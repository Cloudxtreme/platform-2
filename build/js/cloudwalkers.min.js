...
b,c){this.span&&0!=b||this.$container.append(Templates.row);this.span=12>b+this.span?b+this.span:0;a&&(this.$container.children().last().append(a.render().el),this.widgetviews.push(a),a.$el.addClass("span"+b),a.negotiateFunctionalities&&a.negotiateFunctionalities(),this.listenTo(a,"view:update",this.updatewidget.bind(this,a)))},appendhtml:function(a){this.$container.children().last().append(a)},cleanviews:function(){this.widgetviews.length&&(this.destroy(),this.widgetviews=[])},bubblerender:function(){this.trigger("rendered");
$.each(this.widgetviews,function(a,b){b.trigger("rendered")})},updatewidget:function(a){a.render();a.negotiateFunctionalities&&a.negotiateFunctionalities()},destroy:function(){$.each(this.widgetviews,function(a,b){b.remove()})}});Cloudwalkers.Views.Message=Backbone.View.extend({events:{"click .message-action[data-action]":"messageAction","click .children-button.message-children":"showchildren"},className:"message-view",template:"message",tagName:"tr",commentsVisible:!1,commentsView:null,childrencontainer:"comment-container",initialize:function(){var a=this;this.model.on("change",function(){a.render()})},prepareData:function(){var a=jQuery.extend(!0,{},this.model.attributes);a.shortBody=this.model.shortBody();a.humandate=this.model.humandate();
...
a);this.settings={title:this.title,network:this.icon?{icon:this.icon}:{icon:"cloud"},streamid:this.network};this.settings.filterfunc=this.filterfunc;this.settings.footer=this.footer;this.collection=this.model.statistics;this.listenTo(this.collection,"ready",this.fill);this.listenTo(this.collection,"change",this.render);this.network&&(this.settings.network.icon=Cloudwalkers.Session.getStream(this.network).get("network").token)},render:function(){this.$el.html(Mustache.render(Templates.dashboardstat,
...
this.fill();return this},fill:function(){if(this.filled)return!0;$.each(this.charts,function(a,b){b.data.model=this.model;b.data.network=this.network;var c=(new Cloudwalkers.Views.Widgets[b.widget](b.data)).render().el;this.$el.find(this.templatemap[this.template][a]).append(c)}.bind(this));this.filled=!0},negotiateFunctionalities:function(){}});Cloudwalkers.Views.Widgets.TrendingMessage=Backbone.View.extend({initialize:function(a){a&&$.extend(this,a);this.settings={};this.settings.title=this.title;this.model=this.network?Cloudwalkers.Session.getStream(this.network):Cloudwalkers.Session.getChannel("profiles").clone();this.listenTo(this.model,"sync",this.fill);this.timespan.sort||(this.timespan.sort="engagement");this.gettoptrending()},render:function(){this.$el.html(Mustache.render(Templates.trendingmsg,this.settings));this.hideloading();
return this},fill:function(){if((this.message=this.model.get("messages")?this.model.get("messages")[0]:!1)&&0!=this.message.length){var a=this.message,b=a.attachments?a.attachments.filter(function(a){if("link"==a.type)return a}):null,c=a.attachments?a.attachments.filter(function(a){if("image"==a.type)return a}):null;this.settings.statistics=a.statistics;this.settings.from=a.from[0].displayname||"";this.settings.body=a.body.plaintext||"";this.settings.icon=a.from[0].network.icon||"";this.settings.date=
a.dateonly||"";c.length&&(this.settings.image=c[0].url||"");this.settings.links=b||[];this.render()}else this.$el.find(".commentballoon h4").html("No data"),this.hideloading()},gettoptrending:function(){return this.network?this.toptrendingstream(this.network):this.toptrendingall()},toptrendingstream:function(a){this.model.fetch({endpoint:"messages",parameters:{sort:this.timespan.sort,records:1}})},toptrendingall:function(){this.model=Cloudwalkers.Session.getChannel("profiles");this.model.fetch({endpoint:"messages",
