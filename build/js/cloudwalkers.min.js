...
this.end=moment().zone(0).endOf("year"));if("quarter"==this.timespan){var a=3*(this.period+moment().quarter());this.start=moment().startOf("year").add("months",a-3);this.end=moment().startOf("year").add("months",a-1).endOf("month")}else"custom"!=this.timespan&&(0<this.period&&(this.start.add(this.timespan+"s",this.period),this.end.add(this.timespan+"s",this.period)),0>this.period&&(this.start.subtract(this.timespan+"s",Math.abs(this.period)),this.end.subtract(this.timespan+"s",Math.abs(this.period))));
this.timeparams={since:this.start.unix(),until:this.end.unix(),span:this.timespan};return{since:this.start.unix(),until:this.end.unix(),span:this.timespan,period:this.period,reset:!0}},finish:function(){},updatenetwork:function(a){},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},translateWidgets:function(a){if(a.translation)for(k in a.translation)a[k]=Cloudwalkers.Session.polyglot.t(a.translation[k])},mustacheTranslateRender:function(a){this.original="week month quarter year custom time_spans all_networks now show".split(" ");
this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.StatStream=Cloudwalkers.Views.Statistics.extend({networkspecific:{typeA:{facebook:{filterfunc:"age",chart:"PieChart",title:"By Age"},twitter:{filterfunc:"follow",chart:"PieChart",title:"By Type"},linkedin:{filterfunc:"follow",chart:"PieChart",title:"By Type"},youtube:{filterfunc:"follow",chart:"PieChart",title:"By Type"}},typeB:{facebook:{filterfunc:"gender",chart:"PieChart",title:"By gender"},twitter:{filterfunc:"nodata",chart:"PieChart",title:"No data"},youtube:{filterfunc:"nodata",
