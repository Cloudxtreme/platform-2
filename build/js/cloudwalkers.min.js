var Cloudwalkers={Views:{Settings:{},Widgets:{Charts:{}}},Router:{},Models:{},Collections:{},Utilities:{},init:function(){Cloudwalkers.Session.loadEssentialData(function(){Cloudwalkers.RootView=new Cloudwalkers.Views.Root;Cloudwalkers.Router.Instance=new Cloudwalkers.Router;Backbone.history.start()})}};
...
c))},get:function(a){return Cloudwalkers.Session.user.attributes.settings[a]},manage:function(){200<Store.count("messages")&&Store.filter("messages",null,function(a){a.sort(function(a,c){return a.stamp>c.stamp?1:a.stamp<c.stamp?-1:0});a=a.slice(0,100);Store.write("messages",a);Store.filter("touches",null,function(a){var c=Cloudwalkers.Session.getPing().cursor;a=a.filter(function(a){return a.ping==c});Store.write("touches",a)})});25<Store.count("reports")&&Store.filter("reports",null,function(a){a.sort(function(a,
c){return a.stamp>c.stamp?1:a.stamp<c.stamp?-1:0});a=a.slice(0,10);Store.write("messages",a)})},getPing:function(){return this.user.account.ping},ping:function(){this.user.account.ping.request()},getAccount:function(a){return a?this.user.accounts.get(a):this.user.account},getAccounts:function(a){return this.user.accounts},getChannel:function(a){return"number"==typeof a?this.user.account.channels.get(a):this.user.account.channels.findWhere({type:a})},getChannels:function(a){return this.user.account.channels},
storeChannel:function(a){a.channels&&a.channels.length&&(a.channels=a.channels.map(function(a){Cloudwalkers.Session.storeChannel(a);return a.id}));a.streams&&a.streams.length&&(a.streams=a.streams.map(function(a){Store.post("streams",a);return a.id}));Store.post("channels",a)},getStream:function(a){return"number"==typeof a?this.user.account.streams.get(a):this.user.account.streams.findWhere({token:a})},getStreams:function(){return this.user.account.streams},getUser:function(a){return a?this.user.account.users.get(a):
this.user},getUsers:function(){return this.user.account.users},getContact:function(a){return this.user.account.contacts.get(a)},getContacts:function(){return this.user.account.contacts},getMessage:function(a){return this.user.account.messages.get(a)},getMessages:function(){return this.user.account.messages},getNotification:function(a){return this.user.account.notifications.get(a)},getNotifications:function(){return this.user.account.notifications},getStatistic:function(a){return this.user.account.statistics.get(a)},
getStatistics:function(){return this.user.account.statistics},getReport:function(a){return this.user.account.reports.get(a)},getReports:function(){return this.user.account.reports},getComment:function(a){return this.user.account.comments.get(a)},getComments:function(){return this.user.account.comments}};_.extend(Cloudwalkers.Session,Backbone.Events);Cloudwalkers.Router=Backbone.Router.extend({routes:{write:"write",share:"share",scheduled:"scheduled",drafts:"drafts","inbox(/:type)(/:streamid)":"inbox",coworkers:"coworkers","channel/:channel(/:subchannel)(/:stream)(/:messageid)":"channel","timeline/:channel(/:stream)":"timeline","trending/:channel(/:subchannel)(/:stream)(/:messageid)":"trending","monitoring/:channel(/:subchannel)(/:messageid)":"monitoring",keywords:"managekeywords","reports/:streamid":"reports",reports:"statistics","settings(/:sub)":"settings",
...
"":"delete"==a&&(this.endpoint="");return Backbone.sync(a,b,c)},getStream:function(a){var b={};b["string"==typeof a?"token":"id"]=a;return this.streams.findWhere(b)},seedcontacts:function(a){(a=a.get("from"))&&a.length&&this.contacts.add(a)}});Cloudwalkers.Models.Campaign=Backbone.Model.extend({initialize:function(){}});Cloudwalkers.Models.User=Backbone.Model.extend({initialize:function(){},url:function(){return CONFIG_BASE_URL+"json/user/"+this.id},parse:function(a){console.log(a);"number"==typeof a?a={id:a}:this.stamp(a);return a},filterData:function(a){var b=this.attributes;"listitem"==a?b.arrow=!0:b.role=this.getRole();return b},getRole:function(){return 10==this.get("level")?"Administrator":"Co-worker"},saveProfile:function(a){var b={firstname:this.get("firstname"),name:this.get("name"),avatar:this.get("avatarBase64")};
Cloudwalkers.Net.put("user/me",{},b,a)}});Cloudwalkers.Models.Contact=Cloudwalkers.Models.User.extend({initialize:function(){}});Cloudwalkers.Models.Contact.actions={templates:{dm:{name:"Direct message",icon:"message",type:"dialog"}}};Cloudwalkers.Models.Me=Cloudwalkers.Models.User.extend({unreadMessages:null,initialize:function(a){this.once("change",this.activate);this.on("change:id",function(a){this.previous("id")&&Cloudwalkers.Session.home()})},url:function(){var a=Store.exists("me")?"?include_accounts=ids":"";return CONFIG_BASE_URL+"json/user/me"+a},parse:function(a){Store.exists("me")||(a.user=this.firstload(a.user));Store.write("me",[a.user]);Store.set("users",a.user);return a.user},sync:function(a,b,c){"read"==a&&Store.get("me",
...
"</strong> "+b+"</div>")},dialog:function(a,b,c){var d={};d.message=a;d.options=b;a=Mustache.render(Templates.uidialog,d);var e=$(a),f=e.modal();a=function(a){e.find("[data-response="+a.token+"]").click(function(){c(a);f.modal("hide")})};for(d=0;d<b.length;d++)a(b[d])},imagePopups:function(){$("a.image-popup-viewer").fancybox()}});Cloudwalkers.Views.Navigation=Backbone.View.extend({events:{"click .notification-toggle":"toggleNotifications"},initialize:function(){Cloudwalkers.Session.on("change:accounts",this.renderHeader,this);this.listenTo(Cloudwalkers.Session.getChannels(),"sync",this.render);this.listenTo(Cloudwalkers.Session.getChannels(),"remove",this.render);$.get(CONFIG_BASE_URL+"json/version",this.version.bind(this))},headeraction:function(a){a=$(a.currentTarget).data("header-action");"messages"==a&&Cloudwalkers.Router.Instance.navigate("#inbox/messages",
!0);"contacts"==a&&Cloudwalkers.Router.Instance.navigate("#coworkers",!0);"post"==a&&Cloudwalkers.RootView.popup(new Cloudwalkers.Views.Write)},fit:function(){$("#header").html(this.renderHeader().header);$("#sidebar").html(this.render().el);$("#header, #sidebar").on("click","*[data-header-action]",this.headeraction)},version:function(a){"TESTING"==a.platform.name&&(this.development=!0,$("#header").html(this.renderHeader().header))},renderHeader:function(){var a={dev:this.development};a.user=Cloudwalkers.Session.user.attributes;
a.accounts=[];Cloudwalkers.Session.user.accounts.each(function(b){a.accounts.push({name:b.get("name"),id:b.id,active:b.id==Cloudwalkers.Session.get("currentAccount")})});this.header=Mustache.render(Templates.header,a);return this},render:function(){var a=Cloudwalkers.Session.getAccount(),b={reports:[]};b.level=Number(a.get("currentuser").level);if(b.level){var c=a.channels.findWhere({type:"news"});b.news={channelid:c.id,streams:c.streams.models,name:c.get("name")};c=a.channels.findWhere({type:"monitoring"});
b.monitoring={channelid:c.id,channels:c.channels.models,name:c.get("name")}}b.scheduled={channelid:Cloudwalkers.Session.getChannel("internal").id};b.scheduled.streams=a.streams.where({outgoing:1});b.inbox=!0;c=a.channels.findWhere({type:"profiles"});b.profiles={channelid:c.id,streams:c.streams.models,name:c.get("name")};b.reports=a.streams.where({statistics:1}).map(function(a){return a.attributes});this.$el.html(Mustache.render(Templates.navigation,b));this.handleSidebarMenu();return this},handleSidebarMenu:function(){var a=
Backbone.history.fragment;if(!a)return null;this.setActive(a)},setActive:function(a){$("#sidebar .active").removeClass("active");$('a[href="#'+a+'"]').parents("#sidebar .page-sidebar-menu *").addClass("active")},setNotifications:function(a){this.$el.find(".popup-frame ul").html("");for(var b=0;b<a.length;b++)this.addNotification(a[b])},addNotification:function(a){this.$el.find(".popup-frame ul").append('<li><div class="text account"><p>'+a.message+'</p><div class="row"><span class="time"></span></div></div></li>')},
...
countString:"scheduled",link:"#scheduled"},{widget:"coworkers",type:"drafts",size:4,title:"Co-workers wall",color:"yellow",icon:"edit",open:!0,link:"#coworkers"},{widget:"trending",type:"profiles",size:4,title:"Trending Company Posts",color:"grey",icon:"thumbs-up",open:!0,since:7,sublink:"#trending/"},{widget:"trending",type:"news",size:4,title:"Trending Profiles we follow",color:"red",icon:"thumbs-up",open:!0,since:1,sublink:"#trending/"}],initialize:function(){Cloudwalkers.Session.ping()},addDynamicReports:function(){var a=
...
confirm("Are you sure you want to send your message right now? Your schedule details will be lost.")&&(this.clearScheduleDate(),$(a.toElement.form).trigger("submit")):(this.clearScheduleDate(),$(a.toElement.form).trigger("submit"))},selectCampaign:function(a){Number(this.$el.find("select[name=campaign]").val())||this.$el.find(".new-campaign, select[name=campaign]").toggleClass("inactive")},addCampaign:function(a){},getAvailableStreams:function(){var a=Cloudwalkers.Session.getChannel("internal"),a=
new Cloudwalkers.Collections.Streams(a.get("additional").outgoing);if("undefined"!=typeof this.options.actionparameters&&"undefined"!=typeof this.model){var b=Cloudwalkers.Session.getStream(this.model.get("stream"));if(b){for(var c=new Backbone.Collection,d=0;d<a.models.length;d++)a.models[d].get("network").name==b.get("network").name&&c.add(a.models[d]);return c}}return a},render:function(){this.files=[];this.sendnow=this.draft=!1;var a=Cloudwalkers.Session.getAccount();this.actionparameters={};
if("undefined"!=typeof this.options.actionparameters)for(var b=0;b<this.options.actionparameters.length;b++)this.actionparameters[this.options.actionparameters[b].token]=this.options.actionparameters[b];var c=this,d={},e=this.getAvailableStreams(),f={};if(this.model&&this.model.get("streams"))for(b=0;b<this.model.get("streams").length;b++)f[this.model.get("streams")[b].id]=!0;d.channels=[];$.each(e.where({outgoing:1}),function(a,b){b.attributes.checked="undefined"!=typeof f[b.id];d.channels.push(b.attributes)});
d.BASE_URL=CONFIG_BASE_URL;var e=this.model?this.model.scheduledate(!1):!1,g=this.model?this.model.repeat():!1;d.days=[];d.months=[];d.years=[];d.times=[];d.endrepeat={days:[],months:[],years:[]};for(b=1;31>=b;b++)d.days.push({day:b,checked:e?e.getDate()==b:!1}),d.endrepeat.days.push({day:b,checked:g&&g.end&&g.end.getDate()==b});for(b=1;12>=b;b++)d.months.push({month:b,display:Cloudwalkers.Utils.month(b),checked:e?e.getMonth()==b-1:!1}),d.endrepeat.months.push({month:b,display:Cloudwalkers.Utils.month(b),
checked:g&&g.end&&g.end.getMonth()==b-1});for(var h=null,b=0;10>b;b++)h=(new Date).getFullYear()+b,d.years.push({year:h,checked:e?e.getFullYear()==h:!1}),d.endrepeat.years.push({year:h,checked:g&&g.end&&g.end.getFullYear()==h});d.weekdays=[];h="monday tuesday wednesday thursday friday saturday sunday".split(" ");for(b=0;b<h.length;b++){var k=h[b],k=k.charAt(0).toUpperCase()+k.substr(1);d.weekdays.push({name:k,weekday:h[b],checked:g&&g.weekdays&&g.weekdays[h[b].toUpperCase()]})}d.repeatinterval={amount:[],
unit:[]};for(b=0;72>b;b++)d.repeatinterval.amount.push({value:b,name:b,selected:g&&g.interval==b});d.repeatinterval.unit.push({value:"minutes",name:"Minute(s)",selected:g&&"minutes"==g.unit});d.repeatinterval.unit.push({value:"hours",name:"Hour(s)",selected:g&&"hours"==g.unit});d.repeatinterval.unit.push({value:"days",name:"Day(s)",selected:g&&"days"==g.unit});d.repeatinterval.unit.push({value:"weeks",name:"Week(s)",selected:g&&"weeks"==g.unit});d.repeatinterval.unit.push({value:"months",name:"Month(s)",
selected:g&&"months"==g.unit});for(b=0;24>b;b+=0.25)g=Math.floor(b),h=60*(b-Math.floor(b)),10>g&&(g="0"+g),10>h&&(h="0"+h),e&&e.setMinutes(15*Math.floor(e.getMinutes()/15)),d.times.push({time:g+":"+h,checked:e&&e.getHours()==g&&e.getMinutes()==h});d.canSchedule=!0;d.canSave=!0;d.canSendNow=!0;if(this.model){d.canSave="SCHEDULED"!=this.model.get("status");d.message=jQuery.extend(!0,{},this.model.attributes);"undefined"!=typeof this.actionparameters.message&&""!=this.actionparameters.message.value&&
(d.message.body.plaintext=Cloudwalkers.Utilities.Parser.parseFromMessage(this.actionparameters.message.value,this.model),d.message.body.html=Cloudwalkers.Utilities.Parser.parseFromMessage(this.actionparameters.message.value,this.model));d.sortedattachments={};if(this.model.get("attachments"))for(b=0;b<this.model.get("attachments").length;b++)d.sortedattachments[this.model.get("attachments")[b].type]=[this.model.get("attachments")[b]];d.showschedule=!1;if(this.model.scheduledate(!1)||"undefined"!=
typeof this.model.repeat().interval)d.showschedule=!0}d.campaigns=a.get("campaigns");a=Mustache.render(Templates.write,d);c.$el.html(a);c.$el.find("[name=url]").change(function(){c.updateCounter()});c.afterRender();this.model&&setTimeout(function(){var a=c.model.get("attachments");if("undefined"!=typeof a)for(var b=0;b<a.length;b++)"image"==a[b].type&&c.addUploadedFileToList({name:a[b].name,url:a[b].url,delete_url:"bla"})},100);c.updateCounter();c.$el.find("[name=schedule_random]").click(function(){c.randomTime()});
c.$el.find("[data-toggle-id].inactive").hide();c.$el.find("[data-toggle-target]").click(function(){var a=$(this).attr("data-toggle-target"),b;c.$el.find("[data-toggle-id="+a+"]").is(":visible")?(c.$el.find("[data-toggle-id="+a+"]").hide(),b=!1):(b=!0,c.$el.find("[data-toggle-id="+a+"]").show());$(this).toggleClass("black",b);$(this).toggleClass("blue",!b)}).each(function(){var a=$(this);setTimeout(function(){var b=a.attr("data-toggle-target"),b=c.$el.find("[data-toggle-id="+b+"]").is(":visible");
a.toggleClass("black",b);a.toggleClass("blue",!b)},200)});c.$el.find(".inactive[data-toggle-id]").hide();setTimeout(function(){c.trigger("content:change");c.$el.find("#schedule-btn-toggle").toggleClass("black",c.$el.find(".message-schedule").is(":visible"));c.$el.find("#schedule-btn-toggle").toggleClass("blue",!c.$el.find(".message-schedule").is(":visible"))},200);return this},addUploadedFileToList:function(a){var b=this,c=$(document.createElement("div"));c.addClass("uploaded-file");var d=$(document.createElement("img")),
e=$(document.createElement("a"));d.attr("src",a.url);c.append(d);e.html("Delete");e.attr("href","javascript:void(0);");b.files.push(a.url);e.click(function(){for(var d=0;d<b.files.length;d++)if(b.files[d]==a.url){b.files.splice(d,1);break}c.remove();b.updateCounter();jQuery.ajax({async:!0,cache:!1,data:"",dataType:"json",type:"get",url:a.delete_url})});c.append(e);b.$el.find(".fileupload-feedback").append(c);b.updateCounter()},getValidationRules:function(){var a=this,b=Cloudwalkers.Session.getStreams(),
c=[];$.each(b.where({outgoing:1}),function(b,d){a.$el.find("#channel_"+d.id).is(":checked")&&c.push(d.attributes)});for(var b={},d=0;d<c.length;d++)for(var e in c[d].network.limitations)if("undefined"==typeof b[e])b[e]=c[d].network.limitations[e].limit;else{var f=b[e];"MAXIMUM"==c[d].network.limitations[e].type?f>c[d].network.limitations[e].limit&&(b[e]=c[d].network.limitations[e].limit):"MINIMUM"==c[d].network.limitations[e].type?f<c[d].network.limitations[e].limit&&(b[e]=c[d].network.limitations[e].limit):
alert("Unexpected limitation type: "+c[d].network.limitations[e].type)}return b},afterRender:function(){var a=this;a.$el.find("form").find(".channels label").click(function(){var b=$(this);setTimeout(function(){a.$el.find("form").find("input[type=checkbox]#"+b.attr("for")).is(":checked")?(b.parent().addClass("active"),b.parent().removeClass("inactive")):(b.parent().addClass("inactive"),b.parent().removeClass("active"));a.updateCounter()},100)}).each(function(){var b=$(this);setTimeout(function(){a.$el.find("form").find("input[type=checkbox]#"+
...
for(var c=c.serialize(),d=0;d<b.files.length;d++)c+="&files[]="+escape(b.files[d]);d=CONFIG_BASE_URL+"post/?account="+Cloudwalkers.Session.getAccount().get("id");b.model&&!b.isClone()?d+="&id="+b.model.get("id"):b.isClone()&&(c+="&original_message="+b.model.get("id"));b.draft&&(c+="&draft=true");b.validate(!0)&&(b.trigger("popup:close"),b.$el.html("<p>Please wait, storing message.</p>"),jQuery.ajax({async:!0,cache:!1,data:c,dataType:"json",type:"post",url:d,success:function(a){if(a.success){b.$el.html("<p>Your message has been scheduled.</p>");
...
"select",this.select);this.$container.append(b.render().el)}this.$el.find(".inner-loading").removeClass("inner-loading")},select:function(a){this.list.render({users:a.model.id,records:20})}});Cloudwalkers.Views.Widgets.ScheduledList=Cloudwalkers.Views.Widgets.Widget.extend({id:"scheduledlist",title:"Scheduled messages",parameters:{records:20,sort:"asc"},entries:[],events:{remove:"destroy","click .load-more":"more"},initialize:function(a){a&&$.extend(this,a);this.model.messages||(this.model.messages=new Cloudwalkers.Collections.Messages);this.listenTo(this.model.messages,"seed",this.fill);this.listenTo(this.model.messages,"request",this.showloading)},render:function(a){this.$el.html(Mustache.render(Templates.scheduledlist,
{title:this.title}));this.$container=this.$el.find(".messages-container");this.$el.find(".load-more").hide();this.model.messages.touch(this.model,a?a:this.parameters);return this},showloading:function(){this.$el.find(".icon-cloud-download").show();this.$el.find(".load-more").hide()},hideloading:function(){this.$el.find(".icon-cloud-download").hide();this.$container.removeClass("inner-loading");this.model.messages.cursor&&this.$el.find(".load-more").show()},fill:function(a){this.incremental?this.incremental=
