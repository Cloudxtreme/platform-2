...
e&&(e=m[i]),m[i]>f&&(f=m[i])});g=0<m.length?m.indexOf(Math.max.apply(Math,_.values(m))):-1;c.push({day:a.shift(),value:f,time:g});f=0});c.maxvalue=e;return c}});Backbone.View=Backbone.View.extend({loadListeners:function(a,b){var c=b.length;for(i in b)if(_.isArray(b[i]))for(n in b[i])this.listenTo(a,b[i][n],this.loadRender.bind(this,Number(i)+1,c));else this.listenTo(a,b[i],this.loadRender.bind(this,Number(i)+1,c));this.listenTo(a,"ready:empty",this.emptylist.bind(this));this.on("rendered",this.addLoader)},addLoader:function(){this.container=this.$loadercontainer?this.$loadercontainer:this.$container;this.loader=$(Templates.progressbar).appendTo(this.container)},
...
this.filters.users.string=a,this.model.users.fetch({remove:!1,parameters:{q:a},success:this.loadedusers.bind(this)})))},loadedusers:function(){this.$el.find(".loading-contacts").addClass("hidden");this.filters.users.buffered&&(this.requestusers(this.filters.users.buffered),this.filters.users.buffered=!1)},filter:function(a){$(a.currentTarget).toggleClass("inactive active");a=this.$el.find(".filter.keyword-list .active");var b=[];if(!a.size())return this.list.$container.empty();a.each(function(){b.push($(this).attr("data-keyword-id"))});
a=this.$el.find(".filter.network-list .active");var c=[];if(!a.size())return this.list.$container.empty();a.each(function(){c=c.concat($(this).attr("data-network-streams").split(" "))});this.category.messages.touch(this.category,{records:40,channels:b.join(","),streams:c.join(",")});return this},fill:function(a){$.each(this.entries,function(a,b){b.remove()});this.entries=[];for(n in a){var b=new Cloudwalkers.Views.User({model:a[n],template:"smalluser",type:"listitem"});this.entries.push(b);this.listenTo(b,
"select",this.select);this.$container.append(b.render().el)}this.$el.find(".inner-loading").removeClass("inner-loading")},select:function(a){this.list.render({users:a.model.id,records:20})}});Cloudwalkers.Views.Widgets.CoworkersList=Cloudwalkers.Views.Widgets.Widget.extend({id:"coworkersparent",title:"Co-workers messages",parameters:{records:20},entries:[],events:{remove:"destroy","click .load-more":"more"},initialize:function(a){a&&$.extend(this,a);this.model.messages||(this.model.messages=new Cloudwalkers.Collections.Messages);this.listenTo(this.model.messages,"seed",this.fill);this.listenTo(this.model.messages,"request",this.showloading);this.loadListeners(this.model.messages,["request",
"sync","ready"]);this.updateable(this.model,"h3.page-title")},render:function(a){this.$el.html(Mustache.render(Templates.coworkerslist,{title:this.title}));this.$container=this.$el.find(".messages-container");this.$loadercontainer=this.$el.find(".portlet-body");this.$el.find(".load-more").hide();this.model.messages.touch(this.model,a?a:this.parameters);return this},showloading:function(){this.$el.find(".icon-cloud-download").show();this.$el.find(".load-more").hide()},hideloading:function(){this.$el.find(".icon-cloud-download").hide();
this.$container.removeClass("inner-loading");this.model.messages.cursor&&this.$el.find(".load-more").show()},fill:function(a){this.incremental?this.incremental=!1:($.each(this.entries,function(a,b){b.remove()}),this.entries=[]);for(n in a){var b=new Cloudwalkers.Views.Entry({model:a[n],type:"full",template:"coworkersmessage"});this.entries.push(b);a[n].get("from")?this.model.seedusers(a[n]):this.listenTo(a[n],"change:from",this.model.seedusers.bind(this.model));this.$container.append(b.render().el)}this.hideloading()},
...
this.category.messages.cursor&&this.$el.find(".load-more").show();this.$container.removeClass("inner-loading")},fill:function(a){this.incremental?this.incremental=!1:($.each(this.entries,function(a,b){b.remove()}),this.entries=[]);for(n in a){var b=new Cloudwalkers.Views.Entry({model:a[n],type:"full",template:"messagefullentry"});this.entries.push(b);this.$container.append(b.render().el)}},more:function(){this.incremental=!0;this.category.messages.more(this.category,this.parameters)||this.$el.find(".load-more").hide()},
...
"click .load-more":"more"},initialize:function(a,b){a&&$.extend(this,a);this.model=this.options.channel;this.collection=this.model[this.collectionstring];this.listenTo(this.collection,"seed",this.fill);this.listenTo(this.collection,"request",this.showloading);this.listenTo(this.collection,"sync",this.hideloading);this.listenTo(this.model.contacts,"add",this.comparesuggestions);this.loadListeners(this.collection,["request","sync",["ready","loaded"]]);this.listenTo(this.collection,"ready:empty",this.isempty);
this.listenTo(this.collection,"request",this.unsetempty);this.updateable(this.model,"h3.page-title")},toggleall:function(){this.filternetworks(null,!0);this.togglestreams(!0)},togglestreams:function(a){this.$el.find("[data-networks], [data-streams]").addClass(a?"active":"inactive").removeClass(a?"inactive":"active");this.$el.find(".toggleall").addClass(a?"inactive":"active").removeClass(a?"active":"inactive")},render:function(){var a={streams:[],networks:[]};this.model.streams.each(function(b){b.get(this.check)&&
