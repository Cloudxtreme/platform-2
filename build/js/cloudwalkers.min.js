...
c&&(b.icon=c.get("network").icon,b.networktoken=c.get("network").token,b.networkdescription=c.get("name"));a.attachments?(a.attachments.length&&(b.media=a.attachments[a.attachments.length-1],b.media="image"==b.media.type?"picture":b.media.type),b.attached={},$.each(a.attachments,function(a,c){b.attached[c.type]||(b.attached[c.type]=c)})):b.media="reorder";a.engagement&&(b.trending=1E3>a.engagement?a.engagement:"+999");b.schedule&&(b.scheduledate=moment(b.schedule.date).format("DD MMM YYYY HH:mm"));
b.body.intro=a.body.plaintext?a.body.plaintext.substr(0,72):" ";a.date&&(b.fulldate=moment(a.date).format("DD MMM YYYY HH:mm"),b.dateonly=moment(a.date).format("DD MMM YYYY"),b.time=moment(a.date).format("HH:mm"));return b},generateintro:function(){this.get("body")&&!this.get("body").intro&&(this.attributes.body.intro=this.get("body").plaintext.substr(0,72))},filterActions:function(){if(!this.get("actiontokens"))return[];var a=this.actions.rendertokens();a.map(function(a){"note"==a.token&&this.notes.updated&&
(a.value=this.notes.length);return a}.bind(this));return a},filterCalReadable:function(){var a=this.get("objectType"),b=a&&"reorder"!=this.get("media");this.calNode||(this.calNode={id:this.id});this.get("schedule")?this.calNode.start=a?$.fullCalendar.moment(this.get("scheduledate")):$.fullCalendar.moment():this.calNode.start=a?$.fullCalendar.moment(this.get("date")):$.fullCalendar.moment();this.calNode.title=a?(this.get("title")?this.get("title"):this.get("body").plaintext).substring(0,b?12:16):"...";
this.calNode.className=a?this.get("networktoken")+"-color":"hidden";this.calNode.networkdescription=a?this.get("networkdescription"):null;this.calNode.intro=a?this.get("body").intro:null;this.calNode.icon=a?this.get("icon"):null;this.calNode.media=b?this.get("media"):null;a&&72<=this.calNode.intro.length&&(this.calNode.intro+="...");return this},attach:function(a,b,c){c=this.get("attachments")||[];a=b&&c[b]?$.extend(c[b],a):c.push(a);this.set({attachments:c});return a},unattach:function(a){(this.get("attachments")||
[]).splice(a,1)},addcampaign:function(){},setvariation:function(a,b,c){var e=this.get("variations")||[],f=e.filter(function(b){if(b.stream==a)return b});if(0==f.length){f={stream:a};if("image"==b||"link"==b){var g=[c];f.attachments=g}else b&&(f[b]=c);e.push(f)}else f=f[0],"image"==b||"link"==b?(g=f.attachments||[],0==g.length&&"image"==b?(g.push(c),f.attachments=g):"image"==b?g.push(c):(b=g.filter(function(a){if("link"==a.type)return a}),0<b.length?b[0].url=c.url:0!=g.length?f.attachments.push(c):
...
$.extend(this,a);this.streams=Cloudwalkers.Session.getChannel("outgoing").streams;this.action&&(this.type=this.action.get("token"));if(this.action&&"share"==this.type){if(a=Cloudwalkers.Session.getStream(this.reference.get("stream")),this.draft=this.reference.cloneSanitized(),this.draft.attributes.variations=[],"web"==this.reference.get("networktoken")&&"MONITORING"==a.get("type")&&(a=this.reference.geturl()))this.draft.attributes.body={html:a,plaintext:a,intro:a},this.weburl=!0}else this.action&&
this.reference?(this.action.parent=this.reference,this.actionstreams=[],this.actionview=!0,this.action.params={},$.extend(!0,this.action.params,this.action.parameters),delete this.action.parameters,this.listenTo(this.action,"change",this.editstreams),this.action.fetch()):this.model&&(this.type="edit",this.state="loading",this.draft=this.model.clone(),this.draft.get("streams").length&&(this.initstream=this.draft.get("streams")[0].id),this.draft.fetch({endpoint:"original",success:this.original.bind(this)}));
this.draft||(this.draft=new Cloudwalkers.Models.Message({variations:[],attachments:[],streams:[],body:{},schedule:{}}),this.listenTo(this.draft,"invalid",this.invalid));this.loadmylisteners();"reply"==this.type&&"twitter"==this.reference.get("networktoken")&&(a='<short contenteditable="false">@'+this.reference.get("from")[0].username+"</short>&nbsp;",this.draft.set("body",{html:a}));this.censurecompose();this.translateTitles()},loadmylisteners:function(){this.loadListeners(this.draft,["request","sync"],
