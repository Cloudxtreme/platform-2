...
this.get("attachments").length:this.get("body").html},checkloaded:function(a){a.objectType&&setTimeout(function(a){a.trigger("loaded")},1,this)},filterData:function(a){var b={};$.each("id objectType actiontokens subject body date engagement from read stream streams attachments parent statistics canHaveChildren children_count schedule variations".split(" "),function(c,f){b[f]=a[f]});var c=Cloudwalkers.Session.getStream(a.stream);c&&(b.icon=c.get("network").icon,b.networktoken=c.get("network").token,
b.networkdescription=c.get("name"));a.attachments?(a.attachments.length&&(b.media=a.attachments[a.attachments.length-1],b.media="image"==b.media.type?"picture":b.media.type),b.attached={},$.each(a.attachments,function(a,c){b.attached[c.type]=c})):b.media="reorder";a.engagement&&(b.trending=1E3>a.engagement?a.engagement:"+999");b.schedule&&(b.scheduledate=moment(b.schedule.date).format("DD MMM YYYY HH:mm"));a.body&&(b.body.intro=a.body.plaintext?a.body.plaintext.substr(0,72):"...");a.date&&(b.fulldate=
moment(a.date).format("DD MMM YYYY HH:mm"),b.dateonly=moment(a.date).format("DD MMM YYYY"),b.time=moment(a.date).format("HH:mm"));return b},filterActions:function(){return this.get("actiontokens")?this.actions.rendertokens():[]},filterCalReadable:function(){var a=this.get("objectType"),b=a&&"reorder"!=this.get("media");this.calNode||(this.calNode={id:this.id});this.calNode.start=a?$.fullCalendar.moment(this.get("date")):$.fullCalendar.moment();this.calNode.title=a?(this.get("title")?this.get("title"):
this.get("body").plaintext).substring(0,b?12:16):"...";this.calNode.className=a?this.get("networktoken")+"-color":"hidden";this.calNode.networkdescription=a?this.get("networkdescription"):null;this.calNode.intro=a?this.get("body").intro:null;this.calNode.icon=a?this.get("icon"):null;this.calNode.media=b?this.get("media"):null;a&&72<=this.calNode.intro.length&&(this.calNode.intro+="...");return this},attach:function(a,b,c){c=this.get("attachments")||[];a=b&&c[b]?$.extend(c[b],a):c.push(a);this.set({attachments:c});
return a},unattach:function(a){(this.get("attachments")||[]).splice(a,1)},addcampaign:function(){},setvariation:function(a,b,c){var e=this.get("variations")||[],f=e.filter(function(b){if(b.stream==a)return b});if(0==f.length){f={stream:a};if("image"==b||"link"==b){var g=[c];f.attachments=g}else f[b]=c;e.push(f)}else f=f[0],"image"==b||"link"==b?(g=f.attachments||[],0==g.length&&"image"==b?(g.push(c),f.attachments=g):"image"==b?g.push(c):(b=g.filter(function(a){if("link"==a.type)return a}),0<b.length?
b[0].url=c.url:0!=g.length?f.attachments.push(c):f.attachments=[c])):f[b]=c},getvariation:function(a,b){var c=this.get("variations")||[],c=_.findWhere(c,{stream:a});if("image"==b||"link"==b){if(c&&c.attachments)return c.attachments.filter(function(a){if(a.type==b)return a})}else{if(c&&c[b])return c[b];if(c&&!b)return c}},removevarimg:function(a,b){var c=this.getvariation(a).attachments;if(_.isObject(b))this.addexclude(a,b);else for(n in c)"image"==c[n].type&&c[n].name==b&&c.splice(n,1)},addexclude:function(a,
b){var c=this.getvariation(a);c.excludes?c.excludes.push(b):c.excludes=[b]},checkexclude:function(a,b){var c=this.getvariation(a);if(c)c=c.excludes;else return!1;if(c)for(n in c)if(c[n].name==b.name)return!0;return!1},location:function(){return"#"},afterChange:function(){this.addInternalActions();this.get("parent")&&this.get("parentmodel").set(this.get("parent"))},getActionIcon:function(a){return"internal-share"==a.token?"share-alt":"internal-edit"==a.token?"edit":"internal-delete"==a.token||"delete"==
a.token?"remove":"like"==a.token?"thumbs-up":"unlike"==a.token?"thumbs-down":"comment"==a.token?"comment-alt":"reply"==a.token?"comment":"retweet"==a.token?"retweet":"dm"==a.token?"envelope":"internal-reply"==a.token?"comment":"favorite"==a.token?"star":"unfavorite"==a.token?"star-empty":a.token},addInternalActions:function(){var a=this;if("undefined"!=typeof this.attributes.actions)for(var b=0;b<this.attributes.actions.length;b++){if("internal-"==this.attributes.actions[b].token.substr(0,9))return}else this.attributes.actions=
[];"INCOMING"==this.attributes.type?this.attributes.actions.push({name:"Share",parameters:[],token:"internal-share",callback:function(a){Cloudwalkers.RootView.shareMessage(a)}}):"OUTGOING"==this.attributes.type&&(this.attributes.actions.push({name:"Edit",parameters:[],token:"internal-edit",callback:function(b){Cloudwalkers.RootView.editMessage(a)}}),this.attributes.actions.push({name:"Delete",parameters:[],token:"internal-delete",callback:function(b){a.deleteMessage()}}));var c=this.get("parentmodel");
...
[{label:"Skip once",description:"Message will be skipped once",token:"skip"},{label:"Delete forever",description:"Message will never be repeated",token:"remove"}],function(b){b="post/?"+b.token+"="+a.get("id");jQuery.ajax({dataType:"json",type:"get",url:b,success:function(b){a.trigger("destroy",{wait:!0});window.location.reload()}})}):Cloudwalkers.RootView.confirm("Are you sure you want to remove this message?",function(){a.destroy({success:function(){a.trigger("destroy",a,a.collection)}})})},humandate:function(a){a=
new Date(a?a:this.get("date"));return Cloudwalkers.Utils.longdate(a)},shortdate:function(){var a=this.date();return Cloudwalkers.Utils.shortdate(a)},time:function(){var a=this.date();return Cloudwalkers.Utils.time(a)},date:function(){return new Date(this.get("date"))},getAction:function(a){for(var b=this.get("actions"),c=0;c<b.length;c++)if(b[c].token==a)return b[c];return null},act:function(a,b,c){Cloudwalkers.RootView.growl(a.name,"The "+a.token+" is planned with success.");var e=this;"undefined"==
typeof b&&(b={});a={actions:[{token:a.token,parameters:b}]};b=CONFIG_BASE_URL+"json/message/"+this.get("id")+"?account="+Cloudwalkers.Session.getAccount().get("id");jQuery.ajax({data:JSON.stringify(a),dataType:"json",type:"put",url:b,processData:!1,cache:!1,success:function(a){a.removed?e.trigger("destroy",e,e.collection):e.set(a.message);c()}})},scheduledate:function(a){"undefined"==typeof a&&(a=!0);var b=this.get("schedule");return b?a?"ASAP"==b.date?b.date:this.humandate(b.date):"ASAP"==b.date?
!1:new Date(b.date):!1},calculateIntervalFromSeconds:function(a){for(var b={},c=[{unit:"minutes",value:60},{unit:"hours",value:3600},{unit:"days",value:86400},{unit:"weeks",value:604800},{unit:"months",value:2678400}],e=c.length-1;0<=e;e--)if(0==a%c[e].value)return b.interval=Math.round(a/c[e].value),b.unit=c[e].unit,b;for(e=0;e<c.length;e++)if(72>a/c[e].value)return b.interval=Math.round(a/c[e].value),b.unit=c[e].unit,b;return!1},repeat:function(){var a=this.get("schedule"),b={repeat:!1,weekdays:{}};
...
f.readAsDataURL(e)}},addimage:function(a){var b=this.activestream?this.activestream.id:!1;b?this.draft.setvariation(b,"image",a):this.draft.attach(a);this.summarizeimages(a);this.listimage(a)},summarizeimages:function(a){if(a){var b=a.data||a.url,c=this.$el.find("[data-collapsable=images] .summary");$("<li></li>").prependTo(c).attr("data-filename",a.name).css("background-image","url("+b+")")}},listimage:function(a){var b=a.data||a.url,c=this.$el.find("ul.pictures-container");$("<li></li>").prependTo(c).attr("data-filename",
a.name).addClass("images-thumb").css("background-image","url("+b+")")},updateimages:function(){var a=this.activestream?this.activestream.id:!1;this.$el.find("[data-collapsable=images] .summary").empty();var b=this.$el.find("ul.pictures-container").empty();if(a&&this.draft.getvariation(a,"image")){var c=this.draft.getvariation(a,"image");$.each(c,function(a,b){this.summarizeimages(b);this.listimage(b)}.bind(this))}this.draft.get("attachments")&&(c=this.draft.get("attachments").filter(function(a){if("image"==
a.type)return a}),$.each(c,function(b,c){if(a&&this.draft.checkexclude(a,c))return!0;this.summarizeimages(c);this.listimage(c)}.bind(this)));b.append('<li class="add-file">+</li>')},toggleimages:function(a,b){this.$el.find("[data-collapsable=images]")[a?"removeClass":"addClass"]("hidden")},addsnapshot:function(a){this.$el.find(".photo-booth").removeClass("hidden");this.media||(this.media=new MediaClass({video:!0}));this.media.start()},listentobooth:function(){if(this.media){var a=this.media.snapshot();
$("<li></li>").prependTo("ul.snapshots-container").attr("data-filename",moment().unix()).addClass("images-thumb").css("background-image","url("+a+")");this.draft.attach({type:"image",data:a,name:moment().unix()});this.$el.find(".photo-booth").addClass("hidden")}},removefile:function(a){var b=this.activestream?this.activestream.id:!1;a=$(a.currentTarget).remove();var c=this.draft.get("attachments")||[],e;for(n in c)if("image"==c[n].type&&c[n].name==a.data("filename")){e=n;break}b?this.draft.removevarimg(b,
c[e]||a.data("filename")):c.splice(e,1);this.$el.find('[data-collapsable=images] .summary [data-filename="'+a.data("filename")+'"]').remove()},togglelink:function(a){this.$el.find("[data-collapsable=link]")[a?"removeClass":"addClass"]("hidden");return this},listentolink:function(a){var b=this.activestream?this.activestream.id:!1;a=$(a.currentTarget).val();b?this.draft.setvariation(b,"link",{type:"link",url:a}):(b=this.draft.get("attachments").filter(function(a){if("link"==a.type)return a}),b.length?
b[0].url=a:this.draft.attach({type:"link",url:a}));this.summarizelink()},summarizelink:function(){var a=this.activestream?this.activestream.id:!1,b=this.$el.find("[data-collapsable=link] .summary").empty(),c=this.$el.find("[data-collapsable=link] input");if(a&&this.draft.getvariation(a,"link")&&0<this.draft.getvariation(a,"link").length)a=this.draft.getvariation(a,"link"),a=a[0].url;else if(this.draft.get("attachments"))links=this.draft.get("attachments").filter(function(a){if("link"==a.type)return a}),
a=links[0]?links[0].url:!1;else return;a&&(b.html("<span>"+a+"</span>"),c.val(a));return this},togglecampaign:function(a){this.$el.find("[data-collapsable=campaign]")[a?"removeClass":"addClass"]("hidden");return this},summarizecampaign:function(){var a=this.draft.get("campaign"),b=this.$el.find("[data-collapsable=campaign] .summary").empty();if(a){var c=Cloudwalkers.Session.getAccount().get("campaigns").filter(function(b){if(b.id==a)return b}).shift();b.html("<span>"+c.name+"</span>")}return this},
listentocampaign:function(a){a=$(a.currentTarget).val();this.draft.set("campaign",Number(a));this.summarizecampaign().$el.find("[data-collapsable=campaign]").addClass("collapsed")},toggleaddcampaign:function(){this.$el.find(".chosen-container, .campaign-name").toggleClass("hidden");return this},listentoaddcampaign:function(a){a=$(a.currentTarget).val();Cloudwalkers.Session.getAccount().addcampaign(a,function(a){a=a.get("campaigns");this.draft.set("campaign",a[a.length-1].id);this.reloadcampaigns(a);
this.summarizecampaign().$el.find("[data-collapsable=campaign]").addClass("collapsed")}.bind(this))},reloadcampaigns:function(a){var b=this.$el.find(".campaign-list").empty();for(n in a)b.append($("<option value='"+a[n].id+"'>"+a[n].name+"</option>"));b.trigger("chosen:updated")},toggleschedule:function(a){this.$el.find("[data-collapsable=schedule]")[a?"removeClass":"addClass"]("hidden");return this},togglerepeat:function(a){this.$el.find("[data-collapsable=repeat]")[a?"removeClass":"addClass"]("hidden");
return this},toggleschedentry:function(a,b){$(a)[b?"removeClass":"addClass"]("inactive").find("label")[b?"removeClass":"addClass"]("icon-circle-blank")[b?"addClass":"removeClass"]("icon-ok-circle");return this},parsemoment:function(a){a=$(a);if(!a.val())return null;a=moment(a.val(),"DD-MM-YYYY DD-MM-YY DD/MM/YYYY DD/MM/YY DDMMYYYY YYYYMMDD MM-DD-YYYY MM-DD-YY".split(" "));return a.isValid()?a.unix()<moment().unix()?void 0:a:void 0},monitorschedule:function(a,b){var c=b||$(a.currentTarget);this.activestream&&
!this.draft.getvariation(this.activestream.id,"schedule")&&this.draft.setvariation(this.activestream.id,"schedule",{});var e=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,e=e?e:this.draft.get("schedule"),f=c.data("set")?c:c.parents("[data-set]").eq(0),g=f.data("set");"now"==g?(this.toggleschedentry("[data-set=in], [data-set=on]").toggleschedentry("[data-set=now]",!0),$("[data-set=on] input").val(""),$("[data-set=in] select").val(600),this.draft.set("schedule",{repeat:e.repeat||
{}})):"in"==g?(this.toggleschedentry("[data-set=now], [data-set=on]").toggleschedentry("[data-set=in]",!0),$("[data-set=on] input").val("")):"on"==g?(this.toggleschedentry("[data-set=now], [data-set=in]").toggleschedentry("[data-set=on]",!0),$("[data-set=in] select").val(600),void 0===this.parsemoment("#delay-date")&&(this.datepicker.datepicker("hide").val(""),Cloudwalkers.RootView.alert("Please set your Schedule to a date in the future"))):"every"==g?(c=c.data("set")?!f.hasClass("inactive"):!1,this.toggleschedentry("[data-set=every]",
!c),this.toggleschedentry(c?"[data-set=repeat], [data-set=until]":Number($("#repeat-amount").val())?"[data-set=repeat]":"[data-set=until]",!c),$("#repeat-interval").val(c?0:Number($("#repeat-interval").val())?$("#repeat-interval").val():1),c&&($("#repeat-amount").val(0),$("#repeat-until").val(""))):"repeat"==g?(this.toggleschedentry("[data-set=every], [data-set=repeat]",!0).toggleschedentry("[data-set=until]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),Number($("#repeat-amount").val())||
$("#repeat-amount").val(1),$("#repeat-until").val("")):"until"==g&&(this.toggleschedentry("[data-set=every], [data-set=until]",!0).toggleschedentry("[data-set=repeat]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),$("#repeat-amount").val(0));this.parsescheduled();a.stopPropagation();return this},parsescheduled:function(){this.activestream&&!this.draft.getvariation(this.activestream.id,"schedule")&&this.draft.setvariation(this.activestream.id,"schedule",{});var a=this.activestream?
this.draft.getvariation(this.activestream.id,"schedule"):!1,a=a?a:this.draft.get("schedule"),b=this.$el.find("section[data-collapsable] .schedule-entry").not(".inactive").find("#delay-select, #delay-date, #delay-time, #repeat-interval, #every-select, #every-select-weekday, #repeat-amount, #repeat-until");if(b.filter("#delay-select").val())a.date=moment().add("seconds",$("#delay-select").val()).unix();else if(b.filter("#delay-date").val()){var c=this.parsemoment("#delay-date");if(b.filter("#delay-date").val())var e=
$("#delay-time").val().split(":");1<e.length&&c.add("minutes",60*Number(e[0])+Number(e[1]));a.date=c.unix()}b.filter("#repeat-interval").val()&&(a.repeat||(a.repeat={interval:604800}),a.repeat.interval=$("#repeat-interval").val()?3600*$("#repeat-interval").val()*$("#every-select").val():0,$("#every-select-weekday").val()&&(a.repeat.weekdays=[$("#every-select-weekday").val()]),$("#every-select-weekday").attr("disabled",604800>a.repeat.interval),a.repeat.amount=Number(b.filter("#repeat-amount").val())?
Number($("#repeat-amount").val()):!1,a.repeat.until=b.filter("#repeat-until").val()?moment($("#repeat-until").val(),["DD-MM-YYYY","DD-MM-YY","DD/MM/YYYY"]).unix():!1,e={},e.interval=$("#repeat-interval").val()||null,e.every=$("#every-select").val()||null,e.everyweek=$("#every-select-weekday").val()||null,a.repeat.settings=e);return a},togglebesttime:function(a){$(a.currentTarget).toggleClass("inactive");$(a.currentTarget).hasClass("inactive")?$("#delay-time").attr("disabled",!1):$("#delay-time").attr("disabled",
!0).val("");this.draft.get("schedule").besttimetopost=!$(a.currentTarget).hasClass("inactive");return this},summarizeschedule:function(){var a=this.parsescheduled(),b=this.$el.find("[data-collapsable=schedule] .summary").empty();if(a.date){var c=a.besttimetopost?"Best time to post":moment.unix(a.date).format("HH:mm");b.html("<span><i class='icon-time'></i> "+moment.unix(a.date).format("dddd, D MMMM YYYY")+"<em class='negative'>"+c+"</em></span>")}this.summarizerepeat();return this},summarizerepeat:function(){var a=
this.parsescheduled();if(!a.repeat)return this;var b=this.$el.find("[data-collapsable=repeat] .summary").empty(),c=a.date?moment.unix(a.date):moment();if(a.repeat.until)var e=moment.unix(a.repeat.until),c=e.diff(c)/1E3,f=Math.round(c/a.repeat.interval);else a.repeat.interval&&(f=a.repeat.amount,e=c.clone().add("seconds",f*a.repeat.interval));f&&b.html("<span>"+f+" times "+(e?"<em class='negative'>until "+e.format("dddd, D MMMM YYYY")+"</em>":"")+"</span>");return this},updateschedule:function(){var a=
this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,a=a?a:this.draft.get("schedule");a.date&&($(this.datepicker.get(0)).datepicker("update",moment.unix(a.date).format("DD/MM/YYYY")),this.timepicker.timepicker("setTime",moment.unix(a.date).format("HH:mm")));if(a.repeat){a.repeat.until&&$(this.datepicker.get(1)).datepicker("update",moment.unix(a.repeat.until).format("DD/MM/YYYY"));var b=a.repeat.settings||{};b.interval&&$("#repeat-interval").val(b.interval);b.every&&$("#every-select").val(b.every);
b.everyweek&&$("#every-select-weekday").val(b.everyweek);a.repeat.amount&&$("#repeat-amount").val(a.repeat.amount)}},preview:function(){this.$el.addClass("switch-mode");this.preview=new Cloudwalkers.Views.Preview({model:this.draft,previewtype:"default",streamid:this.activestream.id});this.$el.find(".switch-container").append(this.preview.render().el)},endpreview:function(){this.preview.remove();this.$el.removeClass("switch-mode")},save:function(a){if(!this.draft.validateCustom())return Cloudwalkers.RootView.information("Not saved",
"You need a bit of content.",this.$el.find(".modal-footer"));this.disablefooter();a&&_.isString(a)||(a="draft");this.draft.save({status:a},{patch:this.draft.id?!0:!1,success:this.thankyou.bind(this)})},post:function(){if(!this.draft.validateCustom())return Cloudwalkers.RootView.information("Not saved","You need a bit of content.",this.$el.find(".modal-footer"));this.disablefooter();this.draft.get("streams").length?this.draft.save({status:"scheduled"},{patch:this.draft.id?!0:!1,success:this.thankyou.bind(this)}):
...
this.loadListeners(this.model.messages,["request","sync","ready"]);this.updateable(this.model,"h3.page-title")},render:function(a){this.$el.html(Mustache.render(Templates.coworkerslist,{title:this.title}));this.$container=this.$el.find(".messages-container");this.$loadercontainer=this.$el.find(".portlet-body");this.$el.find(".load-more").hide();this.model.messages.touch(this.model,a?a:this.parameters);return this},showloading:function(){this.$el.find(".icon-cloud-download").show();this.$el.find(".load-more").hide()},
hideloading:function(){this.$el.find(".icon-cloud-download").hide();this.$container.removeClass("inner-loading");this.model.messages.cursor&&(this.hasmore=!0)},showmore:function(){this.hasmore&&this.$el.find(".load-more").show()},fill:function(a){this.incremental?this.incremental=!1:($.each(this.entries,function(a,b){b.remove()}),this.entries=[]);for(n in a){var b=new Cloudwalkers.Views.Entry({model:a[n],type:"full",template:"coworkersmessage"});this.entries.push(b);a[n].get("from")?this.model.seedusers(a[n]):
this.listenTo(a[n],"change:from",this.model.seedusers.bind(this.model));this.$container.append(b.render().el)}this.hideloading()},more:function(){this.incremental=!0;this.model.messages.more(this.model,this.parameters)||this.$el.find(".load-more").hide()},negotiateFunctionalities:function(){this.listenTo(Cloudwalkers.Session,"destroy:view",this.remove);this.addScroll()},addScroll:function(){this.$el.find(".scroller").slimScroll({size:"6px",color:"#a1b2bd",height:$("#inner-content").height()-165+"px",
