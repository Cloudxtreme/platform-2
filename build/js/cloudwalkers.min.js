...
addNotification:function(a){this.$el.find(".popup-frame ul").append('<li><div class="text account"><p>'+a.message+'</p><div class="row"><span class="time"></span></div></div></li>')},toggleNotifications:function(){this.$el.find(".notification-popup").toggle()},mapViews:function(){var a={};for(n in this.views)if(a[this.views[n].name]={streams:[]},this.views[n].children)for(i in this.views[n].children)a[this.views[n].children[i].name]={streams:[]};return a}});Cloudwalkers.Views.Compose=Backbone.View.extend({id:"compose",className:"modal hide",type:"post",network:"default",actionstreams:[],actionview:!1,titles:{post:"Write Post",edit:"Edit Post",share:"Share message",comment:"Write Comment",reply:"Write Reply",dm:"Write Message",retweet:"Retweet",like:"Like",favourite:"Favourite",plusone:"Plus one"},limitations:{},options:{"false":"editor link images campaign schedule repeat".split(" "),twitter:"editor images link campaign schedule repeat".split(" "),facebook:"editor fullbody link images campaign schedule repeat".split(" "),
linkedin:"editor link images campaign schedule repeat".split(" "),tumblr:"subject editor fullbody link images campaign schedule repeat".split(" "),"google-plus":"editor fullbody link images campaign schedule repeat".split(" "),youtube:"subject editor fullbody link video campaign schedule repeat".split(" "),blog:"subject editor fullbody link images campaign schedule repeat".split(" "),group:"editor images link campaign schedule repeat".split(" "),"mobile-phone":["editor","schedule","repeat"],comment:["editor"],
reply:["editor"],dm:["editor"],retweet:["icon"],like:["icon","schedule"],favorite:["icon"],plusone:["icon"]},icons:{retweet:"retweet",like:"thumbs-up",favorite:"star",plusone:"plus-square"},events:{"click li[data-streams]":"togglestreams","click .stream-tabs div":"togglestream","click [data-toggle]":"toggleoption","keyup [data-option] input":"monitor","change [data-option] #compose-content":"monitor","keyup [data-option] #compose-content":"monitor","click .add-file":"addfile","click .add-snapshot":"addsnapshot",
"change [data-collapsable=images] input":"listentofiles","click .photo-booth":"listentobooth","click li.images-thumb":"removefile","blur [data-collapsable=link] input":"listentolink","change select.campaign-list":"listentocampaign","blur .campaign-name":"listentoaddcampaign","click .add-campaign":"toggleaddcampaign","click .schedule-entry":"monitorschedule","blur [data-collapsable=schedule] input, [data-collapsable=repeat] input":"monitorschedule","change [data-collapsable=schedule] select, [data-collapsable=repeat] select":"monitorschedule",
"change [data-collapsable=schedule] #delay-time":"monitorschedule","changeDate input":"monitorschedule","click [data-set=on] .btn-white":"togglebesttime","click .end-preview":"endpreview","click #previewbtn":"preview","click #save":"save","click #post":"post","click #action, [data-type=icon] > i":"postaction"},initialize:function(a){a&&$.extend(this,a);this.streams=Cloudwalkers.Session.getChannel("outgoing").streams;this.action&&(this.type=this.action.get("token"));this.action&&"share"==this.type?
this.draft=this.reference.cloneSanitized():this.action&&this.reference?(this.action.parent=this.reference,this.actionstreams=[],this.actionview=!0,this.listenTo(this.action,"change",this.editstreams),this.action.fetch()):this.model&&(this.type="edit",this.draft=this.model.clone(),this.draft.fetch({endpoint:"original",success:this.original.bind(this)}));this.draft||(this.draft=new Cloudwalkers.Models.Message({variations:[],attachments:[],streams:[],body:{},schedule:{}}),this.listenTo(this.draft,"invalid",
this.invalid));this.loadListeners(this.draft,["request","sync"],!0);"reply"==this.type&&"twitter"==this.reference.get("networktoken")&&this.draft.set("body",{html:Mustache.render(this.action.parameters[0].value,{from:this.reference.get("from")[0]})})},original:function(){var a;this.draft.get("schedule")&&(this.draft.get("schedule").date=moment(this.draft.get("schedule").date).unix());(a=this.draft.get("variations"))&&$.each(a,function(a,c){c.schedule&&(c.schedule.date=moment(c.schedule.date).unix())});
this.render()},render:function(){var a={streams:this.actionstreams.length?this.actionstreams:this.streams.models,title:this.titles[this.type],campaigns:Cloudwalkers.Session.getAccount().get("campaigns"),actionview:this.actionview?this.type:!1},a=Mustache.render(Templates.compose,a);this.$el.html(a);this.editor=new Cloudwalkers.Views.Editor({draft:this.draft,parent:this});this.$el.find("[data-type=post]").append(this.editor.render().el);this.listenTo(this.editor,"imageadded",this.addimage);this.listenTo(this.editor,
"change:content",this.monitor);this.$el.find(".campaign-list").chosen({width:"50%"});this.$el.find("#delay-date, #repeat-until").datepicker({format:"dd-mm-yyyy",weekStart:1});this.datepicker=this.$el.find("#delay-date, #repeat-until").datepicker({format:"dd-mm-yyyy",orientation:"auto"});this.timepicker=this.$el.find("#delay-time").timepicker({template:"dropdown",minuteStep:5,showMeridian:!1});this.$loadercontainer=this.$el.find(".modal-footer");this.defaultstreams();this.togglesubcontent();this.trigger("rendered");
return this},editstreams:function(a){var b=a.get("actions").filter(function(b){if(b.token==a.token)return b.streams})[0];this.actionstreams=[];for(n in b.streams)this.actionstreams.push(Cloudwalkers.Session.getStream(b.streams[n]));this.render();this.$el.find("aside li").eq(0).click()},monitor:function(a){var b=a?$(a.target):this.$el.find("#compose-content");a=this.activestream?this.activestream.id:!1;var c=b.html()||b.val(),e=b.text(),b=b.attr("data-option");"body"==b&&(c={html:c,plaintext:e});a?
this.draft.setvariation(a,b,c):this.draft.set(b,c)},toggleoption:function(a){a=$(a.currentTarget);var b=a.data("toggle");if(a.parent().toggleClass("collapsed").hasClass("collapsed")&&this["summarize"+b])this["summarize"+b]()},closealloptions:function(){this.$el.find("[data-collapsable]").not(".collapsed").addClass("collapsed").each(function(a,b){this["summarize"+$(b).data("collapsable")]()}.bind(this))},defaultstreams:function(){var a=this.draft.get("variations");a&&$.each(a,function(a,c){this.$el.find("li[data-streams="+
c.stream+"]").click()}.bind(this))},addstreamtab:function(a){this.$el.find(".tabs-container > section").append('<div class="stream-tab" data-stream="'+a.id+'"><i class="icon-'+a.get("network").icon+'"></i> '+a.get("defaultname")+"</div>")},togglestreams:function(a){var b=this.$el.find(".tabs-container > section");a=$(a.currentTarget);var c=a.data("streams"),e=Cloudwalkers.Session.getStream(c),f=this.draft.get("streams")||[];a.hasClass("active")?(this.$el.find("[data-stream="+c+"]").remove(),b.find("div.active").size()||
(b=this.$el.find(".stream-tabs div").eq(0),b.data("stream"),b.addClass("active"),this.trigger("update:streams",f)),f.splice(f.indexOf(c),1)):(this.addstreamtab(e),f.push(c));a.toggleClass("inactive active");this.isscrolling()?this.$el.find(".stream-tabs").addClass("scrolling"):this.$el.find(".stream-tabs").removeClass("scrolling")},togglestream:function(a){a=$(a.currentTarget);var b=a.data("stream"),b=b?Cloudwalkers.Session.getStream(b):!1;this.$el.find(".stream-tabs div.active").removeClass("active social-icon-colors");
a.addClass("active social-icon-colors");this.togglesubcontent(b)},togglesubcontent:function(a){this.activestream=a;if(this.actionview)var b=this.options[this.type],c=!1;else var c=a?a.get("network").token:!1,e=a?a.get("network").icon:"default",b=this.options[c],f=a?a.id:!1;this.network=c;this.$el.find("#previewbtn").attr("disabled",this.network?!1:!0);this.$el.find(".modal-body").get(0).className="modal-body";this.$el.find(".modal-body").addClass(e+"-theme");0<=b.indexOf("subject")?this.$el.find("[data-option=subject].hidden").removeClass("hidden"):
...
this.updateschedule();this.closealloptions();this.toggleimages(0<=b.indexOf("images"),0<=b.indexOf("multiple"));this.togglelink(0<=b.indexOf("link"));this.togglecampaign(0<=b.indexOf("campaign"));this.toggleschedule(0<=b.indexOf("schedule"));this.togglerepeat(0<=b.indexOf("repeat"));this.trigger("update:stream",{id:f,data:this.draft.getvariation(f,"body")});this.updatesubject();this.updateimages();this.summarizelink();this.summarizeschedule()},updatesubject:function(){var a=this.activestream?this.activestream.id:
...
b.readAsDataURL(a);break}},addimage:function(a){var b=this.activestream?this.activestream.id:!1;b?this.draft.setvariation(b,"image",a):this.draft.attach(a);this.summarizeimages(a);this.listimage(a);this.disableimages()},summarizeimages:function(a){if(a){var b=a.data||a.url,c=this.$el.find("[data-collapsable=images] .summary");$("<li></li>").prependTo(c).attr("data-filename",a.name).css("background-image","url("+b+")")}},listimage:function(a){var b=a.data||a.url,c=this.$el.find("ul.pictures-container");
$("<li></li>").prependTo(c).attr("data-filename",a.name).addClass("images-thumb").css("background-image","url("+b+")")},updateimages:function(){var a=this.activestream?this.activestream.id:!1;this.$el.find("[data-collapsable=images] .summary").empty();var b=this.$el.find("ul.pictures-container").empty(),b=this.$el.find("ul.snapshots-container").empty();if(a&&this.draft.getvariation(a,"image")){var c=this.draft.getvariation(a,"image");$.each(c,function(a,b){this.summarizeimages(b);this.listimage(b)}.bind(this))}this.draft.get("attachments")&&
(c=this.draft.get("attachments").filter(function(a){if("image"==a.type)return a}),$.each(c,function(b,c){if(a&&this.draft.checkexclude(a,c))return!0;this.summarizeimages(c);this.listimage(c)}.bind(this)));0==b.find("li.images-thumb").length&&this.enableimages()},toggleimages:function(a,b){this.$el.find("[data-collapsable=images]")[a?"removeClass":"addClass"]("hidden")},addsnapshot:function(a){this.$el.find(".photo-booth").removeClass("hidden");this.media||(this.media=new MediaClass({video:!0}));this.media.start()},
listentobooth:function(){if(this.media){var a=this.media.snapshot();$("<li></li>").prependTo("ul.snapshots-container").attr("data-filename",moment().unix()).addClass("images-thumb").css("background-image","url("+a+")");this.disableimages();this.draft.attach({type:"image",data:a,name:moment().unix()});this.$el.find(".photo-booth").addClass("hidden")}},removefile:function(a){var b=this.activestream?this.activestream.id:!1;a=$(a.currentTarget).remove();var c=this.draft.get("attachments")||[],e;for(n in c)if("image"==
c[n].type&&c[n].name==a.data("filename")){e=n;break}b?this.draft.removevarimg(b,c[e]||a.data("filename")):c.splice(e,1);this.$el.find('[data-collapsable=images] .summary [data-filename="'+a.data("filename")+'"]').remove();0==this.$el.find("ul.pictures-container li.images-thumb").length&&this.enableimages()},enableimages:function(){this.$el.find("ul.pictures-container").append('<li class="add-file">+</li>');this.$el.find("ul.snapshots-container").append('<li class="add-snapshot">+</li>')},disableimages:function(){this.$el.find("li.add-file").remove();
this.$el.find("li.add-snapshot").remove()},togglelink:function(a){this.$el.find("[data-collapsable=link]")[a?"removeClass":"addClass"]("hidden");return this},listentolink:function(a){var b=this.activestream?this.activestream.id:!1;a=$(a.currentTarget).val();b?this.draft.setvariation(b,"link",{type:"link",url:a}):(b=this.draft.get("attachments").filter(function(a){if("link"==a.type)return a}),b.length?b[0].url=a:this.draft.attach({type:"link",url:a}));this.summarizelink()},summarizelink:function(){var a=
this.activestream?this.activestream.id:!1,b=this.$el.find("[data-collapsable=link] .summary").empty(),c=this.$el.find("[data-collapsable=link] input");if(a&&this.draft.getvariation(a,"link")&&0<this.draft.getvariation(a,"link").length)a=this.draft.getvariation(a,"link"),a=a[0].url;else if(this.draft.get("attachments"))links=this.draft.get("attachments").filter(function(a){if("link"==a.type)return a}),a=links[0]?links[0].url:!1;else return;a&&(b.html("<span>"+a+"</span>"),c.val(a));return this},togglecampaign:function(a){this.$el.find("[data-collapsable=campaign]")[a?
"removeClass":"addClass"]("hidden");return this},summarizecampaign:function(){var a=this.draft.get("campaign"),b=this.$el.find("[data-collapsable=campaign] .summary").empty();if(a){var c=Cloudwalkers.Session.getAccount().get("campaigns").filter(function(b){if(b.id==a)return b}).shift();b.html("<span>"+c.name+"</span>")}return this},listentocampaign:function(a){a=$(a.currentTarget).val();var b=$(".chosen-single > span").get(0).textContent;0==a?this.removecampaign():(this.draft.set("campaign",Number(a)),
this.trigger("update:campaign",b));this.summarizecampaign().$el.find("[data-collapsable=campaign]").addClass("collapsed")},toggleaddcampaign:function(){this.$el.find(".chosen-container, .campaign-name").toggleClass("hidden");return this},listentoaddcampaign:function(a){a=$(a.currentTarget).val();$(".chosen-single > span").get(0);Cloudwalkers.Session.getAccount().addcampaign(a,function(a){a=a.get("campaigns");this.draft.set("campaign",a[a.length-1].id);this.trigger("update:campaign",a[a.length-1].name);
this.reloadcampaigns(a);this.summarizecampaign().$el.find("[data-collapsable=campaign]").addClass("collapsed")}.bind(this))},reloadcampaigns:function(a){var b=this.$el.find(".campaign-list").empty();for(n in a)b.append($("<option value='"+a[n].id+"'>"+a[n].name+"</option>"));b.trigger("chosen:updated")},removecampaign:function(){this.draft.get("campaign")&&(delete this.draft.attributes.campaign,this.trigger("update:campaign",null))},toggleschedule:function(a){this.$el.find("[data-collapsable=schedule]")[a?
"removeClass":"addClass"]("hidden");return this},togglerepeat:function(a){this.$el.find("[data-collapsable=repeat]")[a?"removeClass":"addClass"]("hidden");return this},toggleschedentry:function(a,b){$(a)[b?"removeClass":"addClass"]("inactive").find("label")[b?"removeClass":"addClass"]("icon-circle-blank")[b?"addClass":"removeClass"]("icon-ok-circle");return this},parsemoment:function(a,b){var c=$(a),e=$(b);if(!c.val()||!e.val())return null;c=moment(c.val(),"DD-MM-YYYY DD-MM-YY DD/MM/YYYY DD/MM/YY DDMMYYYY YYYYMMDD MM-DD-YYYY MM-DD-YY".split(" "));
e=e.val().split(":");c=_.clone(c);1<e.length&&c.add("minutes",60*Number(e[0])+Number(e[1]));return c.isValid()?c.unix()<moment().unix()?void 0:c:void 0},monitorschedule:function(a,b){var c=b||$(a.currentTarget);this.activestream&&!this.draft.getvariation(this.activestream.id,"schedule")&&this.draft.setvariation(this.activestream.id,"schedule",{});var e=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,e=e?e:this.draft.get("schedule"),c=(c.data("set")?c:c.parents("[data-set]").eq(0)).data("set");
"now"==c?(this.toggleschedentry("[data-set=in], [data-set=on]").toggleschedentry("[data-set=now]",!0),$("[data-set=on] input").val(""),$("[data-set=in] select").val(600),this.draft.set("schedule",{repeat:e.repeat||{}})):"in"==c?(this.toggleschedentry("[data-set=now], [data-set=on]").toggleschedentry("[data-set=in]",!0),$("[data-set=on] input").val("")):"on"==c?(this.toggleschedentry("[data-set=now], [data-set=in]").toggleschedentry("[data-set=on]",!0),$("[data-set=in] select").val(600),void 0===this.parsemoment("#delay-date",
"#delay-time")&&(this.datepicker.datepicker("hide").val(""),Cloudwalkers.RootView.alert("Please set your Schedule to a date in the future"))):"onlyonce"==c?(this.toggleschedentry("[data-set=onlyonce]",!0).toggleschedentry("[data-set=repeat], [data-set=until], [data-set=every]",!1),$("#repeat-interval, #repeat-amount").val(0),$("#repeat-until").val("")):"every"==c?this.toggleschedentry("[data-set=every]",!0).toggleschedentry("[data-set=onlyonce]",!1):"repeat"==c?(this.toggleschedentry("[data-set=every], [data-set=repeat]",
!0).toggleschedentry("[data-set=onlyonce], [data-set=until]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),Number($("#repeat-amount").val())||$("#repeat-amount").val(1),$("#repeat-until").val("")):"until"==c&&(this.toggleschedentry("[data-set=every], [data-set=until]",!0).toggleschedentry("[data-set=onlyonce], [data-set=repeat]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),$("#repeat-amount").val(0));this.parsescheduled();a.stopPropagation();return this},
parsescheduled:function(){var a=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,a=a?a:this.draft.get("schedule"),b=this.$el.find("section[data-collapsable] .schedule-entry").not(".inactive").find("#delay-select, #delay-date, #delay-time, #repeat-interval, #every-select, #every-select-weekday, #repeat-amount, #repeat-until");if(b.filter("#delay-select").val())a.date=moment().add("seconds",$("#delay-select").val()).unix();else if(b.filter("#delay-date").val()){var c=this.parsemoment("#delay-date",
"#delay-time");a.date=c.unix()}b.filter("#repeat-interval").val()&&(a.repeat||(a.repeat={interval:604800}),a.repeat.interval=$("#repeat-interval").val()?3600*$("#repeat-interval").val()*$("#every-select").val():0,$("#every-select-weekday").val()&&(a.repeat.weekdays=[$("#every-select-weekday").val()]),$("#every-select-weekday").attr("disabled",604800>a.repeat.interval),a.repeat.amount=Number(b.filter("#repeat-amount").val())?Number($("#repeat-amount").val()):!1,a.repeat.until=b.filter("#repeat-until").val()?
moment($("#repeat-until").val(),["DD-MM-YYYY","DD-MM-YY","DD/MM/YYYY"]).unix():!1,b={},b.interval=$("#repeat-interval").val()||null,b.every=$("#every-select").val()||null,b.everyweek=$("#every-select-weekday").val()||null,a.repeat.settings=b);return a},togglebesttime:function(a){$(a.currentTarget).toggleClass("inactive");$(a.currentTarget).hasClass("inactive")?$("#delay-time").attr("disabled",!1):$("#delay-time").attr("disabled",!0).val("");this.draft.get("schedule").besttimetopost=!$(a.currentTarget).hasClass("inactive");
return this},summarizeschedule:function(){var a=this.parsescheduled(),b=this.$el.find("[data-collapsable=schedule] .summary").empty();if(a&&a.date){var c=a.besttimetopost?"Best time to post":moment.unix(a.date).format("HH:mm");b.html("<span><i class='icon-time'></i> "+moment.unix(a.date).format("dddd, D MMMM YYYY")+"<em class='negative'>"+c+"</em></span>")}this.summarizerepeat();return this},summarizerepeat:function(){var a=this.parsescheduled();if(!a||!a.repeat)return this;var b=this.$el.find("[data-collapsable=repeat] .summary").empty(),
c=a.date?moment.unix(a.date):moment();if(a.repeat.until)var e=moment.unix(a.repeat.until),c=e.diff(c)/1E3,f=Math.round(c/a.repeat.interval);else a.repeat.interval&&(f=a.repeat.amount,e=c.clone().add("seconds",f*a.repeat.interval));f&&b.html("<span>"+f+" times "+(e?"<em class='negative'>until "+e.format("dddd, D MMMM YYYY")+"</em>":"")+"</span>");return this},updateschedule:function(){var a=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1;(a=a?a:this.draft.get("schedule"))&&
a.date&&($(this.datepicker.get(0)).datepicker("update",moment.unix(a.date).format("DD/MM/YYYY")),this.timepicker.timepicker("setTime",moment.unix(a.date).format("HH:mm")));if(a&&a.repeat){a.repeat.until&&$(this.datepicker.get(1)).datepicker("update",moment.unix(a.repeat.until).format("DD/MM/YYYY"));var b=a.repeat.settings||{};b.interval&&$("#repeat-interval").val(b.interval);b.every&&$("#every-select").val(b.every);b.everyweek&&$("#every-select-weekday").val(b.everyweek);a.repeat.amount&&$("#repeat-amount").val(a.repeat.amount)}},
preview:function(){this.$el.addClass("switch-mode");this.preview=new Cloudwalkers.Views.Preview({model:this.draft,previewtype:"default",streamid:this.activestream.id});this.$el.find(".switch-container").append(this.preview.render().el)},endpreview:function(){this.preview.remove();this.$el.removeClass("switch-mode")},save:function(a){if(!this.draft.validateCustom())return Cloudwalkers.RootView.information("Not saved","You need a bit of content.",this.$el.find(".modal-footer"));this.disablefooter();
a&&_.isString(a)||(a="draft");this.draft.sanitizepost();this.draft.id?this.draft.save({body:this.draft.get("body"),variations:this.draft.get("variations"),streams:this.draft.get("streams"),update:!0},{patch:!0,endpoint:"original",success:this.thankyou.bind(this)}):this.draft.save({status:a},{success:this.thankyou.bind(this)})},post:function(){if(!this.draft.validateCustom())return Cloudwalkers.RootView.information("Not saved:","You need a bit of content.",this.$el.find(".modal-footer"));if(1>=this.$el.find(".stream-tabs .stream-tab").length)return Cloudwalkers.RootView.information("Not posted:",
"Please select a network first.",this.$el.find(".modal-footer"));this.disablefooter();this.draft.sanitizepost();this.draft.get("streams").length?this.draft.id?this.draft.save({body:this.draft.get("body"),variations:this.draft.get("variations"),streams:this.draft.get("streams"),status:"scheduled",update:!0},{patch:!0,endpoint:"original",success:this.thankyou.bind(this)}):this.draft.save({status:"scheduled"},{success:this.thankyou.bind(this)}):this.save("scheduled")},postaction:function(){var a=[];
this.$el.find(".action-tabs div").each(function(){a.push($(this).data("stream"))});if(!a.length)return Cloudwalkers.RootView.information("Not saved","Select at least 1 network",this.$el.find(".modal-footer"));if(0<=this.options[this.type].indexOf("editor")&&!this.draft.get("body").html)return Cloudwalkers.RootView.information("Not saved","Provide some text first",this.$el.find(".modal-footer"));this.disablefooter();var b=this.reference.actions.create({streams:a,message:this.draft.get("body").html,
actiontype:this.type},{success:this.thankyou.bind(this)});this.loadListeners(b,["request:action","sync"]);b.trigger("request:action")},thankyou:function(){var a=Mustache.render(Templates.thankyou);setTimeout(function(){this.$el.addClass("switch-mode").addClass("thanks");this.$el.find(".switch-container").append(a);setTimeout(function(){this.$el.modal("hide")}.bind(this),1E3)}.bind(this),400);"edit"==this.type&&this.model.fetch()},disablefooter:function(){this.$el.find(".modal-footer .btn").attr("disabled",
...
<<<<<<< HEAD
return this},unfollow:function(){this.model.trigger("unfollow",this.model);this.model.parent=Cloudwalkers.Session.getAccount();this.model.save({following:!1},{patch:!0});this.remove()}});Cloudwalkers.Views.Editor=Backbone.View.extend({id:"editor",className:"clearfix",oldUrl:"",urldata:{},currenturl:!1,urlprocessing:!1,limit:null,content:"",charlength:0,pos:0,hasbeenwarned:!1,urls:[],posmap:[],restrictedstreams:{twitter:140,linkedin:700},events:{"update:content":"render","update:limit":"renderlimit","append:content":"append","change:charlength":"greyout","keyup #compose-content":"listentochange","paste #compose-content":"listentochange","blur #compose-content":"endchange","click #swaplink":"swaplink",
'click [data-type="title"] .addcontent':"addoecontent",'click [data-type="content"] .addcontent':"addoecontent",'click [data-type="image"] i':"addoeimg","mouseup #compose-content":"savepos"},xtrimmable:/<[^>]*>|&nbsp;|\s/g,xurlbasic:/http|[w]{3}/g,xshortens:/http|[w]{3}/g,xurlpattern:/(^|\s|\r|\n|\u00a0)((https?:\/\/|[w]{3})?[\w-]+(\.[\w-]+)+\.?(:\d+)?(\/\S*)?)(\s|\r|\n|\u00a0)/g,xurlendpattern:/((https?:\/\/|[w]{3})?[\w-]+(\.[\w-]+)+\.?(:\d+)?(\/\S*)?)/g,initialize:function(a){a&&$.extend(this,a);
this.listenTo(Cloudwalkers.Session.UrlShortener,"sync",this.parseurl);this.listenTo(this.parent,"update:stream",function(a){this.togglecontent(a,!0)}.bind(this));this.listenTo(this.parent,"update:campaign",this.campaignupdated);navigator.userAgent.match(/(firefox(?=\/))\/?\s*(\d+)/i)&&(this.isfirefox=!0)},render:function(a){void 0!==a&&(this.content=a);a=Math.min.apply(Math,_.values(this.restrictedstreams));this.restrictedstreams["default"]=a;this.$el.html(Mustache.render(Templates.editor,{limit:a}));
this.limit=a;this.$contenteditable=this.$el.find("#compose-content").eq(0);this.contenteditable=this.$contenteditable.get(0);this.document=this.contenteditable.ownerDocument||this.contenteditable.document;this.win=document.defaultView||document.parentWindow;this.$contenteditable.html(this.content);this.content&&this.listentochange();return this},savepos:function(a){a=a.target.tagName;"I"!=a&&"SHORT"!=a&&(this.pos=this.cursorpos())},listentochange:function(a,b){var c=this.$contenteditable.text();this.pos=
this.cursorpos();if(this.content!==c||b)!this.currenturl&&this.listentourl(c)&&this.processurl(),this.charlength!=this.$contenteditable.text().length&&(this.charlength=this.$contenteditable.text().length,this.trigger("change:charlength",this.charlength));this.updatecounter(extrachars);this.content=this.$contenteditable.text();this.isdefault&&!b&&(this.$contenteditable.removeClass("withdefault"),this.isdefault=!1);this.trigger("change:content",this.content);b&&this.clearselections()},endchange:function(a){this.$contenteditable.html().match(this.xurlendpattern)&&
this.$contenteditable.append(" \u200b\u200b");this.listentochange(a,!0)},listentourl:function(a,b){var c=this.win.getSelection(),e=this.document.createRange(),f,g,h;e.selectNodeContents(this.$contenteditable.get(0));f=this.parsenodes(e.startContainer.childNodes);if(this.currenturl)return g=f.textContent.replace(/&nbsp;/gi,""),g=g.indexOf(this.currenturl),h=g+this.currenturl.length,e.setStart(f,g),e.setEnd(f,h),c.removeAllRanges(),c.addRange(e),this.contenteditable.designMode="on",document.execCommand("createLink",
!1,this.currenturl),this.contenteditable.designMode="off",f=document.createTextNode(" \u200b\u200b"),e.insertNode(f),this.$contenteditable.find("a").after(f),e.setStartAfter(f),c.removeAllRanges(),c.addRange(e),!0},processurl:function(){this.$contenteditable.find("a").attr("contenteditable",!1);this.filterurl(this.currenturl)},parsenodes:function(a){var b=null;$.each(a,function(c,e){var f=!1,g=e.textContent;g&&(f=g.match(this.xurlpattern));a.length==c+1&&(f=g.match(this.xurlendpattern));if(f)return b=
3==e.nodeType?e:this.getnode(e,null,3),this.currenturl=f[0],!0}.bind(this));return b},filterurl:function(a){var b=a.match(this.xurlpattern),c=this.draft.get("campaign"),e=Cloudwalkers.Session.getAccount().get("campaigns").filter(function(a){if(a.id==c)return a}),e=e.length?e[0].name:null;b&&!this.urlprocessing&&(this.urlprocessing=!0,this.urls=b,this.shortenurls(b,e));return a},shortenurls:function(a,b){var c={error:this.releaseurlprocessing};a.forEach(function(a){c.q=a.trim();c.campaign=b;Cloudwalkers.Session.UrlShortener.fetch(c)})},
parseurl:function(a){var b=" <short contenteditable='false' data-url='"+a.longurl+"'>"+a.get("shortUrl")+"<i class='icon-link' id='swaplink'></i></short>",c=this;this.currenturl=a.longurl;this.shorturl=a.get("shortUrl");this.$contenteditable.find("a").replaceWith(b);this.trigger("change:content",this.content);a='<div><a href="'+this.currenturl+'" class="oembed"></a></div>';this.$el.find("#out").empty().html(a);this.$el.find(".oembed").oembed(null,null,this.embed).each(function(){this.def.done(function(){c.$el.find("#out").addClass("expanded")})})},
releaseurlprocessing:function(){this.urlprocessing=!1},campaignupdated:function(a){var b=this.urls;b.length&&this.shortenurls(b,a)},greyout:function(a){if(!(0<this.limit-a)){a=this.win.getSelection();var b=this.document.createRange(),c=this.cursorpos(),e,f;this.content.length>=this.$contenteditable.text().length&&this.removegrey();b.selectNodeContents(this.$contenteditable.get(0));e=b.endContainer.childNodes[b.endContainer.childNodes.length-1];f=this.cursorpos(this.limit,!0);b.setStart(f.node,f.offset);
b.setEndAfter(e);a.removeAllRanges();a.addRange(b);this.contenteditable.designMode="on";document.execCommand("backColor",!1,"#fcc");this.contenteditable.designMode="off";this.mergechars();this.cursorpos(c)}},removegrey:function(a){a=this.win.getSelection();var b=this.document.createRange();this.$contenteditable.find("span").length&&(b.selectNodeContents(this.$contenteditable.find("span").get(0)),a.removeAllRanges(),a.addRange(b),this.contenteditable.designMode="on",document.execCommand("removeFormat",
!1,null),this.contenteditable.designMode="off")},mergechars:function(){$.each(this.$contenteditable.find("span"),function(a,b){$(b).html($(b).text())}.bind(this))},cursorpos:function(a,b){var c,e;if(void 0===a)a=0,this.win.getSelection()&&this.win.getSelection().rangeCount&&(e=this.win.getSelection().getRangeAt(0),c=e.cloneRange(),c.selectNodeContents(this.contenteditable),c.setEnd(e.endContainer,e.endOffset),a=c.toString().length);else{this.getnode(this.$contenteditable[0],a);var f=this.currentnode;
if(b)return{node:f.node,offset:f.offset};e=this.document.createRange();c=this.win.getSelection();e.setStart(f.node,f.offset);e.collapse(!0);c.removeAllRanges();c.addRange(e)}return a},getnode:function(a,b,c){var e;$.each(a.childNodes,function(a,g){if(c){if(g.nodeType==c)if(g.childNodes.length)e=this.getnode(g,null,c);else return e=g,!1}else{var h=g.textContent.length;return h>=b?(g.childNodes.length?this.getnode(g,b):this.currentnode={node:g,offset:b},!1):b-=h}}.bind(this));return e},updatecounter:function(a){var b=
this.$el.find(".limit-counter");20<=a&&b.removeClass().addClass("limit-counter");20>a&&b.removeClass().addClass("limit-counter").addClass("color-notice");0>a&&b.removeClass().addClass("limit-counter").addClass("color-warning");b.empty().html(a)},swaplink:function(){var a=this.$contenteditable.find("short").data("url").replace(/\s/g,"")==this.currenturl.replace(/\s/g,""),b="<short contenteditable='false' data-url='"+this.shorturl+"'>"+this.currenturl+"<i class='icon-link' id='swaplink'></i></short>",
=======
return this},unfollow:function(){this.model.trigger("unfollow",this.model);this.model.parent=Cloudwalkers.Session.getAccount();this.model.save({following:!1},{patch:!0});this.remove()}});Cloudwalkers.Views.Editor=Backbone.View.extend({id:"editor",className:"clearfix",oldUrl:"",urldata:{},currenturl:!1,urlprocessing:!1,limit:null,content:"",pos:0,hasbeenwarned:!1,urls:[],posmap:[],restrictedstreams:{twitter:140,linkedin:700},events:{"update:content":"render","update:limit":"renderlimit","append:content":"append","keyup #compose-content":"listentochange","paste #compose-content":"listentochange","blur #compose-content":"endchange","click #swaplink":"swaplink",'click [data-type="title"] .addcontent':"addoecontent",
'click [data-type="content"] .addcontent':"addoecontent",'click [data-type="image"] i':"addoeimg","mouseup #compose-content":"savepos"},xtrimmable:/<[^>]*>|&nbsp;|\s/g,xurlbasic:/http|[w]{3}/g,xshortens:/http|[w]{3}/g,xurlpattern:/(^|\s|\r|\n|\u00a0)((https?:\/\/|[w]{3})?[\w-]+(\.[\w-]+)+\.?(:\d+)?(\/\S*)?)(\s|\r|\n|\u00a0)/g,initialize:function(a){a&&$.extend(this,a);this.listenTo(Cloudwalkers.Session.UrlShortener,"sync",this.parseurl);this.listenTo(this.parent,"update:stream",function(a){this.togglecontent(a,
!0)}.bind(this));this.listenTo(this.parent,"update:campaign",this.campaignupdated);navigator.userAgent.match(/(firefox(?=\/))\/?\s*(\d+)/i)&&(this.isfirefox=!0)},render:function(a){void 0!==a&&(this.content=a);a=Math.min.apply(Math,_.values(this.restrictedstreams));this.restrictedstreams["default"]=a;this.$el.html(Mustache.render(Templates.editor,{limit:a}));this.limit=a;this.$contenteditable=this.$el.find("#compose-content").eq(0);this.contenteditable=this.$contenteditable.get(0);this.document=this.contenteditable.ownerDocument||
this.contenteditable.document;this.win=document.defaultView||document.parentWindow;this.$contenteditable.html(this.content);this.content&&this.listentochange();return this},savepos:function(a){a=a.target.tagName;"I"!=a&&"SHORT"!=a&&(this.pos=this.cursorpos())},listentochange:function(a,b){var c=this.$contenteditable.text(),e=this.limit-this.$contenteditable.text().length;this.pos=this.cursorpos();if(this.content!==c||b)!this.currenturl&&this.listentourl(c)&&this.processurl(),0>=e&&this.greyout(e,
this.limit,a);this.updatecounter(e);this.content=this.$contenteditable.text();this.isdefault&&!b&&(this.$contenteditable.removeClass("withdefault"),this.isdefault=!1);this.trigger("change:content",this.content);b&&this.clearselections()},listentourl:function(a,b){var c=this.win.getSelection(),e=this.document.createRange(),f,g,h;e.selectNodeContents(this.$contenteditable.get(0));f=this.parsenodes(e.startContainer.childNodes);if(this.currenturl)return g=f.textContent.replace(/&nbsp;/gi,""),g=g.indexOf(this.currenturl),
h=g+this.currenturl.length,e.setStart(f,g),e.setEnd(f,h),c.removeAllRanges(),c.addRange(e),this.contenteditable.designMode="on",document.execCommand("createLink",!1,this.currenturl),this.contenteditable.designMode="off",f=document.createTextNode(" \u200b\u200b"),e.insertNode(f),this.$contenteditable.find("a").after(f),e.setStartAfter(f),c.removeAllRanges(),c.addRange(e),!0},processurl:function(){this.$contenteditable.find("a").attr("contenteditable",!1);this.filterurl(this.currenturl)},parsenodes:function(a){var b=
null;$.each(a,function(a,e){var f=!1,g=e.textContent;g&&(f=g.match(this.xurlpattern));if(f)return b=3==e.nodeType?e:this.getnode(e,null,3),this.currenturl=f[0],!0}.bind(this));return b},filterurl:function(a){var b=a.match(this.xurlpattern),c=this.draft.get("campaign"),e=Cloudwalkers.Session.getAccount().get("campaigns").filter(function(a){if(a.id==c)return a}),e=e.length?e[0].name:null;b&&!this.urlprocessing&&(this.urlprocessing=!0,this.urls=b,this.shortenurls(b,e));return a},shortenurls:function(a,
b){var c={error:this.releaseurlprocessing};a.forEach(function(a){c.q=a.trim();c.campaign=b;Cloudwalkers.Session.UrlShortener.fetch(c)})},parseurl:function(a){var b=" <short contenteditable='false' data-url='"+a.longurl+"'>"+a.get("shortUrl")+"<i class='icon-link' id='swaplink'></i></short>",c=this;this.currenturl=a.longurl;this.shorturl=a.get("shortUrl");this.$contenteditable.find("a").replaceWith(b);this.trigger("change:content",this.content);a='<div><a href="'+this.currenturl+'" class="oembed"></a></div>';
this.$el.find("#out").empty().html(a);this.$el.find(".oembed").oembed(null,null,this.embed).each(function(){this.def.done(function(){c.$el.find("#out").addClass("expanded")})})},campaignupdated:function(a){var b=this.urls;b.length&&this.shortenurls(b,a)},greyout:function(a,b,c){a=this.win.getSelection();b=this.document.createRange();c=this.cursorpos();var e,f;this.content.length>=this.$contenteditable.text().length&&this.removegrey();b.selectNodeContents(this.$contenteditable.get(0));e=b.endContainer.childNodes[b.endContainer.childNodes.length-
1];f=this.cursorpos(this.limit,!0);b.setStart(f.node,f.offset);b.setEndAfter(e);a.removeAllRanges();a.addRange(b);this.contenteditable.designMode="on";document.execCommand("backColor",!1,"#fcc");this.contenteditable.designMode="off";this.mergechars();this.cursorpos(c)},removegrey:function(a){a=this.win.getSelection();var b=this.document.createRange();this.$contenteditable.find("span").length&&(b.selectNodeContents(this.$contenteditable.find("span").get(0)),a.removeAllRanges(),a.addRange(b),this.contenteditable.designMode=
"on",document.execCommand("removeFormat",!1,null),this.contenteditable.designMode="off")},mergechars:function(){$.each(this.$contenteditable.find("span"),function(a,b){$(b).html($(b).text())}.bind(this))},cursorpos:function(a,b){var c,e;if(void 0===a)a=0,this.win.getSelection()&&this.win.getSelection().rangeCount&&(e=this.win.getSelection().getRangeAt(0),c=e.cloneRange(),c.selectNodeContents(this.contenteditable),c.setEnd(e.endContainer,e.endOffset),a=c.toString().length);else{this.getnode(this.$contenteditable[0],
a);var f=this.currentnode;if(b)return{node:f.node,offset:f.offset};e=this.document.createRange();c=this.win.getSelection();e.setStart(f.node,f.offset);e.collapse(!0);c.removeAllRanges();c.addRange(e)}return a},getnode:function(a,b,c){var e;$.each(a.childNodes,function(a,g){if(c){if(g.nodeType==c)if(g.childNodes.length)e=this.getnode(g,null,c);else return e=g,!1}else{var h=g.textContent.length;return h>=b?(g.childNodes.length?this.getnode(g,b):this.currentnode={node:g,offset:b},!1):b-=h}}.bind(this));
return e},updatecounter:function(a){var b=this.$el.find(".limit-counter");20<=a&&b.removeClass().addClass("limit-counter");20>a&&b.removeClass().addClass("limit-counter").addClass("color-notice");0>a&&b.removeClass().addClass("limit-counter").addClass("color-warning");b.empty().html(a)},swaplink:function(){var a=this.$contenteditable.find("short").data("url").replace(/\s/g,"")==this.currenturl.replace(/\s/g,""),b="<short contenteditable='false' data-url='"+this.shorturl+"'>"+this.currenturl+"<i class='icon-link' id='swaplink'></i></short>",
>>>>>>> FETCH_HEAD
c="<short contenteditable='false' data-url='"+this.currenturl+"'>"+this.shorturl+"<i class='icon-link' id='swaplink'></i></short>",e=this.$contenteditable.text().length;a?this.$contenteditable.find("short").replaceWith(b):this.$contenteditable.find("short").replaceWith(c);b=this.$contenteditable.text().length-e;a=a?this.$contenteditable.text().indexOf(this.currenturl):this.$contenteditable.text().indexOf(this.shorturl);this.pos>=a?this.cursorpos(this.pos+b):this.cursorpos(this.pos);this.pos=this.cursorpos();
this.trigger("change:content",this.content)},togglecontent:function(a,b){if(a)var c=_.isNumber(a.id)?a.id:null,e=_.isObject(a.data)?a.data.html:null,f=c?Cloudwalkers.Session.getStream(c).get("network").token:null;this.network=f?f:"default";f&&!e?(this.$contenteditable.empty().html(this.draft.get("body").html),this.$contenteditable.addClass("withdefault"),this.isdefault=!0):(c?(e||(e=""),this.$contenteditable.empty().html(e)):this.$contenteditable.empty().html(this.draft.get("body").html),this.$contenteditable.removeClass("withdefault"));
this.limit=this.restrictedstreams[this.network];this.removegrey(!0);this.listentochange(null,!0)},limitwarning:function(){Cloudwalkers.RootView.alert("You have reached the limit number of characters. Exceeding text will not be shown.");this.hasbeenwarned=!0},addoetitle:function(a){},addoecontent:function(a){a=a.currentTarget.parentElement.textContent;var b=this.$contenteditable.html();this.$contenteditable.html(b+"<br/>"+a);this.trigger("change:content")},addoeimg:function(a){a=this.$el.find('[data-type="image"] img').get(0).src;
this.trigger("imageadded",{type:"image",url:a,name:a})},clearselections:function(){window.getSelection?window.getSelection().empty?window.getSelection().empty():window.getSelection():document.selection&&document.selection.empty()}});Cloudwalkers.Widget=Backbone.View.extend({title:"...",color:"blue",icon:"cloud",events:{"click .portlet-title.line":"collapse"},initialize:function(){this.bind("destroy",this.destroy)},innerRender:function(){this.$el.addClass("inner-empty")},render:function(){this.$el.html(Mustache.render(Templates.widget,{title:this.options.title|this.title,color:this.options.color|this.color,icon:this.options.icon|this.icon}));"auto"==this.options.height&&this.$el.find(".scroller").remove();"undefined"!=typeof this.options.counter&&
