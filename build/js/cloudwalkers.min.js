...
Store.filter("streams",null,function(a){this.streams.add(a)}.bind(this));Store.filter("channels",null,function(a){this.channels.add(a)}.bind(this));Store.filter("campaigns",null,function(a){this.campaigns.add(a)}.bind(this));Store.filter("messages",null,function(a){this.messages.add(a)}.bind(this));Store.filter("reports",null,function(a){this.reports.add(a)}.bind(this));this.get("plan")&&(this.limits=this.get("plan").limits);this.ping=new Cloudwalkers.Session.Ping({id:this.id})},monitorlimit:function(a,
...
c.push(f)}c=new Cloudwalkers.Views.Widgets.CombinedStatistics({reports:c,stream:a});c.color=a.network.icon+"-color";c.network=a.network;this.add(c)},addReportWidget:function(a,b){b.stream=a;var c=new Cloudwalkers.Models.Report(b),d=c.getWidget(),e=this.datepicker.getDateRange();c.setDateRange(e[0],e[1]);d.color=a.network.icon+"-color";d.network=a.network;d.showLink=!1;d.showStreamName=!1;this.datepicker.on("date:change",function(a,b){d.getDataset().setDateRange(a,b)});this.add(d)}});Cloudwalkers.Views.Statistics=Cloudwalkers.Views.Pageview.extend({id:"statistics",start:0,end:0,timespan:"week",period:0,custom:!1,events:{remove:"destroy","click #add":"addperiod","click #subtract":"subtractperiod","click #now":"changespan","click #show":"changecustom","change .stats-header select":"changespan"},widgets:[{widget:"StatSummary",data:{columnviews:["contacts","score-trending","outgoing","coworkers"]},span:12},{widget:"Chart",data:{chart:"PolarArea"},span:6}],initialize:function(a){a&&
$.extend(this,a);this.model.reports||(this.model.reports=new Cloudwalkers.Collections.Reports);this.collection=this.model.reports;this.listenTo(this.collection,"request",this.showloading);this.listenTo(this.collection,"sync",this.hideloading)},showloading:function(){this.$el.addClass("loading")},hideloading:function(a,b){this.$el.removeClass("loading")},render:function(){this.cleanviews();this.$el.html(Mustache.render(Templates.statsview,this.timemanager()));this.$container=this.$el.find("#widgetcontainer").eq(0);
this.$el.find("select").chosen({width:"200px",disable_search_threshold:10,inherit_select_classes:!0});"custom"==this.timespan&&this.$el.find("#start, #end").datepicker({format:"dd-mm-yyyy"});for(n in this.widgets){this.widgets[n].data.model=this.model;var a=new Cloudwalkers.Views.Widgets[this.widgets[n].widget](this.widgets[n].data);console.log("widget:",a,this.widgets[n].span);this.appendWidget(a,this.widgets[n].span)}this.collection.touch(this.model,this.filterparameters());return this},timemanager:function(){var a=
this.filterparameters(),b=moment.unix(a.start),c=24<b.date()||"quarter"==a.span?11==b.month()?"DD MMM YYYY":"DD MMM":"DD";a.periodstring=this.period||"custom"==a.span?-1==this.period?"last ":"":"this ";a.timeview=b.format(c)+" - "+moment.unix(a.end).format("DD MMM YYYY");a[a.span+"Active"]=!0;a.startstring=moment.unix(a.start).format("DD-MM-YYYY");a.endstring=moment.unix(a.end).format("DD-MM-YYYY");return a},addperiod:function(){this.period+=1;this.render()},subtractperiod:function(){this.period-=
1;this.render()},changespan:function(){this.period=0;this.render()},changecustom:function(){this.start=moment(this.$el.find("#start").val(),"DD-MM-YYYY");this.end=moment(this.$el.find("#end").val(),"DD-MM-YYYY");this.render()},filterparameters:function(){var a=this.$el.find(".stats-header select").val();a&&(this.timespan=a);"now"==this.timespan&&(this.period=0);"week"==this.timespan&&(this.start=moment().startOf("week"),this.end=moment().endOf("week"));"month"==this.timespan&&(this.start=moment().startOf("month"),
this.end=moment().endOf("month"));"year"==this.timespan&&(this.start=moment().startOf("year"),this.end=moment().endOf("year"));"quarter"==this.timespan?(a=3*(this.period+moment().quarter()),this.start=moment().startOf("year").add("months",a-3),this.end=moment().startOf("year").add("months",a-1).endOf("month")):"custom"!=this.timespan&&(0<this.period&&(this.start.add(this.timespan+"s",this.period),this.end.add(this.timespan+"s",this.period)),0>this.period&&(this.start.subtract(this.timespan+"s",Math.abs(this.period)),
this.end.subtract(this.timespan+"s",Math.abs(this.period))));return{start:this.start.unix(),end:this.end.unix(),span:this.timespan,period:this.period}},finish:function(){}});Cloudwalkers.Views.Settings=Cloudwalkers.Views.Pageview.extend({title:"Settings",tabs:[],initialize:function(){this.endpoint=this.options.endpoint;this.level=Cloudwalkers.Session.user.level},setAction:function(a){this.action=a},render:function(){this.level&&(this.tabs=[{url:"#settings/users",name:"Manage users"},{url:"#settings/services",name:"Social connections"},{url:"#settings/account",name:"Account settings"}]);this.tabs.push({url:"#settings/profile",name:"Profile settings"});this.$el.html(Mustache.render(Templates.tabview,
