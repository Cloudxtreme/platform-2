...
!1;e.showStreamName=!1;this.datepicker.on("date:change",function(a,b){e.getDataset().setDateRange(a,b)});this.add(e)}});Cloudwalkers.Views.Statistics=Cloudwalkers.Views.Pageview.extend({id:"statistics",start:0,end:0,timespan:"week",period:0,custom:!1,views:[],events:{remove:"destroy","click #add":"addperiod","click #subtract":"subtractperiod","click #subtractempty":"subtractempty","click #now":"now","click #show":"changecustom","change .stats-header select.networks":"changestream","change .stats-header select.time":"changespan","click .dashboard-stat":"updatenetwork"},widgets:[{widget:"StatSummary",data:{columnviews:["contacts",
"score-trending","outgoing","coworkers"]},span:12},{widget:"TitleSeparator",data:{translation:{title:"contacts_info"}}},{widget:"Chart",data:{filterfunc:"contacts",chart:"PieChart",translation:{title:"contacts"},display:"divided"},span:6},{widget:"CompoundChart",span:6,data:{template:"2col1row",chartdata:[{widget:"Chart",data:{filterfunc:"age",chart:"PieChart",translation:{title:"by_age"}}},{widget:"Chart",data:{filterfunc:"gender",chart:"PieChart",translation:{title:"by_gender"}}},{widget:"Chart",
data:{filterfunc:"contact-evolution",chart:"LineChart",translation:{title:"contacts_evolution"}}}]}},{widget:"TitleSeparator",data:{translation:{title:"new_this"}}},{widget:"Info",data:{translation:{title:"contact_evolution"},filterfunc:"contact-evolution"},span:3},{widget:"Info",data:{translation:{title:"post_activity"},filterfunc:"post-activity"},span:3},{widget:"Info",data:{title:"Activity?",filterfunc:"activity"},span:3},{widget:"Info",data:{title:"Page Views?",filterfunc:"page-views"},span:3},
{widget:"TitleSeparator",data:{translation:{title:"messages_info"}}},{widget:"TrendingMessage",data:{translation:{title:"top_rated_comment"}},span:12},{widget:"BestTimeToPost",data:{filterfunc:"besttime",chart:"LineChart",translation:{title:"best_time_to_post"}},span:4},{widget:"Chart",data:{filterfunc:"message-evolution",chart:"LineChart",translation:{title:"messages_evolution"}},span:4},{widget:"HeatCalendar",data:{filterfunc:"activity",translation:{title:"activity_calendar"}},span:4},{widget:"TitleSeparator",
...
timemanager:function(){var a=this.filterparameters(),b=moment.unix(a.since),c=24<b.date()||"quarter"==a.span?11==b.month()?"DD MMM YYYY":"DD MMM":"DD";a.periodstring=this.period||"custom"==a.span?-1==this.period?"last ":"":"this ";a.timeview=b.format(c)+" - "+moment.unix(a.until).format("DD MMM YYYY");a[a.span+"Active"]=!0;a.startstring=moment.unix(a.since).format("DD-MM-YYYY");a.endstring=moment.unix(a.until).format("DD-MM-YYYY");a.fullperiod="pt_PT"==Cloudwalkers.Session.user.attributes.locale?
...
this.hideloading();var a=this.translateString("empty_statistics_data")+'<br/><a id="subtractempty">'+this.translateString("show_last_statistics")+"</a>",a=new Cloudwalkers.Views.Widgets.EmptyData({message:a});this.appendWidget(a,8,null,2)},now:function(){this.period=0;this.trigger("change:period",this.period)},addperiod:function(a){a=$(a.target).attr("disabled");0<=this.period||"disabled"==a||(this.period+=1,this.trigger("change:period",this.period))},subtractperiod:function(a){"disabled"!=$(a.target).attr("disabled")&&
(this.period-=1,this.trigger("change:period",this.period))},subtractempty:function(){this.period-=1;this.trigger("change:period",this.period)},changestream:function(){var a=Number(this.$el.find("select.networks").val());Cloudwalkers.Router.Instance.navigate(a?"#statistics/"+a:"#statistics",{trigger:!0})},changespan:function(){this.timespan=this.$el.find("select.time").val();this.trigger("change:period")},changecustom:function(){this.start=moment(this.$el.find("#start").val(),"DD-MM-YYYY");this.end=
moment(this.$el.find("#end").val(),"DD-MM-YYYY");this.trigger("change:period")},filterparameters:function(){"now"==this.timespan&&(this.period=0);"week"==this.timespan&&(this.start=moment().zone(0).startOf("isoweek"),this.end=moment().zone(0).endOf("isoweek"));"month"==this.timespan&&(this.start=moment().zone(0).startOf("month"),this.end=moment().zone(0).endOf("month"));"year"==this.timespan&&(this.start=moment().zone(0).startOf("year"),this.end=moment().zone(0).endOf("year"));if("quarter"==this.timespan){var a=
3*(this.period+moment().quarter());this.start=moment().startOf("year").add("months",a-3);this.end=moment().startOf("year").add("months",a-1).endOf("month")}else"custom"!=this.timespan&&(0<this.period&&(this.start.add(this.timespan+"s",this.period),this.end.add(this.timespan+"s",this.period)),0>this.period&&(this.start.subtract(this.timespan+"s",Math.abs(this.period)),this.end.subtract(this.timespan+"s",Math.abs(this.period))));return{since:this.start.unix(),until:this.end.unix(),span:this.timespan,
period:this.period,reset:!0}},finish:function(){},updatenetwork:function(a){},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},translateWidgets:function(a){if(a.translation)for(k in a.translation)a[k]=Cloudwalkers.Session.polyglot.t(a.translation[k])},mustacheTranslateRender:function(a){this.original="week month quarter year custom time_spans all_networks now show".split(" ");this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),
a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.StatStream=Cloudwalkers.Views.Statistics.extend({networkspecific:{typeA:{facebook:{filterfunc:"age",chart:"PieChart",title:"By Age"},twitter:{filterfunc:"follow",chart:"PieChart",title:"By Type"},linkedin:{filterfunc:"follow",chart:"PieChart",title:"By Type"},youtube:{filterfunc:"follow",chart:"PieChart",title:"By Type"}},typeB:{facebook:{filterfunc:"gender",chart:"PieChart",title:"By gender"},twitter:{filterfunc:"nodata",chart:"PieChart",title:"No data"},youtube:{filterfunc:"nodata",
