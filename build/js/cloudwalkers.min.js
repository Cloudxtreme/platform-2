...
this.original[k]]=this.translated[k]},mustacheTranslateRender:function(a){this.original="compose dashboard message_board compose_message drafts scheduled inbox messages notifications post_message calendar co-workers_wall company_accounts media trending_posts accounts_we_follow keyword_monitoring manage_accounts manage_keywords reports settings manage_users social_connections account_settings profile_settings manage_user_groups notes sent outbox".split(" ");this.translated=[];for(k in this.original)this.translated[k]=
...
this.renderactions();this.model.get("status")&&"FAILED"==this.model.get("status")&&(this.$el.addClass("failed"),this.model.attributes.failed="failed");return this},renderactions:function(){this.actions=new Cloudwalkers.Views.Actions({message:this.model});this.$el.find(".message-actions").html(this.actions.render().el);this.loadedlists=[];if("messagetimeline"==this.template)this.actions.on("actions:update",this.updatetimelineactions.bind(this))},updatetimelineactions:function(){var a=this.actions.message.notes.length;
...
a+"-list").slideDown()},fetchactions:function(a){var b="note"==a?this.model.notes:this.model.notifications;b.parentmodel=this.model;b.parenttype="message";this.listenTo(b,"seed",this.fillactions.bind(this,a));b.touch(this.model,{records:999});this.loadedlists.push(a+"list")},fillactions:function(a,b,c){c=a+"-list";if(!b.length)return this.$el.find(".action-list li").html("No actions found");this.$el.find("."+c).remove();this.$el.find(".action-lists").append('<ul class="action-list '+c+'"></ul>');
...
a.networks=b.filterNetworks(a.streams,!0);this.mustacheTranslateRender(a);this.$el.html(Mustache.render(Templates.inboxlist,a));this.$container=this.$el.find("ul.list");this.collection.touch(this.model,this.filterparameters());return this},toggle:function(a){var b={model:a.model,template:"sentmessage"};this.inboxmessage&&this.inboxmessage.remove();"notes"==this.collectionstring&&(b.template="note");this.inboxmessage=new Cloudwalkers.Views.Widgets.InboxMessage(b);$(".inbox-container").html(this.inboxmessage.render().el);
...
this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Settings.Account=Backbone.View.extend({events:{"click i[data-delete-campaign-id]":"deletecampaign","click #menu a":"scroll","submit form":"editaccount"},initialize:function(){this.streams=Cloudwalkers.Session.getStreams();this.streams=this.streams.filterNetworks();this.account=Cloudwalkers.Session.getAccount();this.triggers=new Cloudwalkers.Collections.Triggers;this.listenTo(this.triggers,"sync",this.filltriggers);this.account=Cloudwalkers.Session.getAccount();this.triggermodel=
{}},render:function(){var a=this.account.attributes;this.mustacheTranslateRender(a);Cloudwalkers.Session.censuretemplate(a);this.$el.html(Mustache.render(Templates.settings.account,a));this.$el.find("#menu").affix();return this},filltriggers:function(a){var b=a.filter(function(a){if("CONTACT-NEW"==a.get("event"))return a});a=a.filter(function(a){if("MESSAGE-RECEIVED"==a.get("event"))return a});b=new Cloudwalkers.Models.Trigger(b[0].attributes);this.twitterview.updatetrigger(b);a=new Cloudwalkers.Models.Trigger(a[0].attributes);
this.dmview.updatetrigger(a)},action:function(a){this[$(a.currentTarget).data("action")](a)},editaccount:function(a){a=this.$el.find("[data-attribute=account-name]").val();this.$el.find(".edit-account-name").addClass("loading");this.$el.find(".edit-account-name .btn").attr("disabled",!0);this.account.save({name:a},{patch:!0,success:function(){Cloudwalkers.RootView.growl(this.translateString("account_settings"),this.translateString("your_account_settings_are_updated"));window.location.reload()}.bind(this),
error:function(){Cloudwalkers.RootView.growl(this.translateString("account_settings"),this.translateString("there_was_an_error_updating_your_settings"));window.location.reload()}.bind(this)})},deletecampaign:function(a){var b=$(a.target).data("delete-campaign-id");Cloudwalkers.RootView.confirm(this.translateString("are_you_sure_you_want_to_remove_this_campaign"),function(){Cloudwalkers.Session.getAccount().removecampaign(b,a.target)})},scroll:function(a){a=$(a.currentTarget).data("hash");var b=this.$el.find("h3[name="+
a+"]").offset().top,b=b-this.$el.find("h3[name="+a+"]").outerHeight();$("html, body").animate({scrollTop:b},"fast");return!1},negotiateFunctionalities:function(a){this.$el.find(".portlet-title").on("click",function(){$(this).parents(".collapse-closed, .collapse-open").toggleClass("collapse-closed collapse-open")})},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original="campaigns account_plan account name save_changes cancel company_name menu campaigns basic manage_campaigns manage_auto_responders message_templates triggers".split(" ");
this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Settings.Trigger=Backbone.View.extend({events:{"click [data-action=save]":"save","click [data-action=reset]":"reset"},initialize:function(a){$.extend(this,a);this.model||(this.model=new Cloudwalkers.Models.Trigger)},render:function(){this.model.parent=Cloudwalkers.Session.getAccount();var a={message:this.model.getmessage("REPLY"),description:this.description};this.$el.html(Mustache.render(Templates.settings.trigger,a));return this},updatetrigger:function(a){this.model=a;this.render();
