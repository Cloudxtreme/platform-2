...
(a={id:this.id});a.stamp=Math.round(0.0010*(new Date).getTime());Store.set(this.typestring,a);return this},loaded:function(a){return void 0!==this.get(a?a:"objectType")}});
...
b.outdated=!0,b.trigger("outdated"))}},outdated:function(a){if(!a)return this.filter(function(a){return a.outdated});this.updates([a])},touch:function(a,b,c){this.parentmodel=a;this.endpoint=this.modelstring+"ids";this.parameters=b;this.seedparameters=c;this.fetch({success:this.touchresponse.bind(this,this.url())})},touchlocal:function(a){a&&a.modelids.length?(this.cursor=a.cursor,this.seed(a.modelids),this.ready()):this.fetch({success:this.touchresponse.bind(this,this.url())})},touchresponse:function(a,
b,c){b=c[this.parenttype][this.typestring];Store.set("touches",{id:a,modelids:b,cursor:this.cursor,ping:Cloudwalkers.Session.getPing().cursor});this.seed(b)},seed:function(a){a||(a=[]);var b=[];a=_.compact(a.map(function(a){var e=this.get(a);e||(e=Cloudwalkers.Session.user.account[this.typestring].get(a));e?this.add(e):e=this.create({id:a});b.push(e);return a},this));a.length&&(this.endpoint=this.parentmodel?this.typestring:null,this.parameters={ids:a.join(",")},this.seedparameters&&$.extend(this.parameters,
this.seedparameters),this.fetch({remove:!1}));this.trigger("seed",b);return b},more:function(a,b){if(!this.cursor)return!1;b.after=this.cursor;this.touch(a,b);return this},ready:function(){var a=this;setTimeout(function(){a.trigger("ready",a)},1,this)}});Cloudwalkers.Session={langs:[{id:"en_EN",name:"International English"},{id:"fr_FR",name:"Fran\u00e7ais"},{id:"nl_NL",name:"Nederlands"},{id:"pt_PT",name:"Portugu\u00eas"}],user:null,version:"1.0.0.0",localversion:null,isLoaded:function(){return null!=this.user},loadEssentialData:function(a){this.user=new Cloudwalkers.Models.Me;this.getversion();this.user.once("activated",function(){this.setLang()}.bind(this));this.listenTo(this,"translation:done",a);this.user.fetch({error:this.user.offline.bind(this.user)})},
refresh:function(){console.log("Session.refresh triggered")},reset:function(a){var b;a&&(b=window.localStorage.getItem("token"));window.localStorage.clear();a&&b&&window.localStorage.setItem("token",b)},home:function(){Cloudwalkers.Router.Instance.home()},updateSetting:function(a,b,c){Cloudwalkers.Session.user.attributes.settings[a]!=b&&(Cloudwalkers.Session.user.attributes.settings[a]=b,c=$.extend(c,{patch:!0})||{patch:!0},Cloudwalkers.Session.user.save({settings:Cloudwalkers.Session.user.attributes.settings},
c))},get:function(a){return this.user.get("settings")[a]},clone:function(a){if(null==a||"object"!=typeof a)return a;var b=a.constructor(),c;for(c in a)b[c]=this.clone(a[c]);return b},viewsettings:function(a,b){var c="account_"+this.getAccount().id;Cloudwalkers.Session.user.attributes.settings.viewsettings||(Cloudwalkers.Session.user.attributes.settings.viewsettings={});Cloudwalkers.Session.user.attributes.settings.viewsettings[c]||(Cloudwalkers.Session.user.attributes.settings.viewsettings[c]=Cloudwalkers.RootView.navigation.mapViews());
"sent"!=a||Cloudwalkers.Session.user.attributes.settings.viewsettings[c].sent||(Cloudwalkers.Session.user.attributes.settings.viewsettings[c].sent={streams:[]});"profiles"!=a||Cloudwalkers.Session.user.attributes.settings.viewsettings[c].profiles||(Cloudwalkers.Session.user.attributes.settings.viewsettings[c].profiles={streams:[]});"news"!=a||Cloudwalkers.Session.user.attributes.settings.viewsettings[c].news||(Cloudwalkers.Session.user.attributes.settings.viewsettings[c].news={streams:[]});var e=
this.clone(this.get("viewsettings"));if(b){if(a&&b)return e[c][a]=$.extend(e[c][a],b),this.updateSetting("viewsettings",e),e[c][a];throw TypeError("Not the right parameters were met for function viewsettings");}return e[c][a]},manage:function(){200<Store.count("messages")&&Store.filter("messages",null,function(a){a.sort(function(a,c){return a.stamp>c.stamp?1:a.stamp<c.stamp?-1:0});a=a.slice(0,100);Store.write("messages",a);Store.filter("touches",null,function(a){var c=Cloudwalkers.Session.getPing().cursor;
a=a.filter(function(a){return a.ping==c});Store.write("touches",a)})});25<Store.count("reports")&&Store.filter("reports",null,function(a){a.sort(function(a,c){return a.stamp>c.stamp?1:a.stamp<c.stamp?-1:0});a=a.slice(0,10);Store.write("messages",a)})},getversion:function(){Store.get("version",null,function(a){a&&(this.localversion=a.version)}.bind(this))},isupdated:function(){return this.localversion?this.localversion==this.version:!1},isAuthorized:function(a){return a?this.user.isauthorized(a):this.user.authorized},
censuretemplate:function(a){a||(a={});var b=this.getUser().censuretokens;a.authorized=b},getPing:function(){return this.user.account.ping},ping:function(){this.user.account.ping.forceping()},getAccount:function(a){return a?this.user.accounts.get(a):this.user.account},getAccounts:function(a){return this.user.accounts},getChannel:function(a){return"number"==typeof a?this.user.account.channels.get(a):this.user.account.channels.findWhere({type:a})},getChannels:function(a){return this.user.account.channels},
storeChannel:function(a){a.channels&&a.channels.length&&(a.channels=a.channels.map(function(a){Cloudwalkers.Session.storeChannel(a);return a.id}));a.streams&&a.streams.length&&(a.streams=a.streams.map(function(a){Store.post("streams",a);return a.id}));Store.post("channels",a)},getStream:function(a){return"number"==typeof a?this.user.account.streams.get(a):this.user.account.streams.findWhere({token:a})},getStreams:function(){return this.user.account.streams},addStream:function(a){return this.user.account.streams.add(a)},
getUser:function(a){return a?this.user.account.users.get(a):this.user},getUsers:function(a){return a?this.user.account.usersbyrole(a):this.user.account.users},getContact:function(a){return this.user.account.contacts.get(a)},getContacts:function(){return this.user.account.contacts},getMessage:function(a){return this.user.account.messages.get(a)},getMessages:function(){return this.user.account.messages},getCannedResponse:function(a){return this.user.account.cannedresponses.get(a)},getCannedResponses:function(){return this.user.account.cannedresponses},
getNotification:function(a){return this.user.account.notifications.get(a)},getNotifications:function(){return this.user.account.notifications},getStatistic:function(a){return this.user.account.statistics.get(a)},getStatistics:function(){return this.user.account.statistics},getReport:function(a){return this.user.account.reports.get(a)},getReports:function(){return this.user.account.reports},getComment:function(a){return this.user.account.comments.get(a)},getComments:function(){return this.user.account.comments},
setLang:function(a){var b;moment.lang(this.user.attributes.locale);b=new Cloudwalkers.Models.Polyglot;b.fetch({success:function(){this.translationLang=b.get("translation");this.polyglot=new Polyglot({phrases:this.translationLang});this.trigger("translation:done")}.bind(this)})}};_.extend(Cloudwalkers.Session,Backbone.Events);Cloudwalkers.Router=Backbone.Router.extend({routes:{"dashboard/:accountid":"changeaccount","demo(/:demotype)":"demo","inbox/drafts":"ib-drafts","inbox/sent":"ib-sent","inbox/scheduled":"ib-scheduled","inbox/trash":"ib-trash",write:"write",share:"share","inbox(/:type)(/:streamid)":"inbox",drafts:"drafts","outbox(/:type)":"outbox",notes:"notes",scheduled:"scheduled",calendar:"calendar",coworkers:"coworkers","channel/:channel(/:subchannel)(/:stream)(/:messageid)":"channel","timeline/:channel(/:stream)":"timeline",
"trending/:channel(/:subchannel)(/:stream)(/:messageid)":"trending","monitoring/accounts":"manageaccounts","monitoring/:channel(/:subchannel)(/:messageid)":"monitoring","monitoring/:channel(/:subchannel)/":"monitoring",keywords:"managekeywords","reports/:streamid":"reports",statistics:"statistics","statistics/:streamid":"statistics","settings(/:sub)(/:service)":"settings",firsttime:"firsttime",work:"coworkdashboard",resync:"resync",home:"home",logout:"home","*path":"dashboard"},initialize:function(){},
demo:function(a){var b=Cloudwalkers.Session.getChannel("inbox");Cloudwalkers.RootView.setView(new Cloudwalkers.Views.Demo({channel:b,type:"messages",demotype:a}))},dashboard:function(){if(Cloudwalkers.Session.getAccount().get("firstTime"))return this.navigate("#firsttime",!0);if(!Cloudwalkers.Session.isAuthorized("_CW_INBOX_VIEW"))return this.navigate("#work",!0);Cloudwalkers.RootView.setView(new Cloudwalkers.Views.Dashboard)},changeaccount:function(a){a!=Cloudwalkers.Session.get("currentAccount")&&
Cloudwalkers.Session.updateSetting("currentAccount",a,{success:this.home.bind(this,!0)})},write:function(){Cloudwalkers.RootView.compose()},share:function(){Cloudwalkers.RootView.viewshare("general")},coworkers:function(){var a=new Cloudwalkers.Views.Coworkers;this.validate(a,"_CW_COWORKERS_VIEW")},"ib-drafts":function(){Cloudwalkers.RootView.setView(new Cloudwalkers.Views.IB({channeltype:"draft",filter:"editors",title:"Drafts"}))},inbox:function(a,b){var c={MESSAGE_READ_INBOX_NOTIFICATIONS:"#messages/notifications",
MESSAGE_READ_INBOX_SCHEDULE:"#scheduled",MESSAGE_READ_DRAFTS:"#drafts",ACCOUNT_NOTES_VIEW:"#notes"},e=_.intersection(_.keys(c),Cloudwalkers.Session.getUser().authorized),f=Cloudwalkers.Session.getChannel("inbox");if(!f||!e||!e.length)return this.home();a||(a="messages");if(!Cloudwalkers.Session.isAuthorized("MESSAGE_READ_INBOX_"+a.toUpperCase())){if(Cloudwalkers.Session.getUser().authorized&&e.length)return this.navigate(c[e[0]],{trigger:!0});window.location="/"}Cloudwalkers.RootView.setView(new Cloudwalkers.Views.Inbox({channel:f,
type:a,streamid:b}))},drafts:function(){var a=new Cloudwalkers.Views.Drafts;this.validate(a,"MESSAGE_READ_DRAFTS")},outbox:function(a){a=new Cloudwalkers.Views.Sent;this.validate(a,"MESSAGE_READ_DRAFTS")},notes:function(){var a=new Cloudwalkers.Views.Notes;this.validate(a,"ACCOUNT_NOTES_VIEW")},scheduled:function(){var a=new Cloudwalkers.Views.Scheduled;this.validate(a,"MESSAGE_READ_SCHEDULE")},calendar:function(){Cloudwalkers.RootView.setView(new Cloudwalkers.Views.Calendar)},timeline:function(a,
b){var c=b?Cloudwalkers.Session.getStream(Number(b)):Cloudwalkers.Session.getChannel(Number(a)),e=Cloudwalkers.Session.getAccount();e.channels.findWhere({type:"news"})&&e.channels.findWhere({type:"news"});e=e.channels.findWhere({type:"profiles"})?e.channels.findWhere({type:"profiles"}).id:null;e=a==e?"company":"thirdparty";c=new Cloudwalkers.Views.Timeline({model:c,showcontact:"thirdparty"==e,parameters:{records:40,markasread:!0}});e=e=="MESSAGE_READ_"+e.toUpperCase();this.validate(c,e)},trending:function(a,
b){var c=Cloudwalkers.Session.getChannel(Number(a)),e=b?Cloudwalkers.Session.getStream(Number(b)):c,c="news"==c.get("type")?1:7,c={since:3600*Math.round(Date.now()/36E5)-86400*c,sort:"engagement",records:40,markasread:!0},e=new Cloudwalkers.Views.Timeline({model:e,trending:!0,parameters:c});this.validate(e,"MESSAGE_READ_THIRDPARTY")},manageaccounts:function(){var a=Cloudwalkers.Session.getChannel("news"),a=new Cloudwalkers.Views.ManageAccounts({channel:a});Cloudwalkers.RootView.setView(a)},monitoring:function(a,
b,c){if(!b)return this.navigate("#keywords",!0);a=new Cloudwalkers.Views.KeywordMonitoring({category:Cloudwalkers.Session.getChannel(Number(b))});this.validate(a,"MESSAGE_READ_MONITORING")},managekeywords:function(){var a=new Cloudwalkers.Views.ManageKeywords;this.validate(a,"CHANNEL_MANAGE_EDIT_MONITORING")},reports:function(a){var b=new Cloudwalkers.Views.Reports({stream:Cloudwalkers.Session.getStream(Number(a))});a&&(b.subnavclass="reports_"+a);this.validate(b,"STATISTICS_VIEW")},statistics:function(a){a=
a?new Cloudwalkers.Views.StatStream({streamid:a}):new Cloudwalkers.Views.Statistics;this.validate(a,"STATISTICS_VIEW")},settings:function(a,b){var c;"users"==a&&(c="USER_INVITE");"services"==a&&(c="SERVICE_CONNECT");"account"==a&&(c="CAMPAIGN_DELETE");var e=new Cloudwalkers.Views.Settings({endpoint:a,serviceid:b});this.validate(e,c)},firsttime:function(){Cloudwalkers.RootView.setView(new Cloudwalkers.Views.Firsttime)},coworkdashboard:function(){Cloudwalkers.RootView.setView(new Cloudwalkers.Views.Coworkdashboard)},
manageusergroups:function(){Cloudwalkers.RootView.setView(new Cloudwalkers.Views.ManageUserGroups)},resync:function(a){this.navigate("#resync");Cloudwalkers.RootView.setView(new Cloudwalkers.Views.Resync({returnto:a}))},home:function(a){a?window.location="/":$.ajax({url:config.authurl+"revoke",headers:{Authorization:"Bearer "+Cloudwalkers.Session.authenticationtoken,Accept:"application/json"},success:function(){window.location="/"}});Cloudwalkers.RootView.view.remove();Cloudwalkers.RootView.navigation.remove();
Cloudwalkers.Session.reset(a);return!1},validate:function(a,b){Cloudwalkers.Session.isAuthorized&&Cloudwalkers.Session.isAuthorized(b)?Cloudwalkers.RootView.setView(a):this.home()},exception:function(a){var b="/";switch(a){case 401:case 503:b="/503.html";default:window.location=b}},rssfeed:function(){Cloudwalkers.RootView.setView(new Cloudwalkers.Views.RSSFeed)},managerss:function(){console.log("Manage RSS");Cloudwalkers.RootView.setView(new Cloudwalkers.Views.ManageRSS)}});Cloudwalkers.Session.Ping=Backbone.Model.extend({interval:3E4,min:1E4,max:9E4,timeout:0,cursor:!1,parameters:{},initialize:function(){Store.exists("ping",{id:this.id})||Store.post("ping",{id:this.id});this.setCursor(!1);this.fetch({success:this.schedule.bind(this),error:this.toing.bind(this)});this.on("change:paging",this.setCursor,this);this.on("change:updates",this.updateModels,this);this.on("change:add",this.addModels,this);this.on("change:remove",this.removeModels,this);this.listenTo(Cloudwalkers.Session,
...
Backbone.history.fragment);a=a.filter(function(a){return a.id==b});return a.length?a[0]:null}});Cloudwalkers.Models.Contact=Cloudwalkers.Models.User.extend({typestring:"contacts",modelstring:"contact",url:function(){var a=[Cloudwalkers.Session.api];this.parent?a.push(this.parent.typestring,this.parent.id,this.typestring,this.id):this.id?a.push("accounts",Cloudwalkers.Session.getAccount().id,this.typestring,this.id):a.push("accounts",Cloudwalkers.Session.getAccount().id,this.typestring);this.endpoint&&a.push(this.endpoint);this.urlparams&&(a=a.concat(this.urlparams));a=a.join("/");return this.parameters?
a+"?"+$.param(this.parameters):a},sync:function(a,b,c){"read"==a&&c.endpoint&&(this.endpoint=c.endpoint);return Backbone.sync(a,b,c)},parse:function(a){a[this.modelstring]&&(a=a[this.modelstring]);this.endpoint&&a[this.endpoint]&&(a=a[this.endpoint]);"number"==typeof a?a={id:a}:this.stamp(a);return a},initialize:function(a){this.gender=this.get("gender")},getMales:function(){return this.gender?this.gender.male:0},getFemales:function(){return this.gender?this.gender.female:0},getOthers:function(){return this.gender?
this.gender.other:0},getGender:function(){return this.gender?this.gender:!1},getAge:function(){return this.age?this.age:!1}});Cloudwalkers.Models.Contact.actions={templates:{dm:{name:"Direct message",icon:"message",type:"dialog"}}};Cloudwalkers.Models.Note=Backbone.Model.extend({typestring:"notes",type_settings:{CONTACT:{icon:"user",model:"Contact",typestring:"contacts"},MESSAGE:{icon:"inbox",model:"Message",typestring:"messages"},ACCOUNT:{icon:"edit",model:"Account",typestring:"accounts"}},type_settings:{CONTACT:{icon:"user",model:"Contact",typestring:"contacts"},MESSAGE:{icon:"inbox",model:"Message",typestring:"messages"},ACCOUNT:{icon:"edit",model:"Account",typestring:"accounts"}},initialize:function(){this.on("action",this.action)},
parse:function(a){if("number"==typeof a)return{id:a};a=a.note?a.note:a;a.date&&(a.fulldate=moment(a.date).format("DD MMM YYYY HH:mm"),a.dateonly=moment(a.date).format("DD MMM YYYY"),a.time=moment(a.date).format("HH:mm"),a.type_icon=this.type_settings[a.model.objectType].icon,a[a.model.objectType]=!0,a.model&&(a.objectType="note"));a.intro=a.text?a.text.substr(0,72):" ";return a},attachParent:function(a,b){a=this.type_settings[a].model;var c=Cloudwalkers.Session["get"+a](b);c&&c.get("objectType")||
(c=new Cloudwalkers.Models[a]({id:b}),c.fetch());return c},url:function(){var a=[Cloudwalkers.Session.api];this.id?a.push(this.typestring,this.id):this.parent?"contacts"!=this.parent.typestring?a.push(this.parent.typestring,this.parent.id,this.typestring):"contacts"==this.parent.typestring&&(a.push("accounts/"+Cloudwalkers.Session.getAccount().id+"/contacts"),a.push(this.parent.id),a.push(this.typestring)):a.push(this.typestring);a=a.join("/");return this.parameters?a+"?"+$.param(this.parameters):
a},action:function(a){"delete"==a&&this.deletenote()},deletenote:function(){var a=this;Cloudwalkers.RootView.confirm("Are you sure you want to remove this note?",function(){a.destroy({success:function(){a.trigger("destroy");a.trigger("destroyed")}})})}});Cloudwalkers.Models.Tag=Backbone.Model.extend({typestring:"tags",initialize:function(a){a&&$.extend(this,a);this.on("action",this.action)},parse:function(a){a=a.tag?a.tag:a;a.date&&(a.fulldate=moment(a.date).format("DD MMM YYYY HH:mm"),a.dateonly=moment(a.date).format("DD MMM YYYY"),a.time=moment(a.date).format("HH:mm"));return a},url:function(){var a=[Cloudwalkers.Session.api];"contacts"==this.parent.typestring&&a.push("accounts/"+Cloudwalkers.Session.getAccount().id);"tags"==this.typestring&&this.id?
a.push(this.parent.typestring,this.parent.id,this.typestring,this.id):this.parent.typestring&&a.push(this.parent.typestring,this.parent.id,this.typestring);a=a.join("/");return this.parameters?a+"?"+$.param(this.parameters):a},deletetag:function(){var a=this;Cloudwalkers.RootView.confirm("Are you sure you want to remove this tag?",function(){a.destroy({success:function(){a.trigger("destroytag")}})})}});Cloudwalkers.Models.Me=Cloudwalkers.Models.User.extend({unreadMessages:null,initialize:function(a){this.once("change",this.activate);Store.exists("me")&&Store.remove("me");Store.exists("channels")&&Store.remove("channels");this.on("change:id",function(a){this.previous("id")&&Cloudwalkers.Session.home()})},url:function(){var a=this.endpoint?this.endpoint:Store.exists("me")?"?include_accounts=ids":"";return Cloudwalkers.Session.api+"/user/me"+a},parse:function(a){Store.exists("me")||(a.user=this.firstload(a.user));
Store.write("me",[a.user]);this.reload&&(this.reload=!1);return a.user},sync:function(a,b,c){c.headers={Authorization:"Bearer "+Cloudwalkers.Session.authenticationtoken,Accept:"application/json"};this.endpoint=c.endpoint?"/"+c.endpoint:!1;"read"==a&&Store.get("me",null,function(a){a&&this.set(a)}.bind(this));return Backbone.sync(a,b,c)},firstload:function(a){$.each(a.accounts,function(a,c){Store.set("accounts",c)}.bind(this));a.settings.currentAccount||(a.settings={currentAccount:a.accounts[0].id});
return a},activate:function(a){if(!this.get("accounts").length)return Cloudwalkers.Router.Instance.exception(401);Store.filter("accounts",null,function(a){this.accounts=new Cloudwalkers.Collections.Accounts(a);this.account=this.getCurrentAccount();this.account.activate();this.set("level",Number(this.account.get("currentuser").level));this.set("rolegroup",Number(this.account.get("currentuser").rolegroup));this.authorized=this.account.get("currentuser").authorized;this.removerole("ACCOUNT_TAGS_VIEW");
this.removerole("ACCOUNT_TAGS_MANAGE");this.parseauthorized();this.censuretokens=this.censure(this.authorized);this.trigger("activated")}.bind(this))},isauthorized:function(a){return _.isArray(a)?0!=_.intersection(this.authorized,a).length:_.contains(this.authorized,a)},removerole:function(a){a=this.authorized.indexOf(a);0<=a&&this.authorized.splice(a,a+1)},censure:function(a){var b={};for(n in a)b[a[n]]=!0;return b},parseauthorized:function(){this.isauthorized(["MESSAGE_OUT_EDIT_OWN","MESSAGE_ACTIONS"])&&
this.authorized.push("_CW_COWORKERS_VIEW");this.isauthorized(["MESSAGE_READ_INBOX_MESSAGES","MESSAGE_READ_INBOX_NOTIFICATIONS","MESSAGE_READ_SCHEDULE","MESSAGE_READ_DRAFTS","ACCOUNT_NOTES_VIEW"])&&this.authorized.push("_CW_INBOX_VIEW")},offline:function(){Cloudwalkers.Session.reset();window.location="/login.html"},getCurrentAccount:function(){var a=Cloudwalkers.Session.get("currentAccount");a||(a=this.accounts.at(0).id,this.save({settings:{currentAccount:a}}));a=this.accounts.get(Number(a));return a&&
a.id?a:(this.save({settings:{currentAccount:this.accounts.at(0).id}}),Cloudwalkers.Session.home())}});Cloudwalkers.Models.Stream=Backbone.Model.extend({parameters:{},initialize:function(a){this.messages=new Cloudwalkers.Collections.Messages;this.contacts=new Cloudwalkers.Collections.Contacts;this.get("statistics")&&(this.reports=new Cloudwalkers.Collections.Reports,this.reports.streamid=this.id)},outdated:function(){this.fetch()},update:function(){this.fetch()},url:function(){return Cloudwalkers.Session.api+"/streams/"+(this.id?this.id:"")+this.endpoint+this.parameters},parse:function(a){a.stream&&
...
"/accounts/"+Cloudwalkers.Session.getAccount().id+"/"+this.typestring)},parse:function(a){if("number"==typeof a)return{id:a};a.message&&(a=a.message);a=this.filterData(a);this.stamp(a);this.checkloaded(a);return a},sync:function(a,b,c){c.headers={Authorization:"Bearer "+Cloudwalkers.Session.authenticationtoken,Accept:"application/json"};this.endpoint=c.endpoint?"/"+c.endpoint:!1;return"update"==a?!1:Backbone.sync(a,b,c)},updatecollection:function(a){a.updated=!0},updateactions:function(a){(a=a.get("actionresult")?
a.get("actionresult").models.update[0]:null)&&$.extend(this.attributes,a)},validateCustom:function(a){var b;if(!this.hascontent()&&0>$.inArray("content",a))return this.translateString("you_need_a_bit_of_content");if(!this.validatecontent())return this.translateString("one_or_more_networks_exceed_the_character_limit");if(!this.get("streams").length&&0>$.inArray("streams",a))return this.translateString("please_select_a_network_first");if((b=this.validateschedules())&&0>$.inArray("schedule",a))return b},
validateschedules:function(){var a;if(a=this.validateschedule(this.get("schedule")))return a;$.each(this.get("variations"),function(b,c){return(a=this.validateschedule(c.schedule))?!1:!0}.bind(this));return a},validateschedule:function(a){if(!a)return!1;var b=a.date||null;a=a.repeat?a.repeat.until:null;return b&&b<moment().unix()?"One or more streams are scheduled into the past.":b&&a&&a<b?"One or more streams have the repeat date happening before the scheduled date.":!b&&a&&a<moment().unix()?"One or more streams have the repeat date set to the past.":
!1},hascontent:function(){var a;if(this.get("attachments")&&this.get("attachments").length)return this.get("attachments").length;if(this.get("body").plaintext)return this.get("body").plaintext.length;if(this.get("variations")&&this.get("variations").length)return $.each(this.get("variations"),function(b,c){if(c.attachments&&c.attachments.length)return a=c.attachments.length,!1;if(c.body&&c.body.plaintext)return a=c.body.plaintext.length,!1}.bind(this)),a},validatecontent:function(){var a=1,b;this.get("streams")&&
this.get("streams").length&&$.each(this.get("streams"),function(c,e){var f=Cloudwalkers.Session.getStream(e).get("network"),f=f.limitations["max-length"]?f.limitations["max-length"].limit:null;if(!f)return!0;if(b=this.getvariation(e)){if(b.body&&b.body.plaintext)return a=f-b.body.plaintext.length,0>a?!1:!0}else if(this.get("body")&&this.get("body").plaintext&&(a=f-this.get("body").plaintext.length,0>a))return!1}.bind(this));return 0<a},sanitizepost:function(){this.attributes.body.html&&delete this.attributes.body.html;
this.attributes.variations&&this.attributes.variations.forEach(function(a,b){a.body&&(a.body.plaintext?a.body.html&&delete a.body.html:delete a.body);0>this.attributes.streams.indexOf(a.stream)&&this.attributes.variations.splice(b,1)}.bind(this))},cloneSanitized:function(a){var b=new Cloudwalkers.Models.Message;$.extend(!0,b,this);b.attributes.date&&delete b.attributes.date;b.attributes.from&&delete b.attributes.from;b.attributes.stream&&!a&&delete b.attributes.stream;a||(b.attributes.streams=[]);
b.id&&(delete b.id,delete b.attributes.id);return b},checkloaded:function(a){var b=this;a.objectType&&setTimeout(function(){b.trigger("loaded")},1,this)},filterData:function(a){a.message&&a.message.body&&(a=a.message);var b={};$.each("id objectType actiontokens subject body date engagement from read stream streams attachments parent statistics stats status canHaveChildren children_count schedule variations url".split(" "),function(c,f){void 0!==a[f]&&(b[f]=a[f])});var c=Cloudwalkers.Session.getStream(a.stream);
c&&(b.icon=c.get("network").icon,b.networktoken=c.get("network").token,b.networkdescription=c.get("name"));a.attachments?(a.attachments.length&&(b.media=a.attachments[a.attachments.length-1],b.media="image"==b.media.type?"picture":b.media.type),b.attached={},$.each(a.attachments,function(a,c){b.attached[c.type]=c})):b.media="reorder";a.engagement&&(b.trending=1E3>a.engagement?a.engagement:"+999");b.schedule&&(b.scheduledate=moment(b.schedule.date).format("DD MMM YYYY HH:mm"));b.body.intro=a.body.plaintext?
a.body.plaintext.substr(0,72):" ";a.date&&(b.fulldate=moment(a.date).format("DD MMM YYYY HH:mm"),b.dateonly=moment(a.date).format("DD MMM YYYY"),b.time=moment(a.date).format("HH:mm"));return b},generateintro:function(){this.get("body")&&!this.get("body").intro&&(this.attributes.body.intro=this.get("body").plaintext.substr(0,72))},filterActions:function(){if(!this.get("actiontokens"))return[];var a=this.actions.rendertokens();a.map(function(a){"note"==a.token&&this.notes.updated&&(a.value=this.notes.length);
return a}.bind(this));return a},filterCalReadable:function(){var a=this.get("objectType"),b=a&&"reorder"!=this.get("media");this.calNode||(this.calNode={id:this.id});this.get("schedule")?this.calNode.start=a?$.fullCalendar.moment(this.get("scheduledate")):$.fullCalendar.moment():this.calNode.start=a?$.fullCalendar.moment(this.get("date")):$.fullCalendar.moment();this.calNode.title=a?(this.get("title")?this.get("title"):this.get("body").plaintext).substring(0,b?12:16):"...";this.calNode.className=
a?this.get("networktoken")+"-color":"hidden";this.calNode.networkdescription=a?this.get("networkdescription"):null;this.calNode.intro=a?this.get("body").intro:null;this.calNode.icon=a?this.get("icon"):null;this.calNode.media=b?this.get("media"):null;a&&72<=this.calNode.intro.length&&(this.calNode.intro+="...");return this},attach:function(a,b,c){c=this.get("attachments")||[];a=b&&c[b]?$.extend(c[b],a):c.push(a);this.set({attachments:c});return a},unattach:function(a){(this.get("attachments")||[]).splice(a,
1)},addcampaign:function(){},setvariation:function(a,b,c){var e=this.get("variations")||[],f=e.filter(function(b){if(b.stream==a)return b});if(0==f.length){f={stream:a};if("image"==b||"link"==b){var g=[c];f.attachments=g}else b&&(f[b]=c);e.push(f)}else f=f[0],"image"==b||"link"==b?(g=f.attachments||[],0==g.length&&"image"==b?(g.push(c),f.attachments=g):"image"==b?g.push(c):(b=g.filter(function(a){if("link"==a.type)return a}),0<b.length?b[0].url=c.url:0!=g.length?f.attachments.push(c):f.attachments=
[c])):f[b]=c;return f},getvariation:function(a,b){var c=this.get("variations")||[],c=_.findWhere(c,{stream:a});if("image"==b||"link"==b){if(c&&c.attachments)return c.attachments.filter(function(a){if(a.type==b)return a})}else{if(c&&c[b])return c[b];if(c&&!b)return c}},original:function(a,b,c){var e=this.get("variations")?this.get("variations").filter(function(b){return a&&b.stream==a.id}):[],f=a?e.length?e[0]:{stream:a.id}:this.attributes;void 0!==c&&(f[b]=c,a&&!e.length&&this.get("variations").push(f));
return f[b]},removevariation:function(a){var b=this.get("variations");b&&$.each(b,function(c,e){if(e.stream==a)return b.splice(c,1),!1})},removevarimg:function(a,b){var c=this.getvariation(a),c=c?c.attachments:[];if(_.isNumber(b))this.addexclude(a,b);else for(n in c)"image"==c[n].type&&c[n].name==b&&c.splice(n,1)},addexclude:function(a,b){var c=this.getvariation(a)||this.setvariation(a);c.excludes?c.excludes.attachments.push(b):c.excludes={attachments:[b]}},checkexclude:function(a,b){var c=this.getvariation(a);
if(c)c=c.excludes;else return!1;if(c)for(n in c.attachments)if(c.attachments[n]==b)return!0;return!1},location:function(){return"#"},afterChange:function(){this.addInternalActions();this.get("parent")&&this.get("parentmodel").set(this.get("parent"))},getActionIcon:function(a){return"internal-share"==a.token?"share-alt":"internal-edit"==a.token?"edit":"internal-delete"==a.token||"delete"==a.token?"remove":"like"==a.token?"thumbs-up":"unlike"==a.token?"thumbs-down":"comment"==a.token?"comment-alt":
"reply"==a.token?"comment":"retweet"==a.token?"retweet":"dm"==a.token?"envelope":"internal-reply"==a.token?"comment":"favorite"==a.token?"star":"unfavorite"==a.token?"star-empty":a.token},addInternalActions:function(){var a=this;if("undefined"!=typeof this.attributes.actions)for(var b=0;b<this.attributes.actions.length;b++){if("internal-"==this.attributes.actions[b].token.substr(0,9))return}else this.attributes.actions=[];"INCOMING"==this.attributes.type?this.attributes.actions.push({name:"Share",
parameters:[],token:"internal-share",callback:function(a){Cloudwalkers.RootView.shareMessage(a)}}):"OUTGOING"==this.attributes.type&&(this.attributes.actions.push({name:"Edit",parameters:[],token:"internal-edit",callback:function(b){Cloudwalkers.RootView.editMessage(a)}}),this.attributes.actions.push({name:"Delete",parameters:[],token:"internal-delete",callback:function(b){a.deleteMessage()}}));var c=this.get("parentmodel");if("undefined"!=typeof c&&c){var e=c.get("actions");if(e)for(b=0;b<e.length;b++)"comment"==
e[b].token&&this.attributes.actions.push({name:"Reply",token:"internal-reply",target:c,originalaction:e[b]})}if("undefined"!=typeof this.attributes.actions)for(b=0;b<this.attributes.actions.length;b++)this.attributes.actions[b].icon=this.getActionIcon(this.attributes.actions[b])},deleteMessage:function(){this.repeat().repeat?Cloudwalkers.RootView.dialog(this.translateString("are_you_sure_you_want_to_remove_this_message"),[{label:this.translateString("skip_once"),description:this.translateString("message_will_be_skipped_once"),
token:"skip"},{label:this.translateString("delete_forever"),description:this.translateString("message_will_never_be_repeated"),token:"remove"}],this.deletetype.bind(this)):Cloudwalkers.RootView.confirm(this.translateString("are_you_sure_you_want_to_remove_this_message"),function(){this.destroy({success:this.destroysuccess.bind(this)})}.bind(this))},deletetype:function(a){"skip"==a.token&&this.save({trigger:Math.random()},{patch:!0,endpoint:"skip",success:this.skipwatcher.bind(this)});"remove"==a.token&&
this.destroy({wait:!0,success:this.destroysuccess.bind(this)})},destroysuccess:function(){this.trigger("destroyed",this,this.collection);this.trigger("destroy",this,this.collection)},skipwatcher:function(a){"REMOVED"==a.get("status")&&this.destroysuccess()},humandate:function(a){a=new Date(a?a:this.get("date"));return Cloudwalkers.Utils.longdate(a)},shortdate:function(){var a=this.date();return Cloudwalkers.Utils.shortdate(a)},time:function(){var a=this.date();return Cloudwalkers.Utils.time(a)},date:function(){return new Date(this.get("date"))},
getAction:function(a){for(var b=this.get("actions"),c=0;c<b.length;c++)if(b[c].token==a)return b[c];return null},act:function(a,b,c){Cloudwalkers.RootView.growl(a.name,this.translateString("the")+" "+a.token+" "+this.translateString("is_planned_with_success"));var e=this;"undefined"==typeof b&&(b={});a={actions:[{token:a.token,parameters:b}]};b=CONFIG_BASE_URL+"json/message/"+this.get("id")+"?account="+Cloudwalkers.Session.getAccount().get("id");jQuery.ajax({data:JSON.stringify(a),dataType:"json",
type:"put",url:b,processData:!1,cache:!1,success:function(a){a.removed?(e.trigger("destroy",e,e.collection),e.trigger("destroyed",e,e.collection)):e.set(a.message);c()}})},scheduledate:function(a){"undefined"==typeof a&&(a=!0);var b=this.get("schedule");return b?a?"ASAP"==b.date?b.date:this.humandate(b.date):"ASAP"==b.date?!1:new Date(b.date):!1},calculateIntervalFromSeconds:function(a){for(var b={},c=[{unit:"minutes",value:60},{unit:"hours",value:3600},{unit:"days",value:86400},{unit:"weeks",value:604800},
{unit:"months",value:2678400}],e=c.length-1;0<=e;e--)if(0==a%c[e].value)return b.interval=Math.round(a/c[e].value),b.unit=c[e].unit,b;for(e=0;e<c.length;e++)if(72>a/c[e].value)return b.interval=Math.round(a/c[e].value),b.unit=c[e].unit,b;return!1},repeat:function(){var a=this.get("schedule"),b={repeat:!1,weekdays:{}};if("undefined"!=typeof a&&"undefined"!=typeof a.repeat){b.repeat=!0;var c=this.calculateIntervalFromSeconds(a.repeat.interval);c&&(b.interval=c.interval,b.unit=c.unit);b.end=null;a.repeat.end&&
(b.end=new Date(a.repeat.end));if(a.repeat.weekdays)for(c=0;c<a.repeat.weekdays.length;c++)b.weekdays[a.repeat.weekdays[c]]=!0}return b},getStream:function(){return this.get("stream")?Cloudwalkers.Session.getAccount().streams.get(this.get("stream")):null},getProcessedAttachments:function(){var a=[],b;if("undefined"!=typeof this.attributes.attachments)for(var c=0;c<this.attributes.attachments.length;c++)b=this.attributes.attachments[c],"link"==b.type?null!=this.attributes.body.plaintext&&-1!==this.attributes.body.plaintext.indexOf(b.url)||
a.push(b):a.push(b);return a},hasAttachement:function(a){if("undefined"!=typeof this.attributes.attachments){for(var b=0;b<this.attributes.attachments.length;b++)if(attachment=this.attributes.attachments[b],attachment.type==a)return this.attributes.attachments[b];return!1}},setRead:function(){if(!this.get("read")){var a=CONFIG_BASE_URL+"json/message/"+this.get("id")+"/read/";jQuery.ajax({type:"get",url:a,success:function(a){}})}},shortBody:function(){var a=this.get("body");return a.plaintext?{html:a.plaintext.substring(0,
...
b.ids.join(" ");e.push(b)});return e},filterReportStreams:function(){return{}}});Cloudwalkers.Collections.Messages=Backbone.Collection.extend({model:Cloudwalkers.Models.Message,typestring:"messages",modelstring:"message",parameters:{},paging:{},cursor:!1,events:{remove:"destroy"},initialize:function(a){a&&$.extend(this,a);Cloudwalkers.Session.user.account&&Cloudwalkers.Session.getMessages().listenTo(this,"add",Cloudwalkers.Session.getMessages().distantAdd);this.on("sync",function(){setTimeout(function(){this.isempty()}.bind(this),1)})},destroy:function(){this.reset()},isempty:function(){0==
this.length&&this.trigger("ready:empty")}});Cloudwalkers.Collections.CannedResponses=Backbone.Collection.extend({model:Cloudwalkers.Models.CannedResponse,touched:!1,initialize:function(){Cloudwalkers.Session.user.account&&Cloudwalkers.Session.getCannedResponses().listenTo(this,"add",Cloudwalkers.Session.getCannedResponses().distantAdd)},url:function(){var a=Cloudwalkers.Session.api+"/streams/"+Cloudwalkers.Session.getAccount().id+":canned/messages";return this.parameters?a+"?"+$.param(this.parameters):a},sync:function(a,b,c){c.headers={Authorization:"Bearer "+
Cloudwalkers.Session.authenticationtoken,Accept:"application/json"};this.parameters=c.parameters;return"update"==a?!1:Backbone.sync(a,b,c)},parse:function(a){return a.stream.messages},store:function(a,b){"delete"==a&&Store.remove("campaigns",{id:b.id})},removecanned:function(a){var b;$.each(this.models,function(c,e){if(e.id==a)return b=this.models.splice(c,1),!1}.bind(this));if(b)return b}});Cloudwalkers.Collections.Related=Cloudwalkers.Collections.Messages.extend({typestring:"related",modelstring:"related",parenttype:"message"});Cloudwalkers.Collections.RelatedNotes=Cloudwalkers.Collections.Notes.extend({typestring:"related",modelstring:"related",parenttype:"note"});Cloudwalkers.Collections.Notifications=Cloudwalkers.Collections.Messages.extend({model:Cloudwalkers.Models.Notification,typestring:"notifications",modelstring:"notification",initialize:function(){Cloudwalkers.Session.user.account&&Cloudwalkers.Session.getNotifications().listenTo(this,"add",Cloudwalkers.Session.getNotifications().distantAdd);this.on("destroy",this.destroy);this.on("sync",function(){setTimeout(function(){this.isempty()}.bind(this),1)})}});Cloudwalkers.Collections.Actions=Backbone.Collection.extend({model:Cloudwalkers.Models.Action,typestring:"actions",modelstring:"action",token:"",templates:{share:{name:"Share",icon:"share-alt",token:"share",type:"write",maxsize:{twitter:140},clone:!0,redirect:!1,valuetag:"shares"},"delete":{name:"Delete",icon:"remove",token:"delete",type:"confirm"},edit:{name:"Edit",icon:"edit",token:"edit",type:"edit",redirect:!1},note_view:{name:"Note",icon:"edit",token:"note",type:"note",compound:"note",valuetag:"notes",
hidemetoken:"hidden"},note_manage:{name:"Add note",icon:"edit",token:"note",type:"note",compound:"note"},tag:{name:"tag",icon:"edit",token:"tag",type:"tag"},resend:{name:"Resend",icon:"arrow-up",token:"resend",type:"write"},reply:{name:"Reply",icon:"comments-alt",token:"reply",type:"write",clone:!0,parameters:[{token:"message",name:"Message",type:"string",required:!1,value:"@{{from.name}} "}]},dm:{name:"DM",icon:"comments-alt",token:"dm",type:"dialog",clone:!0,parameters:[{token:"message",name:"Message",
type:"string",required:!1,value:""}]},comment:{name:"Comment",icon:"comment",token:"comment",type:"dialog",clone:!0,compound:"comment",valuetag:"comments",tokenview:"comment-list",maxsize:{twitter:140},parameters:[{token:"message",name:"Message",type:"string",required:!1,value:""}]},commentlist:{name:"Comment",icon:"comment",token:"comment",type:"dialog",clone:!0,compound:"comment",valuetag:"comments",tokenview:"comment-list",noaction:!0},retweet:{name:"Retweet",icon:"retweet",token:"retweet",type:"options",
valuetag:"retweets"},like:{name:"Like",icon:"thumbs-up",token:"like",type:"options",toggle:"unlike",valuetag:"likes"},unlike:{name:"Unlike",icon:"thumbs-down",token:"unlike",type:"growl",toggle:"like",actiontype:"like"},favorite:{name:"Favorite",icon:"star",token:"favorite",type:"options",toggle:"unfavorite",valuetag:"favourites"},unfavorite:{name:"Unfavorite",icon:"star-empty",token:"unfavorite",type:"growl",toggle:"favorite",actiontype:"favorite"},plusone:{name:"Unfavorite",icon:"google-plus-sign",
token:"plusone",type:"options",toggle:"unplusone"},unplusone:{name:"Unfavorite",icon:"google-plus-sign",token:"unplusone",type:"growl",toggle:"plusone",actiontype:"plusone"}},initialize:function(a,b){b&&$.extend(this,b);this.listenTo(this.parent,"action",this.startaction)},parse:function(a){return a},url:function(){var a=this.parameters?"?"+$.param(this.parameters):"",b=this.parent?this.parent.get("objectType")+"s/"+this.parent.id:"";return Cloudwalkers.Session.api+"/"+b+"/actions"+a},rendertokens:function(a){var b,
c=[],e=this.parent.get("stats");a||(a=this.parent.get("actiontokens"));this.parent.get("canHaveChildren")&&0>a.indexOf("comment")&&a.push("commentlist");for(n in a)if(token=a[n],"commentt"==token&&(a[n]="comment"),b=this.templates[token])e&&(b=this.appendstat(token)),c.push(b);return c},appendstat:function(a){if(this.templates[a]){var b={},c=this.templates[a].valuetag||null;$.extend(b,this.templates[a]);c&&this.parent.get("stats").hasOwnProperty(c)&&(b.value=this.parent.get("stats")[c]);return b}},
startaction:function(a){var b=this.templates[a];this.listenTo(Cloudwalkers.RootView,a.concat(":success"),this.toggleAction);if("confirm"==b.type)this[a](this.parent);else{if("growl"==b.type)return Cloudwalkers.RootView.growl(b.name,this.translateString("the")+" "+b.token+" "+this.translateString("is_planned_with_success")),b.id=1,b.parent=this.parent,(new Cloudwalkers.Models.Action(b)).destroy();if("note"==b.type)Cloudwalkers.RootView.writeNote(this.parent);else var c="edit"==b.type?{model:this.parent}:
{reference:this.parent,action:new Cloudwalkers.Models.Action(b)};Cloudwalkers.RootView.compose(c)}},"delete":function(a){a.deleteMessage()},like:function(){},toggleAction:function(a){var b=this.templates[a];b.toggle&&this.parent.trigger("action:toggle",a,this.templates[b.toggle])},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)}});Cloudwalkers.Collections.Reports=Backbone.Collection.extend({model:Cloudwalkers.Models.Report,typestring:"statistics",modelstring:"report",processing:!1,parameters:{},paging:{},cursor:!1,initialize:function(a){a&&$.extend(this,a);Cloudwalkers.Session.user.account&&Cloudwalkers.Session.getReports().listenTo(this,"add",Cloudwalkers.Session.getReports().distantAdd)},distantAdd:function(a){this.get(a.id)||this.add(a)},url:function(){return Cloudwalkers.Session.api+"/streams/"+this.streamid+"/reports?complete=1"},
...
$.extend(this,a);Cloudwalkers.Session.user.account&&Cloudwalkers.Session.getReports().listenTo(this,"add",Cloudwalkers.Session.getReports().distantAdd)},url:function(a){this.parentmodel&&!this.parenttype&&(this.parenttype=this.parentmodel.get("objectType"));a=Cloudwalkers.Session.api+"/accounts/"+this.parentmodel.id;this.endpoint&&(a+="/"+this.endpoint);return a},touch:function(a){this.parentmodel=Cloudwalkers.Session.getAccount();this.endpoint=this.modelstring+"ids";this.fetch({data:a,success:this.touchresponse.bind(this)})},
touchresponse:function(a,b){var c=b.account.statistics;this.endpoint=this.modelstring+"s";c.length?(this.fetch({reset:!0,data:{ids:c.join(",")},success:this.trigger.bind(this,"sync:data")}),this.trigger("seed",c)):this.trigger("sync:noresults")},latest:function(){return this.at(this.length-1)},first:function(){return this.at(0)},place:function(a){return this.at(a)},parse:function(a){"string"==typeof a&&console.log("is string");this.parentmodel&&(a=a[this.parenttype]);this.setcursor(a.paging);a.paging||
this.ready();return a[this.typestring]},contacts:function(a){var b=this.latest(),c=[];if(!b.get("streams"))return c;a||$.each(b.get("streams"),function(a,b){var g=Cloudwalkers.Session.getStream(b.id);if(g){var g=g.get("network").token,h=b.contacts;h&&c.push({color:this.networkcolors[g],value:Number(h)})}}.bind(this));console.log("contacts list:",c);return c},age:function(a){return{counter:[]}},gender:function(a){return{counter:[]}},regional:function(a){return{counter:[]}},countries:function(a){return{counter:[]}},
cities:function(a){return{counter:[]}},besttime:function(a){return{counter:[]}},activity:function(a){return{counter:[]}},parsebesttime:function(a){var b="Mon Tue Wed Thu Fri Sat Sun".split(" "),c,e=[],f=0,g=0,h,m=this.models;7<this.length&&(m=m.splice(7,m.length-1));$.each(b,function(l,r){var p=m[l],q=[];p&&p.get("streams")&&(p=p.get("streams"),$.each(p,function(b,e){if(a&&a!=e.id)return!0;e=new Cloudwalkers.Models.Stream(e);if(c=e.getbesttime())for(i in 0==q.length&&(q=_.values(c)),c)q[i]+=c[i],
q[i]>f&&(f=q[i]),q[i]>g&&(g=q[i])}));h=0<q.length?q.indexOf(Math.max.apply(Math,_.values(q))):-1;e.push({day:b.shift(),value:g,time:h});g=0});e.maxvalue=f;return e}});Cloudwalkers.Collections.Triggers=Backbone.Collection.extend({model:Cloudwalkers.Models.Trigger,typestring:"triggers",initialize:function(){},parse:function(a){return a.account.triggers},url:function(){var a=[Cloudwalkers.Session.api];this.parent?a.push(this.parent.typestring,this.parent.id,this.typestring):this.typestring&&a.push(this.typestring);return a.join("/")}});Cloudwalkers.Collections.Groups=Backbone.Collection.extend({model:Cloudwalkers.Models.Group,typestring:"groups",modelstring:"group",parenttype:"account",processing:!1,initialize:function(a){a&&$.extend(this,a)},parse:function(a){return"user"==this.modelstring?a.group.users:a.account.groups},url:function(){var a=[CONFIG_BASE_URL+"json"];"user"==this.modelstring?a.push(this.parenttype+"/"+this.parentmodel.groupid):(a.push(this.parenttype+"/"+Cloudwalkers.Session.getAccount().id),this.typestring&&a.push(this.typestring));
this.endpoint&&a.push(this.endpoint);this.parameters&&a.push(this.parameters);return a=a.join("/")},sync:function(a,b,c){"read"==a&&(this.processing=!0,this.parameters=this.parameters?"?"+$.param(this.parameters):"");return Backbone.sync(a,b,c)},updates:function(a){for(n in a){var b=this.get(a[n]);b&&(Store.set(this.typestring,{id:a[n],outdated:!0}),b.outdated=!0,b.trigger("outdated"))}},outdated:function(a){if(!a)return this.filter(function(a){return a.outdated});this.updates([a])},hook:function(a){a.records&&
(this.parameters.records=a.records);this.processing?this.length&&a.success(this):this.fetch({error:a.error});this.on("sync",a.success)}});Cloudwalkers.Collections.Widgets=Backbone.Collection.extend({model:Cloudwalkers.Models.Widget,widgets:{widget_1:{widget:"messagescounters",type:"inbox",source:"streams",size:4,icon:"inbox",open:!0,counter:!0,typelink:"#inbox",countString:"incomingUnread",scrollable:"scrollable",translation:{title:"inbox"}},widget_2:{widget:"messagescounters",type:"monitoring",source:"channels",size:4,icon:"tags",open:!0,counter:!0,countString:"incoming",scrollable:"scrollable",translation:{title:"keywords"}},widget_3:{widget:"schedulecounter",
type:"schedule",source:"outgoing",size:4,icon:"time",open:!0,counter:!0,countString:"scheduled",link:"#scheduled",scrollable:"scrollable",translation:{title:"schedule"}},widget_4:{widget:"coworkers",type:"coworkers",size:4,color:"grey",icon:"edit",open:!0,link:"#coworkers",scrollable:"scrollable",translation:{title:"co-workers_wall"}},widget_5:{widget:"trending",type:"profiles",size:4,color:"red",icon:"thumbs-up",open:!0,since:7,sublink:"#trending/",scrollable:"scrollable",translation:{title:"trending_company_posts"}},
widget_6:{widget:"trending",type:"news",size:4,color:"red",icon:"thumbs-up",open:!0,since:1,sublink:"#trending/",scrollable:"scrollable",translation:{title:"trending_accounts_we_follow"}},widget_7:{widget:"accountswefollow",type:"news",size:4,color:"blue",icon:"globe",open:!0,since:1,sublink:"#trending/",scrollable:"scrollable",translation:{title:"accounts_we_follow"}},widget_8:{widget:"drafts",type:"draft",size:4,color:"blue",icon:"edit",open:!0,since:1,link:"#drafts",scrollable:"scrollable",translation:{title:"drafts"}},
widget_9:{widget:"webcarecounter",type:"open_me",size:3,assigneduser:"me",status:"open",translation:{title:"open_messages_assigned_to_me"}},widget_10:{widget:"webcarecounter",type:"open_mygroups",size:3,assigned:"me",status:"open",translation:{title:"open_messages_assigned_to_my_groups"}},widget_11:{widget:"webcarecounter",type:"handled_me",size:3,assigneduser:"me",status:"closed",translation:{title:"messages_handled_this_week_by_me"}},widget_12:{widget:"webcarecounter",type:"handled_mygroups",size:3,
assigned:"me",status:"closed",translation:{title:"messages_handled_this_week_by_my_groups"}},widget_13:{widget:"write",type:"draft",size:4,color:"blue",icon:"edit",open:!0,since:1,link:"#drafts",scrollable:"scrollable",translation:{title:"quick_draft"}},widget_14:{widget:"write",type:"note",size:4,color:"blue",icon:"edit",open:!0,since:1,link:"#drafts",scrollable:"scrollable",translation:{title:"quick_note"}},widget_15:{widget:"conversation",size:12,since:7,translation:{title:"top_post_on_company_page"}},
widget_16:{widget:"conversation",sort:"date",size:12,since:7,translation:{title:"latest_company_post"}}},initialize:function(a){a&&$.extend(this,a)},startWidgets:function(a){for(i in a){var b=new this.model(this.widgets[a[i]]);this.add(b)}}});Backbone.View=Backbone.View.extend({loadListeners:function(a,b,c){var e=b.length,f="listenTo";c&&(f="listenToOnce");for(i in b)if(_.isArray(b[i]))for(n in b[i])this[f](a,b[i][n],this.loadRender.bind(this,Number(i)+1,e));else this[f](a,b[i],this.loadRender.bind(this,Number(i)+1,e));this.listenTo(a,"ready:empty",this.emptylist.bind(this));this.on("rendered",this.addLoader)},addLoader:function(){this.container=this.$loadercontainer?this.$loadercontainer:this.$container;this.loader=$(Templates.progressbar).appendTo(this.container)},
...
!0,$("#header").html(this.renderHeader().header)},renderHeader:function(){var a={dev:this.development};a.user=Cloudwalkers.Session.user.attributes;a.accounts=[];a.level=Cloudwalkers.Session.getUser().level;Cloudwalkers.Session.isAuthorized("USER_GRANT")|Cloudwalkers.Session.isAuthorized("GROUP_MANAGE")&&(a.manage_user_groups=!0);this.mustacheTranslateRenderHeader(a);Cloudwalkers.Session.user.accounts.each(function(b){a.accounts.push({name:b.get("name"),id:b.id,active:b.id==Cloudwalkers.Session.get("currentAccount")})});
Cloudwalkers.Session.censuretemplate(a);this.header=Mustache.render(Templates.header,a);return this},render:function(){var a=Cloudwalkers.Session.getAccount(),b={reports:[]};b.level=Cloudwalkers.Session.getUser().level;this.mustacheTranslateRender(b);Cloudwalkers.Session.isAuthorized("MESSAGE_READ_THIRDPARTY")&&(b.news=a.channels.findWhere({type:"news"}),b.news&&(b.news=a.channels.findWhere({type:"news"}).id));if(Cloudwalkers.Session.isAuthorized("MESSAGE_READ_MONITORING")){var c=a.channels.findWhere({type:"monitoring"});
for(n in c.channels.models)if(c.channels.models[n].attributes.channels.length){this.first=c.channels.models[n];break}c&&(b.monitoring={channelid:c.id,first:this.first,channels:c.channels.models,name:c.get("name")})}Cloudwalkers.Session.isAuthorized("MESSAGE_READ_SCHEDULE")&&(b.scheduled={channelid:Cloudwalkers.Session.getChannel("internal").id},b.scheduled.streams=a.streams.where({outgoing:1}));Cloudwalkers.Session.isAuthorized("_CW_INBOX_VIEW")&&(b.inbox=!0);Cloudwalkers.Session.isAuthorized("MESSAGE_READ_COMPANY")&&
(c=a.channels.findWhere({type:"profiles"}),b.profiles={channelid:c.id,streams:c.streams.models,name:c.get("name")});Cloudwalkers.Session.isAuthorized("STATISTICS_VIEW")&&(b.reports=a.streams.where({statistics:1}).map(function(a){return a.attributes}));Cloudwalkers.Session.censuretemplate(b);this.$el.html(Mustache.render(Templates.navigation,b));this.handleSidebarMenu();return this},handleSidebarMenu:function(){var a=Backbone.history.fragment;if(!a)return null;this.setActive(a)},compose:function(){Cloudwalkers.RootView.compose()},
writenote:function(){var a=Cloudwalkers.Session.getAccount();Cloudwalkers.RootView.writeNote(a)},setActive:function(a){$("#sidebar .active").removeClass("active");$('a[href="#'+a+'"]').parents("#sidebar .page-sidebar-menu *").addClass("active")},setNotifications:function(a){this.$el.find(".popup-frame ul").html("");for(var b=0;b<a.length;b++)this.addNotification(a[b])},addNotification:function(a){this.$el.find(".popup-frame ul").append('<li><div class="text account"><p>'+a.message+'</p><div class="row"><span class="time"></span></div></div></li>')},
toggleNotifications:function(){this.$el.find(".notification-popup").toggle()},mapViews:function(){var a={};for(n in this.views)if(a[this.views[n].name]={streams:[]},this.views[n].children)for(i in this.views[n].children)a[this.views[n].children[i].name]={streams:[]};return a},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRenderHeader:function(a){this.original=["write_note","clean","support","profile_settings","log_out"];this.translated=[];for(k in this.original)this.translated[k]=
this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]},mustacheTranslateRender:function(a){this.original="compose dashboard message_board compose_message drafts scheduled inbox messages notifications post_message calendar co-workers_wall company_accounts media trending_posts accounts_we_follow keyword_monitoring manage_accounts manage_keywords reports settings manage_users social_connections account_settings profile_settings manage_user_groups notes rss_feed manage_rss sent outbox calendar".split(" ");
this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Compose=Backbone.View.extend({id:"compose",className:"modal hide",type:"post",network:"default",actionstreams:[],actionview:!1,minutestep:5,maxfilesize:3E6,besttimes:{},titles:{post:"Write Post",edit:"Edit Post",share:"Share message",comment:"Write Comment",reply:"Write Reply",dm:"Write Message",retweet:"Retweet",like:"Like",favourite:"Favourite",plusone:"Plus one","default":"Compose"},translated_titles:{},limitations:{},options:{"false":["editor","images","campaign","schedule",
...
_.isString(a)||(a="draft");this.draft.sanitizepost();this.draft.id?this.draft.save({body:this.draft.get("body"),attachments:this.draft.get("attachments"),variations:this.draft.get("variations"),streams:this.draft.get("streams"),schedule:this.draft.get("schedule"),update:!0},{patch:!0,endpoint:"original",success:this.thankyou.bind(this,"save"),error:this.oops.bind(this,"save",a)}):this.draft.save({status:a},{success:this.thankyou.bind(this,"save"),error:this.oops.bind(this,"save",a)});this.loadingalert()},
post:function(){var a;if(a=this.draft.validateCustom())return this.showerror(this.translateString("not_posted")+": ",a);this.disablefooter();this.draft.sanitizepost();this.draft.get("streams").length?this.draft.id?this.draft.save({body:this.draft.get("body"),attachments:this.draft.get("attachments"),variations:this.draft.get("variations"),streams:this.draft.get("streams"),status:"scheduled",schedule:this.draft.get("schedule"),update:!0},{patch:!0,endpoint:"original",success:this.thankyou.bind(this,
"post"),error:this.oops.bind(this,"post")}):this.draft.save({status:"scheduled"},{success:this.thankyou.bind(this,"post"),error:this.oops.bind(this,"post")}):this.save("scheduled");this.loadingalert()},postaction:function(){var a=[],b="content";this.$el.find(".action-tabs div").each(function(){a.push($(this).data("stream"))});0<=this.options[this.type].indexOf("editor")&&!this.draft.get("body").html&&(b=!1);if(b=this.draft.validateCustom([b]))return this.showerror(this.translateString("not_posted")+
": ",b);this.disablefooter();this.$el.find("[data-type=canned] input").is(":checked")&&Cloudwalkers.Session.getCannedResponses().create({body:{plaintext:this.draft.get("body").plaintext},status:"CANNED"});b=this.reference.actions.create({parameters:null,streams:a,message:this.draft.get("body").plaintext,actiontype:this.type},{success:this.thankyou.bind(this,null),error:this.oops.bind(this,"saveaction")});this.loadingalert();this.loadListeners(b,["request:action","sync"]);b.trigger("request:action")},
thankyou:function(a,b){var c=Mustache.render(Templates.thankyou);setTimeout(function(){this.$el.addClass("switch-mode").addClass("thanks");this.$el.find(".switch-container").append(c);setTimeout(function(){this.$el.modal("hide")}.bind(this),1E3)}.bind(this),400);if("edit"==this.type){var e=this.draft.get("variations");e&&e.length?e.forEach(function(b){b.id&&(a&&"post"==a&&Cloudwalkers.RootView.trigger("added:message",this.draft),Cloudwalkers.Session.getMessage(b.id).fetch({success:function(a){a.trigger("change")}}))}.bind(this)):
this.model.fetch()}else"post"==this.type?Cloudwalkers.RootView.trigger("added:message",this.draft):this.trigger("action:success",b);Cloudwalkers.RootView.trigger(this.type.concat(":success"),this.type)},loadingalert:function(){setTimeout(function(){this.showerror("",this.translateString("busy_network"))}.bind(this),2E4)},showerror:function(a,b){Cloudwalkers.RootView.information(a,b,this.$el.find(".modal-footer"))},oops:function(a,b){Cloudwalkers.RootView.confirm("An error ocurred while posting your message. Do you want to retry?",
function(){this.loadmylisteners();"save"==a?this.save(b):this.post()}.bind(this),function(){this.$el.modal("hide")}.bind(this))},disablefooter:function(){this.$el.find(".modal-footer .btn").attr("disabled",!0)},invalid:function(a,b){Cloudwalkers.RootView.alert(a.get("title")+" "+b)},isscrolling:function(){if(this.$el.find(".stream-tabs").length)return this.$el.find(".stream-tabs")[0].scrollWidth>this.$el.find(".stream-tabs").width();if(this.$el.find(".action-tabs").length)return this.$el.find(".action-tabs")[0].scrollWidth>
this.$el.find(".action-tabs").width()},triggerpaste:function(a){this.editor.trigger("paste:content",a)},triggerblur:function(){this.editor.trigger("blur:content")},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},translateTitles:function(a){for(k in this.titles)this.translated_titles[k]=this.translateString(this.titles[k])},mustacheTranslateRender:function(a){this.original="networks default subject images photo_booth pictures camera campaign no_campaign schedule now in mins hour hours day week on best_time repeat no_repeat every days weeks months select_a_day monday tuesday wednesday thursday friday saturday sunday doesnt_matter times until save preview post save_this_as_a_response_template".split(" ");
this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]},mustacheTranslateSummarizeRepeat:function(a){this.original="times until endless_repeat_every hours days weeks months".split(" ");this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.SimpleCompose=Backbone.View.extend({template:"composenote",events:{"click #post":"post","click #cancel":"cancel"},initialize:function(a){a&&$.extend(this,a);this.model||(this.model=new Cloudwalkers.Models.Note);this.parent&&(this.model.parent=this.parent)},render:function(){var a={};this.model.get("text")&&(a.text=this.model.get("text"));this.mustacheTranslateRender(a);a=Mustache.render(Templates[this.template],a);this.$el.html(a);this.model.get("text")&&this.$el.find("h3").remove();
this.$loadercontainer||(this.$loadercontainer=this.$el.find(".modal-footer"));this.loadListeners(this.model,["request","sync"]);this.trigger("rendered");return this},post:function(){var a=this.$el.find("textarea").val(),b=this.model.typestring;if(b)this["post"+b](a)},postnotes:function(a){this.model.save({text:a},{patch:this.model.id?!0:!1,success:this.thanks?this.thankyou.bind(this):this.saved.bind(this)})},postmessages:function(a){this.model.set("body",{html:a,plaintext:a});this.model.save({body:this.model.get("body"),
status:"draft"},{success:this.thanks?this.thankyou.bind(this):this.trigger.bind(this,"save:success")})},cancel:function(){this.trigger("edit:cancel");this.persistent||(this.$el.find("button.close").click(),this.remove())},saved:function(){setTimeout(function(){this.trigger("save:success")}.bind(this),200)},thankyou:function(){var a=Mustache.render(Templates.thankyou);setTimeout(function(){this.$el.addClass("thanks");this.$el.find("section").html(a);setTimeout(function(){this.$el.modal("hide")}.bind(this),
1E3);this.close&&setTimeout(function(){this.model="draft"==this.type?new Cloudwalkers.Models.Message:new Cloudwalkers.Models.Note;this.render()}.bind(this),2E3)}.bind(this),200);Cloudwalkers.RootView.trigger("added:note",this.model)},clean:function(){this.$el.find("textarea").val("")},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=["write_note","save","cancel","private_note"];this.translated=[];for(k in this.original)this.translated[k]=
this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Preview=Backbone.View.extend({id:"preview",events:{},networkclasses:{facebook:"fb",twitter:"twt","google-plus":"gp",linkedin:"li"},events:{"click #viewsummary":"togglesummary"},initialize:function(a){a&&$.extend(this,a);$.extend(this.model.attributes,this.model.getvariation(this.streamid));this.model.attributes.profile=Cloudwalkers.Session.getStream(this.streamid).get("profile");this.network=Cloudwalkers.Session.getStream(this.streamid).get("network").token},render:function(){var a=
...
b,c,e){this.span&&0!=b||this.$container.append(Templates.row);this.span=12>b+this.span?b+this.span:0;a&&(this.$container.children().last().append(a.render().el),this.widgetviews.push(a),a.$el.addClass("span"+b),e&&a.$el.addClass("offset"+e),a.negotiateFunctionalities&&a.negotiateFunctionalities(),this.listenTo(a,"view:update",this.updatewidget.bind(this,a)))},appendhtml:function(a){this.$container.children().last().append(a)},cleanviews:function(){this.widgetviews.length&&(this.destroy(),this.widgetviews=
[])},bubblerender:function(){this.trigger("rendered");$.each(this.widgetviews,function(a,b){b.trigger("rendered")})},updatewidget:function(a){a.render();a.negotiateFunctionalities&&a.negotiateFunctionalities()},destroy:function(){$.each(this.widgetviews,function(a,b){b.remove()})}});Cloudwalkers.Views.Message=Backbone.View.extend({events:{"click .message-action[data-action]":"messageAction","click .children-button.message-children":"showchildren"},className:"message-view",template:"message",tagName:"tr",commentsVisible:!1,commentsView:null,childrencontainer:"comment-container",initialize:function(){var a=this;this.model.on("change",function(){a.render()})},prepareData:function(){var a=jQuery.extend(!0,{},this.model.attributes);a.shortBody=this.model.shortBody();a.humandate=this.model.humandate();
...
"full"==a[c].size?container.addClass("span12"):container.addClass("span"+a[c].size),container.append(a[c].widget.render().el),this.currentline.append(container),a[c].widget.negotiateFunctionalities();setTimeout(function(){b.trigger("content:change")},1)},destroy:function(){for(n in this.widgets)this.widgets[n].widget.trigger("destroy")}});Cloudwalkers.Views.Dashboard=Cloudwalkers.Views.Pageview.extend({title:"Dashboard",statistics:[],widgets:[{widget:"messagescounters",type:"inbox",source:"streams",size:4,title:"Inbox",icon:"inbox",open:!0,counter:!0,typelink:"#inbox",countString:"incomingUnread",scrollable:"scrollable",translation:{title:"inbox"}},{widget:"messagescounters",type:"monitoring",source:"channels",size:4,title:"Keywords",icon:"tags",open:!0,counter:!0,countString:"incoming",scrollable:"scrollable",translation:{title:"keywords"}},
{widget:"schedulecounter",type:"schedule",source:"streams",size:4,title:"Schedule",icon:"time",open:!0,counter:!0,countString:"scheduled",link:"#scheduled",scrollable:"scrollable",translation:{title:"schedule"}},{widget:"coworkers",type:"drafts",size:4,title:"Co-workers wall",color:"yellow",icon:"edit",open:!0,link:"#coworkers",scrollable:"scrollable",translation:{title:"co-workers_wall"}},{widget:"trending",type:"profiles",size:4,title:"Trending Company Posts",color:"grey",icon:"thumbs-up",open:!0,
since:7,sublink:"#trending/",scrollable:"scrollable",translation:{title:"trending_company_posts"}},{widget:"trending",type:"news",size:4,title:"Trending Accounts we follow",color:"red",icon:"thumbs-up",open:!0,since:1,sublink:"#trending/",scrollable:"scrollable",translation:{title:"trending_accounts_we_follow"}}],initialize:function(){Cloudwalkers.Session.ping();this.translateTitle("dashboard");this.collection=new Cloudwalkers.Collections.Statistics;this.listenTo(this.collection,"seed",this.filldynamicreports)},
filldynamicreports:function(){var a=Cloudwalkers.Session.getStreams().where({statistics:1});this.appendWidget(new Cloudwalkers.Views.Widgets.DashboardCleaner({size:12}),12);for(n in a)this.fillstreamwidget(a[n].id)},fillstreamwidget:function(a){var b=Cloudwalkers.Session.getStream(a).get("network").token,c="facebook"==b?"Likes":"Followers",e=[{widget:"Info",data:{title:c,filterfunc:"followers"},span:3},{widget:"Info",data:{title:"Best time to post",filterfunc:"besttimetopost"},span:3}];for(n in e)if("facebook"==
b||"twitter"==b){e[n].data.network=a;e[n].data.parentview=this;e[n].data.footer=Cloudwalkers.Session.getStream(a).get("defaultname");"besttimetopost"!=e[n].data.filterfunc&&(e[n].data.footer=e[n].data.footer+" "+c);var f=new Cloudwalkers.Views.Widgets[e[n].widget](e[n].data);this.statistics.push(f);this.appendWidget(f,e[n].span)}},render:function(){var a=this.widgets;this.$el.html(Mustache.render(Templates.pageview,{title:this.title}));this.$container=this.$el.find("#widgetcontainer").eq(0);for(i in a){this.translateWidgets(a[i]);
switch(a[i].widget){case "messagescounters":var b=this.addMessagesCounters(a[i]);break;case "schedulecounter":b=this.addScheduleCounters(a[i]);break;case "coworkers":b=this.addDashboardDrafts(a[i]);break;case "trending":b=this.addDashboardTrending(a[i]);break;case "report":b=new Cloudwalkers.Views.Widgets.Report(a[i])}b&&this.appendWidget(b,Number(a[i].size))}this.collection.touch(this.filterparameters());return this},addMessagesCounters:function(a){var b=Cloudwalkers.Session.getChannel(a.type);if(b)return $.extend(a,
{name:b.get("name"),open:1,channel:b}),new Cloudwalkers.Views.Widgets.MessagesCounters(a)},addScheduleCounters:function(a){var b=Cloudwalkers.Session.getChannel("outgoing");$.extend(a,{name:b.get("name"),open:1,channel:b});return new Cloudwalkers.Views.Widgets.MessagesCounters(a)},addDashboardDrafts:function(a){a.model=Cloudwalkers.Session.getStream("coworkers");if(a.model)return a.link="#coworkers",new Cloudwalkers.Views.Widgets.DashboardMessageList(a)},addDashboardTrending:function(a){a.trending=
!0;a.model=Cloudwalkers.Session.getChannel(a.type);a.filters={sort:"engagement",since:3600*Math.round(Date.now()/36E5)-86400*a.since};if(a.model)return new Cloudwalkers.Views.Widgets.DashboardMessageList(a)},filterparameters:function(){this.start=moment().subtract(7,"days").zone(0).endOf("day");this.end=moment().zone(0).endOf("day");return{since:this.start.unix(),until:this.end.unix()}},translateWidgets:function(a){if(a.translation)for(k in a.translation)a[k]=Cloudwalkers.Session.polyglot.t(a.translation[k])},
translateTitle:function(a){this.title=Cloudwalkers.Session.polyglot.t(a)}});Cloudwalkers.Views.MessageBoard=Cloudwalkers.Views.Widgets.WidgetContainer.extend({title:"Message Board",widgets:{}});Cloudwalkers.Views.IB=Cloudwalkers.Views.Pageview.extend({id:"inbox",title:"Inbox",className:"container-fluid inbox",initialize:function(){options&&$.extend(this,options);this.model=Cloudwalkers.Session.getStream("draft");if(!this.model)return Cloudwalkers.Router.Instance.exception();Cloudwalkers.Session.viewsettings("ib-"+this.channeltype);this.listenTo(Cloudwalkers.RootView,"resize",this.resize)},render:function(){this.$el.html(Mustache.render(Templates.pageview,{title:this.title}));this.$container=
...
this.appendWidget(a,4);var b=new Cloudwalkers.Views.Widgets.DraftsList({model:this.model});this.appendWidget(b,8);a.list=b;return this},translateTitle:function(a){this.title=Cloudwalkers.Session.polyglot.t(a)}});Cloudwalkers.Views.Notes=Cloudwalkers.Views.Pageview.extend({title:"Notes",className:"container-fluid inbox inbox-notes",initialize:function(a){this.model=Cloudwalkers.Session.getAccount();this.translateTitle("notes")},render:function(){this.translateTitle("notes");this.$el.html(Mustache.render(Templates.pageview,{title:this.title}));this.$container=this.$el.find("#widgetcontainer").eq(0);var a=new Cloudwalkers.Views.Widgets.InboxNotesList({check:"hasnotes",collectionstring:"notes",listtype:"notes",
model:this.model});this.appendWidget(a,4);this.appendhtml(Templates.inboxcontainer);return this},translateTitle:function(a){this.title=Cloudwalkers.Session.polyglot.t(a)},resize:function(a){this.$el.find("#widgetcontainer").height(a-140)},finish:function(){this.resize(Cloudwalkers.RootView.height());$message=this.$el.find(".inbox-container").wrap("<div class='scroller'>");$message.parent().slimScroll({height:"inherit"})},translateTitle:function(a){this.title=Cloudwalkers.Session.polyglot.t(a)},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)}});Cloudwalkers.Views.Scheduled=Cloudwalkers.Views.Pageview.extend({title:"Schedule",className:"container-fluid scheduled",initialize:function(){this.model=Cloudwalkers.Session.getStream("scheduled");if(!this.model)return Cloudwalkers.Session.home();var a=Cloudwalkers.Session.viewsettings("scheduled");a.streams&&(this.options.filters={streams:a.streams});this.listenTo(this.model,"sync",this.render);this.translateTitle("scheduled")},render:function(){this.$el.html(Mustache.render(Templates.pageview,{title:this.title}));
this.$container=this.$el.find("#widgetcontainer").eq(0);var a=new Cloudwalkers.Views.Widgets.ScheduledFilters({model:this.model,filters:this.options.filters});this.appendWidget(a,4);var b=new Cloudwalkers.Views.Widgets.ScheduledList({model:this.model,filters:this.options.filters});this.appendWidget(b,8);a.list=b;return this},translateTitle:function(a){this.title=Cloudwalkers.Session.polyglot.t(a)}});Cloudwalkers.Views.Calendar=Cloudwalkers.Views.Pageview.extend({title:"Calendar",className:"container-fluid calendar",viewtype:"month",views:[],parameters:{records:5},events:{remove:"destroy","change select":"toggleView","click #subtract":"prev","click #add":"next","click #now":"now"},initialize:function(){this.posted=Cloudwalkers.Session.getChannel("profiles");this.scheduled=Cloudwalkers.Session.getStream("scheduled");if(!this.posted)return Cloudwalkers.Session.home();this.translateTitle("calendar");
this.listenTo(this.posted,"sync",this.render)},render:function(){var a={monthActive:!0};this.mustacheTranslateRender(a);this.$el.html(Mustache.render(Templates.calendar,a));this.$container=this.$el.find("#widgetcontainer").eq(0);a=new Cloudwalkers.Views.Widgets.CalendarFilters({model:this.posted,calview:this});this.appendWidget(a,3);a=new Cloudwalkers.Views.Widgets.CalSummary({calview:this});this.views.push(a);this.appendWidget(a,9);this.CalSummary=a;this.$el.find("select").chosen({width:"200px",
disable_search_threshold:10,inherit_select_classes:!0});setTimeout(this.initCalendar.bind(this),1E3);this.loaderswitch=[];return this},populate:function(a,b,c,e,f){f?(this.parameters.records=50,$("#calendar").fullCalendar("removeEvents",function(b){if(b.start.date()==a.date())return!0}),m_item=0):(this.loader("on",""),a=1<a.date()?a.startOf("month").add(1,"month"):a.startOf("month"),b=b.endOf("month").subtract(1,"month").endOf("month"));m_item=a.date()-1;this.datedisplay(a,b);f||this.CalSummary.getTotal(a,
b);for(this.listenTo(this.posted.messages,"sync",this.fill.bind(this,e));m_item<b.date();)e=c=f?moment(a):moment(a).add(m_item,"days"),c=moment(c).hours("0").minutes("0"),e=moment(e).hours("22").minutes("59"),$.extend(this.parameters,{since:c.unix(),until:e.unix()}),this.posted.messages.touch(this.posted,this.parameters),m_item++},populate_scheduled:function(a,b,c,e,f){f?(this.parameters.records=50,$("#calendar").fullCalendar("removeEvents",function(b){if(b.start.date()==a.date())return!0}),m_item=
0):(this.loader("on",""),a=1<a.date()?a.startOf("month").add(1,"month"):a.startOf("month"),b=b.endOf("month").subtract(1,"month").endOf("month"),m_item=a.date()-1);m_item=a.date()-1;this.datedisplay(a,b);for(this.listenTo(this.scheduled.messages,"sync",this.fill.bind(this,e));m_item<b.date();)e=c=f?moment(a):moment(a).add(m_item,"days"),c=moment(c).hours("0").minutes("0"),e=moment(e).hours("22").minutes("59"),$.extend(this.parameters,{since:c.unix(),until:e.unix()}),this.scheduled.messages.touch(this.scheduled,
this.parameters),m_item++},datedisplay:function(a,b){var c=moment();a.isAfter(c)||b.isBefore(c)?this.$el.find("#now").removeClass("hidden"):this.$el.find("#now").addClass("hidden");var e="agendaWeek"==this.viewtype?"week":"month",c=(c=a.diff(c,e+"s"))?-2<c&&0>c?"last ":2>c&&0<c?"next ":"":"this ",f=24<a.date()?11==a.month()?"DD MMM YYYY":"DD MMM":"DD",f=a.format(f)+" - "+b.format("DD MMM YYYY");this.$el.find("h3.stats-header-timeview").html("<strong>"+c+e+": </strong>"+f)},fill:function(a,b,c){if(b.models[0]){fillfrom=
moment(b.models[0].filterCalReadable().calNode.start).hours("0").minutes("0");fillto=moment(fillfrom).hours("22").minutes("59");fillparameters={since:fillfrom.unix(),until:fillto.unix(),records:5};c=[];if(!b)return a(c);for(n in b.models)c.push(b.models[n].filterCalReadable().calNode);0<c.length&&c[0].networkdescription&&(5==this.parameters.records&&"Scheduled messages"!=c[0].networkdescription?stream.fetch({endpoint:"messageids",parameters:fillparameters,success:this.validateViewmore.bind(this,c,
this.scheduled,this.posted)}):this.renderDayEvents(c));this.scheduled&&this.scheduled.messages.destroy();this.posted&&this.posted.messages.destroy()}},validateViewmore:function(a,b,c,e){e.attributes.paging.cursors.after&&a.push({className:"calendar-more",icon:"plus",id:0,allDay:!1,title:this.translateString("view_more"),start:moment(a[0].start).hours("23").minutes("59")});this.renderDayEvents(a)},renderDayEvents:function(a){for(eventNr in a){if("..."==a[eventNr].title)return;"Scheduled messages"==
a[eventNr].networkdescription&&(a[eventNr].start=moment(a[eventNr].start._i,"DD MMM YYYY HH:mm").format("YYYY-MM-DDTHH:mm:ssZ"));var b={id:a[eventNr].id,start:a[eventNr].start,title:a[eventNr].title,className:a[eventNr].className,networkdescription:a[eventNr].networkdescription,intro:a[eventNr].intro,icon:a[eventNr].icon,allDay:!1};$("#calendar").fullCalendar("renderEvent",b)}this.loader("off","off")},toggleView:function(a){this.viewtype=$(a.currentTarget).val();"list"==this.viewtype?this.initList():
$("#calendar").removeClass("hidden");setTimeout(this.initCalendar.bind(this),1E3);$("#calendar").fullCalendar("changeView",this.viewtype)},prev:function(){$("#calendar").fullCalendar("prev")},next:function(){$("#calendar").fullCalendar("next")},now:function(){$("#calendar").fullCalendar("today")},initCalendar:function(){$("#calendar").fullCalendar({header:!1,slotMinutes:30,editable:!1,droppable:!1,allDayDefault:!1,allDaySlot:!1,firstDay:1,axisFormat:"H:mm",defaultTimedEventDuration:"00:30:00",slotEventOverlap:!1,
eventSources:[this.populate.bind(this),this.populate_scheduled.bind(this)],eventRender:function(a,b){if(!a.icon)return!1;$(b).find(".fc-event-time").html("<i class='icon-"+a.icon+"'></i>");a.media&&$(b).find(".fc-event-inner").prepend("<i class='fc-event-media pull-right icon-"+a.media+"'></i>");$(b).data("title",a.intro).tooltip({html:!0,delay:500,placement:"top"});a.rendered=!0},eventClick:function(a,b,c){"calendar-more"==a.className[0]&&(b=new Date(a.start),$("select").val("agendaDay"),$("select").trigger("chosen:updated"),
$("#calendar").fullCalendar("changeView","agendaDay"),$("#calendar").fullCalendar("gotoDate",b),to=from=a.start,dayview=!0,callback=timezone="",this.populate(from,to,timezone,callback,dayview),this.populate_scheduled(from,to,timezone,callback,dayview))}.bind(this)})},loader:function(a,b){a&&(this.loaderswitch.a=a);b&&(this.loaderswitch.b=b);"on"==a?(this.$el.find("#calcontainer").css("display","none"),this.$el.find(".container-loading").css("display","block"),this.$el.find(".stats-summary-counter").html("--")):
"off"==this.loaderswitch.a&&"off"==this.loaderswitch.b&&(this.$el.find(".container-loading").css("display","none"),this.$el.find("#calcontainer").css("display","block"))},translateTitle:function(a){this.title=Cloudwalkers.Session.polyglot.t(a)},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=["day","week","month","list_view","now"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),
a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Coworkers=Cloudwalkers.Views.Pageview.extend({title:"Co-workers wall",className:"container-fluid coworkers",initialize:function(){this.model=Cloudwalkers.Session.getStream("coworkers");if(!this.model)return Cloudwalkers.Session.home();this.listenTo(this.model,"outdated",this.model.fetch);this.listenTo(this.model,"sync",this.render);this.translateTitle("co-workers_wall")},render:function(){this.$el.html(Mustache.render(Templates.pageview,{title:this.title}));this.$container=this.$el.find("#widgetcontainer").eq(0);
var a=new Cloudwalkers.Views.Widgets.CoworkersFilters({model:this.model});this.appendWidget(a,4);var b=new Cloudwalkers.Views.Widgets.CoworkersList({model:this.model});this.appendWidget(b,8);a.list=b;return this},translateTitle:function(a){this.title=Cloudwalkers.Session.polyglot.t(a)}});Cloudwalkers.Views.Timeline=Cloudwalkers.Views.Pageview.extend({id:"timeline",parameters:{records:20,markasread:!0},entries:[],check:"hasMessages",filters:{streams:[]},events:{"click *[data-action]":"action","click [data-toggle]":"togglefilter","click [data-streams]":"filterstreams","click .load-more .more":"more","click .load-more .timeline-icon":"more","click [data-networks]":"filternetworks","click .toggleall.networks.active":"toggleallnetworks","click .toggleall.active":"toggleall"},initialize:function(a){a&&
$.extend(this,a);this.collection=this.model.messages;this.$el.addClass("loading");this.listenTo(this.collection,"seed",this.fill);a=Cloudwalkers.Session.viewsettings(this.model.get("type"));a.streams&&(this.filters={streams:a.streams})},hideloading:function(){this.$el.removeClass("loading");this.$el.find(".timeline-loading.timeline-blue").hide();this.$el.find(".load-more .timeline-icon").removeClass("entry-loading");this.$el.find(".load-more .timeline-body span").html(this.translateString("view_more"))},
toggleall:function(){this.filternetworks(null,!0);this.togglestreams(!0);this.$el.find("[id^=filter_]").slideUp("fast");this.$el.find("[data-toggle=streams]").removeClass("selected")},render:function(){var a={streams:[],networks:[]};"news"==this.model.get("type")?this.rendernews(a):this.rendercompany(a);this.mustacheTranslateRender(a);this.$el.html(Mustache.render(Templates.timeline,a));this.filters.streams.length&&(this.$el.find("[data-streams], [data-networks], .toggleall").toggleClass("inactive active"),
this.$el.find(this.filters.streams.map(function(a){return'[data-networks~="'+a+'"],[data-streams="'+a+'"]'}).join(",")).toggleClass("inactive active"));this.reload(a);this.resize(Cloudwalkers.RootView.height());return this},rendernews:function(a){this.timelinetype="news";this.contacts=new Cloudwalkers.Collections.Contacts([],{});this.listenTo(this.contacts,"sync",this.fillcontacts.bind(this,a));this.contacts.touch(null,{records:200})},rendercompany:function(a){this.timelinetype="company";this.model.streams.each(function(b){b.get(this.check)&&
a.streams.push({id:b.id,icon:b.get("network").icon,name:b.get("defaultname"),network:b.get("network")})}.bind(this));a.networks=this.model.streams.filterNetworks(a.streams,!0)},reload:function(a){this.reloadui(a);this.collection.touch(this.model,this.filterparameters())},reloadui:function(a){this.$el.addClass("loading");a||(a={},this.mustacheTranslateRender(a));this.$el.find("ul.messages-container").empty().html(Mustache.render(Templates.timelinemessagelist,a));"news"!=this.timelinetype||this.newsloaded||
this.$el.find(".filter-bg").addClass("loading");this.$container=this.$el.find("ul.timeline").eq(0);this.$loadmore=this.$el.find(".load-more").remove();this.$nocontent=this.$el.find(".no-content").remove()},fillcontacts:function(a,b){var c=b.models,e=[];for(n in c)c[n].loaded()&&(e.push({id:c[n].id,icon:c[n].get("network").icon,name:c[n].get("defaultname"),network:c[n].get("network")}),this.$el.find("#filter_streams ul").append(Mustache.render(Templates.filterstream,c[n].attributes)));a.streams=e;
a.networks=this.model.streams.filterNetworks(a.streams,!0);for(n in a.networks)c='<div class="btn-white filter" data-networks="'+a.networks[n].ids+'"><i class="icon-'+a.networks[n].icon+'"></i></div>',this.$el.find(".caption").after(c);this.$el.find(".filter-bg .loadingafter").remove();this.newsloaded=!0;this.filters.streams.length&&(this.$el.find("[data-streams], [data-networks], .toggleall").toggleClass("inactive active"),this.$el.find(this.filters.streams.map(function(a){return'[data-networks~="'+
a+'"],[data-streams="'+a+'"]'}).join(",")).toggleClass("inactive active"))},filternetworks:function(a,b){b||(b=this.button&&this.button.data("networks")==$(a.currentTarget).data("networks"));this.togglestreams(b);b||(this.button=$(a.currentTarget).addClass("active").removeClass("inactive"));var c=b?null:String(this.button.data("networks")).split(" ");b&&(this.button=!1);c?($(c.map(function(a){return'[data-streams="'+a+'"]'}).join(",")).removeClass("inactive").addClass("active"),this.filters.streams=
c):this.filters.streams=[];this.reload();this.storeview();return this},filterstreams:function(a,b){b||(b=this.button&&this.button.data("streams")==$(a.currentTarget).data("streams"));this.togglestreams(b);b||(this.button=$(a.currentTarget).addClass("active").removeClass("inactive"));var c=b?null:Number(this.button.data("streams"));b&&(this.button=!1);c?($('[data-networks~="'+c+'"]').removeClass("inactive").addClass("active"),this.filters.streams=[c]):this.filters.streams=[];this.reload();this.storeview();
return this},filterparameters:function(){var a={},b="news"==this.timelinetype?"contacts":"streams";$.extend(!0,a,this.parameters);a[b]=this.filters.streams.length?this.filters.streams.join(","):[];return a},togglefilter:function(a){a=$(a.currentTarget);var b=a.data("toggle"),c=a.hasClass("selected");this.$el.find("[data-toggle].selected").removeClass("selected");this.$el.find("[id^=filter_]").slideUp("fast");c||(a.addClass("selected"),this.$el.find("#filter_"+b).slideDown("fast"))},fill:function(a){a.models&&
(a=a.models);this.incremental?this.incremental=!1:($.each(this.entries,function(a,b){b.remove()}),this.entries=[]);for(n in a){a[n].attributes.showcontact=this.showcontact;var b=new Cloudwalkers.Views.Entry({model:a[n],template:"newmessagetimeline",type:"full",parameters:{trendview:this.trending}});this.entries.push(b);this.$container.append(b.render().el)}this.collection.cursor?this.$container.append(this.$loadmore):a.length?this.$loadmore.remove():this.$container.append(this.$nocontent);this.hideloading()},
toggleallnetworks:function(a){this.filternetworks(null,!0)},togglefilters:function(a){a=a.join(" ");this.$el.find("[data-network-streams]").removeClass("inactive");this.$el.find("[data-network-streams='"+a+"']").addClass("inactive");this.$el.find(".toggleall").addClass("inactive")},togglestreams:function(a){this.$el.find("[data-networks], [data-streams]").addClass(a?"active":"inactive").removeClass(a?"inactive":"active");this.$el.find(".toggleall").addClass(a?"inactive":"active").removeClass(a?"active":
"inactive")},storeview:function(){var a=Cloudwalkers.Session.viewsettings(this.model.get("type"));a&&(a.streams||(a.streams=[]),JSON.stringify(a.streams)!=JSON.stringify(this.filters.streams)&&(a.streams=this.filters.streams,Cloudwalkers.Session.viewsettings(this.model.get("type"),a)))},more:function(){this.incremental=!0;this.$el.find(".load-more .timeline-icon").addClass("entry-loading");this.$el.find(".load-more .timeline-body span").html(this.translateString("loading")+"...");this.collection.more(this.model,
this.filterparameters())||this.$el.find(".load-more").hide()},resize:function(a){this.$el.css("min-height",a)},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original="loading view_more no_messages comments filters select_all more".split(" ");this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Trending=Cloudwalkers.Views.Pageview.extend({initialize:function(a){a&&$.extend(this,a);this.listenTo(this.model.streams,"seed",this.fill);this.listenTo(this.model.streams,"request",this.showloading);this.listenTo(this.model.streams,"sync",this.hideloading)},render:function(){var a={streams:[],networks:this.model.streams.filterNetworks(null,!0)};this.model.streams.each(function(b){a.streams.push({id:b.id,icon:b.get("network").icon,name:b.get("defaultname")})});this.$el.html(Mustache.render(Templates.trending,
...
!1;e.showStreamName=!1;this.datepicker.on("date:change",function(a,b){e.getDataset().setDateRange(a,b)});this.add(e)}});Cloudwalkers.Views.Statistics=Cloudwalkers.Views.Pageview.extend({id:"statistics",start:0,end:0,timespan:"default",period:0,custom:!1,views:[],events:{remove:"destroy","click #add":"addperiod","click #subtract":"subtractperiod","click #subtractempty":"subtractempty","click #addempty":"addempty","click #showreports":"showreports","click #now":"now","click #show":"changecustom","change .stats-header select.networks":"changestream","change .stats-header select.time":"changespan","click .dashboard-stat":"updatenetwork"},
widgets:[{widget:"StatSummary",data:{columnviews:["contacts","score-trending","outgoing","coworkers"]},span:12},{widget:"TitleSeparator",data:{translation:{title:"contacts_info"}}},{widget:"Chart",data:{filterfunc:"contacts",chart:"PieChart",translation:{title:"contacts"},display:"divided"},span:6},{widget:"CompoundChart",span:6,data:{template:"2col1row",chartdata:[{widget:"Chart",data:{filterfunc:"age",chart:"PieChart",translation:{title:"by_age"}}},{widget:"Chart",data:{filterfunc:"gender",chart:"PieChart",
translation:{title:"by_gender"}}},{widget:"Chart",data:{filterfunc:"contact-evolution",chart:"LineChart",translation:{title:"contacts_evolution"}}}]}},{widget:"TitleSeparator",data:{translation:{title:"new_this"}}},{widget:"Info",data:{translation:{title:"contact_evolution"},filterfunc:"contact-evolution"},span:3},{widget:"Info",data:{translation:{title:"post_activity"},filterfunc:"post-activity"},span:3},{widget:"Info",data:{title:"Activity?",filterfunc:"activity"},span:3},{widget:"Info",data:{title:"Page Views?",
filterfunc:"page-views"},span:3},{widget:"TitleSeparator",data:{translation:{title:"messages_info"}}},{widget:"TrendingMessage",data:{translation:{title:"top_rated_comment"}},span:12},{widget:"BestTimeToPost",data:{filterfunc:"besttime",chart:"LineChart",translation:{title:"best_time_to_post"}},span:4},{widget:"Chart",data:{filterfunc:"message-evolution",chart:"LineChart",translation:{title:"messages_evolution"}},span:4},{widget:"HeatCalendar",data:{filterfunc:"activity",translation:{title:"activity_calendar"}},
span:4},{widget:"TitleSeparator",data:{translation:{title:"geo_graphics"}}},{widget:"Chart",data:{filterfunc:"geo",type:"dots",chart:"GeoChart",translation:{title:"countries"},connect:!0},span:8},{widget:"CompoundChart",span:4,data:{template:"2row",chartdata:[{widget:"Chart",data:{filterfunc:"regional",chart:"PieChart",translation:{title:"countries"}},connect:"regional"},{widget:"Chart",data:{filterfunc:"cities",chart:"PieChart",translation:{title:"cities"}}}]}}],initialize:function(a){a&&$.extend(this,
a);this.collection=new Cloudwalkers.Collections.Statistics;this.listenTo(this.collection,"request",this.showloading);this.listenTo(this.collection,"seed",this.fillcharts);this.listenTo(this.collection,"sync:data",this.hideloading);this.listenTo(this.collection,"sync:noresults",this.showempty);translate={new_this:this.translateString("new_this"),new_this_m:this.translateString("new_this_m"),top_rated_comment:this.translateString("top_rated_comment"),messages_evolution:this.translateString("messages_evolution"),
activity_calendar:this.translateString("activity_calendar")};this.streamid=parseInt(this.streamid)},render:function(){var a=this.timemanager();a.streams=Cloudwalkers.Session.getStreams().where({statistics:1}).map(function(a){a.attributes.selected=a.id==this.streamid;return a.attributes}.bind(this));this.mustacheTranslateRender(a);this.$el.html(Mustache.render(Templates.statsview,a));this.$container=this.$el.find("#widgetcontainer").eq(0);this.$el.find("select").chosen({width:"200px",disable_search_threshold:10,
inherit_select_classes:!0});"custom"==this.timespan&&this.$el.find("#start, #end").datepicker({format:"dd-mm-yyyy"});0==this.period&&this.$el.find("#add").attr("disabled",!0);google.load("visualization","1",{callback:this.request.bind(this),packages:["corechart"]});return this},writeui:function(){var a=this.timemanager();this.$el.find(".stats-header-timeview").eq(0).html("<strong>"+a.fullperiod+": </strong>"+a.timeview)},request:function(a){a&&"default"==this.timespan&&(this.timespan="week");this.writeui();
this.collection.touch(this.filterparameters());this.currentperiod=a},fillcharts:function(a){if(a&&!a.length)return this.showempty();this.cleanviews();this.widgets.forEach(function(a){(a.data.translation||a.data.chartdata)&&this.translatechart(a);if(this.streamid&&this.streamdata(a))a.data=this.streamdata(a);else if(this.streamid)return;a.data.parentview=this;a.data.timespan={since:this.start.unix(),to:this.end.unix()};var c=new Cloudwalkers.Views.Widgets[a.widget](a.data);this.views.push(c);this.appendWidget(c,
a.span)}.bind(this))},translatechart:function(a){if(a.data.translation)this.translateWidgets(a.data);else if(a.data.chartdata)for(var b in a.data.chartdata)this.translateWidgets(a.data.chartdata[b].data)},timemanager:function(){var a=this.filterparameters(),b=moment.unix(a.since).zone(0),c=24<b.date()||"quarter"==a.span?11==b.month()?"DD MMM YYYY":"DD MMM":"DD";"default"==this.timespan?(a.periodstring="last 7 days",a.span=""):a.periodstring=this.period||"custom"==a.span?-1==this.period?"last ":"":
"this ";a.timeview=b.format(c)+" - "+moment.unix(a.until).zone(0).format("DD MMM YYYY");a[a.span+"Active"]=!0;a.startstring=moment.unix(a.since).zone(0).format("DD-MM-YYYY");a.endstring=moment.unix(a.until).zone(0).format("DD-MM-YYYY");a.fullperiod="pt_PT"==Cloudwalkers.Session.user.attributes.locale?this.translateString(a.span)+" "+this.translateString(a.periodstring):this.translateString(a.periodstring)+" "+this.translateString(a.span);return a},showloading:function(){this.$el.addClass("loading")},
hideloading:function(){this.$el.removeClass("loading");this.$el.find(".period-buttons .btn").attr("disabled",!1);this.$el.find("select").prop("disabled",!1).trigger("chosen:updated");this.listenToOnce(this,"change:period",this.request);this.currentperiod&&(this.period=this.currentperiod)},showempty:function(){this.cleanviews();this.hideloading();var a=new Cloudwalkers.Views.Widgets.EmptyStatisticsData({timeparams:this.timeparams,stream:this.streamid});this.views.push(a);this.appendWidget(a,8,null,
2)},now:function(){this.period=0;this.trigger("change:period",this.period)},addperiod:function(a){a=$(a.target).attr("disabled");0<=this.period||"disabled"==a||(this.period+=1,this.trigger("change:period",this.period))},subtractperiod:function(a){"disabled"!=$(a.target).attr("disabled")&&(this.period-=1,this.trigger("change:period",this.period))},subtractempty:function(){this.period-=1;this.trigger("change:period",this.period)},addempty:function(){this.period+=1;this.trigger("change:period",this.period)},
showreports:function(){var a=Number(this.$el.find("select.networks").val());a||(a=Cloudwalkers.Session.getStreams().where({statistics:1})[0].id);Cloudwalkers.Router.Instance.navigate(a?"#reports/"+a:"#reports",{trigger:!0})},changestream:function(){var a=Number(this.$el.find("select.networks").val());Cloudwalkers.Router.Instance.navigate(a?"#statistics/"+a:"#statistics",{trigger:!0})},changespan:function(){this.timespan=this.$el.find("select.time").val();this.trigger("change:period")},changecustom:function(){this.start=
moment(this.$el.find("#start").val(),"DD-MM-YYYY");this.end=moment(this.$el.find("#end").val(),"DD-MM-YYYY");this.trigger("change:period")},filterparameters:function(){"default"==this.timespan&&(this.start=moment().subtract(7,"days").zone(0).endOf("day"),this.end=moment().zone(0).endOf("day"));"now"==this.timespan&&(this.period=0);"week"==this.timespan&&(this.start=moment().zone(0).startOf("isoweek"),this.end=moment().zone(0).endOf("isoweek"));"month"==this.timespan&&(this.start=moment().zone(0).startOf("month"),
this.end=moment().zone(0).endOf("month"));"year"==this.timespan&&(this.start=moment().zone(0).startOf("year"),this.end=moment().zone(0).endOf("year"));if("quarter"==this.timespan){var a=3*(this.period+moment().quarter());this.start=moment().startOf("year").add("months",a-3);this.end=moment().startOf("year").add("months",a-1).endOf("month")}else"custom"!=this.timespan&&(0<this.period&&(this.start.add(this.timespan+"s",this.period),this.end.add(this.timespan+"s",this.period)),0>this.period&&(this.start.subtract(this.timespan+
"s",Math.abs(this.period)),this.end.subtract(this.timespan+"s",Math.abs(this.period))));0!=this.period?this.$el.find("#now").eq(0).removeClass("hidden"):this.$el.find("#now").eq(0).addClass("hidden");this.timeparams={since:this.start.unix(),until:this.end.unix(),span:this.timespan};return{since:this.start.unix(),until:this.end.unix(),span:this.timespan,period:this.period,reset:!0}},finish:function(){},updatenetwork:function(a){},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},
translateWidgets:function(a){if(a.translation)for(k in a.translation)a[k]=Cloudwalkers.Session.polyglot.t(a.translation[k])},mustacheTranslateRender:function(a){this.original="week month quarter year custom time_spans all_networks now show".split(" ");this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.StatStream=Cloudwalkers.Views.Statistics.extend({widgets:[{widget:"StatSummary",data:{columnviews:["contacts-network","score-trending-network","outgoing-network","besttime"]},span:12},{widget:"TitleSeparator",data:{title:"Contacts info"}},{widget:"Chart",data:{filterfunc:"contact-evolution-network",chart:"LineChart",title:"Contacts Evolution"},span:6},{widget:"Chart",data:{filterfunc:"age",chart:"PieChart",title:"By Age"},span:3,networks:["facebook"]},{widget:"Chart",data:{filterfunc:"gender",
chart:"PieChart",title:"By gender"},span:3,networks:["facebook"]},{widget:"Chart",data:{filterfunc:"follow",chart:"PieChart",title:"By Type"},span:3,networks:["twitter","linkedin","youtube"]},{widget:"TitleSeparator",data:{title:"Network info"}},{widget:"Info",data:{title:"New impressions",filterfunc:"page-views-network"},span:3,nonetworks:["twitter"]},{widget:"Info",data:{title:"New shares",filterfunc:"shares"},span:3,nonetworks:["twitter"]},{widget:"Info",data:{title:"New posts",filterfunc:"posts"},
span:3,nonetworks:["twitter"]},{widget:"Info",data:{title:"New direct messages",filterfunc:"dms"},span:3,nonetworks:["twitter"]},{widget:"Info",data:{title:"New shares",filterfunc:"shares"},span:4,networks:["twitter"]},{widget:"Info",data:{title:"New posts",filterfunc:"posts"},span:4,networks:["twitter"]},{widget:"Info",data:{title:"New direct messages",filterfunc:"dms"},span:4,networks:["twitter"]},{widget:"TitleSeparator",data:{title:"Messages info"}},{widget:"TrendingMessage",data:{title:"Top rated comment"},
span:12},{widget:"BestTimeToPost",data:{filterfunc:"besttime",chart:"LineChart",title:"Best Time to Post"},span:4},{widget:"Chart",data:{filterfunc:"message-evolution-network",chart:"LineChart",title:"Messages Evolution"},span:4},{widget:"HeatCalendar",data:{filterfunc:"activity",title:"Activity Calendar"},span:4},{widget:"TitleSeparator",data:{title:"Geo Graphics"},networks:["facebook"]},{widget:"Chart",data:{filterfunc:"geo",type:"dots",chart:"GeoChart",title:"Countries",connect:!0},networks:["facebook"],
span:8},{widget:"CompoundChart",span:4,data:{template:"2row",chartdata:[{widget:"Chart",data:{filterfunc:"regional",chart:"PieChart",title:"Countries"},connect:"regional"},{widget:"Chart",data:{filterfunc:"cities",chart:"PieChart",title:"Cities"}}]},networks:["facebook"]}],streamdata:function(a){var b=Cloudwalkers.Session.getStream(Number(this.streamid)).get("network").token,c={};if(a.networks&&0>a.networks.indexOf(b)||a.nonetworks&&0<=a.nonetworks.indexOf(b))return!1;c.network=this.streamid;return _.extend(a.data,
c)}});Cloudwalkers.Views.Settings=Cloudwalkers.Views.Pageview.extend({title:"Settings",tabs:[],initialize:function(){this.endpoint=this.options.endpoint;this.level=Cloudwalkers.Session.user.level},setAction:function(a){this.action=a},render:function(){var a={};this.tabs=[];this.mustacheTranslateRender(a);Cloudwalkers.Session.isAuthorized("USER_INVITE")&&this.tabs.push({url:"#settings/users",name:a.translate_manage_users});Cloudwalkers.Session.isAuthorized("SERVICE_CONNECT")&&this.tabs.push({url:"#settings/services",
...
["manage_users","social_connections","account_settings","manage_user_groups","profile_settings"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Firsttime=Cloudwalkers.Views.Pageview.extend({title:"First Time",events:{remove:"destroy","click [data-add-service]":"addService","click #ready":"ready"},initialize:function(){this.services=new Cloudwalkers.Collections.Services;this.listenTo(this.services,"available:ready",this.appendOptions);this.services.fetchAvailable()},render:function(){Backbone.history.fragment="settings/services";$("#sidebar").addClass("collapsed");$("#inner-content").addClass("expanded");params={displayname:Cloudwalkers.Session.user.get("displayname")};
...
translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=["welcome","lets_get_started","update_your_profile"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.RSSFeed=Cloudwalkers.Views.Pageview.extend({id:"rss",parameters:{records:20,markasread:!0},entries:[],events:{"click .message":"openMessage","click .message *":"openMessage","click [data-rss-select]":"toggleList"},initialize:function(a){a&&$.extend(this,a);this.translateTitle("rss_feed");this.$el.addClass("loading")},hideloading:function(){this.$el.removeClass("loading");this.$el.find(".timeline-loading").hide()},render:function(){var a={title:this.title};this.mustacheTranslateRender(a);
this.$el.html(Mustache.render(Templates.rssfeed,a));return this},openMessage:function(a){$(a.target).closest(".rssfeed-container").children().children().removeClass("opened");$(a.target).closest(".rssfeed-container").children().removeClass("span12");$(a.target).closest(".rssfeed-container").children().addClass("span6");$(a.target).hasClass("message")?($(a.target).addClass("opened"),$(a.target).parent().removeClass("span6"),$(a.target).parent().addClass("span12")):($(a.target).closest(".message").addClass("opened"),
$(a.target).closest(".message").parent().removeClass("span6"),$(a.target).closest(".message").parent().addClass("span12"))},toggleList:function(a){var b=$(a.target).data().rssSelect;this.$el.find(".rssfeed-container").removeClass("cards");this.$el.find(".rssfeed-container").removeClass("list");this.$el.find(".rssfeed-container").addClass(b);this.$el.find(".rssfeed-selector").children().removeClass("active");$(a.target).hasClass("option")?$(a.target).addClass("active"):$(a.target).parent().addClass("active")},
translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},translateTitle:function(a){this.title=Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=["loading"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.Widget=Backbone.View.extend({title:"Untitled widget",icon:"inbox",color:"blue",network:null,entries:[],events:{},tools:[],initialize:function(){this.options.color||(this.options.color=this.color);this.initializeWidget()},initializeWidget:function(){this.bind("destroy",this.onDestroy);this.bind("add",function(){console.log("View add triggrered")})},innerRender:function(a){a.html("No inner content set.")},render:function(){"undefined"!=typeof this.options.title&&(this.title=
...
disableFadeOut:!0})},onDestroy:function(){$.each(this.entries,function(a,b){b.trigger("destroy")});this.$el.find(".scroller").slimScroll({destroy:1});this.remove()},collapse:function(){this.$el.toggleClass("collapse-closed collapse-open")}});Cloudwalkers.Views.Entry=Backbone.View.extend({tagName:"li",template:"messageentry",notifications:[],parameters:{},tokenmap:{favorite:"favourites",retweet:"retweets",like:"likes"},loadedlists:[],events:{remove:"destroy","click [data-notifications]":"loadNotifications","click *[data-action]":"action",click:"toggle"},initialize:function(a){this.parameters={};a&&$.extend(this,a);this.loadmylisteners()},loadmylisteners:function(){this.listenTo(this.model,"change",this.render);this.listenTo(this.model,
"action:toggle",this.toggleaction);this.listenTo(this.model,"destroyed",this.remove)},render:function(){$.extend(this.parameters,this.model.attributes);"full"==this.type&&this.model.get("objectType")&&(this.parameters.actions=this.model.filterActions());"newmessagetimeline"==this.template&&this.formatactions(this.parameters);Cloudwalkers.Session.censuretemplate(this.parameters);for(n in this.parameters.actions)this.parameters.actions[n].name_translated=this.translateString(this.parameters.actions[n].name);
this.parameters.hasactions=this.parameters.actions&&!this.parameters.actions.length?!1:!0;for(n in this.parameters.statistics)this.parameters.statistics[n].name_translated=this.translateString(this.parameters.statistics[n].name);this.mustacheTranslateRender(this.parameters);this.parameters.hasnotes=this.model.hasnotes();this.$el.html(Mustache.render(Templates[this.template],this.parameters));this.$el.find("[data-date]")&&this.time();this.checkunread&&this.model.get("objectType")&&this.checkUnread();
Cloudwalkers.Session.isAuthorized("ACCOUNT_NOTES_VIEW")&&(this.$el.find(".note-list").html("<li>"+Mustache.render(Templates.messagenote)+"</li>"),this.loadnoteui());this.parameters.hasactions&&this.renderactions();this.model.get("status")&&"FAILED"==this.model.get("status")&&(this.$el.addClass("failed"),this.model.attributes.failed="failed");this.$el.find(".youtube-video").colorbox({iframe:!0,innerWidth:640,innerHeight:390,opacity:0.7});return this},loadsentiment:function(){var a=this.parameters.stats;
a&&(a=_.isObject(a)?a["sentiment-fake"]:null)&&(a=5>Math.floor(a/2)?Math.floor(a/2):4,a=new Cloudwalkers.Views.Widgets.Sentiment({sentiment:a}),this.$el.find(".sentiment-wrap").append(a.render().el))},renderactions:function(){this.actions=new Cloudwalkers.Views.Actions({message:this.model});this.$el.find(".message-actions").html(this.actions.render().el);this.loadedlists=[]},updatetimelineactions:function(){var a=this.actions.message.notes.length;a&&this.$el.find(".interaction > .notescount").html(a)},
action:function(a){var b=$(a.currentTarget).data("action");if("note"==b||"action-list"==b){var c=$(a.currentTarget).data("token");this.toggleactions(b,c,a)}else if("note-edit"==b)this.editnote();else if("tag-showedit"==b)this.showtagedit();else if("tag-add"==b){if(b=$(a.currentTarget).siblings("input").val())this.submittag(b),$(a.currentTarget).siblings("input").val("")}else"viewcontact"==b?this.parent||(a=this.model.attributes.from?this.model.attributes.from[0]:null)&&Cloudwalkers.RootView.viewContact({model:a}):
this.model.trigger("action",b)},formatactions:function(a){var b=a.actions,c=a.statistics,e=[];c&&($.each(b,function(b,g){if("comment"==g.token&&a.canHaveChildren)e.push({token:g.token,action:{icon:"comment",value:a.children_count}});else{var h=c.filter(function(a){return a.token==this.tokenmap[g.token]}.bind(this));e.push(h.length?{token:g.token,action:h[0]}:{token:g.token,action:g})}}.bind(this)),a.actionstats=e)},editnote:function(){var a=new Cloudwalkers.Views.SimpleCompose({model:this.model});
this.stopListening(this.model);this.listenTo(a,"edit:cancel",this.canceledit);this.listenTo(a,"save:success",this.saved);this.$el.find(".message-body").addClass("note-content").html(a.render().el);this.$el.find(".message-actions").addClass("hidden");this.$el.find(".toggle-note-actions").toggle()},showtagedit:function(){this.$el.find(".message-tags").toggleClass("enabled");this.$el.find(".message-tags .edit").toggleClass("inactive")},canceledit:function(a){this.loadmylisteners();this.$el.find(".message-actions").removeClass("hidden");
a?(this.toggleactions("note","note"),this.$el.find(".note-content .modal-body textarea").val("")):(this.$el.find(".message-body").removeClass("note-content").html(this.model.get("text")),this.$el.find(".toggle-note-actions").toggle())},saved:function(){setTimeout(function(){this.canceledit();this.loadmylisteners()}.bind(this),200)},toggleactions:function(a,b,c){c&&$(c.currentTarget).hasClass("noresults")||(c=this.$el.find("[data-action="+a+"][data-token="+b+"]"),c.hasClass("inactive")?c.hasClass("inactive")&&
this.processaction("close"):this.processaction("open",a,b))},processaction:function(a,b,c){var e=this.$el.find("[data-action="+b+"][data-token="+c+"]"),f=this.$el.find(".actionvalue.inactive"),g=this.$el.find(".actionname.inactive"),h=this.$el.find(".action-list"),m=this.$el.find(".note-content"),l=!1;if(h.is(":visible")||m.is(":visible"))l=!0;f.removeClass("inactive");g.removeClass("inactive");this.$el.find(".action-list").slideUp("fast");this.$el.find(".note-content").slideUp("fast");"close"!=a&&
setTimeout(function(){e.addClass("inactive");"action-list"==b?this.expandlist(c):"note"==b&&m.slideDown()}.bind(this),l?200:1)},expandlist:function(a){if(0>this.loadedlists.indexOf(a+"list"))return this.fetchactions(a);this.$el.find(".action-list."+a+"-list").slideDown()},fetchactions:function(a){var b="note"==a?this.model.notes:this.model.notifications,c={records:999};"notes"!=a&&(c.markasread=!0);b.parentmodel=this.model;b.parenttype="message";this.listenTo(b,"seed",this.fillactions.bind(this,a));
b.touch(this.model,c);this.loadedlists.push(a+"list")},fillactions:function(a,b,c){c=a+"-list";if(!b.length)return this.$el.find(".action-list li").html("No actions found");this.$el.find("."+c).remove();this.$el.find(".action-lists").append('<ul class="action-list '+c+'"></ul>');for(n in b)this.addaction(b[n],a)},addaction:function(a,b){var c={model:a,template:"note"==b?"messagenote":"timelinecomment"},e=b+"-list";this.newaction&&(c.isnew=!0);a="note"==b?new Cloudwalkers.Views.Widgets.NoteEntry(c):
...
this.model.filterActions()));Cloudwalkers.Session.censuretemplate(this.parameters);for(n in this.parameters.actions)this.parameters.actions[n].name_translated=this.translateString(this.parameters.actions[n].name);this.$el.html(Mustache.render(Templates[this.template],this.parameters));this.$el.find("[data-date]")&&this.time();return this}});Cloudwalkers.Views.CounterEntry=Cloudwalkers.Views.Entry.extend({tagName:"a",initialize:function(a){$.extend(this,a);this.listenTo(this.model,"change",this.render)},render:function(){var a=this.data.typelink?this.data.typelink+"/"+(this.model.get("hasMessages")?"messages":"notifications"):this.data.link?this.data.link:"#"+this.data.type+"/"+this.data.channel.id+"/"+this.model.id;this.$el.attr("href",a);this.$el.attr("data-stream",this.model.id);"outgoing"==this.data.channel.get("type")?this.count=
this.model.get("counters").total.scheduled.messages.total:"inbox"==this.data.channel.get("type")?this.count=this.model.get("counters").total.incoming.any.unread:this.count=this.model.get("counters").recent.incoming.any.unread;a={name:this.model.get("name"),url:a,count:this.count,icon:this.model.get("network")?this.model.get("network").icon:this.data.icon};this.$el.html(Mustache.render(Templates.counterentry,a));return this}});Cloudwalkers.Views.User=Backbone.View.extend({tagName:"li",template:"user",events:{click:"select"},initialize:function(a){a&&$.extend(this,a);this.listenTo(this.model,"change",this.render)},render:function(){this.$el.html(Mustache.render(Templates[this.template],this.model.filterData(this.type)));return this},select:function(){this.trigger("select",this)}});Cloudwalkers.Views.Write=Backbone.View.extend({files:[],draft:!1,sendnow:!1,className:"write",navclass:"write",title:"Compose message",events:{"submit form":"submit","keyup textarea[name=message]":"updateCounter","keyup input[name=title]":"updateCounter","click #schedule-btn-toggle":"toggleSchedule","click button[value=draft]":"setDraft","click [name=delay]":"setWithinDate","change select[name=schedule_day],select[name=schedule_month],select[name=schedule_year],select[name=schedule_time]":"resetWithin",
...
wait:!0});this.model.get("stream")&&Cloudwalkers.Session.getStreams().outdated(this.model.get("stream"))},togglecommentcontact:function(){var a=this.model.attributes.from?this.model.attributes.from[0]:null;a&&Cloudwalkers.RootView.viewContact({model:a})}});Cloudwalkers.Views.Widgets.EmptyStatisticsData=Backbone.View.extend({title:"EmptyData",messageconfig:{now:{id:"subtractempty",text:"show_previous_{{span}}_statistics"},nodata:{id:"addempty",text:"show_next_{{span}}_statistics"},reports:{id:"showreports",text:"show_older_reports"}},initialize:function(a){$.extend(this,a)},render:function(){this.checknow();this.$el.html(Mustache.render(Templates.emptystatisticsdata,{message:this.message}));return this},checknow:function(){var a=1E3*this.timeparams.since,
b=1E3*this.timeparams.until,c=moment().utc().valueOf(),e;c<=b&&c>a&&(e="now");this.generatemessage(e)},generatemessage:function(a){var b=this.translateString("empty_statistics_data"),c=this.timeparams.span;if(a){var e=this.messageconfig[a].id;a=this.translateString(Mustache.render(this.messageconfig[a].text,{span:c}));this.message=b+Mustache.render('<br/><a id="{{id}}">{{text}}</a>',{id:e,text:a})}else this.getcreationdate()},getcreationdate:function(){var a;this.stream?(a=Cloudwalkers.Session.getStream(this.stream),
a.get("created")?this.parsecreationdate(a.get("created")):a.fetch({success:this.getstreamdate.bind(this)})):(a=Cloudwalkers.Session.getAccount().get("created"),this.parsecreationdate(a))},parsecreationdate:function(a){1410998400<=moment.unix(Date.parse(a)/1E3).utc().valueOf()/1E3?this.generatemessage("nodata"):this.generatemessage("reports")},getstreamdate:function(a){a.get("created")?this.parsecreationdate(a.get("created")):this.generatemessage("nodata")},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)}});Cloudwalkers.Views.Widgets.Chart=Backbone.View.extend({title:"Chart",events:{},columns:{contacts:"parsecontacts",age:"parseage",gender:"parsegender",regional:"parseregional",besttime:"parsebesttime",cities:"parsecities",networks:"parsenetworks",activity:"parsecalendar","contact-evolution":"parsecontactevolution","message-evolution":"parsemessageevolution",messages:"parsemessages",activities:"parseactivities",impressions:"parseimpressions",notifications:"parsenotifications",followers:"parsefollowers",
...
{};this.settings.title=this.title;this.$el.html(Mustache.render(Templates.chart,this.settings));return this},fill:function(){if(this.collection.latest()&&this.collection.latest().get("streams")){var a,b;b=this[this.columns[this.filterfunc]](this.collection);if("besttime"==this.filterfunc)this.renderbesttime(b);else{var c=this.$el.find(".chart-container").get(0).clientWidth,c={pieHole:0.4,chartArea:{width:"95%",height:"90%"},vAxis:{minValue:0},width:c,height:0.7*c,legend:{textStyle:{fontSize:"13"}},
tooltip:{textStyle:{fontSize:"13"}}};a={displayMode:"markers",colorAxis:{colors:["#E27927","#91828D"]}};"geo"==this.filterfunc&&"dots"==this.type&&$.extend(c,a);$.extend(c,b.options);_.map(b.data,function(a){"Female"==a[0]&&(a[0]=this.translateString("female"));"Male"==a[0]&&(a[0]=this.translateString("male"))}.bind(this));b.data=google.visualization.arrayToDataTable(b.data);a=new google.visualization[this.chart](this.$el.find(".chart-container").get(0));a.draw(b.data,c);this.removeloading()}}},removeloading:function(){this.$el.find(".loading").removeClass("loading")},
renderbesttime:function(a){this.$el.html(Mustache.render(Templates.besttimewrap,this.settings));$.each(a,function(b,c){c.fill=100*c.value/a.maxvalue;this.$el.find(".chart-wrapper").append(Mustache.render(Templates.besttime,c))}.bind(this))},parsecontacts:function(a,b,c){var e={},f=0,g,h={data:[],colors:[],options:{}};g=b?b:this.collection.latest().get("streams");$.each(g,function(a,b){b=new Cloudwalkers.Models.Stream(b);var f=new Cloudwalkers.Models.Network(Cloudwalkers.Session.getStream(b.id).get("network")),
g=b.getcontacts();if(c&&f.gettoken()!=c)return!0;_.isNumber(g)&&_.has(e,f.gettoken())?e[f.gettoken()].addcontacts(g):e[f.gettoken()]=f.addcontacts(g)});e=_.sortBy(e,function(a){return a.get("contacts")});$.each(e,function(a,b){h.data.push([b.gettitle(),b.getcontacts()]);h.colors.push(b.getcolor());f+=b.getcontacts()});if(a&&(0==h.data.length||0==f))return this.emptychartdata();h.options.colors=h.colors;c||b||h.data.unshift(["Network",this.translateString("number_of_contacts")]);return h},parsemessages:function(a,
b,c){var e={},f={data:[],colors:[]};a=b?b:a.latest().get("streams");$.each(a,function(a,b){b=new Cloudwalkers.Models.Stream(b);var f=new Cloudwalkers.Models.Network(Cloudwalkers.Session.getStream(b.id).get("network")),l=b.getmessages();if(c&&f.gettoken()!=c)return!0;_.isNumber(l)&&_.has(e,f.gettoken())?e[f.gettoken()].messages+=l:(e[f.gettoken()]=f,e[f.gettoken()].messages=l)});e=_.sortBy(e,function(a){return a.get("messages")});$.each(e,function(a,b){f.data.push([b.gettitle(),b.messages]);f.colors.push(b.getcolor())});
c||b||f.data.unshift(["Network",this.translateString("number_of_contacts")]);return f},parsecontactevolution:function(a){return this.parseevolution(a,"contacts")},parsecontactevolutionnetwork:function(a){a=a.models;var b=this.network,c=this.$el.find(".chart-container").get(0).clientWidth,e={data:[],options:{colors:["#333333"],chartArea:{width:"70%",height:"70%",left:"40"},width:c,height:0.3*c,curveType:"function"}};$.each(a,function(a,c){var h=moment(c.get("date")).format("D MMM"),m=c.pluck("contacts",
b);e.data.push([h,m])}.bind(this));e.data.unshift(["Day",this.translateString("number_of_contacts")]);return e},parsemessageevolution:function(a){a=a.models;var b=this.$el.find(".chart-container").get(0).clientWidth,c={data:[],options:{colors:["#333333"],chartArea:{width:"90%",height:"70%",left:"40"},width:b,height:350<b?350:0.92*b,legend:{position:"bottom",textStyle:{fontSize:"13"}},curveType:"function",hAxis:{textStyle:{fontSize:"10"}}}};$.each(a,function(a,b){var g=moment(b.get("date")).format("D MMM"),
h=b.pluck("messages");c.data.push([g,h])}.bind(this));c.data.unshift(["Day",this.translateString("number_of_messages")]);return c},parsemessageevolutionnetwork:function(a){a=a.models;var b=this.network,c=this.$el.find(".chart-container").get(0).clientWidth,e={data:[],options:{colors:["#333333"],chartArea:{width:"70%",height:"70%",left:"40"},width:c,height:0.65*c,legend:{position:"bottom"},curveType:"function"}};$.each(a,function(a,c){var h=moment(c.get("date")).format("D MMM"),m=c.pluck("messages",
b);e.data.push([h,m])}.bind(this));e.data.unshift(["Day",this.translateString("number_of_messages")]);return e},parseactivities:function(a,b,c){var e={},f={data:[],colors:[]};a=b?b:a.latest().get("streams");$.each(a,function(a,b){b=new Cloudwalkers.Models.Stream(b);var f=new Cloudwalkers.Models.Network(Cloudwalkers.Session.getStream(b.id).get("network")),l=b.getactivities();if(c&&f.gettoken()!=c)return!0;_.isNumber(l)&&_.has(e,f.gettoken())?e[f.gettoken()].activities+=l:(e[f.gettoken()]=f,e[f.gettoken()].activities=
l)});e=_.sortBy(e,function(a){return a.get("activities")});$.each(e,function(a,b){f.data.push([b.gettitle(),b.activities]);f.colors.push(b.getcolor())});c||b||f.data.unshift(["Network",this.translateString("number_of_contacts")]);return f},parseimpressions:function(a,b,c){var e={},f={data:[],colors:[]};a=b?b:a.latest().get("streams");$.each(a,function(a,b){b=new Cloudwalkers.Models.Stream(b);var f=new Cloudwalkers.Models.Network(Cloudwalkers.Session.getStream(b.id).get("network")),l=b.getimpressions();
if(c&&f.gettoken()!=c)return!0;_.isNumber(l)&&_.has(e,f.gettoken())?e[f.gettoken()].impressions+=l:(e[f.gettoken()]=f,e[f.gettoken()].impressions=l)});e=_.sortBy(e,function(a){return a.get("impressions")});$.each(e,function(a,b){f.data.push([b.gettitle(),b.impressions]);f.colors.push(b.getcolor())});c||b||f.data.unshift(["Network",this.translateString("number_of_contacts")]);return f},parsenotifications:function(a,b,c){var e={},f={data:[],colors:[]};a=b?b:a.latest().get("streams");$.each(a,function(a,
b){b=new Cloudwalkers.Models.Stream(b);var f=new Cloudwalkers.Models.Network(Cloudwalkers.Session.getStream(b.id).get("network")),l=b.getnotifications();if(c&&f.gettoken()!=c)return!0;_.isNumber(l)&&_.has(e,f.gettoken())?e[f.gettoken()].notifications+=l:(e[f.gettoken()]=f,e[f.gettoken()].notifications=l)});e=_.sortBy(e,function(a){return a.get("notifications")});$.each(e,function(a,b){f.data.push([b.gettitle(),b.notifications]);f.colors.push(b.getcolor())});c||b||f.data.unshift(["Network",this.translateString("number_of_contacts")]);
return f},parsefollowers:function(a,b,c){var e={},f={data:[],colors:[]};a=b?b:a.latest().get("streams");$.each(a,function(a,b){b=new Cloudwalkers.Models.Stream(b);var f=new Cloudwalkers.Models.Network(Cloudwalkers.Session.getStream(b.id).get("network")),l=b.getfollowers();if(c&&f.gettoken()!=c)return!0;_.isNumber(l)&&_.has(e,f.gettoken())?e[f.gettoken()].followers+=l:(e[f.gettoken()]=f,e[f.gettoken()].followers=l)});e=_.sortBy(e,function(a){return a.get("followers")});$.each(e,function(a,b){f.data.push([b.gettitle(),
b.followers]);f.colors.push(b.getcolor())});c||b||f.data.unshift(["Network",this.translateString("number_of_contacts")]);return f},parsefollowing:function(a,b,c){var e={},f={data:[],colors:[]};a=b?b:a.latest().get("streams");$.each(a,function(a,b){b=new Cloudwalkers.Models.Stream(b);var f=new Cloudwalkers.Models.Network(Cloudwalkers.Session.getStream(b.id).get("network")),l=b.getfollowing();if(c&&f.gettoken()!=c)return!0;_.isNumber(l)&&_.has(e,f.gettoken())?e[f.gettoken()].following+=l:(e[f.gettoken()]=
f,e[f.gettoken()].following=l)});e=_.sortBy(e,function(a){return a.get("following")});$.each(e,function(a,b){f.data.push([b.gettitle(),b.following]);f.colors.push(b.getcolor())});c||b||f.data.unshift(["Network",this.translateString("number_of_contacts")]);return f},parsefollow:function(a){var b=a.latest();a=b.pluck(["contacts","types","followers"],this.network,3);var b=b.pluck(["contacts","types","following"],this.network,3),c={data:[["Followers",a],["Following",b]],options:{colors:["#2bbedc","#324A4F"]}};
if(0>=b+a)return this.emptychartdata();c.data.unshift(["Follow state",this.translateString("number_of_contacts")]);return c},parseage:function(a){a=a.latest().get("streams");a=this.groupkey(a,"contacts","age");var b=0,c=[],e={data:[],options:{colors:"#2bbedc #2CA7C0 #2E90A4 #2F7988 #30616B #324A4F #333333".split(" ")}};$.each(a,function(a,e){c.push([a,e]);b+=e});c=_.sortBy(c,function(a){return a.title});if(0==c.length||0==b)return this.emptychartdata();e.data=c;e.data.unshift(["Age interval",this.translateString("number_of_contacts")]);
return e},groupkey:function(a,b,c){var e={};$.each(a,function(a,g){if(this.network&&g.id==this.network||!this.network){var h=g[b][c];_.isObject(h)&&(_.isEmpty(e)?e=h:$.each(h,function(a,b){e[a]+=b}))}}.bind(this));return e},parsegender:function(a){var b=a.latest().get("streams"),b=this.groupkey(b,"contacts","gender"),c=0,e=[];a={data:[],options:{colors:["#2bbedc","#F14B68",a.networkcolors.others]}};$.each(b,function(a,b){e.push([this.capitalize(a),b]);c+=b}.bind(this));if(0==e.length||0==c)return this.emptychartdata();
a.data=e;a.data.unshift(["Gender",this.translateString("number_of_contacts")]);return a},capitalize:function(a){return a.charAt(0).toUpperCase()+a.slice(1)},filtercountry:function(a,b){var c={},e=a.latest().get("streams");$.each(e,function(a,e){if(b&&e.id==b||!b){var h=Cloudwalkers.Session.getStream(e.id).get("network").name,m=Cloudwalkers.Session.getStream(e.id).get("network").token;if(_.isObject(e.contacts.geo)){var l=e.contacts.geo.countries;0==_.size(c)?$.each(l,function(a,b){c[b.name]=b;c[b.name].networks=
[];c[b.name].networks[h]={total:b.total,token:m}}):$.each(l,function(a,b){c[b.name]?(c[b.name].total+=b.total,c[b.name].networks[h]?c[b.name].networks[h].total+=b.total:c[b.name].networks[h]={total:b.total,token:m}):(c[b.name]=b,c[b.name].networks=[],c[b.name].networks[h]={total:b.total,token:m})})}}});return c=_(c).sortBy(function(a){return a.total})},parseregional:function(a,b){var c=8,e=0,f=this.filtercountry(a,b||this.network),g={data:[],options:{colors:this.colors}};this.regional=[];if(!c||c>
f.length)c=f.length;if(0==c)return this.emptychartdata();for(;e<c;){var h=f.pop();g.data.push([h.name,h.total]);this.regional.push(h);e++}c=_.reduce(f,function(a,b){return a+b.total},0);g.data.push(["Others",c]);g.data.unshift(["Countries",this.translateString("number_of_contacts")]);return g},parsecities:function(a){var b;a=this.filtercountry(a,this.network);b={};var c;if(0==a.length)return this.emptychartdata();c=a[a.length-1];countrycities=c.cities;$.each(countrycities,function(a,c){b[c.name]=
{name:c.name,value:c.total}});b=_(b).sortBy(function(a){return a.value});a=this.getbiggestdata(b,6);a.data.unshift(["Cities",this.translateString("number_of_contacts")]);this.$el.find("h4").text(c.name+", "+this.translateString("cities"));return a},parsenetworks:function(a){var b=this.connect.regional?this.connect.regional:this.parseregional(a),c={data:[],colors:[]};if(0==b.length)return this.emptychartdata();this.$el.find("h3").html(b[0].name);b=b[0].networks;for(i in b)c.data.push([i,b[i].total]),
c.colors.push(a.networkcolors[b[i].token]);c.data.unshift(["Network",this.translateString("number_of_contacts")]);return c},getbiggestdata:function(a,b){for(var c=0,e={data:[],options:{colors:[]}};c<b;){var f=a.pop();e.data.push([f.name,f.value]);e.options.colors.push(this.countrycolors[c]);c++}if(0==a.length)return e;c=_.reduce(a,function(a,b){return a+b.value},0);e.data.push(["Others",c]);e.options.colors.push(this.collection.networkcolors.others);return e},parsebesttime:function(a){var b="Mon Tue Wed Thu Fri Sat Sun".split(" "),
c,e=[],f=0,g=0,h;a.forEach(function(a){var l=[];a.get("streams").forEach(function(a){a=new Cloudwalkers.Models.Stream(a);if(c=a.getbesttime())if(0==l.length)l=_.values(c);else for(i in c)l[i]+=c[i],l[i]>f&&(f=l[i]),l[i]>g&&(g=l[i])});h=l.indexOf(Math.max.apply(Math,_.values(l)));e.push({day:b.shift(),value:g,time:h});g=0});e.maxvalue=f;return e},parseevolution:function(a,b){for(var c=b?this.columns[b]:this.columns[this.type],e=this.collection.length,f=this.network?this.network:null,g,h=["Day of the week"],
m={},l=this.$el.find(".chart-container").get(0).clientWidth,l={data:[],options:{chartArea:{width:"70%",height:"60%",left:"40",top:"50"},width:l,height:0.4*l,legend:{textStyle:{fontSize:"11"},position:"top"},curveType:"function",colors:[]}},r=0;r<e;r++)g=a.place(r).get("streams"),dailyresult=this[c](null,g,f),_.map(dailyresult.data,function(a){m[a[0]]||(m[a[0]]=[]);m[a[0]][r]=a[1]});for(r=0;r<e;r++){c=[moment(a.place(r).get("date")).format("D MMM")];for(d in m)c.push(m[d].shift());l.data.push(c)}for(d in m)h.push(d),
l.options.colors.push(this.networktokens[d]);l.data.unshift(h);return l},parseallreports:function(a){var b=this.parseevolution(a,"contacts").data,c=this.parseevolution(a,"messages").data,e=this.parseevolution(a,"notifications").data;a=this.$el.find(".chart-container").get(0).clientWidth;a={data:[],colors:["#E27927","#9E1818","#68114F","#783E68","#896C80"],options:{chartArea:{width:"90%",height:"90%"},width:a,height:0.7*a,legend:{textStyle:{fontSize:"13"}},tooltip:{textStyle:{fontSize:"13"}},curveType:"function",
legend:{position:"bottom"}}};b.shift();c.shift();e.shift();$.each(b,function(a,g){b[a].push(c[a][1]);b[a].push(e[a][1])});a.data=b;a.data.unshift(["Day of the week","Contacts","Messages","Notifications"]);return a},parsegeo:function(a){return this.parseregional(a,this.network)},emptychartdata:function(){var a={data:[["No results",1]],options:{colors:["#e5e5e5"]},empty:!0};a.data.unshift(["Results","Number"]);return a},negotiateFunctionalities:function(){},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)}});Cloudwalkers.Views.Widgets.Info=Backbone.View.extend({title:"Info",columns:{"contact-evolution":"parseevolution","post-activity":"parsepostactivity",activity:"parseactivity","page-views":"parseviews",notifications:"parsenotifications","contact-evolution-network":"parseevolutionnetwork","post-activity-network":"parsepostactivitynetwork","activity-network":"parseactivitynetwork","page-views-network":"parseviewsnetwork",besttimetopost:"parsebesttimetopost",followers:"parsefollowers",following:"parsefollowing",
mentions:"parsementions",retweets:"parseretweets",shares:"parseshares",posts:"parseposts",dms:"parsedms"},descriptions:{contacts:{facebook:"New fans",twitter:"New followers",youtube:"New subscriptions",linkedin:"New followers"},messages:{facebook:"New mesages",twitter:"New tweets",youtube:"New uploads",linkedin:"New messages"},notifications:{facebook:"New notifications",twitter:"New notifications",youtube:"New notifications",linkedin:"New notifications"}},streamid:null,initialize:function(a){a&&$.extend(this,
a);!this.network&&this.parentview&&(this.network=this.parentview.streamid);this.settings={title:this.title,network:this.network?{icon:Cloudwalkers.Session.getStream(this.network).get("network").token}:{icon:"cloud"},streamid:this.network};this.settings.filterfunc=this.filterfunc;this.settings.footer=this.footer;this.collection=this.parentview.collection;this.listenTo(this.collection,"ready",this.fill);this.listenTo(this.collection,"change",this.render)},render:function(){this.$el.html(Mustache.render(Templates.dashboardstat,
this.settings));this.settings.details&&this.settings.details.length&&void 0!==this.settings.details[0].content&&this.$el.find(".dashboard-stat").removeClass("portlet-loading");return this},fill:function(){this.settings.details=this[this.columns[this.filterfunc]]();this.render()},parseevolution:function(){var a=this.collection.latest().pluck("contacts"),b=this.collection.first().pluck("contacts"),a=a-b,b=this.translateString("new_contacts");this.settings.network={icon:"group"};return[{content:a,descr:b}]},
parsepostactivity:function(){var a=this.collection.latest().pluck("messages"),b=this.collection.first().pluck("messages"),a=a-b,b=this.translateString("new_messages");this.settings.network={icon:"cloud-upload"};return[{content:a,descr:b}]},parseactivity:function(){var a=this.collection.latest().pluck("activities"),b=this.collection.first().pluck("activities"),a=a-b,b=this.translateString("new_activities");this.settings.network={icon:"cloud-download"};return[{content:a,descr:b}]},parseviews:function(){var a=
this.collection.latest().pluck(["messages","impressions"],!1,!0),b=this.collection.first().pluck(["messages","impressions"],!1,!0),a=a-b,b=this.translateString("new_impressions");return[{content:a,descr:b}]},parsenotifications:function(){var a=this.collection.latest().pluck("notifications"),b=this.collection.first().pluck("notifications"),a=a-b,b=this.translateString("new_notifications");return[{content:a,descr:b}]},parseevolutionnetwork:function(){var a=this.collection.latest().pluck("contacts",
this.network),b=this.collection.first().pluck("contacts",this.network);return[{content:a-b,descr:this.descriptions.contacts[this.icon]}]},parsepostactivitynetwork:function(){var a=this.collection.latest().pluck("messages",this.network),b=this.collection.first().pluck("messages",this.network);return[{content:a-b,descr:this.descriptions.messages[this.icon]}]},parseactivitynetwork:function(){var a=this.collection.latest().pluck("activities",this.network),b=this.collection.first().pluck("activities",
this.network),a=a-b,b=this.translateString("new_activities");return[{content:a,descr:b}]},parseviewsnetwork:function(){var a=this.collection.latest().pluck(["messages","impressions"],this.network,!0),b=this.collection.first().pluck(["messages","impressions"],this.network,!0),a=a-b,b=this.translateString("post_views");return[{content:a,descr:b}]},parsenotificationnetwork:function(){var a=this.collection.latest().pluck("contacts",this.network),b=this.collection.first().pluck("contacts",this.network);
return[{content:a-b,descr:this.descriptions.notifications[this.icon]}]},parsefollowers:function(){var a=this.collection.latest().pluck(["contacts","types","followers"],this.network,3)||this.collection.latest().pluck("contacts",this.network),b=this.collection.first().pluck(["contacts","types","followers"],this.network,3)||this.collection.first().pluck("contacts",this.network),c=a-b,b=0==c?0:Math.floor(100*(c/b));0<=b&&(b="+"+b);0<=c&&(c="+"+c);return[{content:a,descr:"<strong>"+c+"</strong> ("+b+"%) in last 7 days"}]},
parsementions:function(){var a=this.collection.latest().pluck(["notifications","types","comments"],this.network,3),b=this.collection.first().pluck(["notifications","types","comments"],this.network,3);return[{content:a-b,descr:this.title}]},parseretweets:function(){var a=this.collection.latest().pluck(["notifications","types","shares"],this.network,3),b=this.collection.first().pluck(["notifications","types","shares"],this.network,3);return[{content:a-b,descr:this.title}]},parsefollowing:function(){var a=
this.collection.latest().pluck(["contacts","types","following"],this.network,3),b=this.collection.first().pluck(["contacts","types","following"],this.network,3);return[{content:a-b,descr:this.title}]},parseplusones:function(){var a=this.collection.latest().pluck(["notifications","types","favourites"],this.network,3),b=this.collection.first().pluck(["notifications","types","favourites"],this.network,3);return[{content:a-b,descr:this.title}]},parseseniorities:function(){var a=this.collection.latest().pluck(["contacts",
"professional","seniority"],this.network,3),b=this.collection.first().pluck(["contacts","professional","seniority"],this.network,3);return[{content:a-b,descr:this.title}]},parseshares:function(){var a=this.collection.latest().pluck(["messages","types","shares"],this.network,3),b=this.collection.first().pluck(["messages","types","shares"],this.network,3),a=a-b,b="twitter"==Cloudwalkers.Session.getStream(this.network).get("network").token?this.translateString("new_retweets"):this.translateString("new_shares");
return[{content:a,descr:b}]},parseposts:function(){var a=this.collection.latest().pluck(["messages","types","posts"],this.network,3),b=this.collection.first().pluck(["messages","types","posts"],this.network,3),a=a-b,b=this.translateString("new_posts");return[{content:a,descr:b}]},parsedms:function(){var a=this.collection.latest().pluck(["messages","types","dms"],this.network,3),b=this.collection.first().pluck(["messages","types","dms"],this.network,3),a=a-b,b=this.translateString("new_direct_messages");
return[{content:a,descr:b}]},parsebesttimetopost:function(){var a=this.collection.clone().parsebesttime(this.network);if(0==a.length)return null;for(var b={},c=a[0].time,e=1,f=0;f<a.length;f++){var g=a[f].time;0>g||(null==b[g]?b[g]=1:b[g]++,b[g]>e?(c=g,e=b[g]):b[g]==e&&(c=g,e=b[g]))}a=this.title;return 0<=c?[{content:c+"h - "+(c+1)+"h",descr:a}]:!1},negotiateFunctionalities:function(){},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)}});Cloudwalkers.Views.Widgets.Combo=Backbone.View.extend({render:function(){this.$el.html("<div>Combo</div>");return this},negotiateFunctionalities:function(){}});Cloudwalkers.Views.Widgets.DashboardCleaner=Cloudwalkers.Views.Pageview.extend({messagetemplate:"cleaner",title:""});Cloudwalkers.Views.Widgets.Legenda=Backbone.View.extend({title:"Info",initialize:function(a){a&&$.extend(this,a);this.listenTo(this.model,"change",this.render)},render:function(){this.$el.html("<div></div>");return this},negotiateFunctionalities:function(){}});Cloudwalkers.Views.Widgets.HeatCalendar=Backbone.View.extend({title:"Info",filled:!1,nummonths:{quarter:3,year:12},initialize:function(a){a&&$.extend(this,a);this.collection=this.parentview.collection;this.listenTo(this.collection,"ready",this.fill)},render:function(){this.$el.html(Mustache.render(Templates.activitycalendar,this.options));return this},fill:function(){if(this.filled)return!1;var a,b=new CalHeatMap;a=this.calculatedata();var c={itemSelector:this.$el.find("#cal-heatmap").get(0),domain:"month",
subDomain:"x_day",data:a.data,start:Date.parse(a.date),cellSize:36,cellPadding:8,range:1,legend:a.legend,label:{height:30},subDomainTextFormat:"%d"};if("quarter"==this.span||"year"==this.span)a=this.calculatesizes(this.nummonths[this.span]),12==this.nummonths[this.span]&&delete c.subDomainTextFormat,$.extend(c,a);b.init(c);this.filled=!0},calculatesizes:function(a){var b=(this.$el.find("#cal-heatmap").get(0).clientWidth-40)/a;return{subDomain:"day",cellSize:0.15*b,cellPadding:0.15*b/3,range:a}},calculatedata:function(){var a=
$.extend(!0,{},this.collection),b={},c,e=0;if(2<=a.models.length)for(;2<=a.models.length;){c=a.models.shift();var f=a.models[0],g=f.pluck("messages",this.parentview.streamid)-c.pluck("messages",this.parentview.streamid);c=(new Date(f.get("date"))).getTime()/1E3;b[c]=g;0==e&&(e=f.get("date"))}1==a.models.length&&(e=a.models.shift().get("date"));return{data:b,legend:this.generaterange(b,4),date:e}},generaterange:function(a,b){for(var c=_.max(a,function(a){return a}),e=_.min(a,function(a){return a}),
...
this.showmore);this.listenTo(Cloudwalkers.RootView,"added:message",function(){this.model.messages.touch(this.model,this.parameters)}.bind(this));this.translateTitle("scheduled_messages")},render:function(a){this.loadmylisteners();var b={};b.title=this.title;this.mustacheTranslateRender(b);this.$el.html(Mustache.render(Templates.scheduledlist,b));this.$container=this.$el.find(".messages-container");this.$loadercontainer=this.$el.find(".portlet-body");this.$el.find(".load-more").hide();a?(this.parameters=
a,Cloudwalkers.Session.viewsettings("scheduled",{streams:a.target?[a.target]:[]})):this.filters.streams.length&&(this.parameters.target=this.filters.streams.join(","));this.model.messages.touch(this.model,this.parameters);this.addScroll();return this},loadmylisteners:function(){this.loadListeners(this.model.messages,["request","sync",["ready","loaded","destroy"]],!0)},showloading:function(){this.$el.find(".load-more").hide()},hideloading:function(){this.$container.removeClass("inner-loading");this.hasmore=
this.model.messages.cursor?!0:!1},showmore:function(){setTimeout(function(){this.$container.css("max-height",999999);if(!this.hasmore)return this.$el.find("#loadmore").empty();var a=new Cloudwalkers.Views.Widgets.LoadMore({list:this.model.messages,parentcontainer:this.$container});this.$el.find("#loadmore").html(a.render().el);this.loadmore=a}.bind(this),200)},fill:function(a){this.incremental?this.incremental=!1:($.each(this.entries,function(a,b){b.remove()}),this.entries=[]);this.count=a.length;
for(n in a){var b=new Cloudwalkers.Views.Entry({tagName:"tr",model:a[n],type:"full",template:"scheduledentry"});this.entries.push(b);this.$container.append(b.render().el)}this.hideloading()},more:function(){this.incremental=!0;this.loadmore.loadmylisteners();this.model.messages.more(this.model,this.parameters)||this.$el.find(".load-more").hide()},negotiateFunctionalities:function(){this.listenTo(Cloudwalkers.Session,"destroy:view",this.remove)},addScroll:function(){this.$el.find(".scroller").slimScroll({size:"6px",
color:"#a1b2bd",height:$("#inner-content").height()-165+"px",alwaysVisible:!1,railVisible:!1})},destroy:function(){$.each(this.entries,function(a,b){b.remove()})},translateTitle:function(a){this.title=Cloudwalkers.Session.polyglot.t(a)},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=["start_date","networks","message","actions"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),
a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.ScheduledFilters=Cloudwalkers.Views.Widgets.Widget.extend({id:"scheduledfilters",filters:{users:{string:"",list:[]}},events:{remove:"destroy","click *[data-streams]":"filter","click .toggleall.active":"toggleall"},initialize:function(a){a&&$.extend(this,a);this.model.childtype="message";this.streams=Cloudwalkers.Session.getChannel("internal").get("additional").outgoing},render:function(){var a={streams:[]};for(n in this.streams)a.streams.push({id:this.streams[n].id,icon:this.streams[n].network.icon,
name:this.streams[n].name,network:this.streams[n].network});this.mustacheTranslateRender(a);this.$el.html(Mustache.render(Templates.scheduledfilters,a));this.filters.streams.length&&(this.$el.find("[data-streams], [data-networks], .toggleall").toggleClass("inactive active"),this.$el.find(this.filters.streams.map(function(a){return'[data-networks~="'+a+'"],[data-streams="'+a+'"]'}).join(",")).toggleClass("inactive active"));return this},toggleall:function(){this.filter(!0);this.togglefilters(!0)},
togglefilters:function(a){this.$el.find("li").addClass(a?"active":"inactive").removeClass(a?"inactive":"active");this.$el.find(".toggleall").addClass(a?"inactive":"active").removeClass(a?"active":"inactive")},filter:function(a,b){b||(b=this.button&&this.button.data("streams")==$(a.currentTarget).data("streams"));this.togglefilters(b);b||(this.button=$(a.currentTarget).addClass("active").removeClass("inactive"));var c=b?null:this.button.data("streams");b&&(this.button=!1);this.list.render(c?{records:200,
target:c,sort:"asc"}:{records:200,sort:"asc"});return this},fill:function(a){$.each(this.entries,function(a,b){b.remove()});this.entries=[];for(n in a){var b=new Cloudwalkers.Views.User({model:a[n],template:"smalluser",type:"listitem"});this.entries.push(b);this.listenTo(b,"select",this.select);this.$container.append(b.render().el)}this.$el.find(".inner-loading").removeClass("inner-loading")},select:function(a){this.list.render({users:a.model.id,records:20})},addScroll:function(){this.$el.find(".scroller").slimScroll({size:"6px",
color:"#a1b2bd",height:$("#inner-content").height()-165+"px",alwaysVisible:!1,railVisible:!1})},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=["company_accounts","select_all"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.CoworkersFilters=Cloudwalkers.Views.Widgets.Widget.extend({filters:{users:{string:"",list:[]}},events:{remove:"destroy","input .input-rounded":"comparesuggestions","click .load-more":"more","click .toggleall.active":"toggleall"},initialize:function(a){a&&$.extend(this,a);this.model.childtype="message";this.model.users?this.model.users.reset():this.model.users=new Cloudwalkers.Collections.Users;this.model.users.parentmodel=this.model;this.listenTo(this.model.users,"add",
this.comparesuggestions)},render:function(){var a={};this.mustacheTranslateRender(a);Cloudwalkers.Session.censuretemplate(a);this.$el.html(Mustache.render(Templates.coworkersfilters,a));this.$container=this.$el.find("#users-list").eq(0);return this},toggleall:function(){this.showsuggestions(this.model.users.models);this.model.messages.trigger("update:content");this.model.messages.touch(this.model,{records:20})},comparesuggestions:function(a){this.$el.find(".toggleall").addClass("active").removeClass("inactive");
var b=$("#filter_contacts input").val();if(b)b=b.toLowerCase();else return this.hidesuggestions();var c=this.model.users.filter(this.comparenamefilter.bind(this,b));5>c.length&&(!a.cid&&2<b.length)&&this.requestusers(b);if(c.length)this.showsuggestions(c.slice(0,10));else return this.hidesuggestions()},comparenamefilter:function(a,b){return 0<=b.get("displayname").toLowerCase().indexOf(a)||b.get("name")&&0<=b.get("name").toLowerCase().indexOf(a)},hidesuggestions:function(){this.fill(this.model.users.models)},
showsuggestions:function(a){this.fill(a)},requestusers:function(a){a!=this.filters.users.string&&(this.model.users.processing?this.filters.users.buffered=a:(this.$el.find(".loading-contacts").removeClass("hidden"),this.filters.users.string=a,this.model.users.fetch({remove:!1,parameters:{q:a},success:this.loadedusers.bind(this)})))},loadedusers:function(){this.$el.find(".loading-contacts").addClass("hidden");this.filters.users.buffered&&(this.requestusers(this.filters.users.buffered),this.filters.users.buffered=
!1)},filter:function(a){$(a.currentTarget).toggleClass("inactive active");a=this.$el.find(".filter.keyword-list .active");var b=[];if(!a.size())return this.list.$container.empty();a.each(function(){b.push($(this).attr("data-keyword-id"))});a=this.$el.find(".filter.network-list .active");var c=[];if(!a.size())return this.list.$container.empty();a.each(function(){c=c.concat($(this).attr("data-network-streams").split(" "))});this.category.messages.touch(this.category,{records:40,channels:b.join(","),
streams:c.join(",")});return this},fill:function(a){$.each(this.entries,function(a,b){b.remove()});this.entries=[];for(n in a){var b=new Cloudwalkers.Views.User({model:a[n],template:"smalluser",type:"listitem"});this.entries.push(b);this.listenTo(b,"select",this.select);this.$container.append(b.render().el)}this.$el.find(".inner-loading").removeClass("inner-loading")},select:function(a){this.list.render({users:a.model.id,records:20})},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},
mustacheTranslateRender:function(a){this.original=["co-workers","search_co-workers","suggestions","show_all","manage_users"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.CoworkersList=Cloudwalkers.Views.Widgets.Widget.extend({id:"coworkersparent",title:"Co-workers messages",entries:[],events:{remove:"destroy","click .load-more":"more"},initialize:function(a){a&&$.extend(this,a);this.model.messages||(this.model.messages=new Cloudwalkers.Collections.Messages);this.listenTo(this.model.messages,"seed",this.fill);this.listenTo(this.model.messages,"request",this.showloading);this.listenTo(this.model.messages,"ready",this.showmore);this.listenTo(this.model.messages,
"update:content",this.loadmylisteners);this.listenTo(Cloudwalkers.RootView,"added:message",this.messageadded);this.parameters={records:20};this.updateable(this.model,"h3.page-title");this.translateTitle("co-workers_messages")},render:function(a){this.loadmylisteners();var b={};b.title=this.title;this.mustacheTranslateRender(b);this.$el.html(Mustache.render(Templates.coworkerslist,b));this.$container=this.$el.find(".messages-container");this.$loadercontainer=this.$el.find(".portlet-body");this.loadListeners(this.model.messages,
...
"destroy:view",this.remove)},addScroll:function(){this.$el.find(".scroller").slimScroll({size:"6px",color:"#a1b2bd",height:$("#inner-content").height()-165+"px",alwaysVisible:!1,railVisible:!1})},destroy:function(){$.each(this.entries,function(a,b){b.remove()})},translateTitle:function(a){this.title=Cloudwalkers.Session.polyglot.t(a)},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=["load_more","co-workers","search_co-workers"];
this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.MonitorFilters=Cloudwalkers.Views.Widgets.Widget.extend({events:{"click [data-network-streams]":"filternetworks","click [data-keyword-id]":"filterkeywords","click .toggleall.networks.active":"toggleallnetworks","click .toggleall.keywords.active":"toggleallkeywords"},initialize:function(){this.category=this.options.category;this.category.childtype="message";this.streams=[];this.category.channels.each(function(a){this.streams=this.streams.concat(a.streams.models)},this);this.initializeWidget()},
...
hideloading:function(){this.$el.find(".icon-cloud-download").hide();this.hasmore=this.category.messages.cursor?!0:!1;this.$container.removeClass("inner-loading")},showmore:function(){setTimeout(function(){this.$container.css("max-height",999999);if(!this.hasmore)return this.$el.find("#loadmore").empty();var a=new Cloudwalkers.Views.Widgets.LoadMore({list:this.category.messages,parentcontainer:this.$container});this.$el.find("#loadmore").html(a.render().el);this.loadmore=a}.bind(this),200)},fill:function(a){this.incremental?
this.incremental=!1:($.each(this.entries,function(a,b){b.remove()}),this.entries=[]);for(n in a){var b=new Cloudwalkers.Views.Entry({model:a[n],type:"full",template:"messagefullentry"});this.entries.push(b);this.$container.append(b.render().el)}},more:function(){this.incremental=!0;this.loadmore.loadmylisteners();this.category.messages.more(this.category,this.parameters)||this.$el.find(".load-more").hide()},negotiateFunctionalities:function(){this.listenTo(Cloudwalkers.Session,"destroy:view",this.remove)},
addScroll:function(){this.$el.find(".scroller").slimScroll({size:"6px",color:"#a1b2bd",height:$("#inner-content").height()-165+"px",alwaysVisible:!1,railVisible:!1})},destroy:function(){$.each(this.entries,function(a,b){b.remove()})},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)}});Cloudwalkers.Views.Widgets.KeywordsEditor=Cloudwalkers.Views.Widgets.Widget.extend({events:{"submit form[data-add-category]":"addCategory","click button.add-keyword":"addKeyword","click button.update-keyword":"updateKeyword","click .cancel-edit-keyword":"render","click .reset-keyword":"resetFilter","click button[data-keyword-filter]":"toggleFilter"},initialize:function(){this.channel=Cloudwalkers.Session.getChannel("monitoring");this.listenTo(Cloudwalkers.Session.getChannels(),"sync",this.render);
this.listenTo(Cloudwalkers.Session.getChannels(),"remove",this.render);this.listenTo(Cloudwalkers.Session.getAccount(),"sync",this.showFilter)},render:function(a){a&&a.preventDefault&&a.preventDefault();var b=Cloudwalkers.Session.getAccount();(a=b.attributes.filteroptions)||b.fetch({endpoint:"filteroptions",success:this.render.bind(this)});b={filteroptions:a,categories:this.channel.channels.models};b.title=this.title;this.mustacheTranslateRender(b);translation={};this.mustacheTranslateRender(translation);
a&&!this.scriptloaded&&(this.$el.find("#keywordseditor").keywordfilterdestroy(),this.$el.find("#keywordseditor").keywordfilterscript({update:this.translateString("update"),cancel:this.translateString("cancel"),message_contains:this.translateString("message_contains"),message_doesnt_contain:this.translateString("message_doesnt_contain"),author_is:this.translateString("author_is"),author_is_not:this.translateString("author_is_not"),country_is:this.translateString("country_is"),language_is:this.translateString("language_is"),
country_is_not:this.translateString("country_is_not"),language_is_not:this.translateString("language_is_not"),group:this.translateString("group"),end_group:this.translateString("end_group"),and:this.translateString("and"),or:this.translateString("or"),save:this.translateString("save"),there_are:this.translateString("there_are"),groups_left_open_please_close_them_and_resubmit:this.translateString("groups_left_open_please_close_them_and_resubmit"),formula_is_not_valid_add_more_parameters:this.translateString("formula_is_not_valid_add_more_parameters"),
choose_language:this.translateString("choose_language"),choose_country:this.translateString("choose_country"),insert_text:this.translateString("insert_text"),insert_author:this.translateString("insert_author"),countries:a.countries,languages:a.languages,success:function(a){this.formula=a}.bind(this)}));this.$el.html(Mustache.render(Templates.keywordseditor,b));this.$el.find("select").chosen({width:"100%"});a||(this.$el.find("#keyword_filter").addClass("hidden"),this.$el.find(".container-loading").removeClass("hidden"));
return this},showFilter:function(){this.$el.find("#keyword_filter").removeClass("hidden");this.$el.find(".container-loading").addClass("hidden")},addCategory:function(a){a.preventDefault();a={name:$("#category_create_name").val(),remember:$("#category_create_remember").val()};this.channel.channels.create(a,{parent:this.channel.id,wait:!0});this.$el.find(".addcategory .icon-cloud-upload").show()},addKeyword:function(a){a.preventDefault();a=Number($("#keyword_manage_category").val());$("#keywordsfilter * .alert").remove();
if(!a)return Cloudwalkers.RootView.alert(this.translateString("dont_forget_to_select_a_category"));var b=Cloudwalkers.Session.getChannel(a);if(!this.keywordFormula().name)return Cloudwalkers.RootView.alert(this.translateString("dont_forget_to_fill_the_keyword_name"));if(!this.keywordFormula().settings.formula)return Cloudwalkers.RootView.alert(this.translateString("please_add_a_filter_to_your_keyword"));b.channels.create(this.keywordFormula(),{parent:a,wait:!0,error:function(){Cloudwalkers.RootView.information(this.translateString("not_saved"),
this.translateString("something_went_wrong"),this.$el.find(".manage-keyword"));this.$el.find(".managekeyword .icon-cloud-upload").hide()}.bind(this),success:function(){Cloudwalkers.RootView.growl(this.translateString("manage_keywords"),this.translateString("keyword_filter_has_been_successfully_added"))}.bind(this)});this.$el.find(".managekeyword .icon-cloud-upload").show()},updateKeyword:function(a){a.preventDefault();if(!this.keywordFormula().name)return Cloudwalkers.RootView.alert(this.translateString("dont_forget_to_fill_the_keyword_name"));
this.keywordFormula().settings.formula&&(this.editing.endpoint="",this.editing.save(this.keywordFormula(),{success:function(){Cloudwalkers.RootView.growl(this.translateString("manage_keywords"),this.translateString("keyword_filter_has_been_updated"))}.bind(this),error:function(){Cloudwalkers.RootView.growl(this.translateString("not_saved"),this.translateString("something_went_wrong"))}.bind(this)}));this.$el.find(".managekeyword .icon-cloud-upload").show()},fillKeyword:function(a,b){b.preventDefault();
this.render();this.$el.find(".add-keyword, .edit-keyword").toggleClass("inactive");var c=this.editing=Cloudwalkers.Session.getChannel(a),e=c.get("settings");$('#keyword_manage_category option[value="'+c.get("parent")+'"]').attr("selected","selected");$("#keyword_manage_name").val(c.get("name"));c=e.formula;c=c.replace(/\(/g,",(,");c=c.replace(/\)/g,",),");c=c.replace(/ and /gi,",and,");c=c.replace(/ or /gi,",or,");c=c.split(",");for(n in c)c[n]=c[n].trim();c=$.map(c,function(a){return""!==a?a:null});
for(n in c)this.formulaElement(c[n]),this.$el.find("select").chosen({width:"100%"});$("#keyword_filter #demo_plus").remove();this.addPlus("small");this.$el.find("#keyword_manage_category").trigger("chosen:updated")},addPlus:function(a){"small"==a&&$("#keyword_filter").append('<span id="demo_plus" class="demo_bubble demo_drop demo_hit_me"><span class="demo_hit_me">+</spam><span class="demo_options"><ul><li class="add_and">'+translation.translate_and+'</li><li class="add_or">'+translation.translate_or+
"</li></ul></span></span>");"large"==a&&$("#keyword_filter").append('<span id="demo_plus" class="demo_bubble demo_drop demo_hit_me"><span class="demo_hit_me">+</spam><span class="demo_options"><ul><li class="add_message_contains">'+translation.translate_message_contains+'</li><li class="add_message_no_contains" class="demo_doesntcontain">'+translation.translate_message_doesnt_contain+'</li><li class="add_author_is hidden">'+translation.translate_author_is+'</li><li class="add_author_is_not hidden">'+
translation.translate_author_is_not+'</li><li class="add_country_is">'+translation.translate_country_is+'</li><li class="add_country_is_not">'+translation.translate_country_is_not+'</li><li class="add_language_is">'+translation.translate_language_is+'</li><li class="add_language_is_not">'+translation.translate_language_is_not+'</li><li class="add_group">'+translation.translate_group+"</li></ul></span></span>")},formulaElement:function(a){rand_id=this.getRandomInt(1,999);var b=Cloudwalkers.Session.getAccount().attributes.filteroptions;
countries=b.countries;languages=b.languages;b="";for(n in languages)b+='<option value="'+languages[n].token+'">'+languages[n].name+"</option>";var c="";for(n in countries)c+='<option value="'+countries[n].token+'">'+countries[n].name+"</option>";"or"==a&&$("#keyword_filter").append('<span  id="'+rand_id+'" class="demo_bubble demo_drop demo_or" data-value="or " ><span class="sel_value">'+translation.translate_or+'</span><i class="demo_hit_me icon-sort-down"></i><span class="demo_options"><ul><li class="demo_change_val" data-value="or" data-text="'+
translation.translate_or+'">'+translation.translate_or+'</li><li class="demo_change_val" data-value="and" data-text="'+translation.translate_and+'">'+translation.translate_and+'</li></ul></span><i class="demo_remove_filter icon-remove"></i></span>');"and"==a&&$("#keyword_filter").append('<span  id="'+rand_id+'" data-string="sel_value" class="demo_bubble demo_drop demo_and" data-value="and " ><span class="sel_value">'+translation.translate_and+'</span><i class="demo_hit_me icon-sort-down"></i><span class="demo_options"><ul><li class="demo_change_val" data-value="and" data-text="'+
translation.translate_and+'">'+translation.translate_and+'</li><li class="demo_change_val" data-value="or" data-text="'+translation.translate_or+'">'+translation.translate_or+'</li></ul></span><i class="demo_remove_filter icon-remove"></i></span>');"("==a&&$("#keyword_filter").append('<span  id="'+rand_id+'" class="demo_bubble  demo_group" data-option="("><span class="sel_value">'+translation.translate_group+'</span><i class="demo_remove_filter icon-remove"></i></span>');")"==a&&$("#keyword_filter").append('<span  id="'+
rand_id+'" class="demo_bubble  demo_end_group" data-option=")"><span class="sel_value">'+translation.translate_end_group+'</span><i class="demo_remove_filter icon-remove"></i></span>');if(-1!=a.indexOf("language")){splitValue=a.split(" ");splitValue[2]=splitValue[2].replace(/'/g,"");for(n in languages)languages[n].token.toLowerCase()==splitValue[2].toLowerCase()&&(tokenValue=languages[n].token.toLowerCase());tokenValue&&("!="==splitValue[1]?$("#keyword_filter").append('<span id="'+rand_id+'" class="demo_bubble demo_contains" data-option="language != " data-value="'+
splitValue[2]+'">'+translation.translate_language_is_not+'<span class="demo_drop demo_bubble_text"><select class="demo_options" id="languages">'+b+'</select></span><i class="demo_remove_filter icon-remove"></i></span>'):$("#keyword_filter").append('<span id="'+rand_id+'" class="demo_bubble demo_contains" data-option="language = " data-value="'+splitValue[2]+'">'+translation.translate_language_is+'<span class="demo_drop demo_bubble_text"><select class="demo_options" id="languages">'+b+'</select></span><i class="demo_remove_filter icon-remove"></i></span>'),
$("#"+rand_id+" select").val(tokenValue).trigger("change"))}if(-1!=a.indexOf("country")){splitValue=a.split(" ");splitValue[2]=splitValue[2].replace(/'/g,"");for(n in countries)countries[n].token.toUpperCase()==splitValue[2].toUpperCase()&&(tokenValue=countries[n].token.toUpperCase());tokenValue&&("!="==splitValue[1]?$("#keyword_filter").append('<span id="'+rand_id+'" class="demo_bubble demo_contains" data-option="country != " data-value="'+splitValue[2]+'">'+translation.translate_country_is_not+
'<span class="demo_drop demo_bubble_text"><select class="demo_options" id="countries">'+c+'</select></span><i class="demo_remove_filter icon-remove"></i></span>'):$("#keyword_filter").append('<span id="'+rand_id+'" class="demo_bubble demo_contains" data-option="country = " data-value="'+splitValue[2]+'">'+translation.translate_country_is+'<span class="demo_drop demo_bubble_text"><select class="demo_options" id="countries">'+c+'</select></span><i class="demo_remove_filter icon-remove"></i></span>'),
$("#"+rand_id+" select").val(tokenValue).trigger("change"))}-1!=a.indexOf("message")&&(splitValue=a.split("'"),textValue=splitValue[1],splitValue=a.split(" "),splitValue[2]=splitValue[2].replace(/'/g,""),"!contains"==splitValue[1]?$("#keyword_filter").append('<span id="'+rand_id+'" class="demo_bubble demo_contains" data-option="message !contains ">'+translation.translate_message_doesnt_contain+'<span class="demo_hit_me demo_input_text demo_bubble_text"><span class="text">'+textValue+'</span><span class="demo_options"><input type="text" name="lname" value=""><br><div class="modal-footer toload"><button data-option="save" class="btn" onclick="return false;">'+
translation.translate_save+'</button><button data-option="cancel" class="btn btn-primary" onclick="return false;">'+translation.translate_cancel+'</button></div></span></span><i class="demo_remove_filter icon-remove"></i></span>'):$("#keyword_filter").append('<span id="'+rand_id+'" class="demo_bubble demo_contains" data-option="message contains ">'+translation.translate_message_contains+'<span class="demo_hit_me demo_input_text demo_bubble_text"><span class="text">'+textValue+'</span><span class="demo_options"><input type="text" name="lname" value=""><br><div class="modal-footer toload"><button data-option="save" class="btn" onclick="return false;">'+
translation.translate_save+'</button><button data-option="cancel" class="btn btn-primary" onclick="return false;">'+translation.translate_cancel+'</button></div></span></span><i class="demo_remove_filter icon-remove"></i></span>'));-1!=a.indexOf("author")&&(splitValue=a.split("'"),textValue=splitValue[1],splitValue=a.split(" "),splitValue[2]=splitValue[2].replace(/'/g,""),"!="==splitValue[1]?$("#keyword_filter").append('<span id="'+rand_id+'" class="demo_bubble demo_contains" data-option="author != ">'+
...
$("#keyword_filter").append('<span id="'+rand_id+'" class="demo_bubble demo_contains" data-option="author = ">'+translation.translate_author_is+'<span class="demo_hit_me demo_input_text demo_bubble_text"><span class="text">'+textValue+'</span><span class="demo_options"><input type="text" name="lname" value=""><br><div class="modal-footer toload"><button data-option="save" class="btn" onclick="return false;">'+translation.translate_save+'</button><button data-option="cancel" class="btn btn-primary" onclick="return false;">'+
translation.translate_cancel+'</button></div></span></span><i class="demo_remove_filter icon-remove"></i></span>'))},getRandomInt:function(a,b){return Math.floor(Math.random()*(b-a+1))+a},keywordFormula:function(){var a={name:$("#keyword_manage_name").val(),settings:{}};a.settings.formula="";a.settings.formula=this.formula;return a},keywordParameters:function(){var a={name:$("#keyword_manage_name").val(),settings:{}};$("#filter_formula").val()&&(a.settings.formula=$("#filter_formula").val());$("#filter_include").val()&&
(a.settings.include=$("#filter_include").val().split(","));$("#filter_exclude").val()&&(a.settings.exclude=$("#filter_exclude").val().split(","));if(a.settings.include&&a.settings.include.length)for(n in a.settings.include)a.settings.include[n]=a.settings.include[n].trim();if(a.settings.exclude&&a.settings.exclude.length)for(n in a.settings.exclude)a.settings.exclude[n]=a.settings.exclude[n].trim();a.settings.languages=$("#filter_languages").val();a.settings.countries=$("#filter_countries").val();
return a},resetFilter:function(){Cloudwalkers.RootView.confirm(this.translateString("are_you_sure_you_want_to_reset_this_filter"),function(){$("#keyword_filter").html("");this.addPlus("large")}.bind(this))},toggleFilter:function(a){a.preventDefault();a=$(a.target).attr("data-keyword-filter");this.$el.find("div[data-keyword-filter="+a+"]").toggleClass("inactive")},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original="add_category name create add_keyword keyword_name category select_category fine-tune filters include exclude languages countries use_comma_separated_words language country select_from_list all_languages global update cancel message_contains message_doesnt_contain author_is author_is_not country_is language_is country_is_not language_is_not group end_group save insert_text insert_author there_are groups_left_open_please_close_them_and_resubmit formula_is_not_valid_add_more_parameters choose_language choose_country and or reset are_you_sure_you_want_to_reset_this_filter manage_keyword remember_for days".split(" ");
this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.KeywordsOverview=Cloudwalkers.Views.Widgets.Widget.extend({id:"monitorparent",entries:[],events:{"submit form":"editCategory","click .edit-toggler":"toggleEditCategory","click .delete-category":"deleteCategory","click .delete-keyword":"deleteKeyword","click [data-keyword]":"toggleEditKeyword"},initialize:function(){this.channel=Cloudwalkers.Session.getChannel("monitoring");this.editor=this.options.editor;this.listenTo(Cloudwalkers.Session.getChannels(),"sync",this.render)},
...
var c=b?null:String(this.button.data("networks")).split(" ");b&&(this.button=!1);c?($(c.map(function(a){return'[data-streams="'+a+'"]'}).join(",")).removeClass("inactive").addClass("active"),this.filters.streams=c):this.filters.streams=[];this.collection.touch(this.model,this.filterparameters());return this},filterstreams:function(a,b){this.loadmylisteners(!0);b||(b=this.button&&this.button.data("streams")==$(a.currentTarget).data("streams"));this.togglestreams(b);b||(this.button=$(a.currentTarget).addClass("active").removeClass("inactive"));
var c=b?null:Number(this.button.data("streams"));b&&(this.button=!1);c?($('[data-networks~="'+c+'"]').removeClass("inactive").addClass("active"),this.filters.streams=[c]):this.filters.streams=[];this.collection.touch(this.model,this.filterparameters());return this},filterparameters:function(){var a;a="notes"==this.listtype?{all:1}:this.model.get("token")&&"sent"==this.model.get("token")?{records:20}:{records:20,group:1};this.filters.contacts.list.length&&(a.contacts=this.filters.contacts.list.join(","));
this.filters.streams.length&&(this.model.get("token")&&"sent"==this.model.get("token")?a.target=this.filters.streams.join(","):a.streams=this.filters.streams.join(","));this.storeview();return a},storeview:function(){var a=Cloudwalkers.Session.viewsettings(this.collectionstring);a&&(a.streams||(a.streams=[]),JSON.stringify(a.streams)!=JSON.stringify(this.filters.streams)&&(a.streams=this.filters.streams,Cloudwalkers.Session.viewsettings(this.collectionstring,a)))},more:function(){this.incremental=
!0;this.loadmore.loadmylisteners();this.collection.more(this.model,this.filterparameters())||this.$el.find(".load-more").hide()},negotiateFunctionalities:function(){this.listenTo(Cloudwalkers.Session,"destroy:view",this.remove);this.addScroll()},addScroll:function(){this.$el.find(".scroller").slimScroll({height:"inherit"})},destroy:function(){$.each(this.entries,function(a,b){b.remove()})},isempty:function(){$(".inbox-container").empty().addClass("empty-content")},unsetempty:function(){$(".inbox-container").removeClass("empty-content")},
translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original="networks more contacts filters search_contacts suggestions select_all load_more on".split(" ");this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.InboxNotificationList=Cloudwalkers.Views.Widgets.InboxMessageList.extend({collectionstring:"notifications",check:"hasNotifications",toggle:function(a){this.inboxmessage&&this.inboxmessage.remove();a.model.get("parent")?this.showmessage(a.model):this.listenToOnce(a.model,"change",this.showmessage);this.$el.find(".list .active").removeClass("active");a.$el.addClass("active").removeClass("unread")},showmessage:function(a){var b=Cloudwalkers.Session.getMessage(a.get("parent").id);
...
a&&(a.streams||(a.streams=[]),JSON.stringify(a.streams)!=JSON.stringify(this.filters.streams)&&(a.streams=this.filters.streams,Cloudwalkers.Session.viewsettings(this.collectionstring,a)))},more:function(){this.incremental=!0;this.loadmore.loadmylisteners();this.collection.more(this.model,this.filterparameters())||this.$el.find(".load-more").hide()},negotiateFunctionalities:function(){this.listenTo(Cloudwalkers.Session,"destroy:view",this.remove);this.addScroll()},addScroll:function(){this.$el.find(".scroller").slimScroll({height:"inherit"})}});Cloudwalkers.Views.Widgets.SentMessageList=Cloudwalkers.Views.Widgets.InboxMessageList.extend({initialize:function(a,b){a&&$.extend(this,a);this.model||(this.model=this.options.channel);this.collection=this.model[this.collectionstring];this.loadmylisteners()},render:function(){var a={streams:[],networks:[]},b=Cloudwalkers.Session.getChannel("outgoing").streams;b.each(function(b){b.get(this.check)&&a.streams.push({id:b.id,icon:b.get("network").icon,name:b.get("defaultname"),network:b.get("network")})}.bind(this));
a.networks=b.filterNetworks(a.streams,!0);this.mustacheTranslateRender(a);this.$el.html(Mustache.render(Templates.inboxlist,a));this.$container=this.$el.find("ul.list");this.filters.streams.length&&(this.$el.find("[data-streams], [data-networks], .toggleall").toggleClass("inactive active"),this.$el.find(this.filters.streams.map(function(a){return'[data-networks~="'+a+'"],[data-streams="'+a+'"]'}).join(",")).toggleClass("inactive active"));this.collection.touch(this.model,this.filterparameters(),{fields:"defaults,sendlogs"});
return this},toggle:function(a){var b={model:a.model,template:"sentmessage"};this.inboxmessage&&this.inboxmessage.remove();"notes"==this.collectionstring&&(b.template="note");this.inboxmessage=new Cloudwalkers.Views.Widgets.InboxMessage(b);$(".inbox-container").html(this.inboxmessage.render().el);this.$el.find(".list .active").removeClass("active");a.$el.addClass("active")},fill:function(a){var b=this.templates[this.collectionstring];this.incremental?this.incremental=!1:($.each(this.entries,function(a,
b){b.remove()}),this.entries=[]);for(n in a){var c=new Cloudwalkers.Views.Entry({model:a[n],template:b,checkunread:!0,parameters:{sentview:!0}});this.entries.push(c);this.listenTo(c,"toggle",this.toggle);this.$container.append(c.render().el);this.model.seedcontacts&&this.model.seedcontacts(a[n])}this.entries.length?setTimeout(this.toggle.bind(this,this.entries[0]),1):this.hidemore()},storeview:function(){var a=Cloudwalkers.Session.viewsettings("sent");a&&(a.streams||(a.streams=[]),JSON.stringify(a.streams)!=
JSON.stringify(this.filters.streams)&&(a.streams=this.filters.streams,Cloudwalkers.Session.viewsettings("sent",a)))},requestcontacts:function(a){a!=this.filters.contacts.string&&(this.model.contacts.processing?this.filters.contacts.buffered=a:(this.$el.find(".loading-contacts").removeClass("hidden"),this.filters.contacts.string=a,this.model.contacts.fetch({remove:!1,parameters:{q:a,channels:this.model.id},success:this.loadedcontacts.bind(this)})))}});Cloudwalkers.Views.Widgets.InboxMessage=Cloudwalkers.Views.Entry.extend({tagName:"div",className:"message social-box-colors",template:"inboxmessage",related:[],messageview:[],notifications:[],events:{remove:"destroy","click *[data-youtube]":"loadYoutube","click *[data-action]":"action","keyup #tags":"entertag"},render:function(){this.loading(!this.model.get("objectType"));if(this.options.notification)var a={from:this.options.notification.get("from")[0],timeago:moment(this.options.notification.get("date")).fromNow()};
...
Cloudwalkers.Session.isAuthorized("ACCOUNT_TAGS_MANAGE"))&&this.loadtagui());this.time();this.options.notification&&this.model.get("objectType")&&this.togglecomments();this.model.get("objectType")&&!this.model.get("read")&&this.markasread();this.$el.find(".youtube-video").colorbox({iframe:!0,innerWidth:640,innerHeight:390,opacity:"0.7"});return this},incrementaction:function(a){this.actions.incrementaction(a)},loading:function(a){a?$(".inbox").addClass("loading"):$(".inbox").removeClass("loading")},
showrelated:function(){this.loading(!0);this.$related=$("<ul></ul>");this.$prelated=$("<ul></ul>");this.$el.before(this.$prelated.addClass("related-messages social-box-colors"));this.$el.after(this.$related.addClass("related-messages social-box-colors"));this.model.related||(this.model.related=new Cloudwalkers.Collections.Related);this.listenTo(this.model.related,"seed",this.updateCounters);this.listenTo(this.model.related,"seed",this.fillrelated);this.listenTo(this.model.related,"all",this.sendtriggers);
var a=!1;"1"!=this.model.get("read")&&(a=!0);this.model.related.touch(this.model,{records:20,markasread:a})},updateCounters:function(a){},fillrelated:function(a){this.incremental?this.incremental=!1:($.each(this.related,function(a,b){b.remove()}),this.related=[]);var b=!1;a=a.filter(function(a){a.id==this.model.id&&(b=!0);a.append=b;return a.id!=this.model.id}.bind(this));for(n in a){var c=new Cloudwalkers.Views.Entry({model:a[n],template:"inboxrelatedmessage",type:"full",parameters:{notes:this.notes}});
this.related.push(c);this[a[n].append?"$related":"$prelated"].append(c.render().el)}this.loading(!1)},sendtriggers:function(a){this.trigger(a)},addNotifications:function(){this.loading(!0);this.model.notifications||(this.model.notifications=new Cloudwalkers.Collections.Notifications);this.listenTo(this.model.notifications,"seed",this.fillNotifications);this.model.notifications.touch(this.model,{records:50,markasread:!0})},fillNotifications:function(a){this.notifications.length&&($.each(this.notifications,
function(a,b){b.remove()}),this.notifications=[]);a.models&&(a=a.models);var b=this.$el.find(".message-comments ul").eq(0).html("");for(n in a){var c=new Cloudwalkers.Views.Notification({model:a[n],template:"inboxcomment",active:a[n].id==this.options.notification.id});this.notifications.push(c);b.append(c.render().el)}this.loading(!1)},togglecomments:function(){this.$el.find("[data-token=comment][data-action=action-list]").click()},markasread:function(){this.model.save({read:1},{patch:!0,wait:!0});
Cloudwalkers.Session.getStreams().outdated(this.model.get("stream"))},destroy:function(){$.each(this.modelview,function(a,b){b.remove()});$.each(this.notifications,function(a,b){b.remove()});$.each(this.related,function(a,b){b.remove()})},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=["add","on","commented","an_error_occurred_while_sending_this_message"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),
a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.InboxNote=Cloudwalkers.Views.Widgets.InboxMessage.extend({tagName:"div",className:"note",template:"inboxnote",related:[],messageview:[],notifications:[],events:{remove:"destroy","click *[data-youtube]":"loadYoutube","click *[data-action]":"action","keyup #tags":"entertag"},render:function(){this.loading(!this.model.loaded());var a={};$.extend(a,this.model.attributes);this.model.loaded("model")&&this.model.parent.loaded()&&(a[this.model.parent.get("objectType")]=this.model.parent.attributes);
...
" - "+(new Date(this.end)).add({days:-1}).toString("MMMM d, yyyy"));return this}});Cloudwalkers.Views.Widgets.MessagesCounters=Cloudwalkers.Views.Widgets.Widget.extend({entries:[],events:{"click a[href]":"updatesettings"},initialize:function(a){a.color||(this.options.color=this.color);this.collection=a.channel[a.source];"outgoing"==this.options.channel.get("type")&&this.listenTo(Cloudwalkers.RootView,"added:message",this.addedmessage.bind(this))},addedmessage:function(a){a.get("schedule")&&a.get("schedule").date&&this.updatecollection()},render:function(){this.$el.html(Mustache.render(Templates.messagescounters,
this.options));this.$container=this.$el.find("ul.messages-container.messages-list").eq(0);this.fill(this.collection);this.updatecollection();return this},fill:function(a){this.$container.empty();for(n in a.models){var b=new Cloudwalkers.Views.CounterEntry({model:a.models[n],data:this.options});this.$container.append(b.render().el)}},updatecollection:function(){this.listenToOnce(this.collection,"sync",this.fill);"streams"==this.options.source&&(this.options.channel.endpoint="/streams",this.collection.url=
this.options.channel.url(),this.collection.fetch({remove:!1}))},updatesettings:function(a){if("streams"!=this.options.source&&"outgoing"!=this.options.source)return null;a=this.collection.get($(a.currentTarget).data("stream"));var b=a.get("childtypes")[0]+"s";"outgoing"==this.options.channel.get("type")&&(b="scheduled");var c=Cloudwalkers.Session.viewsettings(b);c.streams=[a.id];Cloudwalkers.Session.viewsettings(b,c)},negotiateFunctionalities:function(){this.$el.find(".scroller").length&&this.addScroll();
...
this.parent.appendWidget(view,6)},fillinfo:function(a){var b;for(n in a)this.funcs[a[n]].data.model=this.model,this.funcs[a[n]].data.network=this.network,this.funcs[a[n]].data.icon=this.icon,this.funcs[a[n]].data.footer="+ 0 (+ 0%) SINCE LAST 7 DAYS",b=new Cloudwalkers.Views.Widgets.Info(this.funcs[a[n]].data),this.parent.appendWidget(b,3);this.parent.appendWidget(null,0)},getstreamcontext:function(a){var b={};b.reports=this.reports[a];return b},negotiateFunctionalities:function(){}});Cloudwalkers.Views.Widgets.CompoundChart=Backbone.View.extend({templatemap:{"2col1row":["#col1","#col2","#row1"],"2row":["#row1","#row2"]},initialize:function(a){a&&$.extend(this,a);this.charttemplate="compoundchart"+this.options.template;this.template=this.options.template;this.charts=this.options.chartdata},render:function(){this.settings={};this.settings.title=this.title;this.$el.html(Mustache.render(Templates[this.charttemplate],this.settings));this.fill();return this},fill:function(){if(this.filled)return!0;
$.each(this.charts,function(a,b){b.data.network=this.network;b.data.parentview=this.parentview;var c=(new Cloudwalkers.Views.Widgets[b.widget](b.data)).render().el;this.parentview.views.push(c);this.$el.find(this.templatemap[this.template][a]).append(c)}.bind(this));this.filled=!0},negotiateFunctionalities:function(){}});Cloudwalkers.Views.Widgets.TrendingMessage=Backbone.View.extend({initialize:function(a){a&&$.extend(this,a);this.settings={};this.settings.title=this.title;this.model=this.parentview.streamid?Cloudwalkers.Session.getStream(this.parentview.streamid):Cloudwalkers.Session.getChannel("profiles").clone();this.listenTo(this.model,"sync",this.fill);this.gettoptrending()},render:function(){this.$el.html(Mustache.render(Templates.trendingmsg,this.settings));this.hideloading();return this},fill:function(){if((this.message=
this.model.get("messages")?this.model.get("messages")[0]:!1)&&0!=this.message.length){var a=this.message,b=a.attachments?a.attachments.filter(function(a){if("link"==a.type)return a}):null,c=a.attachments?a.attachments.filter(function(a){if("image"==a.type)return a}):null;this.settings.from=a.from[0].displayname||"";this.settings.body=a.body.plaintext||"";this.settings.icon=a.from[0].network.icon||"";this.settings.date=a.dateonly||"";this.settings.statistics=[];c.length&&(this.settings.image=c[0].url||
"");if(a.stats)for(n in a.stats)"notes"!=n&&this.settings.statistics.push({name:n,value:a.stats[n]});this.settings.links=b||[];this.render()}else this.$el.find(".commentballoon h4").html("No data"),this.hideloading()},gettoptrending:function(){return this.parentview.streamid?this.toptrendingstream(this.parentview.streamid):this.toptrendingall()},toptrendingstream:function(a){this.model.fetch({endpoint:"messages",parameters:{sort:this.timespan.sort,records:1}})},toptrendingall:function(){this.model.fetch({endpoint:"messages",
parameters:{sort:"engagement",records:1,since:this.timespan.since}})},showloading:function(){this.$el.find(".icon-cloud-download").show()},hideloading:function(){$(".inner-loading").removeClass("inner-loading")},negotiateFunctionalities:function(){}});Cloudwalkers.Views.Widgets.BestTimeToPost=Backbone.View.extend({initialize:function(a){a&&$.extend(this,a);this.settings={};this.settings.title=this.title;this.collection=this.parentview.collection;this.listenTo(this.collection,"ready",this.fill)},render:function(){this.$el.html(Mustache.render(Templates.besttimewrap,this.settings));return this},fill:function(){if(!this.filled){var a=this.collection.clone().parsebesttime(this.parentview.streamid);$.each(a,function(b,c){c.fill=100*c.value/a.maxvalue;
c.time=0<=c.time?c.time+"h":"";this.$el.find(".chart-wrapper").append(Mustache.render(Templates.besttime,c))}.bind(this));this.filled=!0}},negotiateFunctionalities:function(){}});Cloudwalkers.Views.Widgets.TitleSeparator=Backbone.View.extend({className:"titleseparator",initialize:function(a){a&&$.extend(this,a);"default"!=this.parentview.timespan&&(this.title+=" "+Cloudwalkers.Session.polyglot.t(this.parentview.timespan));this.settings={};this.settings.title=this.title},render:function(){this.$el.html(Mustache.render(Templates.titleseparator,this.settings));return this},negotiateFunctionalities:function(){}});Cloudwalkers.Views.Widgets.NetworkStatistics=Backbone.View.extend({initialize:function(a){a&&$.extend(this,a);this.collection=this.model.statistics;this.listenTo(this.collection,"ready",this.fill)},render:function(){this.settings={};this.settings.title=this.title;this.$el.html('<div id="netstat"></div>');return this},fill:function(){if(this.filled)return!0;title="Network evolution chart ("+this.network+")";data={chart:"LineChart",filterfunc:"allreports",network:this.network,title:title};data.model=
...
0},parsecontactsnetwork:function(){return{counter:this.collection.latest().pluck("contacts",this.parentview.streamid)}},parsescorenetwork:function(){stat=this.collection.latest();return{counter:stat.pluck("notifications",this.parentview.streamid)+stat.pluck("activities",this.parentview.streamid)}},parsesentnetwork:function(){var a=this.collection.latest(),b=this.collection.first();return{counter:a.pluck("messages",this.parentview.streamid)-b.pluck("messages",this.parentview.streamid)}},parsebesttime:function(){var a=
this.collection.clone().parsebesttime(this.parentview.streamid);if(0==a.length)return null;for(var b={},c=a[0].time,e=1,f=0;f<a.length;f++){var g=a[f].time;0>g||(null==b[g]?b[g]=1:b[g]++,b[g]>e?(c=g,e=b[g]):b[g]==e&&(c=g,e=b[g]))}return 0<=c?{counter:c+"h - "+(c+1)+"h"}:{counter:"--"}},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)}});Cloudwalkers.Views.Widgets.CalSummary=Cloudwalkers.Views.Widgets.Widget.extend({className:"stats-summary cal-summary",columns:[{title:"Total messages",func:"parsetotal",translation:{title:"total_messages"}},{title:"Average",func:"parseavg",translation:{title:"average"}},{title:"Media",func:"parsemedia",translation:{title:"media"}}],initialize:function(a){a&&$.extend(this,a)},render:function(){var a={columns:this.columns};for(k in this.columns)this.translateColumns(this.columns[k]);this.$el.html(Mustache.render(Templates.statsummary,
a));return this},getTotal:function(a,b){(new Cloudwalkers.Models.Channel({id:this.calview.posted.id})).fetch({endpoint:"messageids",parameters:{since:a.unix(),until:b.unix(),records:999},success:this.showtotal.bind(this)})},showtotal:function(a){a=a.attributes.messages.length;this.$el.find("[data-type='parsetotal'] .stats-summary-counter").html(a)},fill:function(a){this.$el.find("[data-type]").each(function(b,c){var e=this[$(c).data("type")](a);$(c).find(".stats-summary-counter").html(e.counter);
e.title&&$(c).find(".stats-summary-title").html(e.title)}.bind(this))},parseavg:function(a){if(!a.length)return{counter:0};var b="agendaWeek"==this.calview.viewtype?{title:this.translateString("daily_average"),divide:7}:{title:this.translateString("weekly_average"),divide:4};a=Math.round(a.length/b.divide);b.counter=a+" <small>message"+(1!=a?"s":"")+"</small>";return b},parsemedia:function(a){if(!a.length)return{counter:0};var b=[],c=a.length,e={image:0,video:0,link:0,text:0};a.each(function(a){if(a=
a.get("attachments"))for(n in a)e[a[n].type]++;else e.text++});e.image&&b.push("<div class='media-entry'><i class='icon-picture'></i> <strong>"+Math.round(100*(e.image/c))+"</strong>% "+this.translateString("images")+"</div>");e.video&&b.push("<div class='media-entry'><i class='icon-facetime-video'></i> <strong>"+Math.round(100*(e.video/c))+"</strong>% "+this.translateString("video")+"</div>");e.link&&b.push("<div class='media-entry'><i class='icon-link'></i> <strong>"+Math.round(100*(e.link/c))+
"</strong>% "+this.translateString("links")+"</div>");e.text&&b.push("<div class='media-entry'><i class='icon-reorder'></i> <strong>"+Math.round(100*(e.text/c))+"</strong>% "+this.translateString("text-only")+"</div>");return{counter:b.join("<br>")}},parsetrending:function(a){a=a.max(function(a){return a.get("engagement")});var b={intro:a.get("body").plaintext.substr(0,140)};140==b.intro.length&&(b.intro+="...");$.extend(b,a.attributes);return{counter:Mustache.render(Templates.caltrendingentry,b)}},
translateColumns:function(a){if(a.translation)for(k in a.translation)a[k]=Cloudwalkers.Session.polyglot.t(a.translation[k])},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)}});Cloudwalkers.Views.Widgets.CalendarFilters=Cloudwalkers.Views.Widgets.Widget.extend({id:"calendarfilters",filters:{},events:{remove:"destroy","click [data-toggle]":"togglefilter","click [data-networks]":"filternetworks","click [data-streams]":"filterstreams","click .toggleall.active":"toggleall"},initialize:function(a){a&&$.extend(this,a);this.collection=this.model.messages;this.streams=Cloudwalkers.Session.getChannel("internal").get("additional").outgoing},render:function(){if(this.model.streams){var a=
...
this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.Sentiment=Backbone.View.extend({events:{"mouseover .sentiment.main":"toggleedit","mouseout .sentiment.main":"toggleedit","click .notification-action":"edit","click .sentiment-edit .sentiment":"update"},initialize:function(a){$.extend(this,a)},render:function(){this.$el.html(Mustache.render(Templates.sentiment,{sentiment:this.sentiment}));return this},edit:function(){this.$el.find(".sentiment-edit")[this.$el.find(".sentiment-edit").hasClass("visible")?"removeClass":"addClass"]("visible")},
toggleedit:function(a){a="mouseout"==a.originalEvent.type;this.$el.find(".notification-action")[a?"addClass":"removeClass"]("hidden")},update:function(a){this.sentiment=$(a.currentTarget).attr("data-sentiment");this.render()}});Cloudwalkers.Views.Widgets.Charts.Linechart=Cloudwalkers.Views.Widgets.Widget.extend({title:"Line chart",placeholder:null,icon:"reorder",size:6,element:null,getDataset:function(){return this.options.model},initialize:function(){var a=this;this.options.model.on("dataset:change",function(b){a.plot(b[0].values)})},innerRender:function(a){this.element=a;var b=this;this.placeholder=$('<div class="chart" style="position: relative;"></div>');a.html("");a.append(this.placeholder);this.options.model.getValues(function(a){b.plot(a)})},
...
a.header.length;e++)b.header.push({name:a.header[e],"class":"undefined"!=typeof a.classes[e]?a.classes[e]:null}),c.push({sSortDataType:"dom-text"});b.rows=[];for(e=0;e<a.rows.length;e++){for(var c=[],f=0;f<a.rows[e].length;f++)c.push({value:a.rows[e][f],"class":"undefined"!=typeof a.classes[f]?a.classes[f]:null});b.rows.push({columns:c})}this.placeholder.html(Mustache.render(Templates.charttable,b))}});Cloudwalkers.Views.Widgets.LoadMore=Backbone.View.extend({events:{click:"more"},initialize:function(a){$.extend(this,a)},loadmylisteners:function(){this.loadListeners(this.list,["request","sync","ready"],!0);this.trigger("rendered")},render:function(){param={};this.mustacheTranslateRender(param);this.$el.html(Mustache.render(Templates.loadmore,param));this.$container=this.$el.find(".load-more-wrap");this.$loadercontainer=this.$el.find(".load-more-wrap");return this},more:function(){this.$el.find(".btn.load-more").addClass("hidden");
this.$el.find(".load-more-wrap").addClass("inner-loading");var a=this.parentcontainer.outerHeight();this.parentcontainer.css("max-height",a)},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=["load_more"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.InboxDemoList=Cloudwalkers.Views.Widgets.Widget.extend({id:"inboxlist",entries:[],check:"hasMessages",collectionstring:"messages",filters:{contacts:{string:"",list:[]},streams:[]},events:{remove:"destroy","click [data-toggle]":"togglefilter","input .input-rounded":"comparesuggestions","click [data-contact]":"filtercontacts","click [data-close-contact]":"filtercontacts","click [data-networks]":"filternetworks","click [data-streams]":"filterstreams","click .toggleall.active":"toggleall",
...
function(){this.parent.$el.find("[data-service="+this.service.id+"]").remove();this.service.destroy({success:this.servicedeleted.bind(this)})}.bind(this))},servicedeleted:function(){this.trigger("service:deleted");this.closedetail()},closedetail:function(){this.parent.closedetail()},processSettings:function(a){var b=this;$.each(a,function(c,e){"link"==e.type&&(a[c].url=b.processLink(e.url))})},processLink:function(a){return a=0<a.indexOf("?")?a+"&return="+encodeURIComponent(window.location):a+"?return="+
encodeURIComponent(window.location)},getServiceData:function(a,b){var c=this;Cloudwalkers.Net.get("wizard/service/"+a,{account:Cloudwalkers.Session.getAccount().get("id")},function(a){c.processSettings(a.service.settings);b(a)})},storeServiceData:function(a,b,c){Cloudwalkers.Net.put("wizard/service/"+a,{account:Cloudwalkers.Session.getAccount().get("id")},b,c)},deleteService:function(a,b){Cloudwalkers.Net.remove("wizard/service/"+a,{account:Cloudwalkers.Session.getAccount().get("id")},b)},setStreamChannels:function(a){function b(a,
b){var c=[];b.each(function(b){if(!b.get("parent")&&b.get("name")){for(var e=!1,f=0;f<a.channels.length;f++)if(a.channels[f]==b.get("id")){e=!0;break}e={channel:b.attributes,selected:e,channels:[]};b.attributes&&b.get("channels");c.push(e)}});return c}function c(a){for(var b={string:[],"boolean":[]},c=0;c<a.settings.length;c++)"undefined"==typeof b[a.settings[c].type]&&(b[a.settings[c].type]=[]),b[a.settings[c].type].push(a.settings[c]);a.groupedsettings=b}function e(g){var h=[];$.each(a.streams,
function(a,l){"undefined"!=typeof l.parent&&l.parent.id==g.id&&(c(l),h.push({parsedchannels:b(l,f),stream:l,substreams:e(l)}))});return h}var f=Cloudwalkers.Session.getChannels();a.parsedstreams=function(){for(var g=[],h=0;h<a.streams.length;h++)a.streams[h].canSetChannels&&"undefined"==typeof a.streams[h].parent&&(c(a.streams[h]),g.push({parsedchannels:b(a.streams[h],f),stream:a.streams[h],substreams:e(a.streams[h])}));return g}()},submit:function(a){var b=this;a.preventDefault();var c=$(a.target),
e={streams:{}};c.find("[data-channel-setting]").each(function(a,b){"checkbox"==$(b).attr("type")?e[$(b).attr("name")]=$(b).is(":checked"):e[$(b).attr("name")]=$(b).val()});$.each(this.service.streams,function(a,b){var h=[];c.find("[data-stream-id="+b.id+"][data-channel-id]:checked").each(function(a,b){h.push($(b).attr("data-channel-id"))});e.streams[b.id]={channels:h};c.find("[data-stream-setting="+b.id+"]").each(function(a,c){"checkbox"==$(c).attr("type")?e.streams[b.id][$(c).attr("name")]=$(c).is(":checked"):
...
a.user=this.model.attributes;a.title=a.user.name;a.roles=[];for(var c=0;c<b.length;c++){var e=b[c];e.checked=this.model.get("rolegroup")==b[c].id;a.roles.push(e)}this.mustacheTranslateRender(a);this.$el.html(Mustache.render(Templates.settings.userdetails,a));return this},submit:function(a){a={rolegroup:$("#level").val()};this.model.parent=Cloudwalkers.Session.getAccount();this.model.save(a,{patch:!0,success:this.success.bind(this),error:this.error})},success:function(a){Cloudwalkers.RootView.growl(this.translateString("manage_users"),
this.translateString("the_user_clearance_is_updated"));this.model.trigger("change:clearance");a.id==Cloudwalkers.Session.getUser().id&&setTimeout(function(){window.location.reload()},1E3)},error:function(a,b){if(!Cloudwalkers.Session.getAccount().monitorlimit("admins",Cloudwalkers.Session.getUsers("Conversation manager").length,null,!0)){var c=b.responseJSON?b.responseJSON.error.message:this.translateString("something_went_wrong");Cloudwalkers.RootView.alert(c)}},disablesave:function(){this.$el.find(".edit-managed-user .btn").attr("disabled",
!0)},enablesave:function(){this.$el.find(".edit-managed-user .btn").attr("disabled",!1)},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=["clearance_level","save"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Settings.Profile=Backbone.View.extend({events:{"click .add-user":"addUser","submit .edit-user-profile":"editUserProfile","submit .edit-user-password":"editUserPassword","click #add-file":"addfile","change input[type=file]":"listentofile","click #upload-file":"uploadfile","change select":"enablereset","keydown input":"enablereset","click [type=reset]":"disablereset"},"class":"section",collections:[],render:function(){var a=Cloudwalkers.Session.getUser(),b=this,c={user:{firstname:a.get("firstname"),
name:a.get("name"),mobile:a.get("mobile"),avatar:a.get("avatar"),role:a.getRole()},langs:Cloudwalkers.Session.langs};this.mustacheTranslateRender(c);Cloudwalkers.Session.censuretemplate(c);this.$el.html(Mustache.render(Templates.settings.profile,c));this.$el.find(".collapse-closed, .collapse-open").each(function(){b.negotiateFunctionalities(this)});this.$el.find("option[value="+a.get("locale")+"]").attr("selected",!0);return this},editUserProfile:function(a){a=Cloudwalkers.Session.getUser();var b=
this.$el.find("[name=firstname]").val(),c=this.$el.find("[name=name]").val(),e=this.$el.find("[name=mobile]").val(),f=this.$el.find("[name=locale]").val();this.$el.find(".edit-user-profile .btn").attr("disabled",!0);this.$el.find(".edit-user-profile").addClass("loading");a.save({firstname:b,name:c,mobile:e,locale:f},{patch:!0,success:function(){Cloudwalkers.RootView.growl(this.translateString("user_profile"),this.translateString("your_profile_settings_are_updated"));window.location.reload()}.bind(this),
error:function(){Cloudwalkers.RootView.growl(this.translateString("user_profile"),this.translateString("there_was_an_error_updating_your_settings"))}.bind(this)})},enablereset:function(){this.$el.find("[type=reset]").attr("disabled",!1)},disablereset:function(a){$(a.currentTarget).closest("form").get(0).reset();this.$el.find("[type=reset]").attr("disabled",!0)},addfile:function(a){$(".settings-profile input[type=file]").click()},listentofile:function(a){var b=this;a=a.target.files;for(var c=0,e;e=
a[c];c++){if(!e.type.match("image.*"))return Cloudwalkers.RootView.information(this.translateString("wrong_file"),this.translateString("you_need_a_valid_image"),this.$el.find(".settings-profile .portlet-body"));var f=new FileReader;f.onload=function(a){return function(a){b.addimage(a.target.result)}}(e);f.readAsDataURL(e)}},addimage:function(a){this.base64data=a;this.$el.find("div.avatar-big").css("background-image","url("+a+")")},uploadfile:function(){this.$el.find(".edit-user-avatar").addClass("loading");
this.base64data?Cloudwalkers.Session.getUser().save({avatar:this.base64data},{patch:!0,success:function(){Cloudwalkers.RootView.growl(this.translateString("user_profile"),this.translateString("you_have_a_new_profile_picture"));this.$el.find(".edit-user-avatar").removeClass("loading");setTimeout(function(){window.location.reload()},1E3)}.bind(this)}):(Cloudwalkers.RootView.growl(this.translateString("no_image"),this.translateString("select_an_image_file_first")),this.$el.find(".edit-user-avatar").removeClass("loading"))},
editUserPassword:function(){var a=this.$el.find("[name=pass0]").val(),b=this.$el.find("[name=pass1]").val();this.$el.find(".edit-user-password").addClass("loading");if(b!=this.$el.find("[name=pass2]").val())return Cloudwalkers.RootView.growl("Oops",this.translateString("please_retype_your_new_password")),null;Cloudwalkers.Session.getUser().save({oldpassword:a,newpassword:b},{patch:!0,endpoint:"password",success:function(){Cloudwalkers.RootView.growl(this.translateString("user_profile"),this.translateString("you_have_a_new_password_now"));
this.$el.find(".edit-user-password").removeClass("loading");this.$el.find("[name=pass0]").val("");this.$el.find("[name=pass1]").val("");this.$el.find("[name=pass2]").val("");this.$el.find("[name=pass2]").blur()}.bind(this),error:function(a,b,f){Cloudwalkers.RootView.growl(this.translateString("user_profile"),error);window.location.reload()}.bind(this)})},negotiateFunctionalities:function(a){$(a).find(".portlet-title").on("click",function(){$(this).parents(".collapse-closed, .collapse-open").toggleClass("collapse-closed collapse-open")})},
translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original="profile_type profile_picture select upload upload_profile_picture change_password current_password new_password retype_new_password cancel your_profile first_name last_name mobile_phone language save_changes cancel no_image".split(" ");this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Settings.Account=Backbone.View.extend({events:{"click i[data-delete-campaign-id]":"deletecampaign","click #menu a":"scroll","submit form#editaccount":"editaccount","keydown [data-attribute=account-name]":"enablebtnaccount","click [data-action=reseetaccount]":"disablebtnaccount"},initialize:function(){this.streams=Cloudwalkers.Session.getStreams();this.streams=this.streams.filterNetworks();this.account=Cloudwalkers.Session.getAccount();this.triggers=new Cloudwalkers.Collections.Triggers;
this.account=Cloudwalkers.Session.getAccount();this.triggermodel={}},render:function(){var a=this.account.attributes;this.mustacheTranslateRender(a);Cloudwalkers.Session.censuretemplate(a);this.$el.html(Mustache.render(Templates.settings.account,a));this.$el.find("#menu").affix();this.twitterview=new Cloudwalkers.Views.Settings.Trigger({event:"CONTACT-NEW",stream:"twitter",description:"Twitter: New follower response"});this.dmview=new Cloudwalkers.Views.Settings.Trigger({event:"MESSAGE-RECEIVED",
description:"DM: Out of office response"});this.$el.find("#triggerlist").append(this.twitterview.render().el);this.$el.find("#triggerlist").append(this.dmview.render().el);this.triggers.parent=this.account;this.triggers.fetch();return this},enablebtnaccount:function(){this.$el.find("[data-action=reseetaccount]").attr("disabled",!1)},disablebtnaccount:function(a){$(a.currentTarget).closest("form").get(0).reset();this.$el.find("[data-action=reseetaccount]").attr("disabled",!0)},filltriggers:function(a){console.log(a);
var b=a.filter(function(a){if("CONTACT-NEW"==a.get("event"))return a});a=a.filter(function(a){if("MESSAGE-RECEIVED"==a.get("event"))return a});b=new Cloudwalkers.Models.Trigger(b[0].attributes);this.twitterview.updatetrigger(b);a=new Cloudwalkers.Models.Trigger(a[0].attributes);this.dmview.updatetrigger(a)},action:function(a){this[$(a.currentTarget).data("action")](a)},editaccount:function(a){a=this.$el.find("[data-attribute=account-name]").val();this.$el.find(".edit-account-name").addClass("loading");
this.$el.find(".edit-account-name .btn").attr("disabled",!0);this.account.save({name:a},{patch:!0,success:function(){Cloudwalkers.RootView.growl(this.translateString("account_settings"),this.translateString("your_account_settings_are_updated"));this.$el.find(".edit-account-name").removeClass("loading");this.$el.find("[data-action=editaccount]").attr("disabled",!1)}.bind(this),error:function(){Cloudwalkers.RootView.growl(this.translateString("account_settings"),this.translateString("there_was_an_error_updating_your_settings"));
this.$el.find(".edit-account-name").removeClass("loading");this.$el.find("[data-action=editaccount]").attr("disabled",!1);this.$el.find("[data-action=editaccount]").attr("disabled",!1)}.bind(this)})},deletecampaign:function(a){var b=$(a.target).data("delete-campaign-id");Cloudwalkers.RootView.confirm(this.translateString("are_you_sure_you_want_to_remove_this_campaign"),function(){Cloudwalkers.Session.getAccount().removecampaign(b,a.target)})},scroll:function(a){a=$(a.currentTarget).data("hash");var b=
this.$el.find("h3[name="+a+"]").offset().top,b=b-this.$el.find("h3[name="+a+"]").outerHeight();$("html, body").animate({scrollTop:b},"fast");return!1},negotiateFunctionalities:function(a){this.$el.find(".portlet-title").on("click",function(){$(this).parents(".collapse-closed, .collapse-open").toggleClass("collapse-closed collapse-open")})},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original="campaigns account_plan account name save_changes cancel company_name menu campaigns basic manage_campaigns manage_auto_responders message_templates triggers".split(" ");
this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Settings.Trigger=Backbone.View.extend({events:{"click [data-action=save]":"save","keyup textarea":"enablereset","click [type=reset]":"reset"},initialize:function(a){$.extend(this,a);this.model||(this.model=new Cloudwalkers.Models.Trigger)},render:function(){this.model.parent=Cloudwalkers.Session.getAccount();var a={message:this.model.getmessage("REPLY"),description:this.description};this.mustacheTranslateRender(a);this.$el.html(Mustache.render(Templates.settings.trigger,a));return this},
updatetrigger:function(a){this.model=a;this.render();this.$el.find(".inner-loading").removeClass("inner-loading");this.$el.find(".loading").removeClass("loading");this.$el.find("textarea").attr("disable",!1)},save:function(){this.$el.addClass("loading");this.$el.find(".submit-btn button").attr("disabled",!0);this.model.setaction("REPLY",{message:this.$el.find("textarea").val()});this.model.save({event:this.model.get("event"),actions:this.model.get("actions")},{patch:this.model.id?!0:!1,success:function(){Cloudwalkers.RootView.growl(this.translateString("user_profile"),
this.translateString("your_profile_settings_are_updated"));this.$el.removeClass("loading");this.$el.find("[type=submit]").attr("disabled",!1)}.bind(this),error:function(){Cloudwalkers.RootView.growl(this.translateString("user_profile"),this.translateString("there_was_an_error_updating_your_settings"));this.$el.removeClass("loading");this.$el.find("[type=submit]").attr("disabled",!1);this.$el.find("[type=reset]").attr("disabled",!1)}.bind(this)})},enablereset:function(){this.$el.find("[type=reset]").attr("disabled",
!1)},reset:function(a){$(a.currentTarget).closest("form").get(0).reset();this.$el.find("[type=reset]").attr("disabled",!0)},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=["save_changes","reset"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Settings.CannedList=Backbone.View.extend({events:{"click i[data-delete-canned-id]":"deletecanned"},initialize:function(){this.cannedresponses=Cloudwalkers.Session.getCannedResponses();this.listenTo(this.cannedresponses,"sync",this.render)},render:function(){this.$el.html(Mustache.render(Templates.settings.cannedlist,{canned:this.cannedresponses.models}));return this},deletecanned:function(a){var b=$(a.target).data("delete-canned-id"),b=this.cannedresponses.removecanned(b);b.length&&
...
b){Cloudwalkers.Net.put("stream/"+this.streamid+"/events",null,a,b)},submit:function(a){var b=this;a.preventDefault();a=JSON.parse(this.$el.find('[name="events"]').val());this.setStreamSettings({events:a},function(a){"undefined"!=typeof a.error?Cloudwalkers.RootView.alert(a.error.message):b.trigger("popup:close")})}});Cloudwalkers.Views.Settings.ManageUserGroups=Backbone.View.extend({events:{"submit .create-group":"createGroup","click .group-container [data-edit-group-id]":"fetchDetails"},"class":"section",collections:[],initialize:function(){this.collection=new Cloudwalkers.Collections.Groups;this.listenTo(this.collection,"seed",this.fillGroups);this.listenTo(this.collection,"request",this.showloading);this.listenTo(this.collection,"sync",this.hideloading);this.loadListeners(this.collection,["request","sync"],
!0)},render:function(){var a={};this.mustacheTranslateRender(a);Cloudwalkers.Session.censuretemplate(a);this.$el.html(Mustache.render(Templates.settings.manageusergroups,a));this.collection.touch();this.$container=this.$el.find(".toload");return this},fillGroups:function(a){var b=this.$el.find(".group-container").eq(-1);b.empty();for(n in a){var c=new Cloudwalkers.Views.Settings.GroupItem({model:a[n],type:"group"});b.append(c.render().el)}this.$el.find(".group-container tr:first-child td:first-child").click()},
createGroup:function(){if(1>$.trim($("input[name=group-name]").val()).length)return Cloudwalkers.RootView.growl("Oops",this.translateString("something_went_wrong")),this;var a={name:$("input[name=group-name]").val()},b="accounts/"+Cloudwalkers.Session.getAccount().get("id")+"/groups";Cloudwalkers.Net.post(b,{},a,function(a){a.error?Cloudwalkers.RootView.growl("Oops",this.translateString("something_went_wrong")):(Cloudwalkers.RootView.growl(this.translateString("group_management"),this.translateString("group_created")),
$("input[name=group-name]").val(""),this.collection.touch(null,{all:1}))}.bind(this))},fetchDetails:function(a){$(".group-container > *").removeClass("active");$(a.currentTarget).closest("tr").addClass("active");a=$(a.currentTarget).data("edit-group-id");(new Cloudwalkers.Models.Group({id:a})).fetch({success:this.fillDetails.bind(this)})},fillDetails:function(a){a=new Cloudwalkers.Views.Settings.GroupDetails({model:a,template:"manageusergroups_details"});var b=this.$el.find(".group-details-container");
b.empty();b.append(a.render().el)},fail:function(){Cloudwalkers.RootView.growl("Oops","Something went sideways, please reload the page.")},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=["user_groups","create_new_group","name","add"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Settings.GroupItem=Backbone.View.extend({tagName:"tr",events:{"click [data-delete-group-id]":"deleteGroup","click [data-delete-group-user-id]":"deleteGroupUser"},initialize:function(a){a&&$.extend(this,a);this.parameters={};this.listenTo(this.model,"change",this.render);translations={};this.mustacheTranslateRender(translations)},render:function(){var a={};a.group=this.model.attributes;a.type=this.type;a.id=a.group.id;a.name="group-user"==a.type?a.group.displayname:a.group.name;
Cloudwalkers.Session.censuretemplate(a);this.$el.html(Mustache.render(Templates.settings.group,a));return this},deleteGroup:function(a){var b=this,c=$(a.currentTarget).parents("tr");Cloudwalkers.RootView.confirm(translations.translate_you_are_about_to_remove_group+this.model.get("name")+translations.translate_sure,function(){c.remove();var a="groups/"+b.model.get("id");Cloudwalkers.Net.remove(a,{},function(){Cloudwalkers.RootView.growl(translations.translate_manage_users,translations.translate_deleted)})})},
deleteGroupUser:function(a){var b=this,c=$(a.currentTarget).parents("tr");Cloudwalkers.RootView.confirm(translations.translate_you_are_about_to_remove_user+this.model.get("name")+translations.translate_from_this_group+translations.translate_sure,function(){c.remove();var a="groups/"+b.model.collection.parentmodel.groupid+"/users/"+b.model.get("id");Cloudwalkers.Net.remove(a,{},function(){Cloudwalkers.RootView.growl(translations.translate_manage_users,translations.translate_deleted)})})},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},
mustacheTranslateRender:function(a){this.original="you_are_about_to_remove_group sure manage_user_groups deleted you_are_about_to_remove_user from_this_group".split(" ");this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Settings.GroupDetails=Backbone.View.extend({tagName:"div",events:{"submit .add-group-user":"addUser","change select.all-user-list":"listentouserdropdown"},initialize:function(){this.users=new Cloudwalkers.Collections.Users({modelstring:"user"});this.listenTo(this.users,"seed",this.fillUsersDropdown);this.groupusers=new Cloudwalkers.Collections.Groups({modelstring:"user",parenttype:"group",typestring:"users"});this.listenTo(this.groupusers,"seed",this.fillGroupItems)},render:function(){var a=
{},a=this.model.attributes;this.mustacheTranslateRender(a);Cloudwalkers.Session.censuretemplate(a);this.$el.html(Mustache.render(Templates.settings.manageusergroups_details,a));this.users.touch(Cloudwalkers.Session.getAccount(),{records:100});this.groupusers.touch({groupid:this.model.id});return this},fillGroupItems:function(a){var b=this.$el.find("."+a[0].collection.parenttype+"-"+a[0].collection.typestring+"-container").eq(-1);b.empty();for(n in a){var c=new Cloudwalkers.Views.Settings.GroupItem({model:a[n],
type:a[0].collection.parenttype+"-"+a[0].collection.modelstring});b.append(c.render().el)}},fillUsersDropdown:function(a){var b=this.$el.find(".all-"+a[0].collection.modelstring+"-list").eq(-1);for(n in a){var c=new Cloudwalkers.Views.Settings.UserDropdown({model:a[n]});b.append(c.render().el)}this.$el.find(".all-"+a[0].collection.modelstring+"-list").chosen({width:"100%"})},listentouserdropdown:function(a){selecteduser={id:$(a.currentTarget).val(),name:$(a.currentTarget).get(0).textContent}},addUser:function(){Cloudwalkers.Net.post("groups/"+
this.model.id+"/users",{},selecteduser,function(a){a.error?Cloudwalkers.RootView.growl("Oops",this.translateString("something_went_wrong")):(Cloudwalkers.RootView.growl(this.translateString("group_management"),this.translateString("user_added")),this.groupusers.touch({groupid:this.model.id}),$("option").prop("selected",!1),$(".portlet-body form *").trigger("chosen:updated"))}.bind(this))},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=
"user_groups create_new_group name add_user user_list streams keyword_monitoring_category add add_stream add_keyword_monitoring_category".split(" ");this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Settings.UserDropdown=Backbone.View.extend({tagName:"option",events:{"click [data-edit-user-id]":"openDetails","click [data-delete-user-id]":"deleteUser"},initialize:function(a){a&&$.extend(this,a);this.parameters={};this.listenTo(this.model,"change",this.render)},render:function(){var a={};a.user=this.model.attributes;a.user.role=this.model.getRole();Cloudwalkers.Session.censuretemplate(a);this.$el.attr("value",a.user.id);this.$el.html(a.user.displayname);return this}});
