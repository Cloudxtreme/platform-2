...
"There is no pong.");this.interval=this.max;this.schedule()}});Cloudwalkers.UrlShortener=Backbone.Model.extend({initialize:function(){},url:function(){return"http://devapi.cloudwalkers.be/urlshortener/shorten?"+$.param(this.parameters)},parse:function(a){return a},sync:function(a,b,c){this.longurl=c.q;this.campaign=c.campaign;this.parameters={url:this.longurl};this.campaign&&(this.parameters.campaign=this.campaign);return Backbone.sync(a,b,c)}});var StorageClassLocal=function(a,b){this.get=function(a,b,f){if(a=JSON.parse(window.localStorage.getItem(a)))if(b){for(key in b)a=a.filter(function(a){return a[key]==b[key]});f(a.length?a.shift():null)}else f(a.shift());else f(null);return this};this.filter=function(a,b,f){if(a=JSON.parse(window.localStorage.getItem(a)))if(b){for(key in b)a=a.filter(function(a){return a[key]==b[key]});f(a.length?a:[])}else f(a);else f([]);return this};this.exists=function(a,b,f){if((a=JSON.parse(window.localStorage.getItem(a)))&&
...
this.type&&"twitter"==this.reference.get("networktoken")&&this.draft.set("body",{html:Mustache.render(this.action.parameters[0].value,{from:this.reference.get("from")[0]})})},original:function(){var a;this.draft.get("schedule")&&(this.draft.get("schedule").date=moment(this.draft.get("schedule").date).unix());(a=this.draft.get("variations"))&&$.each(a,function(a,c){c.schedule&&(c.schedule.date=moment(c.schedule.date).unix())});this.render()},render:function(){var a={streams:this.actionstreams.length?
...
"removeClass":"addClass"]("hidden");return this},summarizecampaign:function(){var a=this.draft.get("campaign"),b=this.$el.find("[data-collapsable=campaign] .summary").empty();if(a){var c=Cloudwalkers.Session.getAccount().get("campaigns").filter(function(b){if(b.id==a)return b}).shift();b.html("<span>"+c.name+"</span>")}return this},listentocampaign:function(a){a=$(a.currentTarget).val();0==a?this.removecampaign():(this.draft.set("campaign",Number(a)),this.trigger("update:campaign",Number(a)));this.summarizecampaign().$el.find("[data-collapsable=campaign]").addClass("collapsed")},
toggleaddcampaign:function(){this.$el.find(".chosen-container, .campaign-name").toggleClass("hidden");return this},listentoaddcampaign:function(a){a=$(a.currentTarget).val();Cloudwalkers.Session.getAccount().addcampaign(a,function(a){a=a.get("campaigns");this.draft.set("campaign",a[a.length-1].id);this.trigger("update:campaign",a[a.length-1].id);this.reloadcampaigns(a);this.summarizecampaign().$el.find("[data-collapsable=campaign]").addClass("collapsed")}.bind(this))},reloadcampaigns:function(a){var b=
...
{}})):"in"==c?(this.toggleschedentry("[data-set=now], [data-set=on]").toggleschedentry("[data-set=in]",!0),$("[data-set=on] input").val("")):"on"==c?(this.toggleschedentry("[data-set=now], [data-set=in]").toggleschedentry("[data-set=on]",!0),$("[data-set=in] select").val(600),void 0===this.parsemoment("#delay-date")&&(this.datepicker.datepicker("hide").val(""),Cloudwalkers.RootView.alert("Please set your Schedule to a date in the future"))):"onlyonce"==c?(this.toggleschedentry("[data-set=onlyonce]",
...
if(b.filter("#delay-select").val())a.date=moment().add("seconds",$("#delay-select").val()).unix();else if(b.filter("#delay-date").val()){var c=this.parsemoment("#delay-date");if(b.filter("#delay-date").val())var e=$("#delay-time").val().split(":");1<e.length&&c.add("minutes",60*Number(e[0])+Number(e[1]));a.date=c.unix()}b.filter("#repeat-interval").val()&&(a.repeat||(a.repeat={interval:604800}),a.repeat.interval=$("#repeat-interval").val()?3600*$("#repeat-interval").val()*$("#every-select").val():
0,$("#every-select-weekday").val()&&(a.repeat.weekdays=[$("#every-select-weekday").val()]),$("#every-select-weekday").attr("disabled",604800>a.repeat.interval),a.repeat.amount=Number(b.filter("#repeat-amount").val())?Number($("#repeat-amount").val()):!1,a.repeat.until=b.filter("#repeat-until").val()?moment($("#repeat-until").val(),["DD-MM-YYYY","DD-MM-YY","DD/MM/YYYY"]).unix():!1,e={},e.interval=$("#repeat-interval").val()||null,e.every=$("#every-select").val()||null,e.everyweek=$("#every-select-weekday").val()||
null,a.repeat.settings=e);return a},togglebesttime:function(a){$(a.currentTarget).toggleClass("inactive");$(a.currentTarget).hasClass("inactive")?$("#delay-time").attr("disabled",!1):$("#delay-time").attr("disabled",!0).val("");this.draft.get("schedule").besttimetopost=!$(a.currentTarget).hasClass("inactive");return this},summarizeschedule:function(){var a=this.parsescheduled(),b=this.$el.find("[data-collapsable=schedule] .summary").empty();if(a&&a.date){var c=a.besttimetopost?"Best time to post":
...
(f=a.repeat.amount,e=c.clone().add("seconds",f*a.repeat.interval));f&&b.html("<span>"+f+" times "+(e?"<em class='negative'>until "+e.format("dddd, D MMMM YYYY")+"</em>":"")+"</span>");return this},updateschedule:function(){var a=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1;(a=a?a:this.draft.get("schedule"))&&a.date&&($(this.datepicker.get(0)).datepicker("update",moment.unix(a.date).format("DD/MM/YYYY")),this.timepicker.timepicker("setTime",moment.unix(a.date).format("HH:mm")));
if(a&&a.repeat){a.repeat.until&&$(this.datepicker.get(1)).datepicker("update",moment.unix(a.repeat.until).format("DD/MM/YYYY"));var b=a.repeat.settings||{};b.interval&&$("#repeat-interval").val(b.interval);b.every&&$("#every-select").val(b.every);b.everyweek&&$("#every-select-weekday").val(b.everyweek);a.repeat.amount&&$("#repeat-amount").val(a.repeat.amount)}},preview:function(){this.$el.addClass("switch-mode");this.preview=new Cloudwalkers.Views.Preview({model:this.draft,previewtype:"default",streamid:this.activestream.id});
