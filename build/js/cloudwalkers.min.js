...
this.get("attachments").length:this.get("body").plaintext},sanitizepost:function(){this.attributes.body.html&&delete this.attributes.body.html;this.attributes.variations&&this.attributes.variations.forEach(function(a){a.body&&(a.body.plaintext?a.body.html&&delete a.body.html:delete a.body,console.log("running varn:",a))})},cloneSanitized:function(){var a=this.clone();a.attributes.date&&delete a.attributes.date;a.attributes.from&&delete a.attributes.from;a.attributes.stream&&delete a.attributes.stream;
a.attributes.streams=[];a.id&&(delete a.id,delete a.attributes.id);return a},checkloaded:function(a){a.objectType&&setTimeout(function(a){a.trigger("loaded")},1,this)},filterData:function(a){var b={};$.each("id objectType actiontokens subject body date engagement from read stream streams attachments parent statistics canHaveChildren children_count schedule variations".split(" "),function(c,f){void 0!==a[f]&&(b[f]=a[f])});var c=Cloudwalkers.Session.getStream(a.stream);c&&(b.icon=c.get("network").icon,
b.networktoken=c.get("network").token,b.networkdescription=c.get("name"));a.attachments?(a.attachments.length&&(b.media=a.attachments[a.attachments.length-1],b.media="image"==b.media.type?"picture":b.media.type),b.attached={},$.each(a.attachments,function(a,c){b.attached[c.type]=c})):b.media="reorder";a.engagement&&(b.trending=1E3>a.engagement?a.engagement:"+999");b.schedule&&(b.scheduledate=moment(b.schedule.date).format("DD MMM YYYY HH:mm"));a.body&&(b.body.intro=a.body.plaintext?a.body.plaintext.substr(0,
72):"...");a.date&&(b.fulldate=moment(a.date).format("DD MMM YYYY HH:mm"),b.dateonly=moment(a.date).format("DD MMM YYYY"),b.time=moment(a.date).format("HH:mm"));return b},filterActions:function(){return this.get("actiontokens")?this.actions.rendertokens():[]},filterCalReadable:function(){var a=this.get("objectType"),b=a&&"reorder"!=this.get("media");this.calNode||(this.calNode={id:this.id});this.calNode.start=a?$.fullCalendar.moment(this.get("date")):$.fullCalendar.moment();this.calNode.title=a?(this.get("title")?
this.get("title"):this.get("body").plaintext).substring(0,b?12:16):"...";this.calNode.className=a?this.get("networktoken")+"-color":"hidden";this.calNode.networkdescription=a?this.get("networkdescription"):null;this.calNode.intro=a?this.get("body").intro:null;this.calNode.icon=a?this.get("icon"):null;this.calNode.media=b?this.get("media"):null;a&&72<=this.calNode.intro.length&&(this.calNode.intro+="...");return this},attach:function(a,b,c){c=this.get("attachments")||[];a=b&&c[b]?$.extend(c[b],a):
c.push(a);this.set({attachments:c});return a},unattach:function(a){(this.get("attachments")||[]).splice(a,1)},addcampaign:function(){},setvariation:function(a,b,c){var e=this.get("variations")||[],f=e.filter(function(b){if(b.stream==a)return b});if(0==f.length){f={stream:a};if("image"==b||"link"==b){var g=[c];f.attachments=g}else f[b]=c;e.push(f)}else f=f[0],"image"==b||"link"==b?(g=f.attachments||[],0==g.length&&"image"==b?(g.push(c),f.attachments=g):"image"==b?g.push(c):(b=g.filter(function(a){if("link"==
a.type)return a}),0<b.length?b[0].url=c.url:0!=g.length?f.attachments.push(c):f.attachments=[c])):f[b]=c},getvariation:function(a,b){var c=this.get("variations")||[],c=_.findWhere(c,{stream:a});if("image"==b||"link"==b){if(c&&c.attachments)return c.attachments.filter(function(a){if(a.type==b)return a})}else{if(c&&c[b])return c[b];if(c&&!b)return c}},original:function(a,b,c){var e=this.get("variations")?this.get("variations").filter(function(b){return a&&b.stream==a.id}):[],f=a?e.length?e[0]:{id:a.id}:
this.attributes;void 0!==c&&(f[b]=c,a&&!e.length&&this.get("variations").push(f));return f[b]},removevarimg:function(a,b){var c=this.getvariation(a).attachments;if(_.isObject(b))this.addexclude(a,b);else for(n in c)"image"==c[n].type&&c[n].name==b&&c.splice(n,1)},addexclude:function(a,b){var c=this.getvariation(a);c.excludes?c.excludes.push(b):c.excludes=[b]},checkexclude:function(a,b){var c=this.getvariation(a);if(c)c=c.excludes;else return!1;if(c)for(n in c)if(c[n].name==b.name)return!0;return!1},
location:function(){return"#"},afterChange:function(){this.addInternalActions();this.get("parent")&&this.get("parentmodel").set(this.get("parent"))},getActionIcon:function(a){return"internal-share"==a.token?"share-alt":"internal-edit"==a.token?"edit":"internal-delete"==a.token||"delete"==a.token?"remove":"like"==a.token?"thumbs-up":"unlike"==a.token?"thumbs-down":"comment"==a.token?"comment-alt":"reply"==a.token?"comment":"retweet"==a.token?"retweet":"dm"==a.token?"envelope":"internal-reply"==a.token?
"comment":"favorite"==a.token?"star":"unfavorite"==a.token?"star-empty":a.token},addInternalActions:function(){var a=this;if("undefined"!=typeof this.attributes.actions)for(var b=0;b<this.attributes.actions.length;b++){if("internal-"==this.attributes.actions[b].token.substr(0,9))return}else this.attributes.actions=[];"INCOMING"==this.attributes.type?this.attributes.actions.push({name:"Share",parameters:[],token:"internal-share",callback:function(a){Cloudwalkers.RootView.shareMessage(a)}}):"OUTGOING"==
...
c:c.parents("[data-set]").eq(0)).data("set"),e;(e=this.draft.original(this.activestream,"schedule"))||(e={repeat:{}});e.now&&delete e.now;"now"==c?(this.toggleschedentry("[data-set=in], [data-set=on]").toggleschedentry("[data-set=now]",!0),$("[data-set=on] #delay-date").val("").attr("disabled",!1),$("[data-set=in] select").val(600),$("[data-set=in] .btn-white").addClass("inactive"),e={repeat:e.repeat,now:!0}):"in"==c?(this.toggleschedentry("[data-set=now], [data-set=on]").toggleschedentry("[data-set=in]",
...
604800>a.repeat.interval),a.repeat.amount=Number(b.filter("#repeat-amount").val())?Number($("#repeat-amount").val()):!1,a.repeat.until=b.filter("#repeat-until").val()?moment($("#repeat-until").val(),["DD-MM-YYYY","DD-MM-YY","DD/MM/YYYY"]).unix():!1,b={},b.interval=$("#repeat-interval").val()||null,b.every=$("#every-select").val()||null,b.everyweek=$("#every-select-weekday").val()||null,a.repeat.settings=b);return a},togglebesttime:function(a){var b;(b=this.draft.original(this.activestream,"schedule"))||
(b={repeat:{}});$(a.currentTarget).toggleClass("inactive");$(a.currentTarget).hasClass("inactive")?$("#delay-time").attr("disabled",!1):$("#delay-time").attr("disabled",!0).val("");b.besttimetopost=!$(a.currentTarget).hasClass("inactive");this.draft.original(this.activestream,"schedule",b);return this},summarizeschedule:function(){var a=this.parsescheduled(),b=this.$el.find("[data-collapsable=schedule] .summary").empty();if(a&&a.date){var c=a.besttimetopost?"Best time to post":moment.unix(a.date).format("HH:mm");
b.html("<span><i class='icon-time'></i> "+moment.unix(a.date).format("dddd, D MMMM YYYY")+"<em class='negative'>"+c+"</em></span>")}this.summarizerepeat();return this},summarizerepeat:function(){var a=this.parsescheduled();if(!a||!a.repeat)return this;var b=this.$el.find("[data-collapsable=repeat] .summary").empty(),c=a.date?moment.unix(a.date):moment();if(a.repeat.until)var e=moment.unix(a.repeat.until),c=e.diff(c)/1E3,f=Math.round(c/a.repeat.interval);else a.repeat.interval&&(f=a.repeat.amount,
e=c.clone().add("seconds",f*a.repeat.interval));f&&b.html("<span>"+f+" times "+(e?"<em class='negative'>until "+e.format("dddd, D MMMM YYYY")+"</em>":"")+"</span>");return this},updateschedule:function(){var a=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,b=a?a:this.draft.get("schedule");b&&b.date?($(this.datepicker.get(0)).datepicker("update",moment.unix(b.date).format("DD/MM/YYYY")),this.timepicker.timepicker("setTime",moment.unix(b.date).format("HH:mm"))):b&&b.now&&
(this.toggleschedentry("[data-set=in], [data-set=on]").toggleschedentry("[data-set=now]",!0),$("[data-set=on] #delay-date").val("").attr("disabled",!1),$("[data-set=in] select").val(600),$("[data-set=in] .btn-white").addClass("inactive"),schedule={repeat:schedule.repeat,now:!0});b&&b.repeat&&(b.repeat.until&&$(this.datepicker.get(1)).datepicker("update",moment.unix(b.repeat.until).format("DD/MM/YYYY")),b.repeat.interval&&(this.toggleschedentry("[data-set=onlyonce], "+(b.repeat.amount?"[data-set=until]":
"[data-set=repeat]"),!1),this.toggleschedentry("[data-set=every], "+(b.repeat.amount?"[data-set=repeat]":"[data-set=until]"),!0)),[720,168,24,1].some(function(a){var e=b.repeat.interval/a/3600;if(Math.round(e)==e)return $("#repeat-interval").val(e),$("#every-select").val(a),!0}),b.repeat.weekdays&&b.repeat.weekdays.length&&$("#every-select-weekday").val(b.repeat.weekdays[0]),b.repeat.amount&&$("#repeat-amount").val(b.repeat.amount))},preview:function(){this.$el.addClass("switch-mode");this.preview=
...
"Please select a network first.",this.$el.find(".modal-footer"));this.disablefooter();this.draft.sanitizepost();this.draft.get("streams").length?this.draft.id?this.draft.save({body:this.draft.get("body"),variations:this.draft.get("variations"),streams:this.draft.get("streams"),status:"scheduled",schedule:this.draft.get("schedule"),update:!0},{patch:!0,endpoint:"original",success:this.thankyou.bind(this)}):this.draft.save({status:"scheduled"},{success:this.thankyou.bind(this)}):this.save("scheduled")},
...
this.listentochange()},listentochange:function(a,b){var c;if(this.content!==this.$contenteditable.text()||b)this.pos=this.cursorpos(),(c=this.listentourl(this.$contenteditable.text()))&&this.processurls(c),this.charlength!=this.$contenteditable.text().length&&this.trigger("change:charlength");this.content=this.$contenteditable.text();this.isdefault&&!b&&(this.$contenteditable.removeClass("withdefault"),this.isdefault=!1);this.trigger("change:content",this.content);b&&this.clearselections()},endchange:function(a){if(this.$contenteditable.html().match(this.xurlendpattern)){var b;
(b=this.listentourl(this.$contenteditable.text(),!0))&&this.processurls(b)}},listentourl:function(a,b){var c=this.win.getSelection(),e=this.document.createRange(),f,g,h,l;e.selectNodeContents(this.$contenteditable.get(0));f=this.parsenodes(e.startContainer.childNodes,b);if(f.length)return $.each(f,function(a,b){g=b.node.textContent.replace(/&nbsp;/gi,"");b.url=b.url.trim();h=g.indexOf(b.url);l=h+b.url.length;0>h&&(h=0);e.setStart(b.node,h);e.setEnd(b.node,l);c.removeAllRanges();c.addRange(e);this.contenteditable.designMode=
"on";document.execCommand("createLink",!1,b.url);this.contenteditable.designMode="off";var m=document.createTextNode("\u200b");e.insertNode(m);this.$contenteditable.find("a").after(m);e.setStartAfter(m);c.removeAllRanges();c.addRange(e);f[a+1]&&(f[a+1].node=b.node.nextSibling)}.bind(this)),_.pluck(f,"url")},parsenodes:function(a,b){var c=[],e;$.each(a,function(a,g){var h=!1,l=g.textContent;if(b&&3!=g.nodeType)return!0;l&&(h=l.match(b?this.xurlendpattern:this.xurlpattern));if(h&&h.length)return $.each(h,
