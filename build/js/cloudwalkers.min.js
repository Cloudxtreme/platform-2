...
this.get(a);d||(d=Cloudwalkers.Session.user.account[this.typestring].get(a),this.add(d));d||(d=this.create({id:a}));b.push(d);if(d.get("objectType")&&!d.outdated)d.stamp();else return a},this));a.length&&(this.endpoint=this.parentmodel?this.typestring:null,this.parameters={ids:a.join(",")},this.fetch({remove:!1}));this.trigger("seed",b);return b},more:function(a,b){if(!this.cursor)return!1;b.after=this.cursor;this.touch(a,b);return this},ready:function(){setTimeout(function(a){a.trigger("ready",a)},
...
new Cloudwalkers.Collections.Statistics;this.reports=new Cloudwalkers.Collections.Reports;this.comments=new Cloudwalkers.Collections.Comments},parse:function(a){this.endpoint="";Store.set("accounts",a.account);return a.account},url:function(){return CONFIG_BASE_URL+"json/account/"+this.id+this.endpoint},sync:function(a,b,c){this.endpoint=c.endpoint?"/"+c.endpoint:"";return Backbone.sync(a,b,c)},firstload:function(){$.each(this.get("channels"),function(a,b){Cloudwalkers.Session.storeChannel(b)})},
...
this.appendWidget(a,4);var b=new Cloudwalkers.Views.Widgets.ScheduledList({model:this.model});this.appendWidget(b,8);a.list=b;return this}});Cloudwalkers.Views.Calendar=Cloudwalkers.Views.Pageview.extend({title:"Calendar",className:"container-fluid calendar",events:{remove:"destroy"},initialize:function(){this.model=Cloudwalkers.Session.getChannel("profiles");if(!this.model)return Cloudwalkers.Session.home();this.listenTo(this.model,"sync",this.render)},render:function(){this.$el.html(Mustache.render(Templates.calendar,{title:this.title}));this.$container=this.$el.find("#widgetcontainer").eq(0);setTimeout(this.initCalendar.bind(this),
10);return this},populate:function(a,b,c){this.listenToOnce(this.model.messages,"seed",this.fill.bind(this,c));this.model.messages.touch(this.model,{records:999,since:Math.round(a.getTime()/1E3),until:Math.round(b.getTime()/1E3)})},fill:function(a,b){var c=[];for(n in b)this.listenToOnce(b[n].filterCalReadable(),"change:date",this.updatenode),c.push(b[n].calNode);a(c)},updatenode:function(a,b){$("#calendar").fullCalendar("updateEvent",a.filterCalReadable().calNode)},filldrafts:function(a){$("#drafts-list").html("");
for(n in a)this.addDraft("Draft message ("+a[n].id+")")},initCalendar:function(){var a={};720>=$("#calendar").parents(".portlet").width()?($("#calendar").addClass("mobile"),a={left:"title, prev, next",center:"",right:"today,month,agendaWeek"}):($("#calendar").removeClass("mobile"),a={left:"title",center:"",right:"prev,next,today,month,agendaWeek"});$("#calendar").fullCalendar("destroy");$("#calendar").fullCalendar({header:a,slotMinutes:30,editable:!1,droppable:!1,allDayDefault:!1,allDaySlot:!1,axisFormat:"H:mm",
defaultEventMinutes:30,slotEventOverlap:!1,events:this.populate.bind(this),eventRender:function(a,c){if(!a.icon)return!1;$(c).find(".fc-event-time").html("<i class='icon-"+a.icon+"'></i>");a.media&&$(c).find(".fc-event-inner").prepend("<i class='fc-event-media pull-right icon-"+a.media+"'></i>");$(c).data("title",a.intro).tooltip({html:!0,delay:500,placement:"top"});a.rendered=!0}})},addDraft:function(a){a=0==a.length?"Untitled Event":a;a=$('<div class="external-event label">'+a+"</div>");$("#drafts-list").append(a);
this.initDrag(a)},initDrag:function(a){var b={title:$.trim(a.text())};a.data("eventObject",b);a.draggable({zIndex:999,revert:!0,revertDuration:0})}});Cloudwalkers.Views.Coworkers=Cloudwalkers.Views.Pageview.extend({title:"Co-workers wall",className:"container-fluid coworkers",initialize:function(){this.model=this.model=Cloudwalkers.Session.getStream("coworkers");if(!this.model)return Cloudwalkers.Session.home();this.listenTo(this.model,"outdated",this.model.fetch);this.listenTo(this.model,"sync",this.render)},render:function(){this.$el.html(Mustache.render(Templates.pageview,{title:this.title}));this.$container=this.$el.find("#widgetcontainer").eq(0);
...
a)},toggleaction:function(a,b){this.$el.find('div[data-notification-action="'+a+'"]').data("action",b.token).find("i").attr("class","").addClass("icon-"+b.icon)},toggleactions:function(a){a="mouseout"==a.originalEvent.type;this.$el.find(".message-actions")[a?"addClass":"removeClass"]("hidden");this.$el.find(".comment-info")[a?"removeClass":"addClass"]("hidden")},markasread:function(){this.model.save({read:1},{patch:!0,wait:!0});this.model.get("stream")&&Cloudwalkers.Session.getStreams().outdated(this.model.get("stream"))}});Cloudwalkers.Views.Widgets.Chart=Backbone.View.extend({title:"Chart",events:{},initialize:function(a){a&&$.extend(this,a);this.listenTo(this.model.statistics,"ready",this.fill)},render:function(){this.$el.html(Mustache.render(Templates.chart,{title:this.title}));this.canvas=this.$el.find("canvas").get(0).getContext("2d");var a=this.parse[this.chart]();(new Chart(this.canvas))[this.chart](a);return this},fill:function(a){this.parse.collection=a;a=this.parse[this.chart](this.model,this.filterfunc);
