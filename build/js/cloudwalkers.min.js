...
function(a,b,f){window.localStorage.setItem(a,JSON.stringify(b));f&&f(b);return this};this.set=function(a,b,f){var g=!1,h=JSON.parse(window.localStorage.getItem(a));h||(h=[]);for(n in h)h[n].id==b.id&&($.extend(h[n],b),g=!0);g||h.push(b);window.localStorage.setItem(a,JSON.stringify(h));f&&f(b);return this};this.update=function(a,b,f,g){var h=JSON.parse(window.localStorage.getItem(a)),l=h;if(b&&l){for(key in b)l=l.filter(function(a,c){return a[key]==b[key]});for(n in l)for(i in h)h[i].id==l[i].id&&
(h[i]=$.extend(l[n],f));window.localStorage.setItem(a,JSON.stringify(h));g&&(l.length?g(l):g(null,f))}else g(null,f);return this};this.updateById=function(a,b,f){var g=JSON.parse(window.localStorage.getItem(a)),h=!1;if(g&&b.id){for(n in g)g[n].id==b.id&&(g[n]=h=$.extend(g[n],b));window.localStorage.setItem(a,JSON.stringify(g));f&&(h?f(h):f(null,b))}else f(null,b);return this};this.remove=function(a,b,f,g){f=JSON.parse(window.localStorage.getItem(a));if(!b)return window.localStorage.removeItem(a),
...
JSON.stringify(f));c&&c(b);return this};this.update=function(a,b,c,e){var f=JSON.parse(window.localStorage.getItem(a)),m=f;if(b&&m){for(key in b)m=m.filter(function(a,c){return a[key]==b[key]});for(n in m)for(i in f)f[i].id==m[i].id&&(f[i]=$.extend(m[n],c));window.localStorage.setItem(a,JSON.stringify(f));e&&(m.length?e(m):e(null,c))}else e(null,c);return this};this.updateById=function(a,b,c){var e=JSON.parse(window.localStorage.getItem(a)),f=!1;if(e&&b.id){for(n in e)e[n].id==b.id&&(e[n]=f=$.extend(e[n],
...
renderHeader:function(){var a={dev:this.development};a.user=Cloudwalkers.Session.user.attributes;a.accounts=[];a.level=Cloudwalkers.Session.getUser().level;Cloudwalkers.Session.user.accounts.each(function(b){a.accounts.push({name:b.get("name"),id:b.id,active:b.id==Cloudwalkers.Session.get("currentAccount")})});this.header=Mustache.render(Templates.header,a);return this},render:function(){var a=Cloudwalkers.Session.getAccount(),b={reports:[]};b.level=Cloudwalkers.Session.getUser().level;if(b.level){b.news=
a.channels.findWhere({type:"news"}).id;var c=a.channels.findWhere({type:"monitoring"});b.monitoring={channelid:c.id,first:c.channels.models[0],channels:c.channels.models,name:c.get("name")}}b.scheduled={channelid:Cloudwalkers.Session.getChannel("internal").id};b.scheduled.streams=a.streams.where({outgoing:1});b.inbox=!0;c=a.channels.findWhere({type:"profiles"});b.profiles={channelid:c.id,streams:c.streams.models,name:c.get("name")};b.reports=a.streams.where({statistics:1}).map(function(a){return a.attributes});
...
addNotification:function(a){this.$el.find(".popup-frame ul").append('<li><div class="text account"><p>'+a.message+'</p><div class="row"><span class="time"></span></div></div></li>')},toggleNotifications:function(){this.$el.find(".notification-popup").toggle()},mapViews:function(){var a={};for(n in this.views)if(a[this.views[n].name]={streams:[]},this.views[n].children)for(i in this.views[n].children)a[this.views[n].children[i].name]={streams:[]};return a}});Cloudwalkers.Views.Compose=Backbone.View.extend({id:"compose",className:"modal hide",type:"post",network:"default",actionstreams:[],actionview:!1,titles:{post:"Write Post",edit:"Edit Post",share:"Share message",comment:"Write Comment",reply:"Write Reply",dm:"Write Message",retweet:"Retweet",like:"Like",favourite:"Favourite",plusone:"Plus one","default":"Compose"},limitations:{},options:{"false":"editor link images campaign schedule repeat".split(" "),twitter:"editor images link campaign schedule repeat".split(" "),
facebook:"editor fullbody link images campaign schedule repeat".split(" "),linkedin:"editor link images campaign schedule repeat".split(" "),tumblr:"subject editor fullbody link images campaign schedule repeat".split(" "),"google-plus":"editor fullbody link images campaign schedule repeat".split(" "),youtube:"subject editor fullbody link video campaign schedule repeat".split(" "),blog:"subject editor fullbody link images campaign schedule repeat".split(" "),group:"editor images link campaign schedule repeat".split(" "),
"mobile-phone":["editor","schedule","repeat"],comment:["editor"],reply:["editor"],dm:["editor"],retweet:["icon"],like:["icon","schedule"],favorite:["icon"],plusone:["icon"]},icons:{retweet:"retweet",like:"thumbs-up",favorite:"star",plusone:"plus-square"},events:{"click li[data-streams]":"togglestreams","click .stream-tabs div":"togglestream","click [data-toggle]":"toggleoption","keyup [data-option] input":"monitor","change [data-option] #compose-content":"monitor","keyup [data-option] #compose-content":"monitor",
"click .add-file":"addfile","click .add-snapshot":"addsnapshot","change [data-collapsable=images] input":"listentofiles","click .photo-booth":"listentobooth","click li.images-thumb":"removefile","blur [data-collapsable=link] input":"listentolink","change select.campaign-list":"listentocampaign","blur .campaign-name":"listentoaddcampaign","click .add-campaign":"toggleaddcampaign","click .schedule-entry":"monitorschedule","blur [data-collapsable=schedule] input, [data-collapsable=repeat] input":"monitorschedule",
"change [data-collapsable=schedule] select, [data-collapsable=repeat] select":"monitorschedule","change [data-collapsable=schedule] #delay-time":"monitorschedule","changeDate input":"monitorschedule","click [data-set=on] .btn-white":"togglebesttime","click .end-preview":"endpreview","click #previewbtn":"preview","click #save":"save","click #post":"post","click #action, [data-type=icon] > i":"postaction"},initialize:function(a){a&&$.extend(this,a);this.streams=Cloudwalkers.Session.getChannel("outgoing").streams;
this.action&&(this.type=this.action.get("token"));this.action&&"share"==this.type?this.draft=this.reference.cloneSanitized():this.action&&this.reference?(this.action.parent=this.reference,this.actionstreams=[],this.actionview=!0,this.listenTo(this.action,"change",this.editstreams),this.action.fetch()):this.model&&(this.type="edit",this.state="loading",this.draft=this.model.clone(),this.draft.fetch({endpoint:"original",success:this.original.bind(this)}));this.draft||(this.draft=new Cloudwalkers.Models.Message({variations:[],
attachments:[],streams:[],body:{},schedule:{}}),this.listenTo(this.draft,"invalid",this.invalid));this.loadListeners(this.draft,["request","sync"],!0);"reply"==this.type&&"twitter"==this.reference.get("networktoken")&&this.draft.set("body",{html:Mustache.render(this.action.parameters[0].value,{from:this.reference.get("from")[0]})})},original:function(){var a;this.draft.get("schedule")&&this.draft.get("schedule").date&&(this.draft.get("schedule").date=moment(this.draft.get("schedule").date).unix());
(a=this.draft.get("variations"))&&$.each(a,function(a,c){c.schedule&&(c.schedule.date=moment(c.schedule.date).unix())});this.state="loaded";this.render()},render:function(){var a={streams:this.actionstreams.length?this.actionstreams:this.streams.models,title:this.titles[this.type],campaigns:Cloudwalkers.Session.getAccount().get("campaigns"),actionview:this.actionview?this.type:!1};"edit"==this.type&&(a.type=this.type);a=Mustache.render(Templates.compose,a);this.$el.html(a);this.editor||(this.editor=
...
"addClass"]("icon-circle-blank")[b?"addClass":"removeClass"]("icon-ok-circle");return this},parsemoment:function(a,b){var c=$(a),e=$(b);if(!c.val()||!e.val())return null;c=moment(c.val(),"DD-MM-YYYY DD-MM-YY DD/MM/YYYY DD/MM/YY DDMMYYYY YYYYMMDD MM-DD-YYYY MM-DD-YY".split(" "));e=e.val().split(":");c=_.clone(c);1<e.length&&c.add("minutes",60*Number(e[0])+Number(e[1]));return c.isValid()?c.unix()<moment().unix()?void 0:c:void 0},monitorschedule:function(a,b){var c=b||$(a.currentTarget);this.activestream&&
<<<<<<< HEAD
!this.draft.getvariation(this.activestream.id,"schedule")&&this.draft.setvariation(this.activestream.id,"schedule",{});var e=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,e=e?e:this.draft.get("schedule"),c=(c.data("set")?c:c.parents("[data-set]").eq(0)).data("set");"now"==c?(this.toggleschedentry("[data-set=in], [data-set=on]").toggleschedentry("[data-set=now]",!0),$("[data-set=on] input").val(""),$("[data-set=in] select").val(600),this.draft.set("schedule",{repeat:e.repeat||
{}})):"in"==c?(this.toggleschedentry("[data-set=now], [data-set=on]").toggleschedentry("[data-set=in]",!0),$("[data-set=on] input").val("")):"on"==c?(this.toggleschedentry("[data-set=now], [data-set=in]").toggleschedentry("[data-set=on]",!0),$("[data-set=in] select").val(600),void 0===this.parsemoment("#delay-date","#delay-time")&&(this.datepicker.datepicker("hide").val(""),Cloudwalkers.RootView.alert("Please set your Schedule to a date in the future"))):"onlyonce"==c?(this.toggleschedentry("[data-set=onlyonce]",
!0).toggleschedentry("[data-set=repeat], [data-set=until], [data-set=every]",!1),$("#repeat-interval, #repeat-amount").val(0),$("#repeat-until").val("")):"every"==c?this.toggleschedentry("[data-set=every]",!0).toggleschedentry("[data-set=onlyonce]",!1):"repeat"==c?(this.toggleschedentry("[data-set=every], [data-set=repeat]",!0).toggleschedentry("[data-set=onlyonce], [data-set=until]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),Number($("#repeat-amount").val())||$("#repeat-amount").val(1),
$("#repeat-until").val("")):"until"==c&&(this.toggleschedentry("[data-set=every], [data-set=until]",!0).toggleschedentry("[data-set=onlyonce], [data-set=repeat]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),$("#repeat-amount").val(0));console.log(c);this.parsescheduled();a.stopPropagation();return this},parsescheduled:function(){var a=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,a=a?a:this.draft.get("schedule"),b=this.$el.find("section[data-collapsable] .schedule-entry").not(".inactive").find("#delay-select, #delay-date, #delay-time, #repeat-interval, #every-select, #every-select-weekday, #repeat-amount, #repeat-until");
if(b.filter("#delay-select").val())a.date=moment().add("seconds",$("#delay-select").val()).unix();else if(b.filter("#delay-date").val()){var c=this.parsemoment("#delay-date","#delay-time");c&&(a.date=c.unix())}b.filter("#repeat-interval").val()&&(a.repeat||(a.repeat={interval:604800}),a.repeat.interval=$("#repeat-interval").val()?3600*$("#repeat-interval").val()*$("#every-select").val():0,$("#every-select-weekday").val()&&(a.repeat.weekdays=[$("#every-select-weekday").val()]),$("#every-select-weekday").attr("disabled",
604800>a.repeat.interval),a.repeat.amount=Number(b.filter("#repeat-amount").val())?Number($("#repeat-amount").val()):!1,a.repeat.until=b.filter("#repeat-until").val()?moment($("#repeat-until").val(),["DD-MM-YYYY","DD-MM-YY","DD/MM/YYYY"]).unix():!1,b={},b.interval=$("#repeat-interval").val()||null,b.every=$("#every-select").val()||null,b.everyweek=$("#every-select-weekday").val()||null,a.repeat.settings=b);return a},togglebesttime:function(a){$(a.currentTarget).toggleClass("inactive");$(a.currentTarget).hasClass("inactive")?
$("#delay-time").attr("disabled",!1):$("#delay-time").attr("disabled",!0).val("");this.draft.get("schedule").besttimetopost=!$(a.currentTarget).hasClass("inactive");return this},summarizeschedule:function(){var a=this.parsescheduled(),b=this.$el.find("[data-collapsable=schedule] .summary").empty();if(a&&a.date){var c=a.besttimetopost?"Best time to post":moment.unix(a.date).format("HH:mm");b.html("<span><i class='icon-time'></i> "+moment.unix(a.date).format("dddd, D MMMM YYYY")+"<em class='negative'>"+
c+"</em></span>")}this.summarizerepeat();return this},summarizerepeat:function(){var a=this.parsescheduled();if(!a||!a.repeat)return this;var b=this.$el.find("[data-collapsable=repeat] .summary").empty(),c=a.date?moment.unix(a.date):moment();if(a.repeat.until)var e=moment.unix(a.repeat.until),c=e.diff(c)/1E3,f=Math.round(c/a.repeat.interval);else a.repeat.interval&&(f=a.repeat.amount,e=c.clone().add("seconds",f*a.repeat.interval));f&&b.html("<span>"+f+" times "+(e?"<em class='negative'>until "+e.format("dddd, D MMMM YYYY")+
"</em>":"")+"</span>");return this},updateschedule:function(){var a=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,b=a?a:this.draft.get("schedule");b&&b.date&&($(this.datepicker.get(0)).datepicker("update",moment.unix(b.date).format("DD/MM/YYYY")),this.timepicker.timepicker("setTime",moment.unix(b.date).format("HH:mm")));b&&b.repeat&&(b.repeat.until&&$(this.datepicker.get(1)).datepicker("update",moment.unix(b.repeat.until).format("DD/MM/YYYY")),b.repeat.interval&&this.toggleschedentry("[data-set=onlyonce]",
!1).toggleschedentry("[data-set=every], "+(b.repeat.amount?"[data-set=repeat]":"[data-set=until]"),!0),[720,168,24,1].some(function(a){var e=b.repeat.interval/a/3600;if(Math.round(e)==e)return $("#repeat-interval").val(e),$("#every-select").val(a),!0}),b.repeat.weekdays&&b.repeat.weekdays.length&&$("#every-select-weekday").val(b.repeat.weekdays[0]),b.repeat.amount&&$("#repeat-amount").val(b.repeat.amount))},preview:function(){this.$el.addClass("switch-mode");this.preview=new Cloudwalkers.Views.Preview({model:this.draft,
previewtype:"default",streamid:this.activestream.id});this.$el.find(".switch-container").append(this.preview.render().el)},endpreview:function(){this.preview.remove();this.$el.removeClass("switch-mode")},save:function(a){if(!this.draft.validateCustom())return Cloudwalkers.RootView.information("Not saved","You need a bit of content.",this.$el.find(".modal-footer"));this.disablefooter();a&&_.isString(a)||(a="draft");this.draft.sanitizepost();this.draft.id?this.draft.save({body:this.draft.get("body"),
variations:this.draft.get("variations"),streams:this.draft.get("streams"),update:!0},{patch:!0,endpoint:"original",success:this.thankyou.bind(this)}):this.draft.save({status:a},{success:this.thankyou.bind(this)})},post:function(){if(!this.draft.validateCustom())return Cloudwalkers.RootView.information("Not saved:","You need a bit of content.",this.$el.find(".modal-footer"));if(1>=this.$el.find(".stream-tabs .stream-tab").length)return Cloudwalkers.RootView.information("Not posted:","Please select a network first.",
this.$el.find(".modal-footer"));this.disablefooter();this.draft.sanitizepost();this.draft.get("streams").length?this.draft.id?this.draft.save({body:this.draft.get("body"),variations:this.draft.get("variations"),streams:this.draft.get("streams"),status:"scheduled",update:!0},{patch:!0,endpoint:"original",success:this.thankyou.bind(this)}):this.draft.save({status:"scheduled"},{success:this.thankyou.bind(this)}):this.save("scheduled")},postaction:function(){var a=[];this.$el.find(".action-tabs div").each(function(){a.push($(this).data("stream"))});
if(!a.length)return Cloudwalkers.RootView.information("Not saved","Select at least 1 network",this.$el.find(".modal-footer"));if(0<=this.options[this.type].indexOf("editor")&&!this.draft.get("body").html)return Cloudwalkers.RootView.information("Not saved","Provide some text first",this.$el.find(".modal-footer"));this.disablefooter();var b=this.reference.actions.create({streams:a,message:this.draft.get("body").html,actiontype:this.type},{success:this.thankyou.bind(this)});this.loadListeners(b,["request:action",
"sync"]);b.trigger("request:action")},thankyou:function(){var a=Mustache.render(Templates.thankyou);setTimeout(function(){this.$el.addClass("switch-mode").addClass("thanks");this.$el.find(".switch-container").append(a);setTimeout(function(){this.$el.modal("hide")}.bind(this),1E3)}.bind(this),400);"edit"==this.type&&this.model.fetch()},disablefooter:function(){this.$el.find(".modal-footer .btn").attr("disabled",!0)},invalid:function(a,b){Cloudwalkers.RootView.alert(a.get("title")+" "+b)},isscrolling:function(){if(this.$el.find(".stream-tabs").length)return this.$el.find(".stream-tabs")[0].scrollWidth>
this.$el.find(".stream-tabs").width();if(this.$el.find(".action-tabs").length)return this.$el.find(".action-tabs")[0].scrollWidth>this.$el.find(".action-tabs").width()},checklimitations:function(a,b){}});Cloudwalkers.Views.Preview=Backbone.View.extend({id:"preview",events:{},networkclasses:{facebook:"fb",twitter:"twt","google-plus":"gp",linkedin:"li"},events:{"click #viewsummary":"togglesummary"},initialize:function(a){a&&$.extend(this,a);$.extend(this.model.attributes,this.model.getvariation(this.streamid));this.model.attributes.profile=Cloudwalkers.Session.getStream(this.streamid).get("profile");this.network=Cloudwalkers.Session.getStream(this.streamid).get("network").token},render:function(){var a=
=======
!this.draft.getvariation(this.activestream.id,"schedule")&&this.draft.setvariation(this.activestream.id,"schedule",{});var e=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,e=e?e:this.draft.get("schedule"),c=(c.data("set")?c:c.parents("[data-set]").eq(0)).data("set");"now"==c?(this.toggleschedentry("[data-set=in], [data-set=on]").toggleschedentry("[data-set=now]",!0),$("[data-set=on] input").val("").attr("disabled",!1),$("[data-set=in] select").val(600),$("[data-set=in] .btn-white").addClass("inactive"),
this.draft.set("schedule",{repeat:e.repeat||{}})):"in"==c?(this.toggleschedentry("[data-set=now], [data-set=on]").toggleschedentry("[data-set=in]",!0),$("[data-set=on] input").val("")):"on"==c?(this.toggleschedentry("[data-set=now], [data-set=in]").toggleschedentry("[data-set=on]",!0),$("[data-set=in] select").val(600),void 0===this.parsemoment("#delay-date","#delay-time")&&(this.datepicker.datepicker("hide").val(""),Cloudwalkers.RootView.alert("Please set your Schedule to a date in the future"))):
"onlyonce"==c?(this.toggleschedentry("[data-set=onlyonce]",!0).toggleschedentry("[data-set=repeat], [data-set=until], [data-set=every]",!1),$("#repeat-interval, #repeat-amount").val(0),$("#repeat-until").val("")):"every"==c?this.toggleschedentry("[data-set=every]",!0).toggleschedentry("[data-set=onlyonce]",!1):"repeat"==c?(this.toggleschedentry("[data-set=every], [data-set=repeat]",!0).toggleschedentry("[data-set=onlyonce], [data-set=until]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),
<<<<<<< HEAD
Number($("#repeat-amount").val())||$("#repeat-amount").val(1),$("#repeat-until").val("")):"until"==c&&(this.toggleschedentry("[data-set=every], [data-set=until]",!0).toggleschedentry("[data-set=onlyonce], [data-set=repeat]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),$("#repeat-amount").val(0));this.parsescheduled();a.stopPropagation();return this},parsescheduled:function(){var a=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,a=a?a:this.draft.get("schedule"),
b=this.$el.find("section[data-collapsable] .schedule-entry").not(".inactive").find("#delay-select, #delay-date, #delay-time, #repeat-interval, #every-select, #every-select-weekday, #repeat-amount, #repeat-until");if(b.filter("#delay-select").val())a.date=moment().add("seconds",$("#delay-select").val()).unix();else if(b.filter("#delay-date").val()){var c=this.parsemoment("#delay-date","#delay-time");c&&(a.date=c.unix())}b.filter("#repeat-interval").val()&&(a.repeat||(a.repeat={interval:604800}),a.repeat.interval=
$("#repeat-interval").val()?3600*$("#repeat-interval").val()*$("#every-select").val():0,$("#every-select-weekday").val()&&(a.repeat.weekdays=[$("#every-select-weekday").val()]),$("#every-select-weekday").attr("disabled",604800>a.repeat.interval),a.repeat.amount=Number(b.filter("#repeat-amount").val())?Number($("#repeat-amount").val()):!1,a.repeat.until=b.filter("#repeat-until").val()?moment($("#repeat-until").val(),["DD-MM-YYYY","DD-MM-YY","DD/MM/YYYY"]).unix():!1,b={},b.interval=$("#repeat-interval").val()||
null,b.every=$("#every-select").val()||null,b.everyweek=$("#every-select-weekday").val()||null,a.repeat.settings=b);return a},togglebesttime:function(a){$(a.currentTarget).toggleClass("inactive");$(a.currentTarget).hasClass("inactive")?$("#delay-time").attr("disabled",!1):$("#delay-time").attr("disabled",!0).val("");this.draft.get("schedule").besttimetopost=!$(a.currentTarget).hasClass("inactive");return this},summarizeschedule:function(){var a=this.parsescheduled(),b=this.$el.find("[data-collapsable=schedule] .summary").empty();
if(a&&a.date){var c=a.besttimetopost?"Best time to post":moment.unix(a.date).format("HH:mm");b.html("<span><i class='icon-time'></i> "+moment.unix(a.date).format("dddd, D MMMM YYYY")+"<em class='negative'>"+c+"</em></span>")}this.summarizerepeat();return this},summarizerepeat:function(){var a=this.parsescheduled();if(!a||!a.repeat)return this;var b=this.$el.find("[data-collapsable=repeat] .summary").empty(),c=a.date?moment.unix(a.date):moment();if(a.repeat.until)var e=moment.unix(a.repeat.until),
c=e.diff(c)/1E3,f=Math.round(c/a.repeat.interval);else a.repeat.interval&&(f=a.repeat.amount,e=c.clone().add("seconds",f*a.repeat.interval));f&&b.html("<span>"+f+" times "+(e?"<em class='negative'>until "+e.format("dddd, D MMMM YYYY")+"</em>":"")+"</span>");return this},updateschedule:function(){var a=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,b=a?a:this.draft.get("schedule");b&&b.date&&($(this.datepicker.get(0)).datepicker("update",moment.unix(b.date).format("DD/MM/YYYY")),
this.timepicker.timepicker("setTime",moment.unix(b.date).format("HH:mm")));b&&b.repeat&&(b.repeat.until&&$(this.datepicker.get(1)).datepicker("update",moment.unix(b.repeat.until).format("DD/MM/YYYY")),b.repeat.interval&&this.toggleschedentry("[data-set=onlyonce]",!1).toggleschedentry("[data-set=every], "+(b.repeat.amount?"[data-set=repeat]":"[data-set=until]"),!0),[720,168,24,1].some(function(a){var e=b.repeat.interval/a/3600;if(Math.round(e)==e)return $("#repeat-interval").val(e),$("#every-select").val(a),
!0}),b.repeat.weekdays&&b.repeat.weekdays.length&&$("#every-select-weekday").val(b.repeat.weekdays[0]),b.repeat.amount&&$("#repeat-amount").val(b.repeat.amount))},preview:function(){this.$el.addClass("switch-mode");this.preview=new Cloudwalkers.Views.Preview({model:this.draft,previewtype:"default",streamid:this.activestream.id});this.$el.find(".switch-container").append(this.preview.render().el)},endpreview:function(){this.preview.remove();this.$el.removeClass("switch-mode")},save:function(a){if(!this.draft.validateCustom())return Cloudwalkers.RootView.information("Not saved",
"You need a bit of content.",this.$el.find(".modal-footer"));this.disablefooter();a&&_.isString(a)||(a="draft");this.draft.sanitizepost();this.draft.id?this.draft.save({body:this.draft.get("body"),variations:this.draft.get("variations"),streams:this.draft.get("streams"),update:!0},{patch:!0,endpoint:"original",success:this.thankyou.bind(this)}):this.draft.save({status:a},{success:this.thankyou.bind(this)})},post:function(){if(!this.draft.validateCustom())return Cloudwalkers.RootView.information("Not saved:",
"You need a bit of content.",this.$el.find(".modal-footer"));if(1>=this.$el.find(".stream-tabs .stream-tab").length)return Cloudwalkers.RootView.information("Not posted:","Please select a network first.",this.$el.find(".modal-footer"));this.disablefooter();this.draft.sanitizepost();this.draft.get("streams").length?this.draft.id?this.draft.save({body:this.draft.get("body"),variations:this.draft.get("variations"),streams:this.draft.get("streams"),status:"scheduled",update:!0},{patch:!0,endpoint:"original",
success:this.thankyou.bind(this)}):this.draft.save({status:"scheduled"},{success:this.thankyou.bind(this)}):this.save("scheduled")},postaction:function(){var a=[];this.$el.find(".action-tabs div").each(function(){a.push($(this).data("stream"))});if(!a.length)return Cloudwalkers.RootView.information("Not saved","Select at least 1 network",this.$el.find(".modal-footer"));if(0<=this.options[this.type].indexOf("editor")&&!this.draft.get("body").html)return Cloudwalkers.RootView.information("Not saved",
"Provide some text first",this.$el.find(".modal-footer"));this.disablefooter();var b=this.reference.actions.create({streams:a,message:this.draft.get("body").html,actiontype:this.type},{success:this.thankyou.bind(this)});this.loadListeners(b,["request:action","sync"]);b.trigger("request:action")},thankyou:function(){var a=Mustache.render(Templates.thankyou);setTimeout(function(){this.$el.addClass("switch-mode").addClass("thanks");this.$el.find(".switch-container").append(a);setTimeout(function(){this.$el.modal("hide")}.bind(this),
1E3)}.bind(this),400);"edit"==this.type&&this.model.fetch()},disablefooter:function(){this.$el.find(".modal-footer .btn").attr("disabled",!0)},invalid:function(a,b){Cloudwalkers.RootView.alert(a.get("title")+" "+b)},isscrolling:function(){if(this.$el.find(".stream-tabs").length)return this.$el.find(".stream-tabs")[0].scrollWidth>this.$el.find(".stream-tabs").width();if(this.$el.find(".action-tabs").length)return this.$el.find(".action-tabs")[0].scrollWidth>this.$el.find(".action-tabs").width()},checklimitations:function(a,
b){}});Cloudwalkers.Views.Preview=Backbone.View.extend({id:"preview",events:{},networkclasses:{facebook:"fb",twitter:"twt","google-plus":"gp",linkedin:"li"},events:{"click #viewsummary":"togglesummary"},initialize:function(a){a&&$.extend(this,a);$.extend(this.model.attributes,this.model.getvariation(this.streamid));this.model.attributes.profile=Cloudwalkers.Session.getStream(this.streamid).get("profile");this.network=Cloudwalkers.Session.getStream(this.streamid).get("network").token},render:function(){var a=
>>>>>>> master
=======
Number($("#repeat-amount").val())||$("#repeat-amount").val(1),$("#repeat-until").val("")):"until"==c&&(this.toggleschedentry("[data-set=every], [data-set=until]",!0).toggleschedentry("[data-set=onlyonce], [data-set=repeat]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),$("#repeat-amount").val(0));this.parsescheduled();a.stopPropagation();return this},parsescheduled:function(){console.log("parse");var a=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):
!1,a=a?a:this.draft.get("schedule"),b=this.$el.find("section[data-collapsable] .schedule-entry").not(".inactive").find("#delay-select, #delay-date, #delay-time, #repeat-interval, #every-select, #every-select-weekday, #repeat-amount, #repeat-until");if(b.filter("#delay-select").val())a.date=moment().add("seconds",$("#delay-select").val()).unix();else if(b.filter("#delay-date").val()){var c=this.parsemoment("#delay-date","#delay-time");c&&(a.date=c.unix())}b.filter("#repeat-interval").val()&&(a.repeat||
(a.repeat={interval:604800}),a.repeat.interval=$("#repeat-interval").val()?3600*$("#repeat-interval").val()*$("#every-select").val():0,$("#every-select-weekday").val()&&(a.repeat.weekdays=[$("#every-select-weekday").val()]),$("#every-select-weekday").attr("disabled",604800>a.repeat.interval),a.repeat.amount=Number(b.filter("#repeat-amount").val())?Number($("#repeat-amount").val()):!1,a.repeat.until=b.filter("#repeat-until").val()?moment($("#repeat-until").val(),["DD-MM-YYYY","DD-MM-YY","DD/MM/YYYY"]).unix():
!1,b={},b.interval=$("#repeat-interval").val()||null,b.every=$("#every-select").val()||null,b.everyweek=$("#every-select-weekday").val()||null,a.repeat.settings=b);return a},togglebesttime:function(a){$(a.currentTarget).toggleClass("inactive");$(a.currentTarget).hasClass("inactive")?$("#delay-time").attr("disabled",!1):$("#delay-time").attr("disabled",!0).val("");this.draft.get("schedule").besttimetopost=!$(a.currentTarget).hasClass("inactive");return this},summarizeschedule:function(){var a=this.parsescheduled(),
b=this.$el.find("[data-collapsable=schedule] .summary").empty();if(a&&a.date){var c=a.besttimetopost?"Best time to post":moment.unix(a.date).format("HH:mm");b.html("<span><i class='icon-time'></i> "+moment.unix(a.date).format("dddd, D MMMM YYYY")+"<em class='negative'>"+c+"</em></span>")}this.summarizerepeat();return this},summarizerepeat:function(){var a=this.parsescheduled();if(!a||!a.repeat)return this;var b=this.$el.find("[data-collapsable=repeat] .summary").empty(),c=a.date?moment.unix(a.date):
moment();if(a.repeat.until)var e=moment.unix(a.repeat.until),c=e.diff(c)/1E3,f=Math.round(c/a.repeat.interval);else a.repeat.interval&&(f=a.repeat.amount,e=c.clone().add("seconds",f*a.repeat.interval));f&&b.html("<span>"+f+" times "+(e?"<em class='negative'>until "+e.format("dddd, D MMMM YYYY")+"</em>":"")+"</span>");return this},updateschedule:function(){var a=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,b=a?a:this.draft.get("schedule");b&&b.date&&($(this.datepicker.get(0)).datepicker("update",
moment.unix(b.date).format("DD/MM/YYYY")),this.timepicker.timepicker("setTime",moment.unix(b.date).format("HH:mm")));b&&b.repeat&&(b.repeat.until&&$(this.datepicker.get(1)).datepicker("update",moment.unix(b.repeat.until).format("DD/MM/YYYY")),b.repeat.interval&&this.toggleschedentry("[data-set=onlyonce]",!1).toggleschedentry("[data-set=every], "+(b.repeat.amount?"[data-set=repeat]":"[data-set=until]"),!0),[720,168,24,1].some(function(a){var e=b.repeat.interval/a/3600;if(Math.round(e)==e)return $("#repeat-interval").val(e),
$("#every-select").val(a),!0}),b.repeat.weekdays&&b.repeat.weekdays.length&&$("#every-select-weekday").val(b.repeat.weekdays[0]),b.repeat.amount&&$("#repeat-amount").val(b.repeat.amount))},preview:function(){this.$el.addClass("switch-mode");this.preview=new Cloudwalkers.Views.Preview({model:this.draft,previewtype:"default",streamid:this.activestream.id});this.$el.find(".switch-container").append(this.preview.render().el)},endpreview:function(){this.preview.remove();this.$el.removeClass("switch-mode")},
save:function(a){if(!this.draft.validateCustom())return Cloudwalkers.RootView.information("Not saved","You need a bit of content.",this.$el.find(".modal-footer"));this.disablefooter();a&&_.isString(a)||(a="draft");this.draft.sanitizepost();this.draft.id?this.draft.save({body:this.draft.get("body"),variations:this.draft.get("variations"),streams:this.draft.get("streams"),update:!0},{patch:!0,endpoint:"original",success:this.thankyou.bind(this)}):this.draft.save({status:a},{success:this.thankyou.bind(this)})},
post:function(){if(!this.draft.validateCustom())return Cloudwalkers.RootView.information("Not saved:","You need a bit of content.",this.$el.find(".modal-footer"));if(1>=this.$el.find(".stream-tabs .stream-tab").length)return Cloudwalkers.RootView.information("Not posted:","Please select a network first.",this.$el.find(".modal-footer"));this.disablefooter();this.draft.sanitizepost();this.draft.get("streams").length?this.draft.id?this.draft.save({body:this.draft.get("body"),variations:this.draft.get("variations"),
streams:this.draft.get("streams"),status:"scheduled",update:!0},{patch:!0,endpoint:"original",success:this.thankyou.bind(this)}):this.draft.save({status:"scheduled"},{success:this.thankyou.bind(this)}):this.save("scheduled")},postaction:function(){var a=[];this.$el.find(".action-tabs div").each(function(){a.push($(this).data("stream"))});if(!a.length)return Cloudwalkers.RootView.information("Not saved","Select at least 1 network",this.$el.find(".modal-footer"));if(0<=this.options[this.type].indexOf("editor")&&
!this.draft.get("body").html)return Cloudwalkers.RootView.information("Not saved","Provide some text first",this.$el.find(".modal-footer"));this.disablefooter();var b=this.reference.actions.create({streams:a,message:this.draft.get("body").html,actiontype:this.type},{success:this.thankyou.bind(this)});this.loadListeners(b,["request:action","sync"]);b.trigger("request:action")},thankyou:function(){var a=Mustache.render(Templates.thankyou);setTimeout(function(){this.$el.addClass("switch-mode").addClass("thanks");
this.$el.find(".switch-container").append(a);setTimeout(function(){this.$el.modal("hide")}.bind(this),1E3)}.bind(this),400);"edit"==this.type&&this.model.fetch()},disablefooter:function(){this.$el.find(".modal-footer .btn").attr("disabled",!0)},invalid:function(a,b){Cloudwalkers.RootView.alert(a.get("title")+" "+b)},isscrolling:function(){if(this.$el.find(".stream-tabs").length)return this.$el.find(".stream-tabs")[0].scrollWidth>this.$el.find(".stream-tabs").width();if(this.$el.find(".action-tabs").length)return this.$el.find(".action-tabs")[0].scrollWidth>
this.$el.find(".action-tabs").width()},checklimitations:function(a,b){}});Cloudwalkers.Views.Preview=Backbone.View.extend({id:"preview",events:{},networkclasses:{facebook:"fb",twitter:"twt","google-plus":"gp",linkedin:"li"},events:{"click #viewsummary":"togglesummary"},initialize:function(a){a&&$.extend(this,a);$.extend(this.model.attributes,this.model.getvariation(this.streamid));this.model.attributes.profile=Cloudwalkers.Session.getStream(this.streamid).get("profile");this.network=Cloudwalkers.Session.getStream(this.streamid).get("network").token},render:function(){var a=
>>>>>>> master
...
return this},unfollow:function(){this.model.trigger("unfollow",this.model);this.model.parent=Cloudwalkers.Session.getAccount();this.model.save({following:!1},{patch:!0});this.remove()}});Cloudwalkers.Views.Editor=Backbone.View.extend({id:"editor",className:"clearfix",oldUrl:"",urldata:{},currenturl:!1,urlprocessing:!1,limit:null,content:"",charlength:0,pos:0,hasbeenwarned:!1,urls:{},posmap:[],restrictedstreams:{twitter:140,linkedin:700},events:{"update:content":"render","update:limit":"renderlimit","append:content":"append","keyup #compose-content":"listentochange","paste #compose-content":"listentopaste","blur #compose-content":"endchange","click #swaplink":"swaplink",'click [data-type="title"] .addcontent':"addoecontent",
'click [data-type="content"] .addcontent':"addoecontent",'click [data-type="image"] i':"addoeimg","mouseup #compose-content":"savepos"},xtrimmable:/<[^>]*>|&nbsp;|\s/g,xurlbasic:/http|[w]{3}/g,xurlpattern:/(^|\s|\r|\n|\u00a0)((https?:\/\/|[w]{3})?[\w-]+(\.[\w-]+)+\.?(:\d+)?(\/\S*)?)(\s|\r|\n|\u00a0)/g,xurlendpattern:/(^|\s|\r|\n)((https?:\/\/|[w]{3})?[\w-]+(\.[\w-]+)+\.?(:\d+)?(\/\S*)?)/g,initialize:function(a){a&&$.extend(this,a);this.listenTo(Cloudwalkers.Session.UrlShortener,"sync",this.shortenurl);
this.listenTo(this.parent,"update:stream",function(a){this.togglecontent(a,!0)}.bind(this));this.listenTo(this.parent,"update:campaign",this.campaignupdated);this.on("change:charlength",this.greyout);navigator.userAgent.match(/(firefox(?=\/))\/?\s*(\d+)/i)&&(this.isfirefox=!0)},render:function(a){void 0!==a&&(this.content=a);a=Math.min.apply(Math,_.values(this.restrictedstreams));this.restrictedstreams["default"]=a;this.$el.html(Mustache.render(Templates.editor,{limit:a}));this.limit=a;this.$contenteditable=
this.$el.find("#compose-content").eq(0);this.contenteditable=this.$contenteditable.get(0);this.document=this.contenteditable.ownerDocument||this.contenteditable.document;this.win=document.defaultView||document.parentWindow;this.$contenteditable.html(this.content);this.content&&this.listentochange();return this},listentopaste:function(a){a.preventDefault();if(a.originalEvent||a.originalEvent.clipboardData)a=a.originalEvent.clipboardData.getData("text/plain"),document.execCommand("insertHTML",!1,a);
this.listentochange()},listentochange:function(a,b){var c;if(this.content!==this.$contenteditable.text()||b)this.pos=this.cursorpos(),(c=this.listentourl(this.$contenteditable.text()))&&this.processurls(c),this.charlength!=this.$contenteditable.text().length&&this.trigger("change:charlength",this.charlength=this.$contenteditable.text().length);this.content=this.$contenteditable.text();this.isdefault&&!b&&(this.$contenteditable.removeClass("withdefault"),this.isdefault=!1);this.trigger("change:content",
this.content);b&&this.clearselections()},endchange:function(a){if(this.$contenteditable.html().match(this.xurlendpattern)){var b;(b=this.listentourl(this.$contenteditable.text(),!0))&&this.processurls(b)}},listentourl:function(a,b){var c=this.win.getSelection(),e=this.document.createRange(),f,g,h,l;e.selectNodeContents(this.$contenteditable.get(0));f=this.parsenodes(e.startContainer.childNodes,b);if(f.length)return $.each(f,function(a,b){g=b.node.textContent.replace(/&nbsp;/gi,"");b.url=b.url.trim();
h=g.indexOf(b.url);l=h+b.url.length;0>h&&(h=0);e.setStart(b.node,h);e.setEnd(b.node,l);c.removeAllRanges();c.addRange(e);this.contenteditable.designMode="on";document.execCommand("createLink",!1,b.url);this.contenteditable.designMode="off";var m=document.createTextNode("\u200b");e.insertNode(m);this.$contenteditable.find("a").after(m);e.setStartAfter(m);c.removeAllRanges();c.addRange(e);f[a+1]&&(f[a+1].node=b.node.nextSibling)}.bind(this)),_.pluck(f,"url")},parsenodes:function(a,b){var c=[],e;$.each(a,
function(a,g){var h=!1,l=g.textContent;l&&(h=l.match(b?this.xurlendpattern:this.xurlpattern));if(h&&h.length)return $.each(h,function(a,b){3==g.nodeType?c.push({node:g,url:b}):(e=this.getnode(g,null,b))&&b&&c.push({node:e,url:b})}.bind(this)),!0}.bind(this));return c},getnode:function(a,b,c){var e;$.each(a.childNodes,function(a,g){if(c){if(3==g.nodeType)if(g.childNodes.length)e=this.getnode(g,null,type);else if(-1<g.textContent.indexOf(c.trim()))return e=g,!1}else{var h=g.textContent.length;return h>=
b?(g.childNodes.length?this.getnode(g,b):this.currentnode={node:g,offset:b},!1):b-=h}}.bind(this));return e},processurls:function(a){this.$contenteditable.find("a").attr("contenteditable",!1);if(!this.campaign){var b=this.draft.get("campaign"),c=Cloudwalkers.Session.getAccount().get("campaigns").filter(function(a){if(a.id==b)return a});this.campaign=c.length?c[0].name:null}a&&!this.urlprocessing&&(this.urlprocessing=!0,this.parseurls(a,this.campaign))},parseurls:function(a,b,c){var e={error:this.releaseurlprocessing};
a.forEach(function(a){!c&&this.urls[a]?this.shortenurl(this.urls[a]):(e.q=a,e.campaign=b,Cloudwalkers.Session.UrlShortener.fetch(e))}.bind(this))},shortenurl:function(a){this.releaseurlprocessing();var b=this,c=a.get("url"),e=a.get("shortUrl");this.urls[c]||(this.urls[c]=a.clone());a=" <short contenteditable='false' data-long='"+c+"' data-short='"+e+"'>"+e+"<i class='icon-link' id='swaplink'></i></short>";this.$contenteditable.find("a[href='"+c+"']").replaceWith(a);this.trigger("change:content");
c='<div><a href="'+c+'" class="oembed"></a></div>';this.$el.find("#out").empty().html(c);this.$el.find(".oembed").oembed(null,null,this.embed).each(function(){this.def.done(function(){b.$el.find("#out").addClass("expanded")})})},releaseurlprocessing:function(){this.urlprocessing=!1},swaplink:function(a){var b=$(a.currentTarget).get(0).parentElement,c=this.$contenteditable.text().length;a=$(b).data("long");var e=$(b).data("short"),f=$(b).get(0).textContent.trim(),g=" <short contenteditable='false' data-long='"+
a+"' data-short='"+e+"'>"+(e==f?a:e)+"<i class='icon-link' id='swaplink'></i></short>";$(b).replaceWith(g);b=this.$contenteditable.text().length-c;a=e==f?this.$contenteditable.text().indexOf(a):this.$contenteditable.text().indexOf(e);this.pos>=a?this.cursorpos(this.pos+b):this.cursorpos(this.pos);this.pos=this.cursorpos();this.trigger("change:content")},campaignupdated:function(a){var b=this.urls;b.length&&this.parseurls(b,a,!0)},addoetitle:function(a){},addoecontent:function(a){a=a.currentTarget.parentElement.textContent;
var b=this.$contenteditable.html();this.$contenteditable.html(b+"<br/>"+a);this.trigger("change:content")},addoeimg:function(a){a=this.$el.find('[data-type="image"] img').get(0).src;this.trigger("imageadded",{type:"image",url:a,name:a})},clearselections:function(){window.getSelection?window.getSelection().empty?window.getSelection().empty():window.getSelection():document.selection&&document.selection.empty()},greyout:function(a){a=this.limit-a;this.updatecounter(a);if(!(0<a)){a=this.win.getSelection();
var b=this.document.createRange(),c=this.cursorpos(),e,f;this.content.length>=this.$contenteditable.text().length&&this.removegrey();b.selectNodeContents(this.$contenteditable.get(0));e=b.endContainer.childNodes[b.endContainer.childNodes.length-1];f=this.cursorpos(this.limit,!0);f.node&&(b.setStart(f.node,f.offset),b.setEndAfter(e),a.removeAllRanges(),a.addRange(b));this.contenteditable.designMode="on";document.execCommand("backColor",!1,this.limit?"#fcc":"#fff");this.contenteditable.designMode="off";
this.mergechars();this.cursorpos(c)}},removegrey:function(a){a=this.win.getSelection();var b=this.document.createRange();this.$contenteditable.find("span").length&&(b.selectNodeContents(this.$contenteditable.find("span").get(0)),a.removeAllRanges(),a.addRange(b),this.contenteditable.designMode="on",document.execCommand("removeFormat",!1,null),this.contenteditable.designMode="off")},mergechars:function(){$.each(this.$contenteditable.find("span"),function(a,b){$(b).html($(b).text())}.bind(this))},updatecounter:function(a){var b=
this.$el.find(".limit-counter");20<=a&&b.removeClass().addClass("limit-counter");20>a&&b.removeClass().addClass("limit-counter").addClass("color-notice");0>a&&b.removeClass().addClass("limit-counter").addClass("color-warning");b.empty().html(a)},limitwarning:function(){Cloudwalkers.RootView.alert("You have reached the limit number of characters. Exceeding text will not be shown.");this.hasbeenwarned=!0},savepos:function(a){a=a.target.tagName;"I"!=a&&"SHORT"!=a&&(this.pos=this.cursorpos())},cursorpos:function(a,
b){var c,e;if(void 0===a)a=0,this.win.getSelection()&&this.win.getSelection().rangeCount&&(e=this.win.getSelection().getRangeAt(0),c=e.cloneRange(),c.selectNodeContents(this.contenteditable),c.setEnd(e.endContainer,e.endOffset),a=c.toString().length);else{this.getnode(this.$contenteditable[0],a);var f=this.currentnode;if(b)return{node:f.node,offset:f.offset};e=this.document.createRange();c=this.win.getSelection();e.setStart(f.node,f.offset);e.collapse(!0);c.removeAllRanges();c.addRange(e)}return a},
togglecontent:function(a,b){if(a)var c=_.isNumber(a.id)?a.id:null,e=_.isObject(a.data)?a.data.html:null,f=c?Cloudwalkers.Session.getStream(c).get("network").token:null;this.network=f?f:"default";f&&!e?(this.$contenteditable.empty().html(this.draft.get("body").html),this.$contenteditable.addClass("withdefault"),this.isdefault=!0):(c?(e||(e=""),this.$contenteditable.empty().html(e)):this.$contenteditable.empty().html(this.draft.get("body").html),this.$contenteditable.removeClass("withdefault"));this.limit=
this.restrictedstreams[this.network];this.removegrey(!0);this.listentochange(null,!0)}});Cloudwalkers.Widget=Backbone.View.extend({title:"...",color:"blue",icon:"cloud",events:{"click .portlet-title.line":"collapse"},initialize:function(){this.bind("destroy",this.destroy)},innerRender:function(){this.$el.addClass("inner-empty")},render:function(){this.$el.html(Mustache.render(Templates.widget,{title:this.options.title|this.title,color:this.options.color|this.color,icon:this.options.icon|this.icon}));"auto"==this.options.height&&this.$el.find(".scroller").remove();"undefined"!=typeof this.options.counter&&
...
"full"==a[c].size?container.addClass("span12"):container.addClass("span"+a[c].size),container.append(a[c].widget.render().el),this.currentline.append(container),a[c].widget.negotiateFunctionalities();setTimeout(function(){b.trigger("content:change")},1)},destroy:function(){for(n in this.widgets)this.widgets[n].widget.trigger("destroy")}});Cloudwalkers.Views.Dashboard=Cloudwalkers.Views.Pageview.extend({title:"Dashboard",widgets:[{widget:"messagescounters",type:"inbox",source:"streams",size:4,title:"Inbox",icon:"inbox",open:!0,counter:!0,typelink:"#inbox",countString:"incomingUnread",scrollable:"scrollable"},{widget:"messagescounters",type:"monitoring",source:"channels",size:4,title:"Keywords",icon:"tags",open:!0,counter:!0,countString:"incoming",scrollable:"scrollable"},{widget:"schedulecounter",type:"schedule",source:"outgoing",size:4,
title:"Schedule",icon:"time",open:!0,counter:!0,countString:"scheduled",link:"#scheduled",scrollable:"scrollable"},{widget:"coworkers",type:"drafts",size:4,title:"Co-workers wall",color:"yellow",icon:"edit",open:!0,link:"#coworkers",scrollable:"scrollable"},{widget:"trending",type:"profiles",size:4,title:"Trending Company Posts",color:"grey",icon:"thumbs-up",open:!0,since:7,sublink:"#trending/",scrollable:"scrollable"},{widget:"trending",type:"news",size:4,title:"Trending Accounts we follow",color:"red",
icon:"thumbs-up",open:!0,since:1,sublink:"#trending/",scrollable:"scrollable"}],initialize:function(){Cloudwalkers.Session.ping()},addDynamicReports:function(){var a=Cloudwalkers.Session.getStreams().where({statistics:1}),b=[];for(n in a)switch(a[n].get("network").token){case "facebook":b.push({widget:"report",size:3,stream:a[n],type:"numbercomparison/likes",dashboard:!0});b.push({widget:"report",size:3,stream:a[n],type:"besttimetoposttext",dashboard:!0});break;case "twitter":b.push({widget:"report",
size:3,stream:a[n],type:"numbercomparison/followers_count",dashboard:!0}),b.push({widget:"report",size:3,stream:a[n],type:"besttimetoposttext",dashboard:!0})}return b},render:function(){this.$el.html(Mustache.render(Templates.pageview,{title:this.title}));this.$container=this.$el.find("#widgetcontainer").eq(0);var a=this.widgets.concat(this.addDynamicReports());for(i in a){switch(a[i].widget){case "messagescounters":var b=this.addMessagesCounters(a[i]);break;case "schedulecounter":b=this.addScheduleCounters(a[i]);
break;case "coworkers":b=this.addDashboardDrafts(a[i]);break;case "trending":b=this.addDashboardTrending(a[i]);break;case "report":b=new Cloudwalkers.Views.Widgets.Report(a[i])}this.appendWidget(b,Number(a[i].size))}return this},addMessagesCounters:function(a){var b=Cloudwalkers.Session.getChannel(a.type);$.extend(a,{name:b.get("name"),open:1,channel:b});return new Cloudwalkers.Views.Widgets.MessagesCounters(a)},addScheduleCounters:function(a){var b=Cloudwalkers.Session.getChannel("internal");b.outgoing=
new Cloudwalkers.Collections.Streams(b.get("additional").outgoing);$.extend(a,{name:b.get("name"),open:1,channel:b});return new Cloudwalkers.Views.Widgets.MessagesCounters(a)},addDashboardDrafts:function(a){a.model=Cloudwalkers.Session.getStream("coworkers");a.link="#coworkers";return new Cloudwalkers.Views.Widgets.DashboardMessageList(a)},addDashboardTrending:function(a){a.trending=!0;a.model=Cloudwalkers.Session.getChannel(a.type);a.filters={sort:"engagement",since:3600*Math.round(Date.now()/36E5)-
86400*a.since};return new Cloudwalkers.Views.Widgets.DashboardMessageList(a)}});Cloudwalkers.Views.MessageBoard=Cloudwalkers.Views.Widgets.WidgetContainer.extend({title:"Message Board",widgets:{}});Cloudwalkers.Views.Inbox=Cloudwalkers.Views.Pageview.extend({title:"Inbox",className:"container-fluid inbox",initialize:function(){var a=Cloudwalkers.Session.viewsettings(this.options.type);a.streams&&(this.options.filters={contacts:{string:"",list:[]},streams:a.streams})},render:function(){this.$el.html(Mustache.render(Templates.pageview,{title:this.options.type}));this.$container=this.$el.find("#widgetcontainer").eq(0);this.options.channel.childtype=this.options.type.slice(0,-1);var a="messages"==
this.options.type?new Cloudwalkers.Views.Widgets.InboxMessageList(this.options):new Cloudwalkers.Views.Widgets.InboxNotificationList(this.options);this.appendWidget(a,4);this.appendhtml(Templates.inboxcontainer);this.listenTo(Cloudwalkers.RootView,"resize",this.resize);return this},resize:function(a){this.$el.find("#widgetcontainer").height(a-140)},finish:function(){this.resize(Cloudwalkers.RootView.height());$message=this.$el.find(".inbox-container").wrap("<div class='scroller'>");$message.parent().slimScroll({height:"inherit"})}});Cloudwalkers.Views.Demo=Cloudwalkers.Views.Pageview.extend({title:"Demo",initialize:function(a){a&&$.extend(this,a)},render:function(){this.$el.html(Mustache.render(Templates.pageview,{title:this.title}));this.$container=this.$el.find("#widgetcontainer").eq(0);this.editor=new Cloudwalkers.Views.Editor({content:null,parent:this});this.appendWidget(this.editor,4);return this}});Cloudwalkers.Views.Drafts=Cloudwalkers.Views.Pageview.extend({title:"Drafts",className:"container-fluid drafts",initialize:function(){this.model=Cloudwalkers.Session.getStream("draft");if(!this.model)return Cloudwalkers.Session.home();this.listenTo(this.model,"sync",this.render)},render:function(){this.$el.html(Mustache.render(Templates.pageview,{title:this.title}));this.$container=this.$el.find("#widgetcontainer").eq(0);var a=new Cloudwalkers.Views.Widgets.DraftsFilters({model:this.model});this.appendWidget(a,
4);var b=new Cloudwalkers.Views.Widgets.DraftsList({model:this.model});this.appendWidget(b,8);a.list=b;return this}});Cloudwalkers.Views.Scheduled=Cloudwalkers.Views.Pageview.extend({title:"Schedule",className:"container-fluid scheduled",initialize:function(){this.model=Cloudwalkers.Session.getStream("scheduled");if(!this.model)return Cloudwalkers.Session.home();this.listenTo(this.model,"sync",this.render)},render:function(){this.$el.html(Mustache.render(Templates.pageview,{title:this.title}));this.$container=this.$el.find("#widgetcontainer").eq(0);var a=new Cloudwalkers.Views.Widgets.ScheduledFilters({model:this.model});
this.appendWidget(a,4);var b=new Cloudwalkers.Views.Widgets.ScheduledList({model:this.model});this.appendWidget(b,8);a.list=b;return this}});Cloudwalkers.Views.Calendar=Cloudwalkers.Views.Pageview.extend({title:"Calendar",className:"container-fluid calendar",viewtype:"month",views:[],parameters:{records:999},events:{remove:"destroy","change select":"toggleView","click #subtract":"prev","click #add":"next","click #now":"now"},initialize:function(){this.model=Cloudwalkers.Session.getChannel("profiles");if(!this.model)return Cloudwalkers.Session.home();this.listenTo(this.model,"sync",this.render)},render:function(){this.$el.html(Mustache.render(Templates.calendar,
{monthActive:!0}));this.$container=this.$el.find("#widgetcontainer").eq(0);var a=new Cloudwalkers.Views.Widgets.CalendarFilters({model:this.model,calview:this});this.appendWidget(a,3);a=new Cloudwalkers.Views.Widgets.CalSummary({calview:this});this.views.push(a);this.appendWidget(a,9);this.$el.find("select").chosen({width:"200px",disable_search_threshold:10,inherit_select_classes:!0});setTimeout(this.initCalendar.bind(this),10);return this},populate:function(a,b,c,e){a=a.startOf("month").add(1,"month");
b=b.endOf("month").subtract(1,"month");this.datedisplay(a,b);this.listenToOnce(this.model.messages,"ready",this.fill.bind(this,e));this.listenToOnce(this.model.messages,"cached",this.fill.bind(this,e));$.extend(this.parameters,{since:a.unix(),until:b.unix()});this.model.messages.touch(this.model,this.parameters)},datedisplay:function(a,b){var c=moment();a.isAfter(c)||b.isBefore(c)?this.$el.find("#now").removeClass("hidden"):this.$el.find("#now").addClass("hidden");var e="agendaWeek"==this.viewtype?
"week":"month",c=(c=a.diff(c,e+"s"))?-2<c&&0>c?"last ":2>c&&0<c?"next ":"":"this ",f=24<a.date()?11==a.month()?"DD MMM YYYY":"DD MMM":"DD",f=a.format(f)+" - "+b.format("DD MMM YYYY");this.$el.find("h3.stats-header-timeview").html("<strong>"+c+e+": </strong>"+f)},fill:function(a,b){var c=[];for(n in b.models)c.push(b.models[n].filterCalReadable().calNode);a(c)},updatenode:function(a,b){$("#calendar").fullCalendar("updateEvent",a.filterCalReadable().calNode)},filldrafts:function(a){$("#drafts-list").html("");
for(n in a)this.addDraft("Draft message ("+a[n].id+")")},toggleView:function(a){this.viewtype=$(a.currentTarget).val();"list"==this.viewtype?this.initList():$("#calendar").removeClass("hidden");$("#calendar").fullCalendar("changeView",this.viewtype)},prev:function(){$("#calendar").fullCalendar("prev")},next:function(){$("#calendar").fullCalendar("next")},now:function(){$("#calendar").fullCalendar("today")},initCalendar:function(){$("#calendar").fullCalendar({header:!1,slotMinutes:30,editable:!1,droppable:!1,
allDayDefault:!1,allDaySlot:!1,firstDay:1,axisFormat:"H:mm",defaultTimedEventDuration:"00:30:00",slotEventOverlap:!1,events:this.populate.bind(this),eventRender:function(a,b){if(!a.icon)return!1;$(b).find(".fc-event-time").html("<i class='icon-"+a.icon+"'></i>");a.media&&$(b).find(".fc-event-inner").prepend("<i class='fc-event-media pull-right icon-"+a.media+"'></i>");$(b).data("title",a.intro).tooltip({html:!0,delay:500,placement:"top"});a.rendered=!0}})},addDraft:function(a){a=0==a.length?"Untitled Event":
a;a=$('<div class="external-event label">'+a+"</div>");$("#drafts-list").append(a);this.initDrag(a)},initDrag:function(a){var b={title:$.trim(a.text())};a.data("eventObject",b);a.draggable({zIndex:999,revert:!0,revertDuration:0})}});Cloudwalkers.Views.Coworkers=Cloudwalkers.Views.Pageview.extend({title:"Co-workers wall",className:"container-fluid coworkers",initialize:function(){this.model=this.model=Cloudwalkers.Session.getStream("coworkers");if(!this.model)return Cloudwalkers.Session.home();this.listenTo(this.model,"outdated",this.model.fetch);this.listenTo(this.model,"sync",this.render)},render:function(){this.$el.html(Mustache.render(Templates.pageview,{title:this.title}));this.$container=this.$el.find("#widgetcontainer").eq(0);
var a=new Cloudwalkers.Views.Widgets.CoworkersFilters({model:this.model});this.appendWidget(a,4);var b=new Cloudwalkers.Views.Widgets.CoworkersList({model:this.model});this.appendWidget(b,8);a.list=b;return this}});Cloudwalkers.Views.Timeline=Cloudwalkers.Views.Pageview.extend({id:"timeline",parameters:{records:20,markasread:!0},entries:[],events:{"click *[data-action]":"action","click .load-more":"more"},initialize:function(a){a&&$.extend(this,a);this.collection=this.model.messages;this.$el.addClass("loading");this.listenTo(this.collection,"seed",this.fill)},hideloading:function(){this.$el.removeClass("loading")},render:function(){this.$el.html(Mustache.render(Templates.timeline,{}));this.$container=this.$el.find("ul.timeline").eq(0);
...
e.endrepeat={days:[],months:[],years:[]};for(b=1;31>=b;b++)e.days.push({day:b,checked:f?f.getDate()==b:!1}),e.endrepeat.days.push({day:b,checked:h&&h.end&&h.end.getDate()==b});for(b=1;12>=b;b++)e.months.push({month:b,display:Cloudwalkers.Utils.month(b),checked:f?f.getMonth()==b-1:!1}),e.endrepeat.months.push({month:b,display:Cloudwalkers.Utils.month(b),checked:h&&h.end&&h.end.getMonth()==b-1});for(var l=null,b=0;10>b;b++)l=(new Date).getFullYear()+b,e.years.push({year:l,checked:f?f.getFullYear()==
l:!1}),e.endrepeat.years.push({year:l,checked:h&&h.end&&h.end.getFullYear()==l});e.weekdays=[];l="monday tuesday wednesday thursday friday saturday sunday".split(" ");for(b=0;b<l.length;b++){var k=l[b],k=k.charAt(0).toUpperCase()+k.substr(1);e.weekdays.push({name:k,weekday:l[b],checked:h&&h.weekdays&&h.weekdays[l[b].toUpperCase()]})}e.repeatinterval={amount:[],unit:[]};for(b=0;72>b;b++)e.repeatinterval.amount.push({value:b,name:b,selected:h&&h.interval==b});e.repeatinterval.unit.push({value:"minutes",
name:"Minute(s)",selected:h&&"minutes"==h.unit});e.repeatinterval.unit.push({value:"hours",name:"Hour(s)",selected:h&&"hours"==h.unit});e.repeatinterval.unit.push({value:"days",name:"Day(s)",selected:h&&"days"==h.unit});e.repeatinterval.unit.push({value:"weeks",name:"Week(s)",selected:h&&"weeks"==h.unit});e.repeatinterval.unit.push({value:"months",name:"Month(s)",selected:h&&"months"==h.unit});for(b=0;24>b;b+=0.25)h=Math.floor(b),l=60*(b-Math.floor(b)),10>h&&(h="0"+h),10>l&&(l="0"+l),f&&f.setMinutes(15*
Math.floor(f.getMinutes()/15)),e.times.push({time:h+":"+l,checked:f&&f.getHours()==h&&f.getMinutes()==l});e.canSchedule=!0;e.canSave=!0;e.canSendNow=!0;if(this.model){e.canSave="SCHEDULED"!=this.model.get("status");e.message=jQuery.extend(!0,{},this.model.attributes);"undefined"!=typeof this.actionparameters.message&&""!=this.actionparameters.message.value&&(e.message.body.plaintext=Cloudwalkers.Utilities.Parser.parseFromMessage(this.actionparameters.message.value,this.model),e.message.body.html=
...
l=c.pluck("messages",b);e.data.push([h,l])}.bind(this));e.data.unshift(["Day","Number of messages"]);return e},parseactivities:function(a,b,c){var e={},f={data:[],colors:[]};a=b?b:a.latest().get("streams");$.each(a,function(a,b){b=new Cloudwalkers.Models.Stream(b);var f=new Cloudwalkers.Models.Network(Cloudwalkers.Session.getStream(b.id).get("network")),k=b.getactivities();if(c&&f.gettoken()!=c)return!0;_.isNumber(k)&&_.has(e,f.gettoken())?e[f.gettoken()].activities+=k:(e[f.gettoken()]=f,e[f.gettoken()].activities=
k)});e=_.sortBy(e,function(a){return a.get("activities")});$.each(e,function(a,b){f.data.push([b.gettitle(),b.activities]);f.colors.push(b.getcolor())});c||b||f.data.unshift(["Network","Number of contacts"]);return f},parseimpressions:function(a,b,c){var e={},f={data:[],colors:[]};a=b?b:a.latest().get("streams");$.each(a,function(a,b){b=new Cloudwalkers.Models.Stream(b);var f=new Cloudwalkers.Models.Network(Cloudwalkers.Session.getStream(b.id).get("network")),k=b.getimpressions();if(c&&f.gettoken()!=
c)return!0;_.isNumber(k)&&_.has(e,f.gettoken())?e[f.gettoken()].impressions+=k:(e[f.gettoken()]=f,e[f.gettoken()].impressions=k)});e=_.sortBy(e,function(a){return a.get("impressions")});$.each(e,function(a,b){f.data.push([b.gettitle(),b.impressions]);f.colors.push(b.getcolor())});c||b||f.data.unshift(["Network","Number of contacts"]);return f},parsenotifications:function(a,b,c){var e={},f={data:[],colors:[]};a=b?b:a.latest().get("streams");$.each(a,function(a,b){b=new Cloudwalkers.Models.Stream(b);
var f=new Cloudwalkers.Models.Network(Cloudwalkers.Session.getStream(b.id).get("network")),k=b.getnotifications();if(c&&f.gettoken()!=c)return!0;_.isNumber(k)&&_.has(e,f.gettoken())?e[f.gettoken()].notifications+=k:(e[f.gettoken()]=f,e[f.gettoken()].notifications=k)});e=_.sortBy(e,function(a){return a.get("notifications")});$.each(e,function(a,b){f.data.push([b.gettitle(),b.notifications]);f.colors.push(b.getcolor())});c||b||f.data.unshift(["Network","Number of contacts"]);return f},parsefollowers:function(a,
b,c){var e={},f={data:[],colors:[]};a=b?b:a.latest().get("streams");$.each(a,function(a,b){b=new Cloudwalkers.Models.Stream(b);var f=new Cloudwalkers.Models.Network(Cloudwalkers.Session.getStream(b.id).get("network")),k=b.getfollowers();if(c&&f.gettoken()!=c)return!0;_.isNumber(k)&&_.has(e,f.gettoken())?e[f.gettoken()].followers+=k:(e[f.gettoken()]=f,e[f.gettoken()].followers=k)});e=_.sortBy(e,function(a){return a.get("followers")});$.each(e,function(a,b){f.data.push([b.gettitle(),b.followers]);f.colors.push(b.getcolor())});
c||b||f.data.unshift(["Network","Number of contacts"]);return f},parsefollowing:function(a,b,c){var e={},f={data:[],colors:[]};a=b?b:a.latest().get("streams");$.each(a,function(a,b){b=new Cloudwalkers.Models.Stream(b);var f=new Cloudwalkers.Models.Network(Cloudwalkers.Session.getStream(b.id).get("network")),k=b.getfollowing();if(c&&f.gettoken()!=c)return!0;_.isNumber(k)&&_.has(e,f.gettoken())?e[f.gettoken()].following+=k:(e[f.gettoken()]=f,e[f.gettoken()].following=k)});e=_.sortBy(e,function(a){return a.get("following")});
...
a.slice(1)},filtercountry:function(a,b){var c={},e=a.latest().get("streams");$.each(e,function(a,e){if(b&&e.id==b||!b){var h=Cloudwalkers.Session.getStream(e.id).get("network").name,l=Cloudwalkers.Session.getStream(e.id).get("network").token;if(_.isObject(e.contacts.geo)){var k=e.contacts.geo.countries;0==_.size(c)?$.each(k,function(a,b){c[b.name]=b;c[b.name].networks=[];c[b.name].networks[h]={total:b.total,token:l}}):$.each(k,function(a,b){c[b.name]?(c[b.name].total+=b.total,c[b.name].networks[h]?
c[b.name].networks[h].total+=b.total:c[b.name].networks[h]={total:b.total,token:l}):(c[b.name]=b,c[b.name].networks=[],c[b.name].networks[h]={total:b.total,token:l})})}}});return c=_(c).sortBy(function(a){return a.total})},parseregional:function(a,b){var c=8,e=0,f=this.filtercountry(a,b||this.network),g={data:[],options:{colors:this.colors}};this.regional=[];if(!c||c>f.length)c=f.length;if(0==c)return this.emptychartdata();for(;e<c;){var h=f.pop();g.data.push([h.name,h.total]);this.regional.push(h);
...
e.options.colors.push(this.countrycolors[c]);c++}if(0==a.length)return e;c=_.reduce(a,function(a,b){return a+b.value},0);e.data.push(["Others",c]);e.options.colors.push(this.collection.networkcolors.others);return e},parsebesttime:function(a){var b="Mon Tue Wed Thu Fri Sat Sun".split(" "),c,e=[],f=0,g=0,h;a.forEach(function(a){var k=[];a.get("streams").forEach(function(a){a=new Cloudwalkers.Models.Stream(a);if(c=a.getbesttime())if(0==k.length)k=_.values(c);else for(i in c)k[i]+=c[i],k[i]>f&&(f=k[i]),
k[i]>g&&(g=k[i])});h=k.indexOf(Math.max.apply(Math,_.values(k)));e.push({day:b.shift(),value:g,time:h});g=0});e.maxvalue=f;return e},parseevolution:function(a,b){for(var c=b?this.columns[b]:this.columns[this.type],e=this.collection.length,f=this.network?this.network:null,g,h=["Day of the week"],l={},k=this.$el.find(".chart-container").get(0).clientWidth,k={data:[],options:{chartArea:{width:"70%",height:"70%",left:"40"},width:k,height:0.3*k,legend:{textStyle:{fontSize:"11"}},tooltip:{textStyle:{fontSize:"13"}},
curveType:"function",legend:{position:"bottom"},colors:[]}},p=0;p<e;p++)g=a.place(p).get("streams"),dailyresult=this[c](null,g,f),_.map(dailyresult.data,function(a){l[a[0]]||(l[a[0]]=[]);l[a[0]][p]=a[1]});for(p=0;p<e;p++){c=[p];for(d in l)c.push(l[d].shift());k.data.push(c)}for(d in l)h.push(d),k.options.colors.push(this.networktokens[d]);k.data.unshift(h);return k},parseallreports:function(a){var b=this.parseevolution(a,"contacts").data,c=this.parseevolution(a,"messages").data,e=this.parseevolution(a,
...
"destroy:view",this.remove);this.addScroll()},addScroll:function(){this.$el.find(".scroller").slimScroll({size:"6px",color:"#a1b2bd",height:$("#inner-content").height()-165+"px",alwaysVisible:!1,railVisible:!1})},destroy:function(){$.each(this.entries,function(a,b){b.remove()})}});Cloudwalkers.Views.Widgets.MonitorFilters=Cloudwalkers.Views.Widgets.Widget.extend({events:{"click [data-network-streams]":"filternetworks","click [data-keyword-id]":"filterkeywords","click .toggleall.networks.active":"toggleallnetworks","click .toggleall.keywords.active":"toggleallkeywords"},initialize:function(){this.category=this.options.category;this.category.childtype="message";this.streams=[];this.category.channels.each(function(a){this.streams=this.streams.concat(a.streams.models)},this);this.initializeWidget()},
...
var c=b?null:String(this.button.data("network-streams")).split(" ");b&&(this.button=!1);c?this.list.parameters.streams=c.join(","):delete this.list.parameters.streams;this.category.messages.touch(this.category,this.list.parameters);return this},filterkeywords:function(a,b){b||(b=this.button&&this.button.data("keyword-id")==$(a.currentTarget).data("keyword-id"));this.togglefilters(b,".keyword-list");b||(this.button=$(a.currentTarget).addClass("active").removeClass("inactive"));var c=b?null:Number(this.button.data("keyword-id"));
b&&(this.button=!1);c?this.list.parameters.channels=c:delete this.list.parameters.channels;this.category.messages.touch(this.category,this.list.parameters);return this}});Cloudwalkers.Views.Widgets.MonitorList=Cloudwalkers.Views.Widgets.Widget.extend({id:"monitorparent",parameters:{records:40,markasread:!0},entries:[],events:{remove:"destroy","click .load-more":"more"},initialize:function(){this.category=this.options.category;this.category.set({messages:[]});this.loadmylisteners();this.updateable(this.category,"h3.page-title")},render:function(){this.$el.html(Mustache.render(Templates.monitorlist,{name:this.category.get("name")}));this.$container=this.$el.find(".messages-container");
...
(this.model.contacts.processing?this.filters.contacts.buffered=a:(this.$el.find(".loading-contacts").removeClass("hidden"),this.filters.contacts.string=a,this.model.contacts.fetch({remove:!1,parameters:{q:a,channels:this.model.id},success:this.loadedcontacts.bind(this)})))},loadedcontacts:function(){this.$el.find(".loading-contacts").addClass("hidden");this.filters.contacts.buffered&&(this.requestcontacts(this.filters.contacts.buffered),this.filters.contacts.buffered=!1)},filtercontacts:function(a){a=
$(a.currentTarget);if(a.data("contact"))a=Cloudwalkers.Session.getContact(a.data("contact")),0>this.filters.contacts.list.indexOf(a.id)&&(this.filters.contacts.list.push(a.id),this.$el.find("ul.contacts-placeholder").append(Mustache.render(Templates.contactselectedentry,a.attributes)));else{var b=a.data("close-contact");this.filters.contacts.list=this.filters.contacts.list.filter(function(a){return a!=b});this.$el.find("[data-close-contact="+b+"]").parent().remove()}this.collection.touch(this.model,
this.filterparameters());return this},filternetworks:function(a,b){this.loadmylisteners(!0);b||(b=this.button&&this.button.data("networks")==$(a.currentTarget).data("networks"));this.togglestreams(b);b||(this.button=$(a.currentTarget).addClass("active").removeClass("inactive"));var c=b?null:String(this.button.data("networks")).split(" ");b&&(this.button=!1);c?($(c.map(function(a){return'[data-streams="'+a+'"]'}).join(",")).removeClass("inactive").addClass("active"),this.filters.streams=c):this.filters.streams=
[];this.collection.touch(this.model,this.filterparameters());return this},filterstreams:function(a,b){this.loadmylisteners(!0);b||(b=this.button&&this.button.data("streams")==$(a.currentTarget).data("streams"));this.togglestreams(b);b||(this.button=$(a.currentTarget).addClass("active").removeClass("inactive"));var c=b?null:Number(this.button.data("streams"));b&&(this.button=!1);c?($('[data-networks~="'+c+'"]').removeClass("inactive").addClass("active"),this.filters.streams=[c]):this.filters.streams=
[];this.collection.touch(this.model,this.filterparameters());return this},filterparameters:function(){var a={records:20,group:1};this.filters.contacts.list.length&&(a.contacts=this.filters.contacts.list.join(","));this.filters.streams.length&&(a.streams=this.filters.streams.join(","));this.storeview();return a},storeview:function(){var a=Cloudwalkers.Session.viewsettings(this.collectionstring);a.streams||(a.streams=[]);JSON.stringify(a.streams)!=JSON.stringify(this.filters.streams)&&(a.streams=this.filters.streams,
Cloudwalkers.Session.viewsettings(this.collectionstring,a))},more:function(){this.incremental=!0;this.collection.more(this.model,this.filterparameters())||this.$el.find(".load-more").hide()},negotiateFunctionalities:function(){this.listenTo(Cloudwalkers.Session,"destroy:view",this.remove);this.addScroll()},addScroll:function(){this.$el.find(".scroller").slimScroll({height:"inherit"})},destroy:function(){$.each(this.entries,function(a,b){b.remove()})},isempty:function(){$(".inbox-container").empty().addClass("empty-content")},
unsetempty:function(){$(".inbox-container").removeClass("empty-content")}});Cloudwalkers.Views.Widgets.InboxNotificationList=Cloudwalkers.Views.Widgets.InboxMessageList.extend({collectionstring:"notifications",check:"hasNotifications",toggle:function(a){this.inboxmessage&&this.inboxmessage.remove();a.model.get("parent")?this.showmessage(a.model):this.listenToOnce(a.model,"change",this.showmessage);this.$el.find(".list .active").removeClass("active");a.$el.addClass("active").removeClass("unread")},showmessage:function(a){var b=Cloudwalkers.Session.getMessage(a.get("parent").id);
...
this.network)}},parsesentnetwork:function(){var a=this.collection.latest(),b=this.collection.first();return{counter:a.pluck("messages",this.network)-b.pluck("messages",this.network)}},parsebesttime:function(){var a=this.collection.parsebesttime();if(0==a.length)return null;for(var b={},c=a[0].time,e=1,f=0;f<a.length;f++){var g=a[f].time;0>g||(null==b[g]?b[g]=1:b[g]++,b[g]>e?(c=g,e=b[g]):b[g]==e&&(c=g,e=b[g]))}return{counter:c+"h"}}});Cloudwalkers.Views.Widgets.CalSummary=Cloudwalkers.Views.Widgets.Widget.extend({className:"stats-summary cal-summary",columns:[{title:"Total messages",func:"parsetotal"},{title:"Average",func:"parseavg"},{title:"Media",func:"parsemedia"}],initialize:function(a){a&&$.extend(this,a);this.listenTo(this.calview.model.messages,"ready",this.fill)},render:function(){this.$el.html(Mustache.render(Templates.statsummary,{columns:this.columns}));return this},fill:function(a){this.$el.find("[data-type]").each(function(b,
c){var e=this[$(c).data("type")](a);$(c).find(".stats-summary-counter").html(e.counter);e.title&&$(c).find(".stats-summary-title").html(e.title)}.bind(this))},parsetotal:function(a){return{counter:a.length}},parseavg:function(a){if(!a.length)return{counter:0};var b="agendaWeek"==this.calview.viewtype?{title:"Daily average",divide:7}:{title:"Weekly average",divide:4};a=Math.round(a.length/b.divide);b.counter=a+" <small>message"+(1!=a?"s":"")+"</small>";return b},parsemedia:function(a){if(!a.length)return{counter:0};
var b=[],c=a.length,e={image:0,video:0,link:0,text:0};a.each(function(a){if(a=a.get("attachments"))for(n in a)e[a[n].type]++;else e.text++});e.image&&b.push("<div class='media-entry'><i class='icon-picture'></i> <strong>"+Math.round(100*(e.image/c))+"</strong>% images</div>");e.video&&b.push("<div class='media-entry'><i class='icon-facetime-video'></i> <strong>"+Math.round(100*(e.video/c))+"</strong>% video</div>");e.link&&b.push("<div class='media-entry'><i class='icon-link'></i> <strong>"+Math.round(100*
(e.link/c))+"</strong>% links</div>");e.text&&b.push("<div class='media-entry'><i class='icon-reorder'></i> <strong>"+Math.round(100*(e.text/c))+"</strong>% text-only</div>");return{counter:b.join("<br>")}},parsetrending:function(a){a=a.max(function(a){return a.get("engagement")});var b={intro:a.get("body").plaintext.substr(0,140)};140==b.intro.length&&(b.intro+="...");$.extend(b,a.attributes);return{counter:Mustache.render(Templates.caltrendingentry,b)}}});Cloudwalkers.Views.Widgets.CalendarFilters=Cloudwalkers.Views.Widgets.Widget.extend({id:"calendarfilters",filters:{},events:{remove:"destroy","click [data-toggle]":"togglefilter","click [data-networks]":"filternetworks","click [data-streams]":"filterstreams","click .toggleall.active":"toggleall"},initialize:function(a){a&&$.extend(this,a);this.collection=this.model.messages;this.streams=Cloudwalkers.Session.getChannel("internal").get("additional").outgoing},render:function(){var a={streams:[]};for(n in this.streams)a.streams.push({id:this.streams[n].id,
...
$("#filter_"+b).removeClass("hidden"))},togglefilters:function(a){this.$el.find("li").addClass(a?"active":"inactive").removeClass(a?"inactive":"active");this.$el.find(".toggleall").addClass(a?"inactive":"active").removeClass(a?"active":"inactive")},togglestreams:function(a){this.$el.find("[data-networks], [data-streams]").addClass(a?"active":"inactive").removeClass(a?"inactive":"active");this.$el.find(".toggleall").addClass(a?"inactive":"active").removeClass(a?"active":"inactive")},filternetworks:function(a,
b){b||(b=this.button&&this.button.data("networks")==$(a.currentTarget).data("networks"));this.togglestreams(b);b||(this.button=$(a.currentTarget).addClass("active").removeClass("inactive"));var c=b?null:String(this.button.data("networks")).split(" ");b&&(this.button=!1);c?($(c.map(function(a){return'[data-streams="'+a+'"]'}).join(",")).removeClass("inactive").addClass("active"),this.filters.streams=c):this.filters.streams=[];this.filterparameters();return this},filterstreams:function(a,b){b||(b=this.button&&
this.button.data("streams")==$(a.currentTarget).data("streams"));this.togglestreams(b);b||(this.button=$(a.currentTarget).addClass("active").removeClass("inactive"));var c=b?null:Number(this.button.data("streams"));b&&(this.button=!1);c?($('[data-networks~="'+c+'"]').removeClass("inactive").addClass("active"),this.filters.streams=[c]):this.filters.streams=[];this.filterparameters();return this},filterparameters:function(){this.filters.streams.length?this.calview.parameters.streams=this.filters.streams.join(","):
