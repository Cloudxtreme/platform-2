...
"</em>":"")+"</span>");return this},updateschedule:function(){var a=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1;(a=a?a:this.draft.get("schedule"))&&a.date&&($(this.datepicker.get(0)).datepicker("update",moment.unix(a.date).format("DD/MM/YYYY")),this.timepicker.timepicker("setTime",moment.unix(a.date).format("HH:mm")));if(a&&a.repeat){a.repeat.until&&$(this.datepicker.get(1)).datepicker("update",moment.unix(a.repeat.until).format("DD/MM/YYYY"));var b=a.repeat.settings||
{};b.interval&&$("#repeat-interval").val(b.interval);b.every&&$("#every-select").val(b.every);b.everyweek&&$("#every-select-weekday").val(b.everyweek);a.repeat.amount&&$("#repeat-amount").val(a.repeat.amount)}},preview:function(){this.$el.addClass("switch-mode");this.preview=new Cloudwalkers.Views.Preview({model:this.draft,previewtype:"default",streamid:this.activestream.id});this.$el.find(".switch-container").append(this.preview.render().el)},endpreview:function(){this.preview.remove();this.$el.removeClass("switch-mode")},
