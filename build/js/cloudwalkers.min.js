...
a[n].id+"'>"+a[n].name+"</option>"));b.trigger("chosen:updated")},toggleschedule:function(a){this.$el.find("[data-collapsable=schedule]")[a?"removeClass":"addClass"]("hidden");return this},togglerepeat:function(a){this.$el.find("[data-collapsable=repeat]")[a?"removeClass":"addClass"]("hidden");return this},toggleschedentry:function(a,b){$(a)[b?"removeClass":"addClass"]("inactive").find("label")[b?"removeClass":"addClass"]("icon-circle-blank")[b?"addClass":"removeClass"]("icon-ok-circle");return this},
parsemoment:function(a){a=$(a);if(!a.val())return null;a=moment(a.val(),"DD-MM-YYYY DD-MM-YY DD/MM/YYYY DD/MM/YY DDMMYYYY YYYYMMDD MM-DD-YYYY MM-DD-YY".split(" "));return a.isValid()?a.unix()<moment().unix()?void 0:a:void 0},monitorschedule:function(a,b){var c=b||$(a.currentTarget),e=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,e=e?e:this.draft.get("schedule"),f=c.data("set")?c:c.parents("[data-set]").eq(0),g=f.data("set");"now"==g?(this.toggleschedentry("[data-set=in], [data-set=on]").toggleschedentry("[data-set=now]",
!0),$("[data-set=on] input").val(""),$("[data-set=in] select").val(600),this.draft.set("schedule",{repeat:e.repeat||{}})):"in"==g?(this.toggleschedentry("[data-set=now], [data-set=on]").toggleschedentry("[data-set=in]",!0),$("[data-set=on] input").val("")):"on"==g?(this.toggleschedentry("[data-set=now], [data-set=in]").toggleschedentry("[data-set=on]",!0),$("[data-set=in] select").val(600),void 0===this.parsemoment("#delay-date")&&(this.picker.datepicker("hide").val(""),Cloudwalkers.RootView.alert("Please set your Schedule to a date in the future"))):
"every"==g?(c=c.data("set")?!f.hasClass("inactive"):!1,this.toggleschedentry("[data-set=every]",!c),this.toggleschedentry(c?"[data-set=repeat], [data-set=until]":Number($("#repeat-amount").val())?"[data-set=repeat]":"[data-set=until]",!c),$("#repeat-interval").val(c?0:Number($("#repeat-interval").val())?$("#repeat-interval").val():1),c&&($("#repeat-amount").val(0),$("#repeat-until").val(""))):"repeat"==g?(this.toggleschedentry("[data-set=every], [data-set=repeat]",!0).toggleschedentry("[data-set=until]",
!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),Number($("#repeat-amount").val())||$("#repeat-amount").val(1),$("#repeat-until").val("")):"until"==g&&(this.toggleschedentry("[data-set=every], [data-set=until]",!0).toggleschedentry("[data-set=repeat]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),$("#repeat-amount").val(0));this.parsescheduled();a.stopPropagation();return this},parsescheduled:function(){var a=this.activestream?this.draft.getvariation(this.activestream.id,
"schedule"):!1,a=a?a:this.draft.get("schedule"),b=this.$el.find("section[data-collapsable] .schedule-entry").not(".inactive").find("#delay-select, #delay-date, #delay-time, #repeat-interval, #every-select, #every-select-weekday, #repeat-amount, #repeat-until");if(b.filter("#delay-select").val())a.date=moment().add("seconds",$("#delay-select").val()).unix();else if(b.filter("#delay-date").val()){var c=this.parsemoment("#delay-date");if(b.filter("#delay-date").val())var e=$("#delay-date").val().split(":");
1<e.length&&c.add("minutes",60*Number(e[0])+Number(e[1]));a.date=c.unix()}b.filter("#repeat-interval").val()&&(a.repeat||(a.repeat={interval:604800}),a.repeat.interval=$("#repeat-interval").val()?3600*$("#repeat-interval").val()*$("#every-select").val():0,$("#every-select-weekday").val()&&(a.repeat.weekdays=[$("#every-select-weekday").val()]),$("#every-select-weekday").attr("disabled",604800>a.repeat.interval),a.repeat.amount=Number(b.filter("#repeat-amount").val())?Number($("#repeat-amount").val()):
!1,a.repeat.until=b.filter("#repeat-until").val()?moment($("#repeat-until").val(),["DD-MM-YYYY","DD-MM-YY","DD/MM/YYYY"]).unix():!1);return a},togglebesttime:function(a){$(a.currentTarget).toggleClass("inactive");$(a.currentTarget).hasClass("inactive")?$("#delay-time").attr("disabled",!1):$("#delay-time").attr("disabled",!0).val("");this.draft.get("schedule").besttimetopost=!$(a.currentTarget).hasClass("inactive");return this},summarizeschedule:function(){var a=this.parsescheduled(),b=this.$el.find("[data-collapsable=schedule] .summary").empty();
if(a.date){var c=a.besttimetopost?"Best time to post":moment.unix(a.date).format("HH:mm");b.html("<span><i class='icon-time'></i> "+moment.unix(a.date).format("dddd, D MMMM YYYY")+"<em class='negative'>"+c+"</em></span>")}this.summarizerepeat();return this},summarizerepeat:function(){var a=this.parsescheduled();if(!a.repeat)return this;var b=this.$el.find("[data-collapsable=repeat] .summary").empty(),c=a.date?moment.unix(a.date):moment();if(a.repeat.until)var e=moment.unix(a.repeat.until),c=e.diff(c)/
1E3,f=Math.round(c/a.repeat.interval);else a.repeat.interval&&(f=a.repeat.amount,e=c.clone().add("seconds",f*a.repeat.interval));f&&b.html("<span>"+f+" times "+(e?"<em class='negative'>until "+e.format("dddd, D MMMM YYYY")+"</em>":"")+"</span>");return this},preview:function(){this.$el.addClass("switch-mode");this.preview=new Cloudwalkers.Views.Preview({model:this.draft,previewtype:"default",streamid:this.activestream.id});this.$el.find(".switch-container").append(this.preview.render().el)},endpreview:function(){this.preview.remove();
