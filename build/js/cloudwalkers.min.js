...
"json/"+this.typestring+"/"+this.id+this.endpoint:CONFIG_BASE_URL+"json/"+this.typestring+"/"+this.id:CONFIG_BASE_URL+"json/accounts/"+Cloudwalkers.Session.getAccount().id+"/"+this.typestring},parse:function(a){if("number"==typeof a)return{id:a};a.message&&(a=a.message);a=this.filterData(a);this.stamp(a);this.checkloaded(a);return a},sync:function(a,b,c){this.endpoint=c.endpoint?"/"+c.endpoint:!1;return"update"==a?!1:Backbone.sync(a,b,c)},validateCustom:function(a){if(!this.hascontent())return"You need a bit of content.";
if(!this.checkcontent())return"One or more networks exceed the character limit.";if(!this.get("streams").length)return"Please select a network first."},hascontent:function(){var a;if(this.get("attachments")&&this.get("attachments").length)return this.get("attachments").length;if(this.get("body").plaintext)return this.get("body").plaintext.length;if(this.get("variations")&&this.get("variations").length)return $.each(this.get("variations"),function(b,c){if(c.attachments&&c.attachments.length)return a=
c.attachments.length,!1;if(c.body&&c.body.plaintext)return a=c.body.plaintext.length,!1}.bind(this)),a},checkcontent:function(){var a=1;if(this.get("body").plaintext&&0>140-this.get("body").plaintext.length)return!1;this.get("variations")&&this.get("variations").length&&$.each(this.get("variations"),function(b,c){var e=Cloudwalkers.Session.getStream(c.stream).get("network"),e=e.limitations["max-length"]?e.limitations["max-length"].limit:null;if(!e)return!0;if(c.body&&c.body.plaintext)return a=e-c.body.plaintext.length,
0>a?!1:!0}.bind(this));return 0<a},sanitizepost:function(){this.attributes.body.html&&delete this.attributes.body.html;this.attributes.variations&&this.attributes.variations.forEach(function(a,b){a.body&&(a.body.plaintext?a.body.html&&delete a.body.html:delete a.body);0>this.attributes.streams.indexOf(a.stream)&&this.attributes.variations.splice(b,1)}.bind(this))},cloneSanitized:function(){var a=this.clone();a.attributes.date&&delete a.attributes.date;a.attributes.from&&delete a.attributes.from;a.attributes.stream&&
delete a.attributes.stream;a.attributes.streams=[];a.id&&(delete a.id,delete a.attributes.id);return a},checkloaded:function(a){a.objectType&&setTimeout(function(a){a.trigger("loaded")},1,this)},filterData:function(a){var b={};$.each("id objectType actiontokens subject body date engagement from read stream streams attachments parent statistics canHaveChildren children_count schedule variations".split(" "),function(c,f){void 0!==a[f]&&(b[f]=a[f])});var c=Cloudwalkers.Session.getStream(a.stream);c&&
(b.icon=c.get("network").icon,b.networktoken=c.get("network").token,b.networkdescription=c.get("name"));a.attachments?(a.attachments.length&&(b.media=a.attachments[a.attachments.length-1],b.media="image"==b.media.type?"picture":b.media.type),b.attached={},$.each(a.attachments,function(a,c){b.attached[c.type]=c})):b.media="reorder";a.engagement&&(b.trending=1E3>a.engagement?a.engagement:"+999");b.schedule&&(b.scheduledate=moment(b.schedule.date).format("DD MMM YYYY HH:mm"));a.body&&(b.body.intro=a.body.plaintext?
a.body.plaintext.substr(0,72):"...");a.date&&(b.fulldate=moment(a.date).format("DD MMM YYYY HH:mm"),b.dateonly=moment(a.date).format("DD MMM YYYY"),b.time=moment(a.date).format("HH:mm"));return b},filterActions:function(){return this.get("actiontokens")?this.actions.rendertokens():[]},filterCalReadable:function(){var a=this.get("objectType"),b=a&&"reorder"!=this.get("media");this.calNode||(this.calNode={id:this.id});this.calNode.start=a?$.fullCalendar.moment(this.get("date")):$.fullCalendar.moment();
this.calNode.title=a?(this.get("title")?this.get("title"):this.get("body").plaintext).substring(0,b?12:16):"...";this.calNode.className=a?this.get("networktoken")+"-color":"hidden";this.calNode.networkdescription=a?this.get("networkdescription"):null;this.calNode.intro=a?this.get("body").intro:null;this.calNode.icon=a?this.get("icon"):null;this.calNode.media=b?this.get("media"):null;a&&72<=this.calNode.intro.length&&(this.calNode.intro+="...");return this},attach:function(a,b,c){c=this.get("attachments")||
[];a=b&&c[b]?$.extend(c[b],a):c.push(a);this.set({attachments:c});return a},unattach:function(a){(this.get("attachments")||[]).splice(a,1)},addcampaign:function(){},setvariation:function(a,b,c){var e=this.get("variations")||[],f=e.filter(function(b){if(b.stream==a)return b});if(0==f.length){f={stream:a};if("image"==b||"link"==b){var g=[c];f.attachments=g}else f[b]=c;e.push(f)}else f=f[0],"image"==b||"link"==b?(g=f.attachments||[],0==g.length&&"image"==b?(g.push(c),f.attachments=g):"image"==b?g.push(c):
(b=g.filter(function(a){if("link"==a.type)return a}),0<b.length?b[0].url=c.url:0!=g.length?f.attachments.push(c):f.attachments=[c])):f[b]=c},getvariation:function(a,b){var c=this.get("variations")||[],c=_.findWhere(c,{stream:a});if("image"==b||"link"==b){if(c&&c.attachments)return c.attachments.filter(function(a){if(a.type==b)return a})}else{if(c&&c[b])return c[b];if(c&&!b)return c}},original:function(a,b,c){var e=this.get("variations")?this.get("variations").filter(function(b){return a&&b.stream==
a.id}):[],f=a?e.length?e[0]:{id:a.id}:this.attributes;void 0!==c&&(f[b]=c,a&&!e.length&&this.get("variations").push(f));return f[b]},removevariation:function(a){var b=this.get("variations");b&&$.each(b,function(c,e){if(e.stream==a)return b.splice(c,1),!1})},removevarimg:function(a,b){var c=this.getvariation(a).attachments;if(_.isObject(b))this.addexclude(a,b);else for(n in c)"image"==c[n].type&&c[n].name==b&&c.splice(n,1)},addexclude:function(a,b){var c=this.getvariation(a);c.excludes?c.excludes.push(b):
...
"change .schedule-group select":"monitorschedule","click .schedule-group input":"monitorschedule","click [data-set=on] .btn-white":"togglebesttime","click .end-preview":"endpreview","click #previewbtn":"preview","click #save":"save","click #post":"post","click #action, [data-type=icon] > i":"postaction"},initialize:function(a){a&&$.extend(this,a);this.streams=Cloudwalkers.Session.getChannel("outgoing").streams;this.action&&(this.type=this.action.get("token"));this.action&&"share"==this.type?this.draft=
this.reference.cloneSanitized():this.action&&this.reference?(this.action.parent=this.reference,this.actionstreams=[],this.actionview=!0,this.listenTo(this.action,"change",this.editstreams),this.action.fetch()):this.model&&(this.type="edit",this.state="loading",this.draft=this.model.clone(),this.draft.get("streams").length&&(this.initstream=this.draft.get("streams")[0].id),this.draft.fetch({endpoint:"original",success:this.original.bind(this)}));this.draft||(this.draft=new Cloudwalkers.Models.Message({variations:[],
...
"removeClass":"addClass"]("hidden");return this},toggleschedentry:function(a,b){$(a)[b?"removeClass":"addClass"]("inactive").find("label")[b?"removeClass":"addClass"]("icon-circle-blank")[b?"addClass":"removeClass"]("icon-ok-circle");return this},parsemoment:function(a,b){var c=$(a),e=$(b);e.val()||this.restarttime();if(!c.val()||!e.val())return null;c=moment(c.val(),"DD-MM-YYYY DD-MM-YY DD/MM/YYYY DD/MM/YY DDMMYYYY YYYYMMDD MM-DD-YYYY MM-DD-YY".split(" "));e=e.val().split(":");c=_.clone(c);1<e.length&&
c.add("minutes",60*Number(e[0])+Number(e[1]));return c.isValid()?c.unix()<moment().unix()?void 0:c:void 0},updateschedule:function(){var a;(a=this.draft.original(this.activestream,"schedule"))||(a=this.draft.original(null,"schedule"));this.toggleschedentry(".schedule-group [data-set]",!1).toggleschedentry("[data-set=now], [data-set=onlyonce]",!0);$("#delay-date, #delay-time, #repeat-interval, #repeat-amount, #repeat-until").val("").attr("disabled",!1);$("#repeat-interval, #repeat-amount").val(0);
$("[data-set=in] .btn-white").addClass("inactive");$("#delay-select").val(600);$("#every-select").val(168);$("#every-select-weekday").val(0).attr("disabled",!1);if(a&&a.date&&a.settings&&a.settings.delayselect)this.toggleschedentry("[data-set=now]").toggleschedentry("[data-set=in]",!0),$("#delay-select").val(a.settings.delayselect);else if(a&&a.date&&(this.toggleschedentry("[data-set=now]").toggleschedentry("[data-set=on]",!0),$(this.datepicker.get(0)).datepicker("update",moment.unix(a.date).format("DD/MM/YYYY")),
this.timepicker.timepicker("setTime",moment.unix(a.date).format("HH:mm")),a.besttimetopost)){var b=this.$el.find("[data-set=on] .btn-white");b.hasClass("inactive")&&b.toggleClass("inactive");$("#delay-time").attr("disabled",!0)}a&&a.repeat&&(a.repeat.interval&&(this.toggleschedentry("[data-set=onlyonce]").toggleschedentry("[data-set=every]",!0),[720,168,24,1].some(function(b){var e=a.repeat.interval/b/3600;if(Math.round(e)==e)return $("#repeat-interval").val(e),$("#every-select").val(b),!0})),a.repeat.weekdays&&
a.repeat.weekdays.length&&$("#every-select-weekday").val(a.repeat.weekdays[0]),a.repeat.amount&&(this.toggleschedentry("[data-set=onlyonce]").toggleschedentry("[data-set=repeat]",!0),$("#repeat-amount").val(a.repeat.amount)),a.repeat.until&&(this.toggleschedentry("[data-set=now]").toggleschedentry("[data-set=until]",!0),$(this.datepicker.get(1)).datepicker("update",moment.unix(a.repeat.until).format("DD/MM/YYYY"))))},monitorschedule:function(a,b){var c=b||$(a.currentTarget),c=(c.data("set")?c:c.parents("[data-set]").eq(0)).data("set"),
e;(e=this.draft.original(this.activestream,"schedule"))||(e={repeat:{}});e.now&&delete e.now;"now"==c?(this.toggleschedentry("[data-set=in], [data-set=on]").toggleschedentry("[data-set=now]",!0),$("[data-set=on] input").val("").attr("disabled",!1),$("[data-set=in] select").val(600),$("[data-set=in] .btn-white").addClass("inactive"),e={repeat:e.repeat,now:!0}):"in"==c?(this.toggleschedentry("[data-set=now], [data-set=on]").toggleschedentry("[data-set=in]",!0),$("[data-set=on] input").val("").attr("disabled",
!1),$("[data-set=in] .btn-white").addClass("inactive")):"on"==c?(this.toggleschedentry("[data-set=now], [data-set=in]").toggleschedentry("[data-set=on]",!0),$("[data-set=in] select").val(600),void 0===this.parsemoment("#delay-date","#delay-time")&&(this.datepicker.datepicker("hide").val(""),Cloudwalkers.RootView.alert("Please set your Schedule to a date in the future")),e.settings&&e.settings.delayselect&&delete e.settings.delayselect):"onlyonce"==c?(this.toggleschedentry("[data-set=onlyonce]",!0).toggleschedentry("[data-set=repeat], [data-set=until], [data-set=every]",
!1),$("#repeat-interval, #repeat-amount").val(0),$("#every-select").val(600),$("#every-select-weekday").val(0).attr("disabled",!1),$("#repeat-until").val(""),delete e.repeat):"every"==c?this.toggleschedentry("[data-set=every]",!0).toggleschedentry("[data-set=onlyonce]",!1):"repeat"==c?(this.toggleschedentry("[data-set=every], [data-set=repeat]",!0).toggleschedentry("[data-set=onlyonce], [data-set=until]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),Number($("#repeat-amount").val())||
$("#repeat-amount").val(1),$("#repeat-until").val("")):"until"==c&&(this.toggleschedentry("[data-set=every], [data-set=until]",!0).toggleschedentry("[data-set=onlyonce], [data-set=repeat]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),$("#repeat-amount").val(0));this.draft.original(this.activestream,"schedule",e);this.parsescheduled();a.stopPropagation();return this},togglebesttime:function(a){var b;(b=this.draft.original(this.activestream,"schedule"))||(b={repeat:{}});$(a.currentTarget).toggleClass("inactive");
...
c=e.diff(c)/1E3,f=Math.round(c/a.repeat.interval);else a.repeat.interval&&(f=a.repeat.amount,e=c.clone().add("seconds",f*a.repeat.interval));if(f)b.html("<span>"+f+" times "+(e?"<em class='negative'>until "+e.format("dddd, D MMMM YYYY")+"</em>":"")+"</span>");else if(a.repeat.interval){var g,h;[720,168,24,1].some(function(b){g=a.repeat.interval/b/3600;h=b;if(Math.round(g)==g)return!0});b.html("<span>Endless repeat every "+g+" "+{1:"hours",24:"days",168:"weeks",720:"months"}[h])}return this},preview:function(){this.$el.addClass("switch-mode");
this.preview=new Cloudwalkers.Views.Preview({model:this.draft,previewtype:"default",streamid:this.activestream.id});this.$el.find(".switch-container").append(this.preview.render().el)},endpreview:function(){this.preview.remove();this.$el.removeClass("switch-mode")},save:function(a){var b;if(b=this.draft.validateCustom())return Cloudwalkers.RootView.information("Not saved: ",b,this.$el.find(".modal-footer"));this.disablefooter();a&&_.isString(a)||(a="draft");this.draft.sanitizepost();this.draft.id?
this.draft.save({body:this.draft.get("body"),variations:this.draft.get("variations"),streams:this.draft.get("streams"),schedule:this.draft.get("schedule"),update:!0},{patch:!0,endpoint:"original",success:this.thankyou.bind(this)}):this.draft.save({status:a},{success:this.thankyou.bind(this)})},post:function(){var a;if(a=this.draft.validateCustom())return Cloudwalkers.RootView.information("Not posted: ",a,this.$el.find(".modal-footer"));this.disablefooter();this.draft.sanitizepost();this.draft.get("streams").length?
this.draft.id?this.draft.save({body:this.draft.get("body"),variations:this.draft.get("variations"),streams:this.draft.get("streams"),status:"scheduled",schedule:this.draft.get("schedule"),update:!0},{patch:!0,endpoint:"original",success:this.thankyou.bind(this)}):this.draft.save({status:"scheduled"},{success:this.thankyou.bind(this)}):this.save("scheduled")},postaction:function(){var a=[],b;this.$el.find(".action-tabs div").each(function(){a.push($(this).data("stream"))});if(b=this.draft.validateCustom())return Cloudwalkers.RootView.information("Not posted: ",
...
return this},unfollow:function(){this.model.trigger("unfollow",this.model);this.model.parent=Cloudwalkers.Session.getAccount();this.model.save({following:!1},{patch:!0});this.remove()}});Cloudwalkers.Views.Editor=Backbone.View.extend({id:"editor",className:"clearfix",oldUrl:"",urldata:{},currenturl:!1,urlprocessing:!1,limit:null,content:"",charlength:0,pos:0,urls:{},posmap:[],restrictedstreams:{twitter:140,linkedin:700},events:{"update:content":"render","update:limit":"renderlimit","append:content":"append","keyup #compose-content":"listentochange","paste #compose-content":"listentopaste","blur #compose-content":"endchange","click #swaplink":"swaplink",'click [data-type="title"] .addcontent':"addoecontent",
...
this.listenTo(this.parent,"update:stream",function(a){this.togglecontent(a,!0)}.bind(this));this.listenTo(this.parent,"update:campaign",this.campaignupdated);this.on("change:charlength",this.monitorlimit);navigator.userAgent.match(/(firefox(?=\/))\/?\s*(\d+)/i)&&(this.isfirefox=!0)},render:function(a){void 0!==a&&(this.content=a);a=Math.min.apply(Math,_.values(this.restrictedstreams));this.restrictedstreams["default"]=a;this.$el.html(Mustache.render(Templates.editor,{limit:a}));this.limit=a;this.$contenteditable=
