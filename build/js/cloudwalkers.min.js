...
return this},unfollow:function(){this.model.trigger("unfollow",this.model);this.model.parent=Cloudwalkers.Session.getAccount();this.model.save({following:!1},{patch:!0});this.remove()}});Cloudwalkers.Views.Editor=Backbone.View.extend({id:"editor",className:"clearfix",oldUrl:"",urldata:{},currenturl:!1,urlprocessing:!1,limit:null,content:"",pos:0,posmap:[],teststring:"",restrictedstreams:{twitter:140,linkedin:700},events:{"update:content":"render","update:limit":"renderlimit","append:content":"append","keyup #compose-content":"listentochange","paste #compose-content":"listentochange","blur #compose-content":"endchange","click #swaplink":"swaplink","keydown #composeplaceholder":"updatecontainer",
"mouseup #compose-content":"savepos"},xtrimmable:/<[^>]*>|&nbsp;|\s/g,xurlbasic:/http|[w]{3}/g,xshortens:/http|[w]{3}/g,xurlpattern:/(^|\s|\r|\n|\u00a0)((https?:\/\/|[w]{3})?[\w-]+(\.[\w-]+)+\.?(:\d+)?(\/\S*)?)(\s|\r|\n|\u00a0)/gi,initialize:function(a){a&&$.extend(this,a);this.listenTo(Cloudwalkers.Session.UrlShortener,"sync",this.parseurl)},render:function(a){void 0!==a&&(this.content=a);this.$el.html(Mustache.render(Templates.editor,{limit:this.limit}));this.$contenteditable=this.$el.find("#compose-content").eq(0);
this.contenteditable=this.$contenteditable.get(0);this.$contenteditable.html(this.content);this.content&&this.listentochange();return this},savepos:function(a){a=a.target.tagName;"I"!=a&&"SHORT"!=a&&(this.pos=this.cursorpos())},setrange:function(){},append:function(a){a&&(this.$contenteditable.append(a),this.listentochange())},renderlimit:function(a){"undefined"!=typeof a&&(this.limit=a)},listentochange:function(a){a=this.$contenteditable.html();this.pos=this.cursorpos();this.content===a||this.currenturl||
this.listentourl(a)&&this.processurl()},listentourl:function(a,b){var c=this.contenteditable.ownerDocument||this.contenteditable.document,e,f,g,h,l;e=(c.defaultView||c.parentWindow).getSelection();f=c.createRange();f.selectNodeContents(this.$contenteditable.get(0));g=this.parsenodes(f.startContainer.childNodes);if(this.currenturl)return g=f.startContainer.childNodes[g],(h=g.wholeText)||(h=g.innerHTML,g=g.childNodes[0]),h=h.replace(/&nbsp;/gi,""),h=h.indexOf(this.currenturl),l=h+this.currenturl.length,
this.contenteditable.designMode="on",f.setStart(g,h),f.setEnd(g,l),e.removeAllRanges(),e.addRange(f),c.execCommand("createLink",!1,this.currenturl),this.contenteditable.designMode="off",e.collapse(g,0),c=c.createTextNode(" \u200b\u200b"),f.collapse(!1),f.insertNode(c),this.$contenteditable.find("a").after(c),f.setStartAfter(c),e.removeAllRanges(),e.addRange(f),!0},processurl:function(){this.$contenteditable.find("a").attr("contenteditable",!1);this.filterurl(this.currenturl)},parsenodes:function(a){var b=
null;$.each(a,function(a,e){var f=!1,g=e.wholeText?e.wholeText:e.innerHTML;g&&(f=g.match(this.xurlpattern));if(f)return b=a,this.currenturl=f[0],!0}.bind(this));return b},filterreturn:function(a){var b=this.pos;String.prototype.insert=function(a,b){return 0<a?this.substring(0,a+1)+b+this.substring(a,this.length):b+this}.bind(this);return a.insert(b,"hackchar")},filterurl:function(a){var b=a.match(this.xurlpattern);b&&!this.urlprocessing&&(this.urlprocessing=!0,b.forEach(function(a){Cloudwalkers.Session.UrlShortener.fetch({q:a.trim(),
error:this.releaseurlprocessing})}));return a},parseurl:function(a){var b="<short contenteditable='false' data-url='"+a.longurl+"'>"+a.get("shortUrl")+"<i class='icon-link' id='swaplink'></i></short>",c=this;this.currenturl=a.longurl;this.shorturl=a.get("shortUrl");this.$contenteditable.find("a").replaceWith(b);a='<div><a href="'+this.currenturl+'" class="oembed"></a></div>';this.$el.find("#out").empty().html(a);this.$el.find(".oembed").oembed(null,null,this.embed).each(function(){this.def.done(function(){c.$el.find("#out").addClass("expanded")})})},
releaseurlprocessing:function(){this.urlprocessing=!1},toggleurl:function(){},breakurl:function(){},cursorpos:function(a){var b=this.contenteditable.ownerDocument||this.contenteditable.document,c=b.defaultView||b.parentWindow,e,f,g,h;if(void 0===a)a=0,void 0!==c.getSelection?(f=c.getSelection().getRangeAt(0),e=f.cloneRange(),e.selectNodeContents(this.contenteditable),e.setEnd(f.endContainer,f.endOffset),a=e.toString().length):(e=doc.selection)&&"Control"!=e.type&&(f=e.createRange(),e=b.body.createTextRange(),
e.moveToElementText(this.contenteditable),e.setEndPoint("EndToEnd",f),a=e.text.length);else{$.each(this.$contenteditable[0].childNodes,function(b,c){(g=c.length)||(g=c.id?c.outerText.length:c.childNodes[0]?c.childNodes[0].length:0);return g>=a?(h=c,!1):a-=g});f=b.createRange();e=c.getSelection();if(h)b=h.childNodes[0]?h.childNodes[0]:h;else return!1;f.setStart(b,a);f.collapse(!0);e.removeAllRanges();e.addRange(f)}return a},greyout:function(a,b){var c=this.$contenteditable.text().slice(a),e=this.$contenteditable.text().slice(0,
b);this.currentUrl&&(e=this.parsecontent(e));return e+'<span id="extrachars" $contenteditable="true">'+c+"</span>"},parsecontent:function(a,b){var c="";this.currentUrl&&!b&&(c='<span id="urltag" $contenteditable="true">'+this.currentUrl+'<i class="icon-link" id="swaplink"></i></span>');this.currentUrl&&b&&(c='<div id="urltag placehold"><span $contenteditable=false>'+this.currentUrl+'<i class="icon-link" id="swaplink"></i></span></div>');return a.replace(this.currentUrl,c)},updatecontainer:function(a){console.log(a)},
updatecounter:function(a){0>a&&(a=0);this.$el.find(".limit-counter").empty().html(a)},swaplink:function(){var a=this.$contenteditable.find("short").data("url").replace(/\s/g,"")==this.currenturl.replace(/\s/g,""),b="<short contenteditable='false' data-url='"+this.shorturl+"'>"+this.currenturl+"<i class='icon-link' id='swaplink'></i></short>",c="<short contenteditable='false' data-url='"+this.currenturl+"'>"+this.shorturl+"<i class='icon-link' id='swaplink'></i></short>",e=this.$contenteditable.text().length;
a?this.$contenteditable.find("short").replaceWith(b):this.$contenteditable.find("short").replaceWith(c);b=this.$contenteditable.text().length-e;a=a?this.$contenteditable.text().indexOf(this.currenturl):this.$contenteditable.text().indexOf(this.shorturl);this.pos>=a?this.cursorpos(this.pos+b):this.cursorpos(this.pos);this.pos=this.cursorpos()},togglecontent:function(a){if(a)var b=_.isNumber(a)?a:null,c=_.isObject(a)?a.html:null,b=b?Cloudwalkers.Session.getStream(b).get("network").token:null;this.network=
b?b:"default";b&&!c?(this.$contenteditable.empty().html(Mustache.render(Templates.composeplaceholder,{content:this.content})),this.updatecounter(this.restrictedstreams[this.network]-this.$contenteditable.text().length)):(a?(c||(c=""),this.$contenteditable.empty().html(c)):this.$contenteditable.empty().html(this.content),this.updatecontainer())},addoetitle:function(a){},addoecontent:function(a){a=a.currentTarget.parentElement.parentElement.innerText;var b=this.$contenteditable.html();this.$contenteditable.html(b+
"<br/>"+a);this.updatecontainer();this.trigger("contentadded")},addoeimg:function(a){a=this.$el.find('[data-type="image"] img').get(0).src;this.trigger("imageadded",{type:"image",data:a,name:a})}});Cloudwalkers.Widget=Backbone.View.extend({title:"...",color:"blue",icon:"cloud",events:{"click .portlet-title.line":"collapse"},initialize:function(){this.bind("destroy",this.destroy)},innerRender:function(){this.$el.addClass("inner-empty")},render:function(){this.$el.html(Mustache.render(Templates.widget,{title:this.options.title|this.title,color:this.options.color|this.color,icon:this.options.icon|this.icon}));"auto"==this.options.height&&this.$el.find(".scroller").remove();"undefined"!=typeof this.options.counter&&
