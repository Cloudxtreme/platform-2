...
"change [data-collapsable=repeat] select":"monitorschedule","click [data-collapsable=schedule] input, [data-collapsable=repeat] input, [data-collapsable=schedule] select, [data-collapsable=repeat] select":"monitorschedule","click [data-set=on] .btn-white":"togglebesttime","click .end-preview":"endpreview","click #previewbtn":"preview","click #save":"save","click #post":"post","click #action, [data-type=icon] > i":"postaction"},initialize:function(a){a&&$.extend(this,a);this.streams=Cloudwalkers.Session.getChannel("outgoing").streams;
...
"addClass"]("icon-circle-blank")[b?"addClass":"removeClass"]("icon-ok-circle");return this},parsemoment:function(a,b){var c=$(a),e=$(b);if(!c.val()||!e.val())return null;c=moment(c.val(),"DD-MM-YYYY DD-MM-YY DD/MM/YYYY DD/MM/YY DDMMYYYY YYYYMMDD MM-DD-YYYY MM-DD-YY".split(" "));e=e.val().split(":");c=_.clone(c);1<e.length&&c.add("minutes",60*Number(e[0])+Number(e[1]));return c.isValid()?c.unix()<moment().unix()?void 0:c:void 0},monitorschedule:function(a,b){var c=b||$(a.currentTarget),c=(c.data("set")?
c:c.parents("[data-set]").eq(0)).data("set");(schedule=this.draft.original(this.activestream,"schedule"))||(schedule={repeat:{}});schedule.now&&delete schedule.now;"now"==c?(this.toggleschedentry("[data-set=in], [data-set=on]").toggleschedentry("[data-set=now]",!0),$("[data-set=on] #delay-date").val("").attr("disabled",!1),$("[data-set=in] select").val(600),$("[data-set=in] .btn-white").addClass("inactive"),schedule={repeat:schedule.repeat,now:!0}):"in"==c?(this.toggleschedentry("[data-set=now], [data-set=on]").toggleschedentry("[data-set=in]",
...
$("#repeat-until").val(""),delete schedule.repeat):"every"==c?this.toggleschedentry("[data-set=every]",!0).toggleschedentry("[data-set=onlyonce]",!1):"repeat"==c?(this.toggleschedentry("[data-set=every], [data-set=repeat]",!0).toggleschedentry("[data-set=onlyonce], [data-set=until]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),Number($("#repeat-amount").val())||$("#repeat-amount").val(1),$("#repeat-until").val("")):"until"==c&&(this.toggleschedentry("[data-set=every], [data-set=until]",
!0).toggleschedentry("[data-set=onlyonce], [data-set=repeat]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),$("#repeat-amount").val(0));this.draft.original(this.activestream,"schedule",schedule);this.parsescheduled();a.stopPropagation();return this},parsescheduled:function(){(schedule=this.draft.original(this.activestream,"schedule"))||(schedule={repeat:{}});var a=this.$el.find("section[data-collapsable] .schedule-entry").not(".inactive").find("#delay-select, #delay-date, #delay-time, #repeat-interval, #every-select, #every-select-weekday, #repeat-amount, #repeat-until");
if(a.filter("#delay-select").val())schedule.date=moment().add("seconds",$("#delay-select").val()).unix();else if(a.filter("#delay-date").val()){var b=this.parsemoment("#delay-date","#delay-time");b&&(schedule.date=b.unix())}a.filter("#repeat-interval").val()&&(schedule.repeat||(schedule.repeat={interval:604800}),schedule.repeat.interval=$("#repeat-interval").val()?3600*$("#repeat-interval").val()*$("#every-select").val():0,$("#every-select-weekday").val()&&(schedule.repeat.weekdays=[$("#every-select-weekday").val()]),
$("#every-select-weekday").attr("disabled",604800>schedule.repeat.interval),schedule.repeat.amount=Number(a.filter("#repeat-amount").val())?Number($("#repeat-amount").val()):!1,schedule.repeat.until=a.filter("#repeat-until").val()?moment($("#repeat-until").val(),["DD-MM-YYYY","DD-MM-YY","DD/MM/YYYY"]).unix():!1,a={},a.interval=$("#repeat-interval").val()||null,a.every=$("#every-select").val()||null,a.everyweek=$("#every-select-weekday").val()||null,schedule.repeat.settings=a);return schedule},togglebesttime:function(a){$(a.currentTarget).toggleClass("inactive");
$(a.currentTarget).hasClass("inactive")?$("#delay-time").attr("disabled",!1):$("#delay-time").attr("disabled",!0).val("");this.draft.get("schedule").besttimetopost=!$(a.currentTarget).hasClass("inactive");return this},summarizeschedule:function(){var a=this.parsescheduled(),b=this.$el.find("[data-collapsable=schedule] .summary").empty();if(a&&a.date){var c=a.besttimetopost?"Best time to post":moment.unix(a.date).format("HH:mm");b.html("<span><i class='icon-time'></i> "+moment.unix(a.date).format("dddd, D MMMM YYYY")+
"<em class='negative'>"+c+"</em></span>")}this.summarizerepeat();return this},summarizerepeat:function(){var a=this.parsescheduled();if(!a||!a.repeat)return this;var b=this.$el.find("[data-collapsable=repeat] .summary").empty(),c=a.date?moment.unix(a.date):moment();if(a.repeat.until)var e=moment.unix(a.repeat.until),c=e.diff(c)/1E3,f=Math.round(c/a.repeat.interval);else a.repeat.interval&&(f=a.repeat.amount,e=c.clone().add("seconds",f*a.repeat.interval));f&&b.html("<span>"+f+" times "+(e?"<em class='negative'>until "+
e.format("dddd, D MMMM YYYY")+"</em>":"")+"</span>");return this},updateschedule:function(){var a=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,b=a?a:this.draft.get("schedule");b&&b.date?($(this.datepicker.get(0)).datepicker("update",moment.unix(b.date).format("DD/MM/YYYY")),this.timepicker.timepicker("setTime",moment.unix(b.date).format("HH:mm"))):b&&b.now&&(this.toggleschedentry("[data-set=in], [data-set=on]").toggleschedentry("[data-set=now]",!0),$("[data-set=on] #delay-date").val("").attr("disabled",
!1),$("[data-set=in] select").val(600),$("[data-set=in] .btn-white").addClass("inactive"),schedule={repeat:schedule.repeat,now:!0});b&&b.repeat&&(b.repeat.until&&$(this.datepicker.get(1)).datepicker("update",moment.unix(b.repeat.until).format("DD/MM/YYYY")),b.repeat.interval&&(this.toggleschedentry("[data-set=onlyonce], "+(b.repeat.amount?"[data-set=until]":"[data-set=repeat]"),!1),this.toggleschedentry("[data-set=every], "+(b.repeat.amount?"[data-set=repeat]":"[data-set=until]"),!0)),[720,168,24,
1].some(function(a){var e=b.repeat.interval/a/3600;if(Math.round(e)==e)return $("#repeat-interval").val(e),$("#every-select").val(a),!0}),b.repeat.weekdays&&b.repeat.weekdays.length&&$("#every-select-weekday").val(b.repeat.weekdays[0]),b.repeat.amount&&$("#repeat-amount").val(b.repeat.amount))},preview:function(){this.$el.addClass("switch-mode");this.preview=new Cloudwalkers.Views.Preview({model:this.draft,previewtype:"default",streamid:this.activestream.id});this.$el.find(".switch-container").append(this.preview.render().el)},
