...
"add_category name create add_keyword category select_category fine-tune filters include exclude languages countries use_comma_separated_words language country select_from_list all_languages global update cancel manage_keyword remember_for days".split(" ");this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.KeywordsOverview=Cloudwalkers.Views.Widgets.Widget.extend({id:"monitorparent",entries:[],events:{"submit .category-edit form":"editCategory","click .edit-toggler":"toggleEditCategory","click .delete-category":"deleteCategory","click .delete-keyword":"deleteKeyword","click [data-keyword]":"toggleEditKeyword"},initialize:function(){this.channel=Cloudwalkers.Session.getChannel("monitoring");this.editor=this.options.editor;this.listenTo(Cloudwalkers.Session.getChannels(),"sync",
this.render)},render:function(){categories=this.channel.channels.map(function(a){return{id:a.id,name:a.get("name"),remember:a.get("remember"),keywords:a.channels.models}});var a={categories:categories};this.mustacheTranslateRender(a);Cloudwalkers.Session.censuretemplate(a);this.$el.html(Mustache.render(Templates.keywordsoverview,a));return this},toggleEditCategory:function(a){a.stopPropagation();$(a.target).closest("[data-category]").find(".category-name, .category-edit").toggle()},editCategoryName:function(a){a.stopPropagation();
a=$(a.target).closest("[data-category]");var b=a.find('[name="name"]').val(),c=a.find('[name="remember"]').val(),e=Cloudwalkers.Session.getChannel(Number(a.attr("data-category")));e.endpoint="";e.save({name:b,remember:c});a.find(".name_val").html(b);a.find(".remember_val").html(c);return!1},deleteCategory:function(a){a.stopPropagation();var b=$(a.target).closest("[data-category]");Cloudwalkers.RootView.confirm(this.translateString("are_you_sure_you_want_to_remove_this_category"),function(){Cloudwalkers.Session.getChannel(Number(b.attr("data-category"))).destroy();
