...
!0;a.model=Cloudwalkers.Session.getChannel(a.type);a.filters={sort:"engagement",since:3600*Math.round(Date.now()/36E5)-86400*a.since};if(a.model)return new Cloudwalkers.Views.Widgets.DashboardMessageList(a)},filterparameters:function(){this.start=moment().zone(0).startOf("isoweek");this.end=moment().zone(0).endOf("isoweek");return{since:this.start.unix(),until:this.end.unix()}},translateWidgets:function(a){if(a.translation)for(k in a.translation)a[k]=Cloudwalkers.Session.polyglot.t(a.translation[k])},
...
!1;e.showStreamName=!1;this.datepicker.on("date:change",function(a,b){e.getDataset().setDateRange(a,b)});this.add(e)}});Cloudwalkers.Views.Statistics=Cloudwalkers.Views.Pageview.extend({id:"statistics",start:0,end:0,timespan:"week",period:0,custom:!1,views:[],events:{remove:"destroy","click #add":"addperiod","click #subtract":"subtractperiod","click #subtractempty":"subtractempty","click #addempty":"addempty","click #showreports":"showreports","click #now":"now","click #show":"changecustom","change .stats-header select.networks":"changestream","change .stats-header select.time":"changespan","click .dashboard-stat":"updatenetwork"},
...
inherit_select_classes:!0});"custom"==this.timespan&&this.$el.find("#start, #end").datepicker({format:"dd-mm-yyyy"});0==this.period&&this.$el.find("#add").attr("disabled",!0);google.load("visualization","1",{callback:this.request.bind(this),packages:["corechart"]});return this},writeui:function(){var a=this.timemanager();this.$el.find(".stats-header-timeview").eq(0).html("<strong>"+a.fullperiod+": </strong>"+a.timeview)},request:function(a){this.writeui();this.collection.touch(this.filterparameters());
this.currentperiod=a},fillcharts:function(a){if(a&&!a.length)return this.showempty();this.cleanviews();this.widgets.forEach(function(a){(a.data.translation||a.data.chartdata)&&this.translatechart(a);if(this.streamid&&this.streamdata(a))a.data=this.streamdata(a);else if(this.streamid)return;a.data.parentview=this;a.data.timespan={since:this.start.unix(),to:this.end.unix()};var c=new Cloudwalkers.Views.Widgets[a.widget](a.data);this.views.push(c);this.appendWidget(c,a.span)}.bind(this))},translatechart:function(a){if(a.data.translation)this.translateWidgets(a.data);
else if(a.data.chartdata)for(var b in a.data.chartdata)this.translateWidgets(a.data.chartdata[b].data)},timemanager:function(){var a=this.filterparameters(),b=moment.unix(a.since).zone(0),c=24<b.date()||"quarter"==a.span?11==b.month()?"DD MMM YYYY":"DD MMM":"DD";a.periodstring=this.period||"custom"==a.span?-1==this.period?"last ":"":"this ";a.timeview=b.format(c)+" - "+moment.unix(a.until).zone(0).format("DD MMM YYYY");a[a.span+"Active"]=!0;a.startstring=moment.unix(a.since).zone(0).format("DD-MM-YYYY");
a.endstring=moment.unix(a.until).zone(0).format("DD-MM-YYYY");a.fullperiod="pt_PT"==Cloudwalkers.Session.user.attributes.locale?this.translateString(a.span)+" "+this.translateString(a.periodstring):this.translateString(a.periodstring)+" "+this.translateString(a.span);return a},showloading:function(){this.$el.addClass("loading")},hideloading:function(){this.$el.removeClass("loading");this.$el.find(".period-buttons .btn").attr("disabled",!1);this.$el.find("select").prop("disabled",!1).trigger("chosen:updated");
this.listenToOnce(this,"change:period",this.request);this.currentperiod&&(this.period=this.currentperiod)},showempty:function(){this.cleanviews();this.hideloading();var a=new Cloudwalkers.Views.Widgets.EmptyStatisticsData({timeparams:this.timeparams,stream:this.streamid});this.views.push(a);this.appendWidget(a,8,null,2)},now:function(){this.period=0;this.trigger("change:period",this.period)},addperiod:function(a){a=$(a.target).attr("disabled");0<=this.period||"disabled"==a||(this.period+=1,this.trigger("change:period",
this.period))},subtractperiod:function(a){"disabled"!=$(a.target).attr("disabled")&&(this.period-=1,this.trigger("change:period",this.period))},subtractempty:function(){this.period-=1;this.trigger("change:period",this.period)},addempty:function(){this.period+=1;this.trigger("change:period",this.period)},showreports:function(){var a=Number(this.$el.find("select.networks").val());a||(a=Cloudwalkers.Session.getStreams().where({statistics:1})[0].id);Cloudwalkers.Router.Instance.navigate(a?"#reports/"+
a:"#reports",{trigger:!0})},changestream:function(){var a=Number(this.$el.find("select.networks").val());Cloudwalkers.Router.Instance.navigate(a?"#statistics/"+a:"#statistics",{trigger:!0})},changespan:function(){this.timespan=this.$el.find("select.time").val();this.trigger("change:period")},changecustom:function(){this.start=moment(this.$el.find("#start").val(),"DD-MM-YYYY");this.end=moment(this.$el.find("#end").val(),"DD-MM-YYYY");this.trigger("change:period")},filterparameters:function(){"now"==
this.timespan&&(this.period=0);"week"==this.timespan&&(this.start=moment().zone(0).startOf("isoweek"),this.end=moment().zone(0).endOf("isoweek"));"month"==this.timespan&&(this.start=moment().zone(0).startOf("month"),this.end=moment().zone(0).endOf("month"));"year"==this.timespan&&(this.start=moment().zone(0).startOf("year"),this.end=moment().zone(0).endOf("year"));if("quarter"==this.timespan){var a=3*(this.period+moment().quarter());this.start=moment().startOf("year").add("months",a-3);this.end=moment().startOf("year").add("months",
a-1).endOf("month")}else"custom"!=this.timespan&&(0<this.period&&(this.start.add(this.timespan+"s",this.period),this.end.add(this.timespan+"s",this.period)),0>this.period&&(this.start.subtract(this.timespan+"s",Math.abs(this.period)),this.end.subtract(this.timespan+"s",Math.abs(this.period))));this.timeparams={since:this.start.unix(),until:this.end.unix(),span:this.timespan};return{since:this.start.unix(),until:this.end.unix(),span:this.timespan,period:this.period,reset:!0}},finish:function(){},updatenetwork:function(a){},
translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},translateWidgets:function(a){if(a.translation)for(k in a.translation)a[k]=Cloudwalkers.Session.polyglot.t(a.translation[k])},mustacheTranslateRender:function(a){this.original="week month quarter year custom time_spans all_networks now show".split(" ");this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.StatStream=Cloudwalkers.Views.Statistics.extend({widgets:[{widget:"StatSummary",data:{columnviews:["contacts-network","score-trending-network","outgoing-network","besttime"]},span:12},{widget:"TitleSeparator",data:{title:"Contacts info"}},{widget:"Chart",data:{filterfunc:"contact-evolution-network",chart:"LineChart",title:"Contacts Evolution"},span:6},{widget:"Chart",data:{filterfunc:"age",chart:"PieChart",title:"By Age"},span:3,networks:["facebook"]},{widget:"Chart",data:{filterfunc:"gender",
...
tumblr:"#385775","google-plus":"#DD4C39",youtube:"#CC181E",web:"#f39501",blog:"#f39501","mobile-phone":"#E2EAE9",others:"#E5E5E5"},networktokens:{Facebook:"#3B5998",Twitter:"#01a9da",LinkedIn:"#1783BC",GooglePlus:"#DD4C39",YouTube:"#CC181E",Youtube:"#CC181E",Internal:"#E2EAE9",Others:"#E5E5E5",CoworkerWall:"#CCCCCC"},initialize:function(a){a&&$.extend(this,a);view=this;this.collection=this.parentview.collection;this.listenTo(this.collection,"ready",this.fill)},render:function(){this.settings={};this.settings.title=
