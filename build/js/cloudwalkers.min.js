...
e=Cloudwalkers.Session.getAccount(),f=e.channels.findWhere({type:"news"})?e.channels.findWhere({type:"news"}).id:null,e=e.channels.findWhere({type:"profiles"})?e.channels.findWhere({type:"profiles"}).id:null,c=new Cloudwalkers.Views.Timeline({model:c,parameters:{records:40,markasread:!0}}),g;a==e?g="MESSAGE_READ_COMPANY":a==f&&(g="MESSAGE_READ_THIRDPARTY");this.validate(c,g)},trending:function(a,b){var c=Cloudwalkers.Session.getChannel(Number(a)),e=b?Cloudwalkers.Session.getStream(Number(b)):c,c=
"news"==c.get("type")?1:7,c={since:3600*Math.round(Date.now()/36E5)-86400*c,sort:"engagement",records:40,markasread:!0},e=new Cloudwalkers.Views.Timeline({model:e,trending:!0,parameters:c});this.validate(e,"MESSAGE_READ_THIRDPARTY")},manageaccounts:function(){var a=Cloudwalkers.Session.getChannel("news"),a=new Cloudwalkers.Views.ManageAccounts({channel:a});Cloudwalkers.RootView.setView(a)},monitoring:function(a,b,c){a=new Cloudwalkers.Views.KeywordMonitoring({category:Cloudwalkers.Session.getChannel(Number(b))});
...
Cloudwalkers.Models.Contact.actions={templates:{dm:{name:"Direct message",icon:"message",type:"dialog"}}};Cloudwalkers.Models.Note=Backbone.Model.extend({typestring:"notes",type_settings:{CONTACT:{icon:"user",model:"Contact",typestring:"contacts"},MESSAGE:{icon:"inbox",model:"Message",typestring:"messages"},ACCOUNT:{icon:"edit",model:"Account",typestring:"accounts"}},initialize:function(a){a&&$.extend(this,a);this.on("action",this.action)},parse:function(a){a=a.note?a.note:a;a.date&&(a.fulldate=moment(a.date).format("DD MMM YYYY HH:mm"),a.dateonly=moment(a.date).format("DD MMM YYYY"),a.time=moment(a.date).format("HH:mm"),
a.type_icon=this.type_settings[a.model.objectType].icon);return a},url:function(){var a=[CONFIG_BASE_URL+"json"];this.id?a.push(this.typestring,this.id):this.parent?"contacts"!=this.parent.typestring?a.push(this.parent.typestring,this.parent.id,this.typestring):"contacts"==this.parent.typestring&&(a.push("accounts/"+Cloudwalkers.Session.getAccount().id+"/contacts"),a.push(this.parent.id),a.push(this.typestring)):a.push(this.typestring);a=a.join("/");return this.parameters?a+"?"+$.param(this.parameters):
...
this.attributes.variations&&this.attributes.variations.forEach(function(a,b){a.body&&(a.body.plaintext?a.body.html&&delete a.body.html:delete a.body);0>this.attributes.streams.indexOf(a.stream)&&this.attributes.variations.splice(b,1)}.bind(this))},cloneSanitized:function(){var a=this.clone();a.attributes.date&&delete a.attributes.date;a.attributes.from&&delete a.attributes.from;a.attributes.stream&&delete a.attributes.stream;a.attributes.streams=[];a.id&&(delete a.id,delete a.attributes.id);return a},
...
b);else for(n in c)"image"==c[n].type&&c[n].name==b&&c.splice(n,1)},addexclude:function(a,b){var c=this.getvariation(a)||this.setvariation(a);c.excludes?c.excludes.attachments.push(b):c.excludes={attachments:[b]}},checkexclude:function(a,b){var c=this.getvariation(a);if(c)c=c.excludes;else return!1;if(c)for(n in c.attachments)if(c.attachments[n]==b)return!0;return!1},location:function(){return"#"},afterChange:function(){this.addInternalActions();this.get("parent")&&this.get("parentmodel").set(this.get("parent"))},
...
"s/"+this.parent.id:"";return CONFIG_BASE_URL+"json/"+b+"/actions"+a},rendertokens:function(a){a||(a=this.parent.get("actiontokens"));return a.map(function(a){return this.templates[a]}.bind(this))},startaction:function(a){var b=this.templates[a];this.listenTo(Cloudwalkers.RootView,a.concat(":success"),this.toggleAction);if("confirm"==b.type)this[a](this.parent);else{if("growl"==b.type)return Cloudwalkers.RootView.growl(b.name,this.translateString("the")+" "+b.token+" "+this.translateString("is_planned_with_success")),
...
a["translate_"+this.original[k]]=this.translated[k]},mustacheTranslateSummarizeRepeat:function(a){this.original="times until endless_repeat_every hours days weeks months".split(" ");this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Preview=Backbone.View.extend({id:"preview",events:{},networkclasses:{facebook:"fb",twitter:"twt","google-plus":"gp",linkedin:"li"},events:{"click #viewsummary":"togglesummary"},initialize:function(a){a&&$.extend(this,a);$.extend(this.model.attributes,this.model.getvariation(this.streamid));this.model.attributes.profile=Cloudwalkers.Session.getStream(this.streamid).get("profile");this.network=Cloudwalkers.Session.getStream(this.streamid).get("network").token},render:function(){var a=
...
return this},unfollow:function(){this.model.trigger("unfollow",this.model);this.model.parent=Cloudwalkers.Session.getAccount();this.model.save({following:!1},{patch:!0});this.remove()}});Cloudwalkers.Views.ViewContact=Backbone.View.extend({id:"compose",className:"modal hide viewcontact",entries:[],events:{"click .end-preview td i":"backtolist","click #contactfilter li":"loadmessages","click [data-action=write-note]":"togglecontactnote","click #post":"post","click *[data-action]":"action","keyup #tags":"entertag","click .load-more":"more"},initialize:function(a){a&&$.extend(this,a);if(this.contact=a.contact?a.contact.model:null)this.model=new Cloudwalkers.Models.Contact(this.contact);
...
"arrow",b=new Cloudwalkers.Views.Entry({model:b,template:c,checkunread:!0,parameters:{inboxview:!0,parent:this}}),this.entries.push(b),this.listenTo(b,"toggle",this.loadmessage),this.$container.append(b.render().el);this.refreshview=!1},seed:function(a,b){this.collection.models=b.contact.messages;this.collection.length=this.collection.models.length;this.collection.trigger("seed",this.collection.models)},begintouch:function(a){a||(a="messages");this.type=a;this.loadmylisteners(!0);this.refreshview=
!0;"messages"==a?this.getmessages():this.touch(a)},getmessages:function(){this.model.urlparams=["messages"];this.collection.url=this.model.url();this.collection.parenttype="contact";this.collection.contactinfo=this.contactinfo;this.collection.parse=this.parse;this.collection.fetch({success:this.seed.bind(this)})},parse:function(a){this.parenttype&&(a=a[this.parenttype]);if(a[this.typestring])for(n in a[this.typestring])a[this.typestring][n]=new Cloudwalkers.Models.Message(a[this.typestring][n]),a[this.typestring][n].generateintro(),
a[this.typestring][n].set("read",1),a[this.typestring][n].set("fulldate",moment(a[this.typestring][n].get("date")).format("DD MMM YYYY HH:mm")),a[this.typestring][n].set("icon",this.contactinfo.network?this.contactinfo.network.icon:null),a[this.typestring][n].set("networkdescription",this.contactinfo.network?this.contactinfo.network.name:null),a[this.typestring][n].set("networktoken",this.contactinfo.network?this.contactinfo.network.token:null);this.ready();return a[this.typestring]},touch:function(a,
...
["add","on","commented"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.NoteEntry=Cloudwalkers.Views.Entry.extend({tagName:"li",template:"messagenote",events:{mouseover:"toggleactions",mouseout:"toggleactions","click *[data-notification-action]":"action","click .note-header":"togglenote"},initialize:function(a){this.parameters={};a&&$.extend(this,a);this.listenToOnce(this.model,"change",this.render);this.listenTo(this.model,"destroy",this.remove)},render:function(){Cloudwalkers.Session.censuretemplate(this.model.attributes);this.model.attributes.date=
...
"campaigns account_plan account name save_changes cancel company_name manage_campaigns basic menu".split(" ");this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Settings.Trigger=Backbone.View.extend({events:{"click [data-action=save]":"save","click [data-action=reset]":"reset"},initialize:function(a){$.extend(this,a);this.model||(this.model=new Cloudwalkers.Models.Trigger)},render:function(){this.model.parent=Cloudwalkers.Session.getAccount();var a={message:this.model.getmessage("REPLY"),description:this.description};this.$el.html(Mustache.render(Templates.settings.trigger,a));return this},updatetrigger:function(a){this.model=a;this.render();
