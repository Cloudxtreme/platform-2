...
a);var f=this.currentnode;if(b)return{node:f.node,offset:f.offset};e=this.document.createRange();c=this.win.getSelection();e.setStart(f.node,f.offset);e.collapse(!0);c.removeAllRanges();c.addRange(e)}return a},getnode:function(a,b,c){$.each(a.childNodes,function(a,f){if(c){if(f.nodeName==c)if(f.childNodes.length)this.getnode(f,null,c);else return this.nextnode=f,!1}else{var g=f.textContent.length;return g>=b?(f.childNodes.length?this.getnode(f,b):this.currentnode={node:f,offset:b},!1):b-=g}}.bind(this))},
