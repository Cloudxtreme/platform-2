...
this.get(a);e||(e=Cloudwalkers.Session.user.account[this.typestring].get(a));e?this.add(e):e=this.create({id:a});b.push(e);return a},this));a.length&&(this.endpoint=this.parentmodel?this.typestring:null,this.parameters={ids:a.join(",")},this.fetch({remove:!1}));this.trigger("seed",b);return b},more:function(a,b){if(!this.cursor)return!1;b.after=this.cursor;this.touch(a,b);return this},ready:function(){setTimeout(function(a){a.trigger("ready",a)},1,this)}});Cloudwalkers.Session={langs:[{id:"en_EN",name:"International English"},{id:"nl_NL",name:"Dutch"},{id:"pt_PT",name:"Portugese"}],user:null,isLoaded:function(){return null!=this.user},loadEssentialData:function(a){this.user=new Cloudwalkers.Models.Me;this.user.once("activated",function(){this.setLang()}.bind(this));this.listenTo(this,"translation:done",a);this.user.fetch({error:this.user.offline.bind(this.user)})},refresh:function(){console.log("Session.refresh triggered")},reset:function(){window.localStorage.clear()},
home:function(){Cloudwalkers.Router.Instance.home()},updateSetting:function(a,b,c){Cloudwalkers.Session.user.attributes.settings[a]!=b&&(Cloudwalkers.Session.user.attributes.settings[a]=b,c=$.extend(c,{patch:!0})||{patch:!0},Cloudwalkers.Session.user.save({settings:Cloudwalkers.Session.user.attributes.settings},c))},get:function(a){return this.user.get("settings")[a]},clone:function(a){if(null==a||"object"!=typeof a)return a;var b=a.constructor(),c;for(c in a)b[c]=this.clone(a[c]);return b},viewsettings:function(a,
b){var c="account_"+this.getAccount().id;Cloudwalkers.Session.user.attributes.settings.viewsettings||(Cloudwalkers.Session.user.attributes.settings.viewsettings={});Cloudwalkers.Session.user.attributes.settings.viewsettings[c]||(Cloudwalkers.Session.user.attributes.settings.viewsettings[c]=Cloudwalkers.RootView.navigation.mapViews());var e=this.clone(this.get("viewsettings"));if(b){if(a&&b)return e[c][a]=$.extend(e[c][a],b),this.updateSetting("viewsettings",e),e[c][a];throw TypeError("Not the right parameters were met for function viewsettings");
}return e[c][a]},manage:function(){200<Store.count("messages")&&Store.filter("messages",null,function(a){a.sort(function(a,c){return a.stamp>c.stamp?1:a.stamp<c.stamp?-1:0});a=a.slice(0,100);Store.write("messages",a);Store.filter("touches",null,function(a){var c=Cloudwalkers.Session.getPing().cursor;a=a.filter(function(a){return a.ping==c});Store.write("touches",a)})});25<Store.count("reports")&&Store.filter("reports",null,function(a){a.sort(function(a,c){return a.stamp>c.stamp?1:a.stamp<c.stamp?
-1:0});a=a.slice(0,10);Store.write("messages",a)})},isAuthorized:function(a){return a?this.user.isauthorized(a):this.user.authorized},censuretemplate:function(a){a||(a={});var b=this.getUser().censuretokens;a.authorized=b},getPing:function(){return this.user.account.ping},ping:function(){this.user.account.ping.forceping()},getAccount:function(a){return a?this.user.accounts.get(a):this.user.account},getAccounts:function(a){return this.user.accounts},getChannel:function(a){return"number"==typeof a?
this.user.account.channels.get(a):this.user.account.channels.findWhere({type:a})},getChannels:function(a){return this.user.account.channels},storeChannel:function(a){a.channels&&a.channels.length&&(a.channels=a.channels.map(function(a){Cloudwalkers.Session.storeChannel(a);return a.id}));a.streams&&a.streams.length&&(a.streams=a.streams.map(function(a){Store.post("streams",a);return a.id}));Store.post("channels",a)},getStream:function(a){return"number"==typeof a?this.user.account.streams.get(a):this.user.account.streams.findWhere({token:a})},
getStreams:function(){return this.user.account.streams},getUser:function(a){return a?this.user.account.users.get(a):this.user},getUsers:function(){return this.user.account.users},getContact:function(a){return this.user.account.contacts.get(a)},getContacts:function(){return this.user.account.contacts},getMessage:function(a){return this.user.account.messages.get(a)},getMessages:function(){return this.user.account.messages},getNotification:function(a){return this.user.account.notifications.get(a)},getNotifications:function(){return this.user.account.notifications},
...
Cloudwalkers.Models.Contact.actions={templates:{dm:{name:"Direct message",icon:"message",type:"dialog"}}};Cloudwalkers.Models.Note=Backbone.Model.extend({typestring:"notes",initialize:function(a){a&&$.extend(this,a);this.on("action",this.action)},parse:function(a){a=a.note?a.note:a;a.date&&(a.fulldate=moment(a.date).format("DD MMM YYYY HH:mm"),a.dateonly=moment(a.date).format("DD MMM YYYY"),a.time=moment(a.date).format("HH:mm"));return a},url:function(){var a=[CONFIG_BASE_URL+"json"];this.id?a.push(this.typestring,this.id):this.parent?"contacts"!=this.parent.typestring?a.push(this.parent.typestring,this.parent.id,
this.typestring):"contacts"==this.parent.typestring&&(a.push("accounts/"+Cloudwalkers.Session.getAccount().id+"/contacts"),a.push(this.parent.id),a.push(this.typestring)):a.push(this.typestring);a=a.join("/");return this.parameters?a+"?"+$.param(this.parameters):a},action:function(a){"delete"==a&&this.deletenote()},deletenote:function(){var a=this;Cloudwalkers.RootView.confirm("Are you sure you want to remove this note?",function(){a.destroy({success:function(){a.trigger("destroy")}})})}});Cloudwalkers.Models.Me=Cloudwalkers.Models.User.extend({unreadMessages:null,initialize:function(a){this.once("change",this.activate);this.on("change:id",function(a){this.previous("id")&&Cloudwalkers.Session.home()})},url:function(){var a=this.endpoint?this.endpoint:Store.exists("me")?"?include_accounts=ids":"";return CONFIG_BASE_URL+"json/user/me"+a},parse:function(a){Store.exists("me")||(a.user=this.firstload(a.user));Store.write("me",[a.user]);return a.user},sync:function(a,b,c){this.endpoint=c.endpoint?
...
"json/"+this.typestring+"/"+this.id+this.endpoint:CONFIG_BASE_URL+"json/"+this.typestring+"/"+this.id:CONFIG_BASE_URL+"json/accounts/"+Cloudwalkers.Session.getAccount().id+"/"+this.typestring},parse:function(a){if("number"==typeof a)return{id:a};a.message&&(a=a.message);a=this.filterData(a);this.stamp(a);this.checkloaded(a);return a},sync:function(a,b,c){this.endpoint=c.endpoint?"/"+c.endpoint:!1;return"update"==a?!1:Backbone.sync(a,b,c)},validateCustom:function(a){var b;if(!this.hascontent()&&0>
$.inArray("content",a))return"You need a bit of content.";if(!this.validatecontent())return"One or more networks exceed the character limit.";if(!this.get("streams").length&&0>$.inArray("streams",a))return"Please select a network first.";if((b=this.validateschedules())&&0>$.inArray("schedule",a))return b},validateschedules:function(){var a;if(a=this.validateschedule(this.get("schedule")))return a;$.each(this.get("variations"),function(b,c){return(a=this.validateschedule(c.schedule))?!1:!0}.bind(this));
return a},validateschedule:function(a){if(!a)return!1;var b=a.date||null;a=a.repeat?a.repeat.until:null;return b&&b<moment().unix()?"One or more streams are scheduled into the past.":b&&a&&a<b?"One or more streams have the repeat date happening before the scheduled date.":!b&&a&&a<moment().unix()?"One or more streams have the repeat date set to the past.":!1},hascontent:function(){var a;if(this.get("attachments")&&this.get("attachments").length)return this.get("attachments").length;if(this.get("body").plaintext)return this.get("body").plaintext.length;
if(this.get("variations")&&this.get("variations").length)return $.each(this.get("variations"),function(b,c){if(c.attachments&&c.attachments.length)return a=c.attachments.length,!1;if(c.body&&c.body.plaintext)return a=c.body.plaintext.length,!1}.bind(this)),a},validatecontent:function(){var a=1,b;this.get("streams")&&this.get("streams").length&&$.each(this.get("streams"),function(c,e){var f=Cloudwalkers.Session.getStream(e).get("network"),f=f.limitations["max-length"]?f.limitations["max-length"].limit:
null;if(!f)return!0;if(b=this.getvariation(e)){if(b.body&&b.body.plaintext)return a=f-b.body.plaintext.length,0>a?!1:!0}else if(this.get("body")&&this.get("body").plaintext&&(a=f-this.get("body").plaintext.length,0>a))return!1}.bind(this));return 0<a},sanitizepost:function(){this.attributes.body.html&&delete this.attributes.body.html;this.attributes.variations&&this.attributes.variations.forEach(function(a,b){a.body&&(a.body.plaintext?a.body.html&&delete a.body.html:delete a.body);0>this.attributes.streams.indexOf(a.stream)&&
this.attributes.variations.splice(b,1)}.bind(this))},cloneSanitized:function(){var a=this.clone();a.attributes.date&&delete a.attributes.date;a.attributes.from&&delete a.attributes.from;a.attributes.stream&&delete a.attributes.stream;a.attributes.streams=[];a.id&&(delete a.id,delete a.attributes.id);return a},checkloaded:function(a){a.objectType&&setTimeout(function(a){a.trigger("loaded")},1,this)},filterData:function(a){var b={};$.each("id objectType actiontokens subject body date engagement from read stream streams attachments parent statistics canHaveChildren children_count schedule variations".split(" "),
...
[];if(_.isObject(b))this.addexclude(a,b);else for(n in c)"image"==c[n].type&&c[n].name==b&&c.splice(n,1)},addexclude:function(a,b){var c=this.getvariation(a)||this.setvariation(a);c.excludes?c.excludes.push(b):c.excludes=[b]},checkexclude:function(a,b){var c=this.getvariation(a);if(c)c=c.excludes;else return!1;if(c)for(n in c)if(c[n].name==b.name)return!0;return!1},location:function(){return"#"},afterChange:function(){this.addInternalActions();this.get("parent")&&this.get("parentmodel").set(this.get("parent"))},
...
authorized:0},{name:"profile",title:"Profile settings",icon:"user",authorized:0}]}],events:{"click .notification-toggle":"toggleNotifications","click .btn-compose":"compose","click #writenote":"writenote"},initialize:function(){Cloudwalkers.Session.on("change:accounts",this.renderHeader,this);this.listenTo(Cloudwalkers.Session.getChannels(),"sync",this.render);this.listenTo(Cloudwalkers.Session.getChannels(),"remove",this.render);$.get(CONFIG_BASE_URL+"json/version",this.version.bind(this))},headeraction:function(a){a=
$(a.currentTarget).data("header-action");"messages"==a&&Cloudwalkers.Router.Instance.navigate("#inbox/messages",!0);"contacts"==a&&Cloudwalkers.Router.Instance.navigate("#coworkers",!0);"post"==a&&Cloudwalkers.RootView.compose();"writenote"==a&&this.writenote()},fit:function(){$("#header").html(this.renderHeader().header);$("#sidebar").html(this.render().el);$("#header, #sidebar").on("click","*[data-header-action]",this.headeraction.bind(this))},version:function(a){if("TESTING"==a.platform.name||
"DEVELOPMENT"==a.platform.name)this.development=!0,$("#header").html(this.renderHeader().header)},renderHeader:function(){var a={dev:this.development};a.user=Cloudwalkers.Session.user.attributes;a.accounts=[];a.level=Cloudwalkers.Session.getUser().level;this.mustacheTranslateRenderHeader(a);Cloudwalkers.Session.user.accounts.each(function(b){a.accounts.push({name:b.get("name"),id:b.id,active:b.id==Cloudwalkers.Session.get("currentAccount")})});this.header=Mustache.render(Templates.header,a);return this},
render:function(){var a=Cloudwalkers.Session.getAccount(),b={reports:[]};b.level=Cloudwalkers.Session.getUser().level;this.mustacheTranslateRender(b);if(b.level){b.news=a.channels.findWhere({type:"news"}).id;var c=a.channels.findWhere({type:"monitoring"});b.monitoring={channelid:c.id,first:c.channels.models[0],channels:c.channels.models,name:c.get("name")}}b.scheduled={channelid:Cloudwalkers.Session.getChannel("internal").id};b.scheduled.streams=a.streams.where({outgoing:1});b.inbox=!0;c=a.channels.findWhere({type:"profiles"});
b.profiles={channelid:c.id,streams:c.streams.models,name:c.get("name")};b.reports=a.streams.where({statistics:1}).map(function(a){return a.attributes});Cloudwalkers.Session.censuretemplate(b);this.$el.html(Mustache.render(Templates.navigation,b));this.handleSidebarMenu();return this},handleSidebarMenu:function(){var a=Backbone.history.fragment;if(!a)return null;this.setActive(a)},compose:function(){Cloudwalkers.RootView.compose()},writenote:function(){var a=Cloudwalkers.Session.getAccount();Cloudwalkers.RootView.writeNote(a)},
setActive:function(a){$("#sidebar .active").removeClass("active");$('a[href="#'+a+'"]').parents("#sidebar .page-sidebar-menu *").addClass("active")},setNotifications:function(a){this.$el.find(".popup-frame ul").html("");for(var b=0;b<a.length;b++)this.addNotification(a[b])},addNotification:function(a){this.$el.find(".popup-frame ul").append('<li><div class="text account"><p>'+a.message+'</p><div class="row"><span class="time"></span></div></div></li>')},toggleNotifications:function(){this.$el.find(".notification-popup").toggle()},
mapViews:function(){var a={};for(n in this.views)if(a[this.views[n].name]={streams:[]},this.views[n].children)for(i in this.views[n].children)a[this.views[n].children[i].name]={streams:[]};return a},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRenderHeader:function(a){this.original=["clean","profile_settings","log_out"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]},
mustacheTranslateRender:function(a){this.original="compose dashboard message_board compose_message drafts scheduled inbox messages notifications post_message calendar co-workers_wall company_accounts media trending_posts accounts_we_follow keyword_monitoring manage_accounts manage_keywords reports settings manage_users social_connections account_settings profile_settings".split(" ");this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+
this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Compose=Backbone.View.extend({id:"compose",className:"modal hide",type:"post",network:"default",actionstreams:[],actionview:!1,minutestep:5,titles:{post:"Write Post",edit:"Edit Post",share:"Share message",comment:"Write Comment",reply:"Write Reply",dm:"Write Message",retweet:"Retweet",like:"Like",favourite:"Favourite",plusone:"Plus one","default":"Compose"},limitations:{},options:{"false":["editor","images","campaign","schedule","repeat"],twitter:["editor","images","campaign","schedule",
...
return this},unfollow:function(){this.model.trigger("unfollow",this.model);this.model.parent=Cloudwalkers.Session.getAccount();this.model.save({following:!1},{patch:!0});this.remove()}});Cloudwalkers.Views.ViewContact=Backbone.View.extend({id:"compose",className:"modal hide viewcontact",entries:[],contactid:1583880,events:{"click .end-preview td i":"backtolist","click #contactfilter li":"loadmessages","click [data-action=write-note]":"togglecontactnote","click #post":"post"},initialize:function(a){a&&$.extend(this,a);if(this.contact=a.contact?a.contact.model:null)this.model=new Cloudwalkers.Models.Contact(this.contact);!this.model&&this.contactid&&(this.model=new Cloudwalkers.Models.Contact({id:this.contactid}));
this.loadmylisteners()},loadmylisteners:function(a){"note"==this.type?(this.collection=new Cloudwalkers.Collections.Notes,this.collection.parentmodel="contact",this.collection.parenttype="contact"):this.collection=new Cloudwalkers.Collections.Messages;a&&this.stopListening(this.collection);this.listenTo(this.collection,"seed",this.fill);this.loadListeners(this.collection,["request","sync",["ready","loaded"]],!0)},render:function(){var a=Mustache.render(Templates.viewcontact,this.contactinfo);this.$el.html(a);
this.$container=this.$el.find("ul.list");this.$loadercontainer=this.$el.find("ul.list");this.trigger("rendered");this.updatecontactinfo();this.initializenote();this.begintouch();return this},fill:function(a){var b,c="note"==this.type?"smallentrynote":"smallentry";$.each(this.entries,function(a,b){b.remove()});this.entries=[];for(n in a)b=a[n],b.attributes.arrow="arrow",b=new Cloudwalkers.Views.Entry({model:b,template:c,checkunread:!0,parameters:{inboxview:!0,notes:!0}}),this.entries.push(b),this.listenTo(b,
"toggle",this.loadmessage),this.$container.append(b.render().el)},seed:function(a,b){this.collection.models=b.contact.messages;this.collection.length=this.collection.models.length;this.collection.trigger("seed",this.collection.models)},begintouch:function(a){a||(a="messages");this.type=a;this.loadmylisteners(!0);"messages"==a?this.getmessages():this.touch(a)},getmessages:function(){this.model.urlparams=["messages"];this.collection.url=this.model.url();this.collection.parenttype="contact";this.collection.parse=
this.parse;this.collection.fetch({success:this.seed.bind(this)})},parse:function(a){this.parenttype&&(a=a[this.parenttype]);if(a[this.typestring])for(n in a[this.typestring])a[this.typestring][n]=new Cloudwalkers.Models.Message(a[this.typestring][n]),a[this.typestring][n].generateintro(),a[this.typestring][n].set("read",1);a.paging||this.ready();return a[this.typestring]},touch:function(a){this.model.urlparams=[a+"ids"];this.model.parameters=!1;this.collection.url=this.model.url();this.collection.fetch({success:this.touchresponse.bind(this)})},
touchresponse:function(a,b){var c=b.contact[this.collection.typestring];this.model.urlparams=[this.type+"s"];this.model.parameters={ids:c.join(",")};this.collection.url=this.model.url();this.collection.seed(c)},updatecontactinfo:function(){var a=this.model.attributes;a&&(this.$el.find("aside").html(Mustache.render(Templates.viewcontactaside,a)),setTimeout(function(){this.$el.find("aside").removeClass("nodata")}.bind(this),1),this.hascontactinfo=!0)},initializenote:function(){var a=new Cloudwalkers.Views.ComposeNote({model:this.model,
persistent:!0});this.$el.find("#notecontainer").append(a.render().el);this.listenTo(a.note,"sync",this.doneposting.bind(this,200));this.listenTo(a,"edit:cancel",this.doneposting)},doneposting:function(a){a||(a=0);setTimeout(function(){this.togglecontactnote();this.$el.find("#notecontainer textarea").val("")}.bind(this),a)},loadmessages:function(a){this.backtolist();var b=$(a.currentTarget).attr("data-type");$(a.currentTarget).hasClass("inactive")&&(this.$el.find("#contactfilter .active").removeClass("active").addClass("inactive"),
$(a.currentTarget).removeClass("inactive").addClass("active"),this.begintouch(b))},loadmessage:function(a){var b={model:a.model,notes:a.model.id?!0:!1};"note"==this.type&&(b.template="note");$(".viewcontact").addClass("onmessage");this.$loadercontainer=$(".inbox-container");this.inboxmessage&&this.inboxmessage.remove();this.inboxmessage=new Cloudwalkers.Views.Widgets.InboxMessage(b);this.type&&"messages"!=this.type&&this.loadListeners(this.inboxmessage,["request","sync",["ready","loaded"]],!0);$(".inbox-container").html(this.inboxmessage.render().el);
this.type&&"conversation"==this.type?this.inboxmessage.showrelated():this.type&&"note"==this.type&&this.listenTo(this.inboxmessage.model,"destroy",this.backtolist);this.$el.find(".list .active").removeClass("active");a.$el.addClass("active")},backtolist:function(){$(".viewcontact").removeClass("onmessage");this.$loadercontainer=$("ul.list")},togglecontactnote:function(){$(".viewcontact").hasClass("writenote")?($(".viewcontact").removeClass("writenote"),setTimeout(function(){$(".contactbottom").slideDown("fast")},
100)):($(".contactbottom").slideUp("fast"),setTimeout(function(){$(".viewcontact").addClass("writenote")},100))},filterparams:function(){return{records:20,contacts:this.contactid}}});Cloudwalkers.Views.Editor=Backbone.View.extend({id:"editor",className:"clearfix",oldUrl:"",urldata:{},currenturl:!1,urlprocessing:!1,limit:null,content:"",charlength:0,contentlimitation:0,pos:0,urls:{},posmap:[],restrictedstreams:{twitter:140,linkedin:700},events:{"update:content":"render","update:limit":"renderlimit","append:content":"append","click #swaplink":"swaplink",'click [data-type="title"] .addcontent':"addoecontent",'click [data-type="content"] .addcontent':"addoecontent",'click [data-type="image"] i':"addoeimg",
...
mustacheTranslateRender:function(a){this.original=["load_more","co-workers","search_co-workers"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.MonitorFilters=Cloudwalkers.Views.Widgets.Widget.extend({events:{"click [data-network-streams]":"filternetworks","click [data-keyword-id]":"filterkeywords","click .toggleall.networks.active":"toggleallnetworks","click .toggleall.keywords.active":"toggleallkeywords"},initialize:function(){this.category=this.options.category;this.category.childtype="message";this.streams=[];this.category.channels.each(function(a){this.streams=this.streams.concat(a.streams.models)},this);this.initializeWidget()},
...
this.filters.contacts.list.join(","));this.filters.streams.length&&(a.streams=this.filters.streams.join(","));this.storeview();return a},storeview:function(){var a=Cloudwalkers.Session.viewsettings(this.collectionstring);a&&(a.streams||(a.streams=[]),JSON.stringify(a.streams)!=JSON.stringify(this.filters.streams)&&(a.streams=this.filters.streams,Cloudwalkers.Session.viewsettings(this.collectionstring,a)))},more:function(){this.incremental=!0;this.collection.more(this.model,this.filterparameters())||
this.$el.find(".load-more").hide()},negotiateFunctionalities:function(){this.listenTo(Cloudwalkers.Session,"destroy:view",this.remove);this.addScroll()},addScroll:function(){this.$el.find(".scroller").slimScroll({height:"inherit"})},destroy:function(){$.each(this.entries,function(a,b){b.remove()})},isempty:function(){$(".inbox-container").empty().addClass("empty-content")},unsetempty:function(){$(".inbox-container").removeClass("empty-content")},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},
mustacheTranslateRender:function(a){this.original="networks more contacts filters search_contacts suggestions select_all load_more".split(" ");this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.InboxNotificationList=Cloudwalkers.Views.Widgets.InboxMessageList.extend({collectionstring:"notifications",check:"hasNotifications",toggle:function(a){this.inboxmessage&&this.inboxmessage.remove();a.model.get("parent")?this.showmessage(a.model):this.listenToOnce(a.model,"change",this.showmessage);this.$el.find(".list .active").removeClass("active");a.$el.addClass("active").removeClass("unread")},showmessage:function(a){var b=Cloudwalkers.Session.getMessage(a.get("parent").id);
b||(b=new Cloudwalkers.Models.Message({id:a.get("parent").id}));this.model.messages.add(b);b.get("objectType")||b.fetch();this.inboxmessage=new Cloudwalkers.Views.Widgets.InboxMessage({model:b,notification:a});$(".inbox-container").html(this.inboxmessage.render().el)}});Cloudwalkers.Views.Widgets.InboxMessage=Cloudwalkers.Views.Entry.extend({tagName:"div",className:"message social-box-colors",template:"inboxmessage",related:[],messageview:[],notifications:[],events:{remove:"destroy","click *[data-youtube]":"loadYoutube","click *[data-action]":"action"},render:function(){this.loading(!this.model.get("objectType"));if(this.options.notification)var a={from:this.options.notification.get("from")[0],timeago:moment(this.options.notification.get("date")).fromNow()};a={commented:a};
$.extend(a,this.model.attributes);this.model.filterActions&&$.extend(a,{actions:this.model.filterActions()});this.notes&&(a.notes=!0);this.$el.html(Mustache.render(Templates[this.template],a));this.notes&&this.loadnoteui();this.time();this.options.notification&&this.model.get("objectType")&&this.addNotifications();this.model.get("objectType")&&!this.model.get("read")&&this.markasread();return this},loading:function(a){a?$(".inbox").addClass("loading"):$(".inbox").removeClass("loading")},showrelated:function(){this.loading(!0);
this.$related=$("<ul></ul>");this.$prelated=$("<ul></ul>");this.$el.before(this.$prelated.addClass("related-messages social-box-colors"));this.$el.after(this.$related.addClass("related-messages social-box-colors"));this.model.related||(this.model.related=new Cloudwalkers.Collections.Related);this.listenTo(this.model.related,"seed",this.fillrelated);this.listenTo(this.model.related,"all",this.sendtriggers);this.model.related.touch(this.model,{records:20,markasread:!0})},fillrelated:function(a){this.incremental?
this.incremental=!1:($.each(this.related,function(a,b){b.remove()}),this.related=[]);var b=!1;a=a.filter(function(a){a.id==this.model.id&&(b=!0);a.append=b;return a.id!=this.model.id}.bind(this));for(n in a){var c=new Cloudwalkers.Views.Entry({model:a[n],template:"inboxrelatedmessage",type:"full",parameters:{notes:this.notes}});this.related.push(c);this[a[n].append?"$related":"$prelated"].append(c.render().el)}this.loading(!1)},sendtriggers:function(a){this.trigger(a)},addNotifications:function(){this.loading(!0);
this.model.notifications||(this.model.notifications=new Cloudwalkers.Collections.Notifications);this.listenTo(this.model.notifications,"seed",this.fillNotifications);this.model.notifications.touch(this.model,{records:50,markasread:!0})},fillNotifications:function(a){this.notifications.length&&($.each(this.notifications,function(a,b){b.remove()}),this.notifications=[]);a.models&&(a=a.models);var b=this.$el.find(".message-comments ul").eq(0).html("");for(n in a){var c=new Cloudwalkers.Views.Notification({model:a[n],
template:"inboxcomment",active:a[n].id==this.options.notification.id});this.notifications.push(c);b.append(c.render().el)}this.loading(!1)},markasread:function(){this.model.save({read:1},{patch:!0,wait:!0});Cloudwalkers.Session.getStreams().outdated(this.model.get("stream"))},destroy:function(){$.each(this.modelview,function(a,b){b.remove()});$.each(this.notifications,function(a,b){b.remove()});$.each(this.related,function(a,b){b.remove()})}});Cloudwalkers.Views.Widgets.NoteEntry=Cloudwalkers.Views.Entry.extend({tagName:"li",template:"messagenote",events:{mouseover:"toggleactions",mouseout:"toggleactions","click *[data-notification-action]":"action","click .note-header":"togglenote"},initialize:function(a){this.parameters={};a&&$.extend(this,a);this.listenToOnce(this.model,"change",this.render);this.listenTo(this.model,"destroy",this.remove)},render:function(){this.model.attributes.date=moment(this.model.attributes.date).format("DD MMM YYYY");
...
this.mustacheTranslateRender(c);this.$el.html(Mustache.render(Templates.settings.profile,c));this.$el.find(".collapse-closed, .collapse-open").each(function(){b.negotiateFunctionalities(this)});this.$el.find("option[value="+a.get("locale")+"]").attr("selected",!0);return this},editUserProfile:function(a){a=Cloudwalkers.Session.getUser();var b=this.$el.find("[name=firstname]").val(),c=this.$el.find("[name=name]").val(),e=this.$el.find("[name=mobile]").val(),f=this.$el.find("[name=locale]").val();a.save({firstname:b,
name:c,mobile:e,locale:f},{patch:!0,success:function(){Cloudwalkers.RootView.growl("User Profile","Your profile settings are updated");window.location.reload()}})},addfile:function(a){$(".settings-profile input[type=file]").click()},listentofile:function(a){var b=this;a=a.target.files;for(var c=0,e;e=a[c];c++){if(!e.type.match("image.*"))return Cloudwalkers.RootView.information("Wrong file","You need a valid image.",this.$el.find(".settings-profile .portlet-body"));var f=new FileReader;f.onload=function(a){return function(a){b.addimage(a.target.result)}}(e);
f.readAsDataURL(e)}},addimage:function(a){this.base64data=a;this.$el.find("div.avatar-big").css("background-image","url("+a+")")},uploadfile:function(){this.base64data?Cloudwalkers.Session.getUser().save({avatar:this.base64data},{patch:!0,success:function(){Cloudwalkers.RootView.growl("User Profile","You have a new profile picture.")}}):Cloudwalkers.RootView.information("No image","Select an image file first.",this.$el.find(".settings-profile .portlet-body"))},editUserPassword:function(){var a=this.$el.find("[name=pass0]").val(),
b=this.$el.find("[name=pass1]").val();if(b!=this.$el.find("[name=pass2]").val())return Cloudwalkers.RootView.growl("Oops","Please re-type your new password."),null;Cloudwalkers.Session.getUser().save({oldpassword:a,newpassword:b},{patch:!0,endpoint:"password",success:function(){Cloudwalkers.RootView.growl("User Profile","You have a new password now.")},error:function(a,b,f){b=b.responseJSON.error.message;Cloudwalkers.RootView.growl("Oops",b)}})},negotiateFunctionalities:function(a){$(a).find(".portlet-title").on("click",
function(){$(this).parents(".collapse-closed, .collapse-open").toggleClass("collapse-closed collapse-open")})},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=["profile_type"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Settings.Account=Backbone.View.extend({events:{"submit .edit-account":"editAccount","click i[data-delete-campaign-id]":"deleteCampaign"},render:function(){var a=Cloudwalkers.Session.getAccount().attributes;this.mustacheTranslateRender(a);Cloudwalkers.Session.censuretemplate(a);this.$el.html(Mustache.render(Templates.settings.account,a));return this},editAccount:function(a){a=Cloudwalkers.Session.getAccount();var b=this.$el.find("[name=name]").val();a.save({name:b},{patch:!0,success:function(){Cloudwalkers.RootView.growl("Account settings",
