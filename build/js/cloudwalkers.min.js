...
this.trigger("imageadded",{type:"image",url:a,name:a})},clearselections:function(){window.getSelection?window.getSelection().empty?window.getSelection().empty():window.getSelection():document.selection&&document.selection.empty()},monitorlimit:function(){this.charlength=this.$contenteditable.text().length;this.limit&&this.updatecounter(this.limit-this.charlength)},greyout:function(a,b){var c=this.win.getSelection(),e=this.document.createRange(),f=this.cursorpos(),g,h,l=0,k=this.$contenteditable.find("short");
this.content.length>=this.$contenteditable.text().length&&this.removegrey();e.selectNodeContents(this.$contenteditable.get(0));g=this.cursorpos(this.$contenteditable.text().length,!0);h=this.cursorpos(this.limit,!0);if(h.node){if(b)return;k.removeClass("red");this.isurl(h.node)&&($(h.node.parentNode).addClass("red"),h.node=h.node.parentNode.nextSibling,h.offset=0,l++);e.setStart(h.node,h.offset);e.setEnd(g.node,g.offset);c.removeAllRanges();c.addRange(e);$.each(k,function(a,b){c.containsNode(b)&&
l++})}this.contenteditable.designMode="on";document.execCommand("backColor",!1,this.limit?"#fcc":"#fff");this.contenteditable.designMode="off";if(l)for(;0<l;)$(k[k.length-l]).addClass("red"),l--;this.cursorpos(this.$contenteditable.text().length);this.mergechars();this.cursorpos(f)},isurl:function(a){result=!1;$.each(this.urls,function(b,c){if(c.get("shortUrl")==a.textContent||b==a.textContent)return result=!0,!1});return result},updatecounter:function(a){var b=this.$el.find(".limit-counter");20<=
