...
this.model.get("messages")?this.model.get("messages")[0]:!1)&&0!=this.message.length){var a=this.message,b=a.attachments?a.attachments.filter(function(a){if("link"==a.type)return a}):null,c=a.attachments?a.attachments.filter(function(a){if("image"==a.type)return a}):null;this.settings.statistics=a.statistics;this.settings.from=a.from[0].displayname||"";this.settings.body=a.body.plaintext||"";this.settings.icon=a.from[0].network.icon||"";this.settings.date=a.dateonly||"";c.length&&(this.settings.image=
c[0].url||"");this.settings.links=b||[];this.render()}else this.$el.find(".commentballoon h4").html("No data"),this.hideloading()},gettoptrending:function(){return this.parentview.streamid?this.toptrendingstream(this.parentview.streamid):this.toptrendingall()},toptrendingstream:function(a){this.model.fetch({endpoint:"messages",parameters:{sort:this.timespan.sort,records:1}})},toptrendingall:function(){this.model.fetch({endpoint:"messages",parameters:{sort:"engagement",records:1,since:this.timespan.since}})},
