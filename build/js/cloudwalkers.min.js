...
"changeDate input":"monitorschedule","click [data-set=on] .btn-white":"togglebesttime","click .end-preview":"endpreview","click #previewbtn":"preview","click #save":"save","click #post":"post","click #action, [data-type=icon] > i":"postaction"},initialize:function(a){a&&$.extend(this,a);this.streams=Cloudwalkers.Session.getChannel("outgoing").streams;this.action&&(this.type=this.action.get("token"));this.action&&"share"==this.type?this.draft=this.reference.cloneSanitized():this.action&&this.reference?
(this.action.parent=this.reference,this.actionstreams=[],this.actionview=!0,this.listenTo(this.action,"change",this.editstreams),this.action.fetch()):this.model&&(this.type="edit",this.state="loading",this.draft=this.model.clone(),this.draft.fetch({endpoint:"original",success:this.original.bind(this)}));this.draft||(this.draft=new Cloudwalkers.Models.Message({variations:[],attachments:[],streams:[],body:{},schedule:{}}),this.listenTo(this.draft,"invalid",this.invalid));this.loadListeners(this.draft,
["request","sync"],!0);"reply"==this.type&&"twitter"==this.reference.get("networktoken")&&this.draft.set("body",{html:Mustache.render(this.action.parameters[0].value,{from:this.reference.get("from")[0]})})},original:function(){var a;this.draft.get("schedule")&&this.draft.get("schedule").date&&(this.draft.get("schedule").date=moment(this.draft.get("schedule").date).unix());(a=this.draft.get("variations"))&&$.each(a,function(a,c){c.schedule&&(c.schedule.date=moment(c.schedule.date).unix())});this.state=
"loaded";this.render()},render:function(){var a={streams:this.actionstreams.length?this.actionstreams:this.streams.models,title:this.titles[this.type],campaigns:Cloudwalkers.Session.getAccount().get("campaigns"),actionview:this.actionview?this.type:!1};"edit"==this.type&&(a.type=this.type);a=Mustache.render(Templates.compose,a);this.$el.html(a);this.editor=new Cloudwalkers.Views.Editor({draft:this.draft,parent:this});this.$el.find("[data-type=post]").append(this.editor.render().el);this.listenTo(this.editor,
"imageadded",this.addimage);this.listenTo(this.editor,"change:content",this.monitor);this.$el.find(".campaign-list").chosen({width:"50%"});this.$el.find("#delay-date, #repeat-until").datepicker({format:"dd-mm-yyyy",weekStart:1});this.datepicker=this.$el.find("#delay-date, #repeat-until").datepicker({format:"dd-mm-yyyy",orientation:"auto"});this.timepicker=this.$el.find("#delay-time").timepicker({template:"dropdown",minuteStep:5,showMeridian:!1});this.$loadercontainer=this.$el.find(".modal-footer");
...
b.indexOf("repeat"));this.trigger("update:stream",{id:f,data:this.draft.getvariation(f,"body")});this.updatesubject();this.updateimages();this.summarizelink();this.summarizeschedule()},updatesubject:function(){var a=this.activestream?this.activestream.id:!1,b=this.$el.find("[data-option=subject] > input").empty(),c;a&&this.draft.getvariation(a,"subject")?c=this.draft.getvariation(a,"subject"):this.draft.get("subject")&&(c=this.draft.get("subject")||"");b.val(c)},addfile:function(a){$("[data-collapsable=images] input").click()},
...
return this},parsemoment:function(a,b){var c=$(a),e=$(b);if(!c.val()||!e.val())return null;c=moment(c.val(),"DD-MM-YYYY DD-MM-YY DD/MM/YYYY DD/MM/YY DDMMYYYY YYYYMMDD MM-DD-YYYY MM-DD-YY".split(" "));e=e.val().split(":");c=_.clone(c);1<e.length&&c.add("minutes",60*Number(e[0])+Number(e[1]));return c.isValid()?c.unix()<moment().unix()?void 0:c:void 0},monitorschedule:function(a,b){var c=b||$(a.currentTarget);this.activestream&&!this.draft.getvariation(this.activestream.id,"schedule")&&this.draft.setvariation(this.activestream.id,
"schedule",{});var e=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,e=e?e:this.draft.get("schedule"),c=(c.data("set")?c:c.parents("[data-set]").eq(0)).data("set");"now"==c?(this.toggleschedentry("[data-set=in], [data-set=on]").toggleschedentry("[data-set=now]",!0),$("[data-set=on] input").val(""),$("[data-set=in] select").val(600),this.draft.set("schedule",{repeat:e.repeat||{}})):"in"==c?(this.toggleschedentry("[data-set=now], [data-set=on]").toggleschedentry("[data-set=in]",
!0),$("[data-set=on] input").val("")):"on"==c?(this.toggleschedentry("[data-set=now], [data-set=in]").toggleschedentry("[data-set=on]",!0),$("[data-set=in] select").val(600),void 0===this.parsemoment("#delay-date","#delay-time")&&(this.datepicker.datepicker("hide").val(""),Cloudwalkers.RootView.alert("Please set your Schedule to a date in the future"))):"onlyonce"==c?(this.toggleschedentry("[data-set=onlyonce]",!0).toggleschedentry("[data-set=repeat], [data-set=until], [data-set=every]",!1),$("#repeat-interval, #repeat-amount").val(0),
$("#repeat-until").val("")):"every"==c?this.toggleschedentry("[data-set=every]",!0).toggleschedentry("[data-set=onlyonce]",!1):"repeat"==c?(this.toggleschedentry("[data-set=every], [data-set=repeat]",!0).toggleschedentry("[data-set=onlyonce], [data-set=until]",!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),Number($("#repeat-amount").val())||$("#repeat-amount").val(1),$("#repeat-until").val("")):"until"==c&&(this.toggleschedentry("[data-set=every], [data-set=until]",!0).toggleschedentry("[data-set=onlyonce], [data-set=repeat]",
!1),Number($("#repeat-interval").val())||$("#repeat-interval").val(1),$("#repeat-amount").val(0));this.parsescheduled();a.stopPropagation();return this},parsescheduled:function(){var a=this.activestream?this.draft.getvariation(this.activestream.id,"schedule"):!1,a=a?a:this.draft.get("schedule"),b=this.$el.find("section[data-collapsable] .schedule-entry").not(".inactive").find("#delay-select, #delay-date, #delay-time, #repeat-interval, #every-select, #every-select-weekday, #repeat-amount, #repeat-until");
...
'click [data-type="content"] .addcontent':"addoecontent",'click [data-type="image"] i':"addoeimg","mouseup #compose-content":"savepos"},xtrimmable:/<[^>]*>|&nbsp;|\s/g,xurlbasic:/http|[w]{3}/g,xurlbasic:/(^>)(http|[w]{3})/g,xurlpattern:/(^|\s|\r|\n|\u00a0)((https?:\/\/|[w]{3})?[\w-]+(\.[\w-]+)+\.?(:\d+)?(\/\S*)?)(\s|\r|\n|\u00a0)/g,xurlendpattern:/(^|\s|\r|\n|\u00a0)((https?:\/\/|[w]{3})?[\w-]+(\.[\w-]+)+\.?(:\d+)?(\/\S*)?)(\s|\r|\n|\u00a0)/g,initialize:function(a){a&&$.extend(this,a);this.listenTo(Cloudwalkers.Session.UrlShortener,
>>>>>>> 41406c44491f2f02ed11d33235dfb1a2beee3b3c
"sync",this.parseurl);this.listenTo(this.parent,"update:stream",function(a){this.togglecontent(a,!0)}.bind(this));this.listenTo(this.parent,"update:campaign",this.campaignupdated);this.on("change:charlength",this.greyout);navigator.userAgent.match(/(firefox(?=\/))\/?\s*(\d+)/i)&&(this.isfirefox=!0)},render:function(a){void 0!==a&&(this.content=a);a=Math.min.apply(Math,_.values(this.restrictedstreams));this.restrictedstreams["default"]=a;this.$el.html(Mustache.render(Templates.editor,{limit:a}));this.limit=
a;this.$contenteditable=this.$el.find("#compose-content").eq(0);this.contenteditable=this.$contenteditable.get(0);this.document=this.contenteditable.ownerDocument||this.contenteditable.document;this.win=document.defaultView||document.parentWindow;this.$contenteditable.html(this.content);this.content&&this.listentochange();return this},listentopaste:function(a){a.preventDefault();if(a.originalEvent||a.originalEvent.clipboardData)a=a.originalEvent.clipboardData.getData("text/plain"),document.execCommand("insertHTML",
!1,a);this.listentochange()},listentochange:function(a,b){if(this.content!==this.$contenteditable.text()||b)this.pos=this.cursorpos(),!this.currenturl&&this.listentourl(this.$contenteditable.text())&&this.processurl(),this.charlength!=this.$contenteditable.text().length&&this.trigger("change:charlength",this.charlength=this.$contenteditable.text().length);this.content=this.$contenteditable.text();this.isdefault&&!b&&(this.$contenteditable.removeClass("withdefault"),this.isdefault=!1);this.trigger("change:content",
this.content);b&&this.clearselections()},endchange:function(a){this.$contenteditable.html().match(this.xurlendpattern)&&this.$contenteditable.append(" \u200b\u200b");this.listentochange(a,!0)},listentourl:function(a,b){var c=this.win.getSelection(),e=this.document.createRange(),f,g,h;e.selectNodeContents(this.$contenteditable.get(0));f=this.parsenodes(e.startContainer.childNodes);if(this.currenturl)return g=f.textContent.replace(/&nbsp;/gi,""),g=g.indexOf(this.currenturl),h=g+this.currenturl.length,
e.setStart(f,g),e.setEnd(f,h),c.removeAllRanges(),c.addRange(e),this.contenteditable.designMode="on",document.execCommand("createLink",!1,this.currenturl),this.contenteditable.designMode="off",f=document.createTextNode(" \u200b\u200b"),e.insertNode(f),this.$contenteditable.find("a").after(f),e.setStartAfter(f),c.removeAllRanges(),c.addRange(e),!0},parsenodes:function(a){var b=null;$.each(a,function(c,e){var f=!1,g=e.textContent;g&&(f=g.match(this.xurlpattern));a.length==c+1&&(f=g.match(this.xurlendpattern));
if(f)return b=3==e.nodeType?e:this.getnode(e,null,3),this.currenturl=f[0],!0}.bind(this));return b},processurl:function(){this.$contenteditable.find("a").attr("contenteditable",!1);this.filterurl(this.currenturl)},filterurl:function(a){var b=a.match(this.xurlpattern),c=this.draft.get("campaign"),e=Cloudwalkers.Session.getAccount().get("campaigns").filter(function(a){if(a.id==c)return a}),e=e.length?e[0].name:null;b&&!this.urlprocessing&&(this.urlprocessing=!0,this.urls=b,this.shortenurls(b,e));return a},
shortenurls:function(a,b){var c={error:this.releaseurlprocessing};a.forEach(function(a){c.q=a.trim();c.campaign=b;Cloudwalkers.Session.UrlShortener.fetch(c)})},parseurl:function(a){var b=" <short contenteditable='false' data-url='"+a.longurl+"'>"+a.get("shortUrl")+"<i class='icon-link' id='swaplink'></i></short>",c=this;this.currenturl=a.longurl;this.shorturl=a.get("shortUrl");this.$contenteditable.find("a").replaceWith(b);this.trigger("change:content",this.content);a='<div><a href="'+this.currenturl+
'" class="oembed"></a></div>';this.$el.find("#out").empty().html(a);this.$el.find(".oembed").oembed(null,null,this.embed).each(function(){this.def.done(function(){c.$el.find("#out").addClass("expanded")})})},releaseurlprocessing:function(){this.urlprocessing=!1},swaplink:function(){var a=this.$contenteditable.find("short").data("url").replace(/\s/g,"")==this.currenturl.replace(/\s/g,""),b="<short contenteditable='false' data-url='"+this.shorturl+"'>"+this.currenturl+"<i class='icon-link' id='swaplink'></i></short>",
