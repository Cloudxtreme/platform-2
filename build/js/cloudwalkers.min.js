...
var c=b?null:Number(this.button.data("streams"));b&&(this.button=!1);c?($('[data-networks~="'+c+'"]').removeClass("inactive").addClass("active"),this.filters.streams=[c]):this.filters.streams=[];this.collection.touch(this.model,this.filterparameters());return this},filterparameters:function(){var a="notes"==this.listtype?{all:1}:{records:20,group:1};this.filters.contacts.list.length&&(a.contacts=this.filters.contacts.list.join(","));this.filters.streams.length&&(a.streams=this.filters.streams.join(","));
this.storeview();return a},storeview:function(){var a=Cloudwalkers.Session.viewsettings(this.collectionstring);a&&(a.streams||(a.streams=[]),JSON.stringify(a.streams)!=JSON.stringify(this.filters.streams)&&(a.streams=this.filters.streams,Cloudwalkers.Session.viewsettings(this.collectionstring,a)))},more:function(){this.incremental=!0;this.loadmore.loadmylisteners();this.collection.more(this.model,this.filterparameters())||this.$el.find(".load-more").hide()},negotiateFunctionalities:function(){this.listenTo(Cloudwalkers.Session,
"destroy:view",this.remove);this.addScroll()},addScroll:function(){this.$el.find(".scroller").slimScroll({height:"inherit"})},destroy:function(){$.each(this.entries,function(a,b){b.remove()})},isempty:function(){$(".inbox-container").empty().addClass("empty-content")},unsetempty:function(){$(".inbox-container").removeClass("empty-content")},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original="networks more contacts filters search_contacts suggestions select_all load_more on".split(" ");
this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Widgets.InboxNotificationList=Cloudwalkers.Views.Widgets.InboxMessageList.extend({collectionstring:"notifications",check:"hasNotifications",toggle:function(a){this.inboxmessage&&this.inboxmessage.remove();a.model.get("parent")?this.showmessage(a.model):this.listenToOnce(a.model,"change",this.showmessage);this.$el.find(".list .active").removeClass("active");a.$el.addClass("active").removeClass("unread")},showmessage:function(a){var b=Cloudwalkers.Session.getMessage(a.get("parent").id);
