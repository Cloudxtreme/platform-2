...
this.options));this.$container=this.$el.find("ul.messages-container.messages-list").eq(0);this.fill(this.collection);this.updatecollection();return this},fill:function(a){this.$container.empty();for(n in a.models){var b=new Cloudwalkers.Views.CounterEntry({model:a.models[n],data:this.options});console.log(this.$container);this.$container.append(b.render().el)}},updatecollection:function(){this.listenToOnce(this.collection,"sync",this.fill);"streams"==this.options.source&&(this.options.channel.endpoint=
"/streams",this.collection.url=this.options.channel.url(),this.collection.fetch({remove:!1}))},updatesettings:function(a){if("streams"!=this.options.source&&"outgoing"!=this.options.source)return null;a=this.collection.get($(a.currentTarget).data("stream"));var b=a.get("childtypes")[0]+"s";"outgoing"==this.options.source&&(b="scheduled");var c=Cloudwalkers.Session.viewsettings(b);c.streams=[a.id];Cloudwalkers.Session.viewsettings(b,c)},negotiateFunctionalities:function(){this.$el.find(".scroller").length&&
