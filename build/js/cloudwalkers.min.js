...
!1;e.showStreamName=!1;this.datepicker.on("date:change",function(a,b){e.getDataset().setDateRange(a,b)});this.add(e)}});Cloudwalkers.Views.Statistics=Cloudwalkers.Views.Pageview.extend({id:"statistics",start:0,end:0,timespan:"week",period:0,custom:!1,views:[],events:{remove:"destroy","click #add":"addperiod","click #subtract":"subtractperiod","click #subtractempty":"subtractempty","click #addempty":"addempty","click #now":"now","click #show":"changecustom","change .stats-header select.networks":"changestream","change .stats-header select.time":"changespan","click .dashboard-stat":"updatenetwork"},widgets:[{widget:"StatSummary",
data:{columnviews:["contacts","score-trending","outgoing","coworkers"]},span:12},{widget:"TitleSeparator",data:{translation:{title:"contacts_info"}}},{widget:"Chart",data:{filterfunc:"contacts",chart:"PieChart",translation:{title:"contacts"},display:"divided"},span:6},{widget:"CompoundChart",span:6,data:{template:"2col1row",chartdata:[{widget:"Chart",data:{filterfunc:"age",chart:"PieChart",translation:{title:"by_age"}}},{widget:"Chart",data:{filterfunc:"gender",chart:"PieChart",translation:{title:"by_gender"}}},
{widget:"Chart",data:{filterfunc:"contact-evolution",chart:"LineChart",translation:{title:"contacts_evolution"}}}]}},{widget:"TitleSeparator",data:{translation:{title:"new_this"}}},{widget:"Info",data:{translation:{title:"contact_evolution"},filterfunc:"contact-evolution"},span:3},{widget:"Info",data:{translation:{title:"post_activity"},filterfunc:"post-activity"},span:3},{widget:"Info",data:{title:"Activity?",filterfunc:"activity"},span:3},{widget:"Info",data:{title:"Page Views?",filterfunc:"page-views"},
span:3},{widget:"TitleSeparator",data:{translation:{title:"messages_info"}}},{widget:"TrendingMessage",data:{translation:{title:"top_rated_comment"}},span:12},{widget:"BestTimeToPost",data:{filterfunc:"besttime",chart:"LineChart",translation:{title:"best_time_to_post"}},span:4},{widget:"Chart",data:{filterfunc:"message-evolution",chart:"LineChart",translation:{title:"messages_evolution"}},span:4},{widget:"HeatCalendar",data:{filterfunc:"activity",translation:{title:"activity_calendar"}},span:4},{widget:"TitleSeparator",
data:{translation:{title:"geo_graphics"}}},{widget:"Chart",data:{filterfunc:"geo",type:"dots",chart:"GeoChart",translation:{title:"countries"},connect:!0},span:8},{widget:"CompoundChart",span:4,data:{template:"2row",chartdata:[{widget:"Chart",data:{filterfunc:"regional",chart:"PieChart",translation:{title:"countries"}},connect:"regional"},{widget:"Chart",data:{filterfunc:"cities",chart:"PieChart",translation:{title:"cities"}}}]}}],initialize:function(a){a&&$.extend(this,a);this.collection=new Cloudwalkers.Collections.Statistics;
this.listenTo(this.collection,"request",this.showloading);this.listenTo(this.collection,"seed",this.fillcharts);this.listenTo(this.collection,"sync:data",this.hideloading);this.listenTo(this.collection,"sync:noresults",this.showempty);translate={new_this:this.translateString("new_this"),new_this_m:this.translateString("new_this_m"),top_rated_comment:this.translateString("top_rated_comment"),messages_evolution:this.translateString("messages_evolution"),activity_calendar:this.translateString("activity_calendar")};
this.streamid=parseInt(this.streamid)},render:function(){var a=this.timemanager();a.streams=Cloudwalkers.Session.getStreams().where({statistics:1}).map(function(a){a.attributes.selected=a.id==this.streamid;return a.attributes}.bind(this));this.mustacheTranslateRender(a);this.$el.html(Mustache.render(Templates.statsview,a));this.$container=this.$el.find("#widgetcontainer").eq(0);this.$el.find("select").chosen({width:"200px",disable_search_threshold:10,inherit_select_classes:!0});"custom"==this.timespan&&
this.$el.find("#start, #end").datepicker({format:"dd-mm-yyyy"});0==this.period&&this.$el.find("#add").attr("disabled",!0);google.load("visualization","1",{callback:this.request.bind(this),packages:["corechart"]});return this},writeui:function(){var a=this.timemanager();this.$el.find(".stats-header-timeview").eq(0).html("<strong>"+a.fullperiod+": </strong>"+a.timeview)},request:function(a){this.writeui();this.collection.touch(this.filterparameters());this.currentperiod=a},fillcharts:function(a){if(a&&
!a.length)return this.showempty();this.cleanviews();this.widgets.forEach(function(a){(a.data.translation||a.data.chartdata)&&this.translatechart(a);if(this.streamid&&this.streamdata(a))a.data=this.streamdata(a);else if(this.streamid)return;a.data.parentview=this;a.data.timespan={since:this.start.unix(),to:this.end.unix()};var c=new Cloudwalkers.Views.Widgets[a.widget](a.data);this.views.push(c);this.appendWidget(c,a.span)}.bind(this))},translatechart:function(a){if(a.data.translation)this.translateWidgets(a.data);
...
this.listenToOnce(this,"change:period",this.request);this.currentperiod&&(this.period=this.currentperiod)},showempty:function(){this.cleanviews();this.hideloading();var a=new Cloudwalkers.Views.Widgets.EmptyData({timeparams:this.timeparams});this.views.push(a);this.appendWidget(a,8,null,2)},now:function(){this.period=0;this.trigger("change:period",this.period)},addperiod:function(a){a=$(a.target).attr("disabled");0<=this.period||"disabled"==a||(this.period+=1,this.trigger("change:period",this.period))},
subtractperiod:function(a){"disabled"!=$(a.target).attr("disabled")&&(this.period-=1,this.trigger("change:period",this.period))},subtractempty:function(){this.period-=1;this.trigger("change:period",this.period)},addempty:function(){this.period+=1;this.trigger("change:period",this.period)},changestream:function(){var a=Number(this.$el.find("select.networks").val());Cloudwalkers.Router.Instance.navigate(a?"#statistics/"+a:"#statistics",{trigger:!0})},changespan:function(){this.timespan=this.$el.find("select.time").val();
this.trigger("change:period")},changecustom:function(){this.start=moment(this.$el.find("#start").val(),"DD-MM-YYYY");this.end=moment(this.$el.find("#end").val(),"DD-MM-YYYY");this.trigger("change:period")},filterparameters:function(){"now"==this.timespan&&(this.period=0);"week"==this.timespan&&(this.start=moment().zone(0).startOf("isoweek"),this.end=moment().zone(0).endOf("isoweek"));"month"==this.timespan&&(this.start=moment().zone(0).startOf("month"),this.end=moment().zone(0).endOf("month"));"year"==
this.timespan&&(this.start=moment().zone(0).startOf("year"),this.end=moment().zone(0).endOf("year"));if("quarter"==this.timespan){var a=3*(this.period+moment().quarter());this.start=moment().startOf("year").add("months",a-3);this.end=moment().startOf("year").add("months",a-1).endOf("month")}else"custom"!=this.timespan&&(0<this.period&&(this.start.add(this.timespan+"s",this.period),this.end.add(this.timespan+"s",this.period)),0>this.period&&(this.start.subtract(this.timespan+"s",Math.abs(this.period)),
this.end.subtract(this.timespan+"s",Math.abs(this.period))));this.timeparams={since:this.start.unix(),until:this.end.unix(),span:this.timespan};return{since:this.start.unix(),until:this.end.unix(),span:this.timespan,period:this.period,reset:!0}},finish:function(){},updatenetwork:function(a){},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},translateWidgets:function(a){if(a.translation)for(k in a.translation)a[k]=Cloudwalkers.Session.polyglot.t(a.translation[k])},mustacheTranslateRender:function(a){this.original=
...
wait:!0});this.model.get("stream")&&Cloudwalkers.Session.getStreams().outdated(this.model.get("stream"))},togglecommentcontact:function(){var a=this.model.attributes.from?this.model.attributes.from[0]:null;a&&Cloudwalkers.RootView.viewContact({model:a})}});Cloudwalkers.Views.Widgets.EmptyData=Backbone.View.extend({title:"EmptyData",initialize:function(a){$.extend(this,a);this.timeparams&&this.checknow()},checknow:function(){var a=1E3*this.timeparams.since,b=1E3*this.timeparams.until,c=moment().utc().valueOf();return c<=b&&c>a},render:function(){var a=this.checknow(),b=this.timeparams.span,a=a?this.translateString("empty_statistics_data")+'<br/><a id="subtractempty">'+this.translateString("show_previous_"+b+"_statistics")+"</a>":this.translateString("empty_statistics_data")+
'<br/><a id="addempty">'+this.translateString("show_next_"+b+"_statistics")+"</a>";this.$el.html(Mustache.render(Templates.emptydata,{message:a}));return this},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)}});Cloudwalkers.Views.Widgets.Chart=Backbone.View.extend({title:"Chart",events:{},columns:{contacts:"parsecontacts",age:"parseage",gender:"parsegender",regional:"parseregional",besttime:"parsebesttime",cities:"parsecities",networks:"parsenetworks",activity:"parsecalendar","contact-evolution":"parsecontactevolution","message-evolution":"parsemessageevolution",messages:"parsemessages",activities:"parseactivities",impressions:"parseimpressions",notifications:"parsenotifications",followers:"parsefollowers",
