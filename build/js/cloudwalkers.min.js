...
c))},get:function(a){return this.user.get("settings")[a]},clone:function(a){if(null==a||"object"!=typeof a)return a;var b=a.constructor(),c;for(c in a)b[c]=this.clone(a[c]);return b},viewsettings:function(a,b){console.log(a,b);var c="account_"+this.getAccount().id;Cloudwalkers.Session.user.attributes.settings.viewsettings||(Cloudwalkers.Session.user.attributes.settings.viewsettings={});Cloudwalkers.Session.user.attributes.settings.viewsettings[c]||(Cloudwalkers.Session.user.attributes.settings.viewsettings[c]=
Cloudwalkers.RootView.navigation.mapViews());"sent"!=a||Cloudwalkers.Session.user.attributes.settings.viewsettings[c].sent||(Cloudwalkers.Session.user.attributes.settings.viewsettings[c].sent={streams:[]});var e=this.clone(this.get("viewsettings"));if(b){if(a&&b)return e[c][a]=$.extend(e[c][a],b),this.updateSetting("viewsettings",e),e[c][a];throw TypeError("Not the right parameters were met for function viewsettings");}return e[c][a]},manage:function(){200<Store.count("messages")&&Store.filter("messages",
null,function(a){a.sort(function(a,c){return a.stamp>c.stamp?1:a.stamp<c.stamp?-1:0});a=a.slice(0,100);Store.write("messages",a);Store.filter("touches",null,function(a){var c=Cloudwalkers.Session.getPing().cursor;a=a.filter(function(a){return a.ping==c});Store.write("touches",a)})});25<Store.count("reports")&&Store.filter("reports",null,function(a){a.sort(function(a,c){return a.stamp>c.stamp?1:a.stamp<c.stamp?-1:0});a=a.slice(0,10);Store.write("messages",a)})},getversion:function(){Store.get("version",
null,function(a){a&&(this.localversion=a.version)}.bind(this))},isupdated:function(){return this.localversion?this.localversion==this.version:!1},isAuthorized:function(a){return a?this.user.isauthorized(a):this.user.authorized},censuretemplate:function(a){a||(a={});var b=this.getUser().censuretokens;a.authorized=b},getPing:function(){return this.user.account.ping},ping:function(){this.user.account.ping.forceping()},getAccount:function(a){return a?this.user.accounts.get(a):this.user.account},getAccounts:function(a){return this.user.accounts},
getChannel:function(a){return"number"==typeof a?this.user.account.channels.get(a):this.user.account.channels.findWhere({type:a})},getChannels:function(a){return this.user.account.channels},storeChannel:function(a){a.channels&&a.channels.length&&(a.channels=a.channels.map(function(a){Cloudwalkers.Session.storeChannel(a);return a.id}));a.streams&&a.streams.length&&(a.streams=a.streams.map(function(a){Store.post("streams",a);return a.id}));Store.post("channels",a)},getStream:function(a){return"number"==
typeof a?this.user.account.streams.get(a):this.user.account.streams.findWhere({token:a})},getStreams:function(){return this.user.account.streams},addStream:function(a){return this.user.account.streams.add(a)},getUser:function(a){return a?this.user.account.users.get(a):this.user},getUsers:function(){return this.user.account.users},getContact:function(a){return this.user.account.contacts.get(a)},getContacts:function(){return this.user.account.contacts},getMessage:function(a){return this.user.account.messages.get(a)},
getMessages:function(){return this.user.account.messages},getCannedResponse:function(a){return this.user.account.cannedresponses.get(a)},getCannedResponses:function(){return this.user.account.cannedresponses},getNotification:function(a){return this.user.account.notifications.get(a)},getNotifications:function(){return this.user.account.notifications},getStatistic:function(a){return this.user.account.statistics.get(a)},getStatistics:function(){return this.user.account.statistics},getReport:function(a){return this.user.account.reports.get(a)},
getReports:function(){return this.user.account.reports},getComment:function(a){return this.user.account.comments.get(a)},getComments:function(){return this.user.account.comments},setLang:function(a){var b;moment.lang(this.user.attributes.locale);b=new Cloudwalkers.Models.Polyglot;b.fetch({success:function(){this.translationLang=b.get("translation");this.polyglot=new Polyglot({phrases:this.translationLang});this.trigger("translation:done")}.bind(this)})}};_.extend(Cloudwalkers.Session,Backbone.Events);Cloudwalkers.Router=Backbone.Router.extend({routes:{"dashboard/:accountid":"changeaccount","demo(/:demotype)":"demo","inbox/drafts":"ib-drafts","inbox/sent":"ib-sent","inbox/scheduled":"ib-scheduled","inbox/trash":"ib-trash",write:"write",share:"share","inbox(/:type)(/:streamid)":"inbox",drafts:"drafts","outbox(/:type)":"outbox",notes:"notes",scheduled:"scheduled",calendar:"calendar",coworkers:"coworkers","channel/:channel(/:subchannel)(/:stream)(/:messageid)":"channel","timeline/:channel(/:stream)":"timeline",
...
filterData:function(a){a.message&&a.message.body&&(a=a.message);var b={};$.each("id objectType actiontokens subject body date engagement from read stream streams attachments parent statistics stats status canHaveChildren children_count schedule variations url".split(" "),function(c,f){void 0!==a[f]&&(b[f]=a[f])});var c=Cloudwalkers.Session.getStream(a.stream);c&&(b.icon=c.get("network").icon,b.networktoken=c.get("network").token,b.networkdescription=c.get("name"));a.attachments?(a.attachments.length&&
(b.media=a.attachments[a.attachments.length-1],b.media="image"==b.media.type?"picture":b.media.type),b.attached={},$.each(a.attachments,function(a,c){b.attached[c.type]=c})):b.media="reorder";a.engagement&&(b.trending=1E3>a.engagement?a.engagement:"+999");b.schedule&&(b.scheduledate=moment(b.schedule.date).format("DD MMM YYYY HH:mm"));b.body.intro=a.body.plaintext?a.body.plaintext.substr(0,72):" ";a.date&&(b.fulldate=moment(a.date).format("DD MMM YYYY HH:mm"),b.dateonly=moment(a.date).format("DD MMM YYYY"),
...
(this.calNode={id:this.id});this.get("schedule")?this.calNode.start=a?$.fullCalendar.moment(this.get("scheduledate")):$.fullCalendar.moment():this.calNode.start=a?$.fullCalendar.moment(this.get("date")):$.fullCalendar.moment();this.calNode.title=a?(this.get("title")?this.get("title"):this.get("body").plaintext).substring(0,b?12:16):"...";this.calNode.className=a?this.get("networktoken")+"-color":"hidden";this.calNode.networkdescription=a?this.get("networkdescription"):null;this.calNode.intro=a?this.get("body").intro:
...
this.$loadercontainer||(this.$loadercontainer=this.$el.find(".modal-footer"));this.loadListeners(this.model,["request","sync"]);this.trigger("rendered");return this},post:function(){var a=this.$el.find("textarea").val(),b=this.model.typestring;if(b)this["post"+b](a)},postnotes:function(a){this.model.save({text:a},{patch:this.model.id?!0:!1,success:this.thanks?this.thankyou.bind(this):this.trigger.bind(this,"save:success")})},postmessages:function(a){this.model.set("body",{html:a,plaintext:a});this.model.save({body:this.model.get("body"),
...
new Cloudwalkers.Models.Note;this.render()}.bind(this),2E3)}.bind(this),200);Cloudwalkers.RootView.trigger("added:note",this.model)},clean:function(){this.$el.find("textarea").val("")},translateString:function(a){return Cloudwalkers.Session.polyglot.t(a)},mustacheTranslateRender:function(a){this.original=["write_note","save","cancel","private_note"];this.translated=[];for(k in this.original)this.translated[k]=this.translateString(this.original[k]),a["translate_"+this.original[k]]=this.translated[k]}});Cloudwalkers.Views.Preview=Backbone.View.extend({id:"preview",events:{},networkclasses:{facebook:"fb",twitter:"twt","google-plus":"gp",linkedin:"li"},events:{"click #viewsummary":"togglesummary"},initialize:function(a){a&&$.extend(this,a);$.extend(this.model.attributes,this.model.getvariation(this.streamid));this.model.attributes.profile=Cloudwalkers.Session.getStream(this.streamid).get("profile");this.network=Cloudwalkers.Session.getStream(this.streamid).get("network").token},render:function(){var a=
...
var a=new Cloudwalkers.Views.Widgets.SentMessageList(this.options);this.appendWidget(a,4);this.appendhtml(Templates.inboxcontainer);return this},resize:function(a){this.$el.find("#widgetcontainer").height(a-140)},finish:function(){this.resize(Cloudwalkers.RootView.height());$message=this.$el.find(".inbox-container").wrap("<div class='scroller'>");$message.parent().slimScroll({height:"inherit"})},translateTitle:function(a){this.title=Cloudwalkers.Session.polyglot.t(a)}});Cloudwalkers.Views.Actions=Backbone.View.extend({actionsright:[],actionsleft:[],compounds:[],initialize:function(a){$.extend(this,a);this.listenTo(this.message.notes,"ready",this.updateactions.bind(this,"notes"));this.listenTo(this.message.notes,"destroy",this.updateactions.bind(this,"notes"));this.listenTo(this.message.actions,"ready",this.updateactions);this.listenTo(this.message.actions,"destroy",this.updateactions)},render:function(a){this.selected=this.$el.find(".inactive").data("token");this.actionsleft=
...
Cloudwalkers.Session.isAuthorized("ACCOUNT_NOTES_VIEW")&&(this.$el.find(".note-list").html("<li>"+Mustache.render(Templates.messagenote)+"</li>"),this.loadnoteui());this.parameters.hasactions&&this.renderactions();this.model.get("status")&&"FAILED"==this.model.get("status")&&(this.$el.addClass("failed"),this.model.attributes.failed="failed");return this},loadsentiment:function(){var a=this.parameters.stats;a&&(a=_.isObject(a)?a["sentiment-fake"]:null)&&(a=5>Math.floor(a/2)?Math.floor(a/2):4,a=new Cloudwalkers.Views.Widgets.Sentiment({sentiment:a}),
...
b||(b=new Cloudwalkers.Models.Message({id:a.get("parent").id}));this.model.messages.add(b);b.get("objectType")||b.fetch();this.inboxmessage=new Cloudwalkers.Views.Widgets.InboxMessage({model:b,notification:a});$(".inbox-container").html(this.inboxmessage.render().el)}});Cloudwalkers.Views.Widgets.InboxNotesList=Cloudwalkers.Views.Widgets.InboxMessageList.extend({entrytemplate:"smallentrynote",render:function(){var a={streams:[],networks:[]};this.model.streams.each(function(b){b.get(this.check)&&a.streams.push({id:b.id,icon:b.get("network").icon,name:b.get("defaultname"),network:b.get("network")})}.bind(this));a.networks=this.model.streams.filterNetworks(a.streams,!0);a.note=this.listtype?!0:!1;this.mustacheTranslateRender(a);this.$el.html(Mustache.render(Templates.inboxlist,
a));this.filters.streams.length&&(this.$el.find("[data-streams], [data-networks], .toggleall").toggleClass("inactive active"),this.$el.find(this.filters.streams.map(function(a){return'[data-networks~="'+a+'"],[data-streams="'+a+'"]'}).join(",")).toggleClass("inactive active"));this.$container=this.$el.find("ul.list");this.collection.touch(this.model,this.filterparameters());this.listenTo(this.collection,"ready",this.afterrender);this.listenTo(Cloudwalkers.RootView,"ready:notecontext",this.rendercontext);
return this},fill:function(a){this.incremental?this.incremental=!1:($.each(this.entries,function(a,b){b.remove()}),this.entries=[]);for(n in a){var b=new Cloudwalkers.Views.NoteEntry({model:a[n],template:this.entrytemplate,parameters:{inboxview:!0}});this.entries.push(b);this.listenTo(b,"toggle",this.toggle);this.$container.append(b.render().el)}},afterrender:function(){this.entries.length?setTimeout(this.toggle.bind(this,this.entries[0]),1):this.hidemore()},toggle:function(a){var b={model:a.model},
c=a.model.parent?a.model.parent.get("objectType"):null;this.inboxnote&&this.inboxnote.remove();this.inboxnote=new Cloudwalkers.Views.Widgets.InboxNote(b);$(".inbox-container").html(this.inboxnote.render().el);this.rendercontext();"account"!=c&&this.inboxnote.showrelated();this.$el.find(".list .active").removeClass("active");a.$el.addClass("active")},togglefilter:function(a){a=$(a.currentTarget);var b=a.data("toggle"),c=a.hasClass("selected");this.$el.find("[data-toggle].selected").removeClass("selected");
