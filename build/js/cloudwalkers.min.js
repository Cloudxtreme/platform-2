...
a)return"You need a bit of content.";if(!this.checkcontent())return"One or more networks exceed the character limit.";if(!this.get("streams").length)return"Please select a network first."},hascontent:function(){var a;if(this.get("attachments")&&this.get("attachments").length)return this.get("attachments").length;if(this.get("body").plaintext)return this.get("body").plaintext.length;if(this.get("variations")&&this.get("variations").length)return $.each(this.get("variations"),function(b,c){if(c.attachments&&
c.attachments.length)return a=c.attachments.length,!1;if(c.body&&c.body.plaintext)return a=c.body.plaintext.length,!1}.bind(this)),a},checkcontent:function(){var a=1;if(this.get("body").plaintext&&0>140-this.get("body").plaintext.length)return!1;this.get("variations")&&this.get("variations").length&&$.each(this.get("variations"),function(b,c){var e=Cloudwalkers.Session.getStream(c.stream).get("network"),e=e.limitations["max-length"]?e.limitations["max-length"].limit:null;if(!e)return!0;if(c.body&&
c.body.plaintext)return a=e-c.body.plaintext.length,0>a?!1:!0}.bind(this));return 0<a},sanitizepost:function(){this.attributes.body.html&&delete this.attributes.body.html;this.attributes.variations&&this.attributes.variations.forEach(function(a,b){a.body&&(a.body.plaintext?a.body.html&&delete a.body.html:delete a.body);0>this.attributes.streams.indexOf(a.stream)&&this.attributes.variations.splice(b,1)}.bind(this))},cloneSanitized:function(){var a=this.clone();a.attributes.date&&delete a.attributes.date;
a.attributes.from&&delete a.attributes.from;a.attributes.stream&&delete a.attributes.stream;a.attributes.streams=[];a.id&&(delete a.id,delete a.attributes.id);return a},checkloaded:function(a){a.objectType&&setTimeout(function(a){a.trigger("loaded")},1,this)},filterData:function(a){var b={};$.each("id objectType actiontokens subject body date engagement from read stream streams attachments parent statistics canHaveChildren children_count schedule variations".split(" "),function(c,f){void 0!==a[f]&&
(b[f]=a[f])});var c=Cloudwalkers.Session.getStream(a.stream);c&&(b.icon=c.get("network").icon,b.networktoken=c.get("network").token,b.networkdescription=c.get("name"));a.attachments?(a.attachments.length&&(b.media=a.attachments[a.attachments.length-1],b.media="image"==b.media.type?"picture":b.media.type),b.attached={},$.each(a.attachments,function(a,c){b.attached[c.type]=c})):b.media="reorder";a.engagement&&(b.trending=1E3>a.engagement?a.engagement:"+999");b.schedule&&(b.scheduledate=moment(b.schedule.date).format("DD MMM YYYY HH:mm"));
a.body&&(b.body.intro=a.body.plaintext?a.body.plaintext.substr(0,72):"...");a.date&&(b.fulldate=moment(a.date).format("DD MMM YYYY HH:mm"),b.dateonly=moment(a.date).format("DD MMM YYYY"),b.time=moment(a.date).format("HH:mm"));return b},filterActions:function(){return this.get("actiontokens")?this.actions.rendertokens():[]},filterCalReadable:function(){var a=this.get("objectType"),b=a&&"reorder"!=this.get("media");this.calNode||(this.calNode={id:this.id});this.calNode.start=a?$.fullCalendar.moment(this.get("date")):
$.fullCalendar.moment();this.calNode.title=a?(this.get("title")?this.get("title"):this.get("body").plaintext).substring(0,b?12:16):"...";this.calNode.className=a?this.get("networktoken")+"-color":"hidden";this.calNode.networkdescription=a?this.get("networkdescription"):null;this.calNode.intro=a?this.get("body").intro:null;this.calNode.icon=a?this.get("icon"):null;this.calNode.media=b?this.get("media"):null;a&&72<=this.calNode.intro.length&&(this.calNode.intro+="...");return this},attach:function(a,
b,c){c=this.get("attachments")||[];a=b&&c[b]?$.extend(c[b],a):c.push(a);this.set({attachments:c});return a},unattach:function(a){(this.get("attachments")||[]).splice(a,1)},addcampaign:function(){},setvariation:function(a,b,c){var e=this.get("variations")||[],f=e.filter(function(b){if(b.stream==a)return b});if(0==f.length){f={stream:a};if("image"==b||"link"==b){var g=[c];f.attachments=g}else f[b]=c;e.push(f)}else f=f[0],"image"==b||"link"==b?(g=f.attachments||[],0==g.length&&"image"==b?(g.push(c),
f.attachments=g):"image"==b?g.push(c):(b=g.filter(function(a){if("link"==a.type)return a}),0<b.length?b[0].url=c.url:0!=g.length?f.attachments.push(c):f.attachments=[c])):f[b]=c},getvariation:function(a,b){var c=this.get("variations")||[],c=_.findWhere(c,{stream:a});if("image"==b||"link"==b){if(c&&c.attachments)return c.attachments.filter(function(a){if(a.type==b)return a})}else{if(c&&c[b])return c[b];if(c&&!b)return c}},original:function(a,b,c){var e=this.get("variations")?this.get("variations").filter(function(b){return a&&
b.stream==a.id}):[],f=a?e.length?e[0]:{id:a.id}:this.attributes;void 0!==c&&(f[b]=c,a&&!e.length&&this.get("variations").push(f));return f[b]},removevariation:function(a){var b=this.get("variations");b&&$.each(b,function(c,e){if(e.stream==a)return b.splice(c,1),!1})},removevarimg:function(a,b){var c=this.getvariation(a).attachments;if(_.isObject(b))this.addexclude(a,b);else for(n in c)"image"==c[n].type&&c[n].name==b&&c.splice(n,1)},addexclude:function(a,b){var c=this.getvariation(a);c.excludes?c.excludes.push(b):
...
document.execCommand("insertHTML",!1,b)}this.listentochange(a)},applystyle:function(){var a=this.win.getSelection();console.log(a);this.contenteditable.designMode="on";document.execCommand("backColor",!1,this.limit?"#fcc":"#fff");this.contenteditable.designMode="off"},listentochange:function(a,b){var c;if(this.content!==this.$contenteditable.text()||b)this.pos=this.cursorpos(),(c=this.listentourl(a,this.$contenteditable.text()))&&this.processurls(c),this.charlength!=this.$contenteditable.text().length&&
this.trigger("change:charlength");this.content=this.$contenteditable.text();this.isdefault&&!b&&(this.$contenteditable.removeClass("withdefault"),this.isdefault=!1);b&&this.clearselections()},endchange:function(a){if(this.$contenteditable.html().match(this.xurlendpattern)){var b;(b=this.listentourl(null,this.$contenteditable.text(),!0))&&this.processurls(b)}},listentourl:function(a,b,c){var e=this.win.getSelection(),f=this.document.createRange(),g,h,l,k;if(!a||"keyup"!=a.type||32==a.keyCode)if(a&&
"paste"==a.type&&(c=!0),f.selectNodeContents(this.$contenteditable.get(0)),g=this.parsenodes(f.startContainer.childNodes,c),g.length)return $.each(g,function(a,b){h=b.node.textContent.replace(/&nbsp;/gi,"");b.url=b.url.trim();l=h.indexOf(b.url);k=l+b.url.length;0>l&&(l=0);f.setStart(b.node,l);f.setEnd(b.node,k);e.removeAllRanges();e.addRange(f);this.contenteditable.designMode="on";document.execCommand("createLink",!1,b.url);this.contenteditable.designMode="off";var c=document.createTextNode("\u200b");
f.insertNode(c);this.$contenteditable.find("a").after(c);f.setStartAfter(c);e.removeAllRanges();e.addRange(f);g[a+1]&&(g[a+1].node=b.node.nextSibling)}.bind(this)),_.pluck(g,"url")},parsenodes:function(a,b){var c=[],e;$.each(a,function(a,g){var h=!1,l=g.textContent;if(b&&3!=g.nodeType)return!0;l&&(h=l.match(b?this.xurlendpattern:this.xurlpattern));if(h&&h.length)return $.each(h,function(a,b){3==g.nodeType?c.push({node:g,url:b}):(e=this.getnode(g,null,b))&&b&&c.push({node:e,url:b})}.bind(this)),!0}.bind(this));
return c},getnode:function(a,b,c){var e;$.each(a.childNodes,function(a,g){if(c){if(3==g.nodeType)if(g.childNodes.length)e=this.getnode(g,null,type);else if(-1<g.textContent.indexOf(c.trim()))return e=g,!1}else{var h=g.textContent.length;return h>=b?(g.childNodes.length?this.getnode(g,b):this.currentnode={node:g,offset:b},!1):b-=h}}.bind(this));return e},processurls:function(a){this.$contenteditable.find("a").attr("contenteditable",!1);if(!this.campaign){var b=this.draft.get("campaign"),c=Cloudwalkers.Session.getAccount().get("campaigns").filter(function(a){if(a.id==
b)return a});this.campaign=c.length?c[0].name:null}a&&!this.urlprocessing&&(this.urlprocessing=!0,this.parseurls(a,this.campaign))},parseurls:function(a,b,c){var e={error:this.releaseurlprocessing};a.forEach(function(a){!c&&this.urls[a]?this.shortenurl(this.urls[a]):(e.q=a,e.campaign=b,Cloudwalkers.Session.UrlShortener.fetch(e))}.bind(this))},shortenurl:function(a){this.releaseurlprocessing();var b=this,c=a.get("url"),e=a.get("shortUrl");this.urls[c]||(this.urls[c]=a.clone());a=" <short contenteditable='false' data-long='"+
