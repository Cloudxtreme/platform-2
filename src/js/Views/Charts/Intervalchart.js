...
define(
	...
	function (Widget)
	{
		var Intervalchart = Widget.extend ({
			...
			'plot' : function (values)
			{
				...
				for (var j = 0; j < values.length; j ++)
				{
					if (j > 0)
					{
						values[j][1] = parseInt(values[j][1]);

						if (values[j][1] > 0 && values[j-1][1] > 0)
						{
							growth = (values[j][1] - values[j-1][1]) / Math.max(1, values[j-1][1]);
							growth *= 100;
							growth = Math.floor (growth);

							growth = this.numberOutput (growth) + '%';

							o = plot.pointOffset({ x: values[j][0], y: values[j][1]});
